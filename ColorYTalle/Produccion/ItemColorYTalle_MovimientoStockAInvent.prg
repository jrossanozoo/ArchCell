define class ItemColorYTalle_MovimientoStockAInvent as ItemMovStock of ItemMovStock.Prg
	...
	*-----------------------------------------------------------------------------------------
	function EventoBusquedaDeColor( toBusqueda as Object ) as Void
		...
		if vartype( toBusqueda ) = "O"
			lcCodigos = this.oCompStockProduccion.oColaboradorColoryTalle.ObtenerColores( this.articulo )
			if !empty( lcCodigos )
