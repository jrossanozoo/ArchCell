		...
		for lni = 1 to this.Count
			...
			if vartype( this.Item[lni].ETIQUETA) = 'C'
				if len( alltrim( this.Item[lni].ETIQUETA )) <= 100
					...
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud del valor del atributo ETIQUETA del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform( len( alltrim( this.Item[lni].ETIQUETA ))) + ' Máxima:100), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
