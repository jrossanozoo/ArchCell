...
define class Din_DetalleLIQUIDACIONDETALLERLiquidaciontallerinsumos as DetalleProduccioncolorytalle of DetalleProduccioncolorytalle.Prg
	...
	function Sumarizar() as boolean
		...
		with this
			...
			if this.lEsNavegacion
				if select( 'c_Liquidaciontallerinsumos' ) > 0 and reccount( 'c_Liquidaciontallerinsumos' ) > 0
					sum Cantidad, Monto;
						 to ;
	...
	*--------------------------------------------------------------------------------------------------------
	function oItem_Access() as variant
		if !this.ldestroy and ( !vartype( this.oItem ) = 'O' or isnull( this.oItem ) )
			this.oItem = _Screen.zoo.crearobjeto( 'ItemLIQUIDACIONDETALLERLiquidaciontallerinsumos' )
	...
	*--------------------------------------------------------------------------------------------------------
	Function Cargar() as Void
		...
		select c_Liquidaciontallerinsumos
		lcFiltro = this.ObtenerFiltro()
		scan for &lcFiltro
			loItem = createobject('ItemAuxiliar')

			loItem.Codigo = c_Liquidaciontallerinsumos.Codigo
			loItem.Comportamiento = c_Liquidaciontallerinsumos.Comportamiento
			loItem.IdItemInsumo = c_Liquidaciontallerinsumos.IdItemInsumo
			loItem.NROITEM = c_Liquidaciontallerinsumos.NROITEM
			loItem.Proceso_PK = c_Liquidaciontallerinsumos.Proceso
			loItem.ProcesoDetalle = c_Liquidaciontallerinsumos.ProcesoDetalle
			loItem.Insumo_PK = c_Liquidaciontallerinsumos.Insumo
			loItem.InsumoDetalle = c_Liquidaciontallerinsumos.InsumoDetalle
			loItem.Color_PK = c_Liquidaciontallerinsumos.Color
			loItem.ColorDetalle = c_Liquidaciontallerinsumos.ColorDetalle
			loItem.Talle_PK = c_Liquidaciontallerinsumos.Talle
			loItem.TalleDetalle = c_Liquidaciontallerinsumos.TalleDetalle
			loItem.Cantidad = c_Liquidaciontallerinsumos.Cantidad
			loItem.Articulo_PK = c_Liquidaciontallerinsumos.Articulo
			loItem.ArticuloDetalle = c_Liquidaciontallerinsumos.ArticuloDetalle
			loItem.Costo = c_Liquidaciontallerinsumos.Costo
			loItem.Monto = c_Liquidaciontallerinsumos.Monto
			loItem.NroItem = lnI 
