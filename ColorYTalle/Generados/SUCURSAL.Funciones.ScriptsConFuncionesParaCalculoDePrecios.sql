...
CREATE TYPE dtTablaFiltro AS TABLE (ARTICULO Varchar ( 15 ), CCOLOR Varchar ( 6 ), TALLE Varchar ( 5 ), FALTAFW Datetime, HALTAFW Varchar( 8 ), FMODIFW Datetime, HMODIFW Varchar( 8 ), UALTAFW Varchar( 100 ), UMODIFW Varchar( 100 ), SALTAFW Varchar( 7 ), SMODIFW Varchar( 7 ), BDALTAFW Varchar( 8 ), BDMODIFW Varchar( 8 ), VALTAFW Varchar( 13 ), VMODIFW Varchar( 13 ),CODORI Varchar ( 20 ), ENTORI Varchar ( 40 ), FECHAVIG DateTime, TIMESTAMPA Numeric ( 14,0 ), DESCFW Varchar( 200 ) )
...
( @ARTICULO Varchar( 25 ), @CCOLOR Varchar( 16 ), @TALLE Varchar( 15 ), @VigenciaParaFecha nVarchar( 15 ), @Expresion nVarchar( 4000 ) ) 
...
BEGIN
	...
	Begin
		Select @FuncionPrecioSQL = 
			'[Funciones].[ObtenerPrecioDeLaCombinacionConVigencia]( ' + rtrim(ltrim(@ARTICULO)) + ', ' + rtrim(ltrim(@CCOLOR)) + ', ' + rtrim(ltrim(@TALLE)) + ',  ''' + rtrim(ltrim(@ListaDePrecios)) + ''', ' + rtrim(ltrim(@VigenciaParaFecha)) + ', default)'
...
BEGIN
 ...
 -- Nuevo 
	...
	Select @StrPrec = [Funciones].[EvaluarExpresionFormula]( 'P.ARTICULO', 'P.CCOLOR', 'P.TALLE', 'P.FECHAVIG', @Expresion )
	Select @SqlSel =
	...
	'Insert Into ZOOLOGIC.PrecioAr(FALTAFW, HALTAFW, FMODIFW, HMODIFW, UALTAFW, UMODIFW, SALTAFW, SMODIFW, BDALTAFW, BDMODIFW, VALTAFW, VMODIFW, DESCFW, CODORI, ENTORI, FECHAVIG,
           TimestampA, Codigo, ARTICULO, CCOLOR, TALLE,  ListaPre, PDirecto)' + 
	' Select T.FMODIFW as FALTAFW, T.HMODIFW as HALTAFW, T.FMODIFW as FMODIFW, T.HMODIFW as HMODIFW, T.UMODIFW as UALTAFW, T.UMODIFW as UMODIFW, T.SMODIFW as SALTAFW, T.SMODIFW as SMODIFW, T.BDMODIFW as BDALTAFW, T.BDMODIFW as BDMODIFW, T.VMODIFW as VALTAFW, T.VMODIFW as VMODIFW, T.DESCFW as DESCFW, T.CODORI, T.ENTORI, T.FECHAVIG,
           T.TIMESTAMPA, Funciones.Padr( CONVERT(varchar(14), T.TIMESTAMPA ), 14, '' '') + ''' + @ListaDePrecios + '''  + T.ARTICULO + T.CCOLOR + T.TALLE as Codigo, T.ARTICULO, T.CCOLOR, T.TALLE,  '''  + @ListaDePrecios + ''' as ListaPre, ' + REPLACE( @StrPrec, 'P.', 'T.' ) + ' as PDirecto' + 
	' From #tt T'
   if ( @precioConVigencia = 0)
	begin
   	Select @SqlIns = @SqlIns + ' Left join ZooLogic.PrecioAr P ' + 
   	' on  T.ARTICULO= P.ARTICULO' +
' and T.CCOLOR = P.CCOLOR'+
' and T.TALLE = P.TALLE'+
...
' and T.TALLE = P.TALLE'+
	' And T.ListaPre = P.ListaPre '+
	' And P.timestampa = [Funciones].[ObtenerTimestampVigenteDeLaCombinacion]( T.ARTICULO, T.CCOLOR, T.TALLE, T.LISTAPRE, GETDATE() )'
...
DECLARE @retorno numeric(15,2)
    ...
    set @retorno = 
        ...
        where Prioridad = 1
        order by sign( sc.PDIRECTO ) desc, sc.FECHAVIG desc, sc.Grado desc, sc.TIMESTAMPA desc
        )
...
DECLARE @retorno numeric(15,2)
	...
	set @retorno = coalesce( Funciones.ObtenerPrecioRealDeLaCombinacionConVigencia( @P_ARTICULO, @P_CCOLOR, @P_TALLE, @CodigoLPrecio, @VigenciaParaFecha, @NoValidarSiEsListaCalculada ), 0 )
...
Begin
	Declare
		...
		Select ARTICULO, CCOLOR, TALLE, FALTAFW, HALTAFW, FMODIFW, HMODIFW, UALTAFW, UMODIFW, SALTAFW, SMODIFW, BDALTAFW, BDMODIFW, VALTAFW, VMODIFW, DESCFW, LISTAPRE,CODORI, ENTORI, FECHAVIG, TIMESTAMPA into #PreciosModificados from inserted
	...
	begin
		...
		begin
			...
			insert into @Tabla
			Select PM.ARTICULO, PM.CCOLOR, PM.TALLE, PM.FALTAFW, PM.HALTAFW, PM.FMODIFW, PM.HMODIFW, PM.UALTAFW, PM.UMODIFW, PM.SALTAFW, PM.SMODIFW, PM.BDALTAFW, PM.BDMODIFW, PM.VALTAFW, PM.VMODIFW, PM.CODORI, PM.ENTORI, PM.FECHAVIG, PM.TIMESTAMPA+@id as TIMESTAMPA, PM.DESCFW
