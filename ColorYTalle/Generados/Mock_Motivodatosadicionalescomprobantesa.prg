...
define class Mock_motivodatosadicionalescomprobantesa as MockBase of MockBase.prg
	...
	cClase = 'motivodatosadicionalescomprobantesa'
	...
	*-----------------------------------------------------------------------------------------
	function descripcioncorta_assign( txval as variant ) as void                                                                                                                                                                                              
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'MOTIVODATOSADICIONALESCOMPROBANTESA' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'descripcioncorta_assign', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'descripcioncorta_assign', lcParametros )			endif		endif		Local lxValOld As Variant		lxValOld = this.Descripcioncorta		local lxVal as Variant		lxVal = txVal		if This.CargaManual()			if this.Validar_Descripcioncorta( lxVal )				this.Setear_Descripcioncorta( lxVal )			EndIf		Else			This.Setear_Descripcioncorta( lxVal )		EndIf		dodefault( lxVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function setear_descripcioncorta( txval as variant ) as void                                                                                                                                                                                              
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'MOTIVODATOSADICIONALESCOMPROBANTESA' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'setear_descripcioncorta', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'setear_descripcioncorta', lcParametros )			endif		endif		this.Descripcioncorta = txVal		dodefault( txVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function validar_descripcioncorta( txval as variant ) as boolean                                                                                                                                                                                          
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'MOTIVODATOSADICIONALESCOMPROBANTESA' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'validar_descripcioncorta', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'validar_descripcioncorta', lcParametros )			endif		endif		Return dodefault( txVal )
	EndFunc
