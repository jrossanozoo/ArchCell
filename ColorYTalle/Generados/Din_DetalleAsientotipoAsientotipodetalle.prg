	...
	local lnLargoEntero as integer, lnValorDelDecimal as integer, lnLargoDecimal as integer, llRetorno as boolean, lnI as Integer
		...
		for lni = 1 to this.Count
			...
			if vartype( this.Item[lni].PLANDECUENTASDEBEDETALLE) = 'C'
				if len( alltrim( this.Item[lni].PLANDECUENTASDEBEDETALLE )) <= 90
				else
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud del valor del atributo PLANDECUENTASDEBEDETALLE del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform( len( alltrim( this.Item[lni].PLANDECUENTASDEBEDETALLE ))) + ' Máxima:90), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
