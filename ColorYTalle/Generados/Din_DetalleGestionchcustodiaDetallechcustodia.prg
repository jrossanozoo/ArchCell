		...
		for lni = 1 to this.Count
			...
			if vartype( this.Item[lni].REGISTRO) = 'C'
				if len( alltrim( this.Item[lni].REGISTRO )) <= 20
					...
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud del valor del atributo REGISTRO del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform( len( alltrim( this.Item[lni].REGISTRO ))) + ' Máxima:20), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
			...
			if vartype( this.Item[lni].IDCHEQUE_PK) = 'C'
				if len( alltrim( this.Item[lni].IDCHEQUE_PK )) <= 38
					...
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud del valor del atributo IDCHEQUE del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform( len( alltrim( this.Item[lni].IDCHEQUE_PK ))) + ' Máxima:38), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
