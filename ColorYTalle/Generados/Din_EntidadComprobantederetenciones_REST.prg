...
define class Din_EntidadComprobantederetenciones_REST as ServicioRestOperacionesEntidad of ServicioRestOperacionesEntidad.prg

	cNamespaceDTOs = "ZooLogicSA.OrganicServiciosREST.Colorytalle.Generados.DTO.Comprobantederetenciones"
	cClaseResponse = this.cNamespaceDTOs + ".ComprobantederetencionesResponse"
	cClaseModelo = "ComprobantederetencionesModelo"
	cEntidad = "Comprobantederetenciones"
	...
	function ObtenerClaseRequest( tcOperacion as String ) as String
		...
		do case
			case tcOperacion == "Nuevo"
				lcRetorno = this.cNamespaceDTOs + "." + "ComprobantederetencionesModelo"
			...
			case tcOperacion == "Listar"
				lcRetorno = this.cNamespaceDTOs + "." + "ComprobantederetencionesListarRequest"
	...
	*-----------------------------------------------------------------------------------------
	function ObtenerClaseResponse( tcOperacion as String ) as String
		...
		do case
			case tcOperacion == "Listar"
				lcRetorno = this.cNamespaceDTOs + "." + "ComprobantederetencionesListarResponse"
			otherwise
				lcRetorno = this.cNamespaceDTOs + "." + "ComprobantederetencionesModelo"
		endcase
		return lcRetorno
	...
	*-----------------------------------------------------------------------------------------
	protected function SetearDetalleImpuestosDetalle( toEntidad as Object, toModeloEnRequest as Object ) as Void
		...
		for lnI = 1 to lnCantidad 
			...
			this.SetearAtributoModeloEnEntidad( loItem, "Codimp", toEntidad.ImpuestosDetalle.oItem, "CodImp_PK" )
			this.SetearAtributoModeloEnEntidad( loItem, "Jurisdicciondescripcion", toEntidad.ImpuestosDetalle.oItem, "JurisdiccionDescripcion" )
			this.SetearAtributoModeloEnEntidad( loItem, "Resolucion", toEntidad.ImpuestosDetalle.oItem, "Resolucion" )
			this.SetearAtributoModeloEnEntidad( loItem, "Monto", toEntidad.ImpuestosDetalle.oItem, "Monto" )
	...
	protected function CargarColeccionModeloImpuestosDetalle( toModeloResponse as Object, toEntidad as Object ) as Void
		...
		with toEntidad
			for lnI = 1 to .ImpuestosDetalle.Count
				loItem = _Screen.DotNetBridge.CrearObjeto( this.cNamespaceDTOs + ".ItemCdrimpuestos" )
				this.SetearAtributoString( loItem, "Codigo", .ImpuestosDetalle.Item(lnI).Codigo)
				this.SetearAtributoString( loItem, "CodImp", .ImpuestosDetalle.Item(lnI).CodImp_PK)
				this.SetearAtributoString( loItem, "JurisdiccionDescripcion", .ImpuestosDetalle.Item(lnI).JurisdiccionDescripcion)
				this.SetearAtributoString( loItem, "Resolucion", .ImpuestosDetalle.Item(lnI).Resolucion)
				this.SetearAtributoDecimal( loItem, "Monto", .ImpuestosDetalle.Item(lnI).Monto)
