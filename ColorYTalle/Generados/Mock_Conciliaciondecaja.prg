...
define class Mock_conciliaciondecaja as MockBase of MockBase.prg
	...
	cCaracteres = ""                                                                                                                                                                                                                                          
	cContexto = ""                                                                                                                                                                                                                                            
	cDescripcion =  "Ajuste de movimientos y saldos de caja según comprobantes"                                                                                                                                                                               
	...
	lHabilitarCaja_PK = .T.                                                                                                                                                                                                                                   
	lHabilitarVerificarHistoricos = .T.                                                                                                                                                                                                                       
	...
	*-----------------------------------------------------------------------------------------
	function analizarcaja() as void                                                                                                                                                                                                                           
		...
		return this.ObtenerResultado( 'Analizarcaja', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function caja_pk_assign( txval as variant ) as void                                                                                                                                                                                                       
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'CONCILIACIONDECAJA' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'caja_pk_assign', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'caja_pk_assign', lcParametros )			endif		endif		Local lxValOld As Variant		lxValOld = this.Caja_PK		local lxVal as Variant		local loSugerido as Object				lxVal = txVal		this.Caja_PK = lxVal		if this.Validar_Caja( lxVal, lxValOld )			if ( this.lHabilitarCaja_PK or this.lEstaSeteandoValorSugerido or lxValOld == lxVal )				This.Setear_Caja( lxVal )			else				this.Caja_PK = lxValOld				goServicios.Errores.LevantarExcepcion( "No se puede cambiar este valor (Entidad: Ajuste de movimientos y saldos de caja según comprobantes - Atributo: Caja)" )			endif		endif		dodefault( lxVal )
	EndFunc

	*-----------------------------------------------------------------------------------------
	function cajaauditoria_assign( txval as variant ) as void                                                                                                                                                                                                 
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'CONCILIACIONDECAJA' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'cajaauditoria_assign', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'cajaauditoria_assign', lcParametros )			endif		endif		Local lxValOld As Variant		lxValOld = this.Cajaauditoria		local lxVal as Variant		lxVal = txVal		if This.CargaManual()			if this.Validar_Cajaauditoria( lxVal )				this.Setear_Cajaauditoria( lxVal )			EndIf		Else			This.Setear_Cajaauditoria( lxVal )		EndIf		dodefault( lxVal )
	EndFunc

	*-----------------------------------------------------------------------------------------
	function cambiosdetalleconciliamovimiento() as void                                                                                                                                                                                                       
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Cambiosdetalleconciliamovimiento', lcParametros )
	EndFunc

	*-----------------------------------------------------------------------------------------
	function cambiosdetalleconciliasaldo() as void                                                                                                                                                                                                            
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Cambiosdetalleconciliasaldo', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function conciliamovimiento_access() as variant                                                                                                                                                                                                           
		lnParametros = pcount()lcParametros = ''		lnIdClase = _screen.Mocks.BuscarMock( 'CONCILIACIONDECAJA' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'conciliamovimiento_access', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'conciliamovimiento_access', lcParametros )			endif		endif		if !this.ldestroy and ( !vartype( this.Conciliamovimiento ) = 'O' or isnull( this.Conciliamovimiento ) )			this.Conciliamovimiento = this.crearobjeto( 'DetalleConciliaciondecajaConciliamovimiento' )			this.Conciliamovimiento.inicializar()			this.enlazar( 'Conciliamovimiento.EventoObtenerLogueo', 'inyectarLogueo' )			this.enlazar( 'Conciliamovimiento.EventoObtenerInformacion', 'inyectarInformacion' )			bindevent( this.Conciliamovimiento, 'CambioSumarizado', this, 'CambiosDetalleConciliamovimiento', 1)			this.enlazar( 'Conciliamovimiento.EventoAdvertirLimitePorDiseno', 'EventoAdvertirLimitePorDiseno')			this.enlazar( 'Conciliamovimiento.EventoCancelarCargaLimitePorDiseno', 'EventoCancelarCargaLimitePorDiseno')		endif		return this.Conciliamovimiento
	EndFunc

	*-----------------------------------------------------------------------------------------
	function conciliasaldo_access() as variant                                                                                                                                                                                                                
		lnParametros = pcount()lcParametros = ''		lnIdClase = _screen.Mocks.BuscarMock( 'CONCILIACIONDECAJA' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'conciliasaldo_access', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'conciliasaldo_access', lcParametros )			endif		endif		if !this.ldestroy and ( !vartype( this.Conciliasaldo ) = 'O' or isnull( this.Conciliasaldo ) )			this.Conciliasaldo = this.crearobjeto( 'DetalleConciliaciondecajaConciliasaldo' )			this.Conciliasaldo.inicializar()			this.enlazar( 'Conciliasaldo.EventoObtenerLogueo', 'inyectarLogueo' )			this.enlazar( 'Conciliasaldo.EventoObtenerInformacion', 'inyectarInformacion' )			bindevent( this.Conciliasaldo, 'CambioSumarizado', this, 'CambiosDetalleConciliasaldo', 1)			this.enlazar( 'Conciliasaldo.EventoAdvertirLimitePorDiseno', 'EventoAdvertirLimitePorDiseno')			this.enlazar( 'Conciliasaldo.EventoCancelarCargaLimitePorDiseno', 'EventoCancelarCargaLimitePorDiseno')		endif		return this.Conciliasaldo
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function eventoadvertircancelar() as void                                                                                                                                                                                                                 
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Eventoadvertircancelar', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function eventopoblemasantesdeanalizar()                                                                                                                                                                                                                  
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Eventopoblemasantesdeanalizar', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function eventovalidargrillas() as boolean                                                                                                                                                                                                                
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Eventovalidargrillas', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function ocompajustesdecaja_access() as variant                                                                                                                                                                                                           
		lnParametros = pcount()lcParametros = ''		lnIdClase = _screen.Mocks.BuscarMock( 'CONCILIACIONDECAJA' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'ocompajustesdecaja_access', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'ocompajustesdecaja_access', lcParametros )			endif		endif		if !this.ldestroy and ( !vartype( this.oCompAjustesdecaja ) = 'O' or isnull( this.oCompAjustesdecaja ) )			this.oCompAjustesdecaja = _Screen.zoo.InstanciarComponente( 'ComponenteAjustesdecaja' )		this.oCompAjustesdecaja.Inicializar()		this.enlazar( 'oCompAjustesdecaja.EventoObtenerLogueo', 'inyectarLogueo' )		this.enlazar( 'oCompAjustesdecaja.EventoObtenerInformacion', 'inyectarInformacion' )		endif		return this.oCompAjustesdecaja
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function setear_cajaauditoria( txval as variant ) as void                                                                                                                                                                                                 
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'CONCILIACIONDECAJA' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'setear_cajaauditoria', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'setear_cajaauditoria', lcParametros )			endif		endif		this.Cajaauditoria = txVal		dodefault( txVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function setear_verificarhistoricos( txval as variant ) as void                                                                                                                                                                                           
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'CONCILIACIONDECAJA' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'setear_verificarhistoricos', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'setear_verificarhistoricos', lcParametros )			endif		endif		this.Verificarhistoricos = txVal		dodefault( txVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function validar_cajaauditoria( txval as variant ) as boolean                                                                                                                                                                                             
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'CONCILIACIONDECAJA' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'validar_cajaauditoria', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'validar_cajaauditoria', lcParametros )			endif		endif		Return dodefault( txVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function validar_verificarhistoricos( txval as variant ) as boolean                                                                                                                                                                                       
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'CONCILIACIONDECAJA' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'validar_verificarhistoricos', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'validar_verificarhistoricos', lcParametros )			endif		endif		Return dodefault( txVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function validarquehayaajustesarealizar() as boolean                                                                                                                                                                                                      
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Validarquehayaajustesarealizar', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function valorsugeridofwverificarhistoricos() as void                                                                                                                                                                                                     
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Valorsugeridofwverificarhistoricos', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function valorsugeridoverificarhistoricos() as void                                                                                                                                                                                                       
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Valorsugeridoverificarhistoricos', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function verificarhistoricos_assign( txval as variant ) as void                                                                                                                                                                                           
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'CONCILIACIONDECAJA' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'verificarhistoricos_assign', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'verificarhistoricos_assign', lcParametros )			endif		endif		Local lxValOld As Variant		lxValOld = this.Verificarhistoricos		local lxVal as Variant		lxVal = txVal		if This.CargaManual()			if this.Validar_Verificarhistoricos( lxVal )				if ( this.lHabilitarVerificarhistoricos or this.lEstaSeteandoValorSugerido or lxValOld == lxVal)					this.Setear_Verificarhistoricos( lxVal )				else					this.Verificarhistoricos = lxValOld					goServicios.Errores.LevantarExcepcion( "No se puede cambiar este valor (Entidad: Ajuste de movimientos y saldos de caja según comprobantes - Atributo: Verificarhistoricos)" )				endif			EndIf		Else			This.Setear_Verificarhistoricos( lxVal )		EndIf		dodefault( lxVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	protected function deshabilitarfiltros() as void                                                                                                                                                                                                          
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Deshabilitarfiltros', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	protected function llenardetalles() as void                                                                                                                                                                                                               
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Llenardetalles', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	protected function validarantesdeanalizar() as boolean                                                                                                                                                                                                    
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Validarantesdeanalizar', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	protected function validarcajaabiertayaconciliada() as boolean                                                                                                                                                                                            
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Validarcajaabiertayaconciliada', lcParametros )
	EndFunc

	*-----------------------------------------------------------------------------------------
	protected function validarcajaestado() as boolean                                                                                                                                                                                                         
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Validarcajaestado', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	protected function validarexistenciadecodigosdevalorescondiferencias() as boolean                                                                                                                                                                         
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Validarexistenciadecodigosdevalorescondiferencias', lcParametros )
	EndFunc

	*-----------------------------------------------------------------------------------------
	protected function validarfiltros() as boolean                                                                                                                                                                                                            
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Validarfiltros', lcParametros )
	EndFunc

	*-----------------------------------------------------------------------------------------
	protected function validarquelacajadelajustenosehayacerrado() as boolean                                                                                                                                                                                  
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Validarquelacajadelajustenosehayacerrado', lcParametros )
	EndFunc
