...
define class Din_EntidadDESCARGADECHEQUES as entidad of entidad.prg
	...
	cDescripcion =  [Descarga de cheques de terceros]
	lEntidadEditable =  .t.
	lTieneVendedorComoClaveForanea = .f.
	cAtributoVendedor = ''
	oAtributosCC =  Null
	oCompCuentaBancariaDescargaDeCheques = null
	oColeccionVS = null
	oColeccionVSFW = null
	*-----------------------------------------------------------------------------------------
	...
	*--------------------------------------------------------------------------------------------------------
	function oAD_Access() as variant
		if !this.ldestroy and ( !vartype( this.oAD ) = 'O' or isnull( this.oAD ) )
			this.oAD = this.crearobjeto( 'Din_EntidadDESCARGADECHEQUESAD_' + alltrim( _screen.zoo.app.TipoDeBase ))
	...
	*--------------------------------------------------------------------------------------------------------
	function Chequesdetalle_Access() as variant
		if !this.ldestroy and ( !vartype( this.Chequesdetalle ) = 'O' or isnull( this.Chequesdetalle ) )
			this.Chequesdetalle = this.crearobjeto( 'Din_DetalleDescargadechequesChequesdetalle' )
			this.Chequesdetalle.inicializar()
			this.enlazar( 'Chequesdetalle.EventoObtenerLogueo', 'inyectarLogueo' )
			this.enlazar( 'Chequesdetalle.EventoObtenerInformacion', 'inyectarInformacion' )
			bindevent( this.Chequesdetalle, 'CambioSumarizado', this, 'CambiosDetalleChequesdetalle', 1) 
			this.enlazar( 'Chequesdetalle.EventoAdvertirLimitePorDiseno', 'EventoAdvertirLimitePorDiseno') 
			this.enlazar( 'Chequesdetalle.EventoCancelarCargaLimitePorDiseno', 'EventoCancelarCargaLimitePorDiseno') 
			this.Chequesdetalle.lVerificarLimitesEnDisenoImpresion = .t.
		endif
		return this.Chequesdetalle
	endfunc

	*--------------------------------------------------------------------------------------------------------
	function SetearEstadoNuevoEnItems() as Void
		dodefault()
		this.ChequesDetalle.oItem.lNuevo = this.lNuevo
	endfunc

	*--------------------------------------------------------------------------------------------------------
	function SetearEstadoEdicionEnItems() as Void
		dodefault()
		this.ChequesDetalle.oItem.lEdicion = this.lEdicion
	endfunc

	*--------------------------------------------------------------------------------------------------------
	function CambiosDetalleChequesdetalle() as void
		dodefault()
	endfunc
	...
	*--------------------------------------------------------------------------------------------------------
	...
	local lnLargoDecimal as integer, lnLargoEntero as Integer, lnLargoDecimal as integer, llRetorno as boolean 
		...
		if ( vartype( this.Observ) = 'C' )
			if len( alltrim( this.OBSERV ) ) > 100
				This.AgregarInformacion( 'La longitud del valor del atributo OBSERV es mayor a la permitida.' )
				llRetorno = .F.
			Endif
		else
	...
	*--------------------------------------------------------------------------------------------------------
	function VotacionCambioEstadoNUEVO( tcEstado as string ) as Boolean
		*!*Se ejecuta antes de hacer el cambio de estado
		...
		llVotacion = llVotacion and this.oCompCUENTABANCARIADESCARGADECHEQUES.votarCambioEstadoNUEVO( tcEstado )
		llVotacion = llVotacion and this.ChequesDetalle.oItem.oCompDESCARGADECHEQUES.votarCambioEstadoNUEVO( tcEstado )
		return llVotacion
	endfunc
	...
	*--------------------------------------------------------------------------------------------------------
	function VotacionCambioEstadoELIMINAR( tcEstado as string ) as Boolean
		*!*Se ejecuta antes de hacer el cambio de estado
		local llVotacion as boolean
		llVotacion = dodefault()
		llVotacion = llVotacion and this.oCompCUENTABANCARIADESCARGADECHEQUES.votarCambioEstadoELIMINAR( tcEstado )
		llVotacion = llVotacion and this.ChequesDetalle.oItem.oCompDESCARGADECHEQUES.votarCambioEstadoELIMINAR( tcEstado )
		return llVotacion
	endfunc
	...
	*--------------------------------------------------------------------------------------------------------
	function VotacionCambioEstadoMODIFICAR( tcEstado as string ) as Boolean
		*!*Se ejecuta antes de hacer el cambio de estado
		local llVotacion as boolean
		llVotacion = dodefault()
		llVotacion = llVotacion and this.oCompCUENTABANCARIADESCARGADECHEQUES.votarCambioEstadoMODIFICAR( tcEstado )
		llVotacion = llVotacion and this.ChequesDetalle.oItem.oCompDESCARGADECHEQUES.votarCambioEstadoMODIFICAR( tcEstado )
		return llVotacion
	endfunc
	...
	*--------------------------------------------------------------------------------------------------------
	function VotacionCambioEstadoCANCELAR( tcEstado as string ) as Boolean
		*!*Se ejecuta antes de hacer el cambio de estado
		local llVotacion as boolean
		llVotacion = dodefault()
		llVotacion = llVotacion and this.oCompCUENTABANCARIADESCARGADECHEQUES.votarCambioEstadoCANCELAR( tcEstado )
		llVotacion = llVotacion and this.ChequesDetalle.oItem.oCompDESCARGADECHEQUES.votarCambioEstadoCANCELAR( tcEstado )
		return llVotacion
	endfunc
	...
	*--------------------------------------------------------------------------------------------------------
	function VotacionCambioEstadoGRABAR( tcEstado as string ) as Boolean
		*!*Se ejecuta antes de hacer el cambio de estado
		local llVotacion as boolean
		llVotacion = dodefault()
		llVotacion = llVotacion and this.oCompCUENTABANCARIADESCARGADECHEQUES.votarCambioEstadoGRABAR( tcEstado )
		llVotacion = llVotacion and this.ChequesDetalle.oItem.oCompDESCARGADECHEQUES.votarCambioEstadoGRABAR( tcEstado )
		return llVotacion
	endfunc
	...
	*--------------------------------------------------------------------------------------------------------
	function VotacionCambioEstadoANULAR( tcEstado as string ) as Boolean
		*!*Se ejecuta antes de hacer el cambio de estado
		local llVotacion as boolean
		llVotacion = dodefault()
		llVotacion = llVotacion and this.oCompCUENTABANCARIADESCARGADECHEQUES.votarCambioEstadoANULAR( tcEstado )
		llVotacion = llVotacion and this.ChequesDetalle.oItem.oCompDESCARGADECHEQUES.votarCambioEstadoANULAR( tcEstado )
		return llVotacion
	endfunc
	...
	*--------------------------------------------------------------------------------------------------------
	function ValidacionBasica() as boolean
		...
		With This
			llRetorno = .ValidarDestinodescarga() and llRetorno
	...
	*--------------------------------------------------------------------------------------------------------
	function ValidacionDetalles() as Boolean
		...
		llRetorno = dodefault()
		llRetorno = This.ValidarUnDetalle( this.ChequesDetalle, 'Detalle' ) and llRetorno
		return llRetorno
	endfunc
	...
	*--------------------------------------------------------------------------------------------------------
	function ActualizarEstado() as void
		...
		this.oCompCuentaBancariaDescargaDeCheques.lNuevo = this.lNuevo
		this.oCompCuentaBancariaDescargaDeCheques.lEdicion = this.lEdicion
		this.oCompCuentaBancariaDescargaDeCheques.lEliminar = this.lEliminar
	...
	*--------------------------------------------------------------------------------------------------------
	function ValidarLetra() as boolean
		...
		llRetorno = dodefault()
		if empty( this.Letra )
			this.AgregarInformacion( 'Debe cargar el campo Número comprobante', 9005, 'Letra' )
			llRetorno = .F.
		endif
		return llRetorno
	endfunc
	...
	*--------------------------------------------------------------------------------------------------------
	function ValidarDestinoDescarga() as boolean
		local llRetorno as boolean
		llRetorno = dodefault()
		if empty( this.DestinoDescarga_PK )
			this.AgregarInformacion( 'Debe cargar el campo Destino de descarga', 9005, 'DestinoDescarga' )
			llRetorno = .F.
		else
			If this.DestinoDescarga.oAD.ConsultarPorClavePrimaria()
			Else
				this.AgregarInformacion( 'El dato buscado ' + alltrim(transform( this.DestinoDescarga_PK ))+ ' de la entidad ' +  upper( this.DestinoDescarga.cDescripcion ) + ' no existe.')
				llRetorno = .F.
			endif
		endif
		Return llRetorno
	...
	*--------------------------------------------------------------------------------------------------------
	function obtenerAtributosObligatorios() as ZooColeccion
		...
		loAtributosObligatorios.add( 'DESTINODESCARGA' )
	...
	*--------------------------------------------------------------------------------------------------------
	Protected Function _Buscar() As void
		...
		if empty( This.&lcAtributo )
			...
			lcMensaje = 'El dato buscado NÚMERO COMPROBANTE ' + transform( This.Letra ) + ', PTO. VENTA ' + transform( This.PuntoDeVenta ) + ', NÚMERO ' + transform( This.Numero ) + ' de la entidad ' + upper( this.cDescripcion ) + ' no existe.'
		else
	...
	*--------------------------------------------------------------------------------------------------------
	function SetearColeccionSentenciasAnterior_NUEVO() as Void
		...
		dodefault()
		this.oCompCUENTABANCARIADESCARGADECHEQUES.SetearColeccionSentenciasAnterior_NUEVO()
		this.ChequesDetalle.oItem.oCompDESCARGADECHEQUES.SetearColeccionSentenciasAnterior_NUEVO()
	...
	*--------------------------------------------------------------------------------------------------------
	function SetearColeccionSentenciasAnterior_MODIFICAR() as Void
		...
		dodefault()
		this.oCompCUENTABANCARIADESCARGADECHEQUES.SetearColeccionSentenciasAnterior_MODIFICAR()
		this.ChequesDetalle.oItem.oCompDESCARGADECHEQUES.SetearColeccionSentenciasAnterior_MODIFICAR()
	...
	*--------------------------------------------------------------------------------------------------------
	function SetearColeccionSentenciasAnterior_ANULAR() as Void
		...
		dodefault()
		this.oCompCUENTABANCARIADESCARGADECHEQUES.SetearColeccionSentenciasAnterior_ANULAR()
		this.ChequesDetalle.oItem.oCompDESCARGADECHEQUES.SetearColeccionSentenciasAnterior_ANULAR()
	...
	*--------------------------------------------------------------------------------------------------------
	function SetearColeccionSentenciasAnterior_ELIMINAR() as Void
		...
		dodefault()
		this.oCompCUENTABANCARIADESCARGADECHEQUES.SetearColeccionSentenciasAnterior_ELIMINAR()
		this.ChequesDetalle.oItem.oCompDESCARGADECHEQUES.SetearColeccionSentenciasAnterior_ELIMINAR()
	
	Endfunc
	*--------------------------------------------------------------------------------------------------------
	function ObtenerFuncionalidades() as String
		return '<NOIMPO><NOEXPO><VENTAS><ANULABLE>'
	Endfunc
	*--------------------------------------------------------------------------------------------------------
	...
	*--------------------------------------------------------------------------------------------------------
	function Inicializar() as Void
		...
		this.oProveedorAtributosAnulacion = newobject( "din_entidadDESCARGADECHEQUESAtributosAnulacion", "din_entidadDESCARGADECHEQUESAtributosAnulacion.prg" )
		...
		oICol = goServicios.SaltosDeCampoYValoresSugeridos.ObtenerValorSugeridoDeUnaEntidadDetalle( 'DESCARGADECHEQUES', '')
	...
	*--------------------------------------------------------------------------------------------------------
	function Destinodescarga_PK_Assign( txVal as variant ) as void

		Local lxValOld As Variant
		lxValOld = this.Destinodescarga_PK
		local lxVal as Variant
		local loSugerido as Object
		
		lxVal = txVal
		lxVal = iif( len( lxVal ) < 10, padr( lxVal, 10 ), lxVal )
		this.Destinodescarga_PK = lxVal

		if this.Validar_Destinodescarga( lxVal, lxValOld )
			This.Setear_Destinodescarga( lxVal )
		endif
		dodefault( lxVal )
	...
	*--------------------------------------------------------------------------------------------------------
	function Setear_Destinodescarga( txVal as variant ) as void

		this.Destinodescarga.CODIGO = txVal
		dodefault( txVal )
	...
	*--------------------------------------------------------------------------------------------------------
	function Validar_Destinodescarga( txVal as variant, txValOld as variant ) as Boolean
	...
	*--------------------------------------------------------------------------------------------------------
	function Destinodescarga_Access() as variant
		if this.ldestroy
		else
			if This.lInstanciarSubEntidadaDemanda
				if ( !vartype( this.Destinodescarga ) = 'O' or isnull( this.Destinodescarga ) )
					this.Destinodescarga = _Screen.zoo.instanciarentidad( 'Destinodescargacheques' )
					this.Destinodescarga.lEsSubEntidad = .t.
					this.enlazar( 'Destinodescarga.EventoObtenerLogueo', 'inyectarLogueo' )
					this.enlazar( 'Destinodescarga.EventoObtenerInformacion', 'inyectarInformacion' )
				endif
				if !this.lBuscandoCodigo
					if this.Destinodescarga.CODIGO # this.Destinodescarga_PK
						this.Destinodescarga.CODIGO = this.Destinodescarga_PK
					endif
				endif
			endif
		endif
		return this.Destinodescarga
	endfunc
	...
	*--------------------------------------------------------------------------------------------------------
	function oCompCuentabancariadescargadecheques_Access() as variant
		if !this.ldestroy and ( !vartype( this.oCompCuentabancariadescargadecheques ) = 'O' or isnull( this.oCompCuentabancariadescargadecheques ) )
			this.oCompCuentabancariadescargadecheques = _Screen.zoo.InstanciarComponente( 'ComponenteCuentabancariadescargadecheques' )
		this.oCompCuentabancariadescargadecheques.Inicializar()
		this.enlazar( 'oCompCuentabancariadescargadecheques.EventoObtenerLogueo', 'inyectarLogueo' )
		this.enlazar( 'oCompCuentabancariadescargadecheques.EventoObtenerInformacion', 'inyectarInformacion' )
		endif
		return this.oCompCuentabancariadescargadecheques
	endfunc
	...
	*--------------------------------------------------------------------------------------------------------
	...
	endfunc
	*--------------------------------------------------------------------------------------------------------
	Function ValorSugeridoDestinodescarga() as void
		local lcExpresionValorSugeridoDefinidoPorElUsuario as String, lvValorSugeridoDefinidoPorElUsuario as Variant, loError as Object, loEx as Object, loExTipoDato as Object
		lcExpresionValorSugeridoDefinidoPorElUsuario = null
		with this
			try
				lcExpresionValorSugeridoDefinidoPorElUsuario = goServicios.SaltosDeCampoYValoresSugeridos.ObtenerValorSugerido( "DESCARGADECHEQUES", "", "Destinodescarga" )
				if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
					if !empty( lcExpresionValorSugeridoDefinidoPorElUsuario )
						...
						This.EventoEstaSeteandoValorSugerido( 'Destinodescarga_PK' )
						.Destinodescarga_PK = lvValorSugeridoDefinidoPorElUsuario
						.Destinodescarga.CODIGO = .Destinodescarga_PK
					endif
				endif
			Catch to loError
				.Destinodescarga_PK=[]
				...
				if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
					...
					loEx.AgregarInformacion( "Se produjo un error al intentar asignar el valor sugerido " + lcExpresionValorSugeridoDefinidoPorElUsuario + " del atributo Destino de descarga para la entidad Descarga de cheques de terceros de tipo discontinuado." + chr( 10 ) + "Verifique el valor especificado en la entidad Comportamiento de atributos." + chr( 10 ) + "El tipo de dato del atributo es caracter." )
	...
	*--------------------------------------------------------------------------------------------------------
	Function ValorSugeridoConcepto() as void
		...
		with this
			try
				lcExpresionValorSugeridoDefinidoPorElUsuario = goServicios.SaltosDeCampoYValoresSugeridos.ObtenerValorSugerido( "DESCARGADECHEQUES", "", "Concepto" )
				if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
					if !empty( lcExpresionValorSugeridoDefinidoPorElUsuario )
						lvValorSugeridoDefinidoPorElUsuario = &lcExpresionValorSugeridoDefinidoPorElUsuario
						if vartype( lvValorSugeridoDefinidoPorElUsuario ) # "C"
	...
	*--------------------------------------------------------------------------------------------------------
	Function ValorSugeridoFecha() as void
		...
		lcExpresionValorSugeridoDefinidoPorElUsuario = null
		with this
			lcExpresionValorSugeridoDefinidoPorElUsuario = goServicios.SaltosDeCampoYValoresSugeridos.ObtenerValorSugerido( "DESCARGADECHEQUES", "", "Fecha" )
			if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
				if !empty( lcExpresionValorSugeridoDefinidoPorElUsuario )
					try
						lvValorSugeridoDefinidoPorElUsuario = &lcExpresionValorSugeridoDefinidoPorElUsuario
	...
	*--------------------------------------------------------------------------------------------------------
	Function ValorSugeridoLetra() as void
		...
		lcExpresionValorSugeridoDefinidoPorElUsuario = null
		with this
			lcExpresionValorSugeridoDefinidoPorElUsuario = goServicios.SaltosDeCampoYValoresSugeridos.ObtenerValorSugerido( "DESCARGADECHEQUES", "", "Letra" )
			if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
				if !empty( lcExpresionValorSugeridoDefinidoPorElUsuario )
					try
						lvValorSugeridoDefinidoPorElUsuario = &lcExpresionValorSugeridoDefinidoPorElUsuario
	...
	*--------------------------------------------------------------------------------------------------------
	Function ValorSugeridoPuntodeventa() as void
		...
		lcExpresionValorSugeridoDefinidoPorElUsuario = null
		with this
			lcExpresionValorSugeridoDefinidoPorElUsuario = goServicios.SaltosDeCampoYValoresSugeridos.ObtenerValorSugerido( "DESCARGADECHEQUES", "", "Puntodeventa" )
			if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
				if !empty( lcExpresionValorSugeridoDefinidoPorElUsuario )
					try
						lvValorSugeridoDefinidoPorElUsuario = &lcExpresionValorSugeridoDefinidoPorElUsuario
	...
	*--------------------------------------------------------------------------------------------------------
	Function ValorSugeridoNumero() as void
		...
		lcExpresionValorSugeridoDefinidoPorElUsuario = null
		with this
			lcExpresionValorSugeridoDefinidoPorElUsuario = goServicios.SaltosDeCampoYValoresSugeridos.ObtenerValorSugerido( "DESCARGADECHEQUES", "", "Numero" )
			if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
				if !empty( lcExpresionValorSugeridoDefinidoPorElUsuario )
					try
						lvValorSugeridoDefinidoPorElUsuario = &lcExpresionValorSugeridoDefinidoPorElUsuario
	...
	*--------------------------------------------------------------------------------------------------------
	Function ValorSugeridoObserv() as void
		...
		lcExpresionValorSugeridoDefinidoPorElUsuario = null
		with this
			lcExpresionValorSugeridoDefinidoPorElUsuario = goServicios.SaltosDeCampoYValoresSugeridos.ObtenerValorSugerido( "DESCARGADECHEQUES", "", "Observ" )
			if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
				if !empty( lcExpresionValorSugeridoDefinidoPorElUsuario )
					try
						lvValorSugeridoDefinidoPorElUsuario = &lcExpresionValorSugeridoDefinidoPorElUsuario
	...
	*--------------------------------------------------------------------------------------------------------
	function Fecha_DespuesDeAsignar() as void
		this.ProcesarDespuesDeSetearFecha()                                                                 
	endfunc
	...
	*--------------------------------------------------------------------------------------------------------
	function Destroy()
		...
		this.lDestroy = .t.
		this.oCompCuentaBancariaDescargaDeCheques = null
