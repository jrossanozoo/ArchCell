...
define class Mock_selechequeainsertar as MockBase of MockBase.prg
	...
	Codigo = ""                                                                                                                                                                                                                                               
	CriterioDeSeleccion = 0                                                                                                                                                                                                                                   
	DetalleChequesAInsertar = null                                                                                                                                                                                                                            
	Diferencia = 0                                                                                                                                                                                                                                            
	ESTADOTRANSFERENCIA = ""                                                                                                                                                                                                                                  
	FECHAALTAFW = ctod( '  /  /    ' )                                                                                                                                                                                                                        
	FECHAEXPO = ctod( '  /  /    ' )                                                                                                                                                                                                                          
	...
	cCaracteres = ""                                                                                                                                                                                                                                          
	cContexto = ""                                                                                                                                                                                                                                            
	cDescripcion =  "Selección de cheques"                                                                                                                                                                                                                    
	...
	cXMLChequesDeTercerosPendientes = ""                                                                                                                                                                                                                      
	cXMLChequesDeTercerosSeleccionados = ""                                                                                                                                                                                                                   
	...
	lHabilitarCriterioDeSeleccion = .T.                                                                                                                                                                                                                       
	lHabilitarDiferencia = .T.                                                                                                                                                                                                                                
	lHabilitarOrdenamiento = .T.                                                                                                                                                                                                                              
	lHabilitarSaldoPendiente = .T.                                                                                                                                                                                                                            
	...
	*-----------------------------------------------------------------------------------------
	function aplicarcriterioseleccion( tccursor as string ) as void                                                                                                                                                                                           
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		local lxParam1
		...
		return this.ObtenerResultado( 'Aplicarcriterioseleccion', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function cambiosdetalledetallechequesainsertar() as void                                                                                                                                                                                                  
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Cambiosdetalledetallechequesainsertar', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function cargarchequesdetercerosseleccionados( tcaccion as string ) as void                                                                                                                                                                               
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		local lxParam1
		...
		return this.ObtenerResultado( 'Cargarchequesdetercerosseleccionados', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function criteriodeseleccion_assign( txval as variant ) as void                                                                                                                                                                                           
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'SELECHEQUEAINSERTAR' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'criteriodeseleccion_assign', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'criteriodeseleccion_assign', lcParametros )			endif		endif		Local lxValOld As Variant		lxValOld = this.Criteriodeseleccion		local lxVal as Variant		lxVal = txVal		if This.CargaManual()			if this.Validar_Criteriodeseleccion( lxVal )				if ( this.lHabilitarCriteriodeseleccion or this.lEstaSeteandoValorSugerido or lxValOld == lxVal)					this.Setear_Criteriodeseleccion( lxVal )				else					this.Criteriodeseleccion = lxValOld					goServicios.Errores.LevantarExcepcion( "No se puede cambiar este valor (Entidad: Selección de cheques - Atributo: Criteriodeseleccion)" )				endif			EndIf		Else			This.Setear_Criteriodeseleccion( lxVal )		EndIf		dodefault( lxVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function detallechequesainsertar_access() as variant                                                                                                                                                                                                      
		lnParametros = pcount()lcParametros = ''		lnIdClase = _screen.Mocks.BuscarMock( 'SELECHEQUEAINSERTAR' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'detallechequesainsertar_access', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'detallechequesainsertar_access', lcParametros )			endif		endif		if !this.ldestroy and ( !vartype( this.Detallechequesainsertar ) = 'O' or isnull( this.Detallechequesainsertar ) )			this.Detallechequesainsertar = this.crearobjeto( 'Din_DetalleSelechequeainsertarDetallechequesainsertar' )			this.Detallechequesainsertar.inicializar()			this.enlazar( 'Detallechequesainsertar.EventoObtenerLogueo', 'inyectarLogueo' )			this.enlazar( 'Detallechequesainsertar.EventoObtenerInformacion', 'inyectarInformacion' )			bindevent( this.Detallechequesainsertar, 'CambioSumarizado', this, 'CambiosDetalleDetallechequesainsertar', 1)			this.enlazar( 'Detallechequesainsertar.EventoAdvertirLimitePorDiseno', 'EventoAdvertirLimitePorDiseno')			this.enlazar( 'Detallechequesainsertar.EventoCancelarCargaLimitePorDiseno', 'EventoCancelarCargaLimitePorDiseno')		endif		return this.Detallechequesainsertar
	EndFunc

	*-----------------------------------------------------------------------------------------
	function diferencia_assign( txval as variant ) as void                                                                                                                                                                                                    
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'SELECHEQUEAINSERTAR' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'diferencia_assign', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'diferencia_assign', lcParametros )			endif		endif		Local lxValOld As Variant		lxValOld = this.Diferencia		local lxVal as Variant		lxVal = txVal		if This.CargaManual()			if this.Validar_Diferencia( lxVal )				if ( this.lHabilitarDiferencia or this.lEstaSeteandoValorSugerido or lxValOld == lxVal)					this.Setear_Diferencia( lxVal )				else					this.Diferencia = lxValOld					goServicios.Errores.LevantarExcepcion( "No se puede cambiar este valor (Entidad: Selección de cheques - Atributo: Diferencia)" )				endif			EndIf		Else			This.Setear_Diferencia( lxVal )		EndIf		dodefault( lxVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function eventovaciargrilla() as void                                                                                                                                                                                                                     
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Eventovaciargrilla', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function saldopendiente_assign( txval as variant ) as void                                                                                                                                                                                                
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'SELECHEQUEAINSERTAR' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'saldopendiente_assign', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'saldopendiente_assign', lcParametros )			endif		endif		Local lxValOld As Variant		lxValOld = this.Saldopendiente		local lxVal as Variant		lxVal = txVal		if This.CargaManual()			if this.Validar_Saldopendiente( lxVal )				if ( this.lHabilitarSaldopendiente or this.lEstaSeteandoValorSugerido or lxValOld == lxVal)					this.Setear_Saldopendiente( lxVal )				else					this.Saldopendiente = lxValOld					goServicios.Errores.LevantarExcepcion( "No se puede cambiar este valor (Entidad: Selección de cheques - Atributo: Saldopendiente)" )				endif			EndIf		Else			This.Setear_Saldopendiente( lxVal )		EndIf		dodefault( lxVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function setear_criteriodeseleccion( txval as variant ) as void                                                                                                                                                                                           
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'SELECHEQUEAINSERTAR' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'setear_criteriodeseleccion', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'setear_criteriodeseleccion', lcParametros )			endif		endif		local llRetorno as Boolean				llRetorno = dodefault( txVal )		if llRetorno and this.SaldoPendiente > 0			this.CargarChequesDeTercerosPendientes()		endif
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function setear_saldopendiente( txval as variant ) as void                                                                                                                                                                                                
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'SELECHEQUEAINSERTAR' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'setear_saldopendiente', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'setear_saldopendiente', lcParametros )			endif		endif		this.Saldopendiente = txVal		dodefault( txVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function validar_criteriodeseleccion( txval as variant ) as boolean                                                                                                                                                                                       
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'SELECHEQUEAINSERTAR' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'validar_criteriodeseleccion', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'validar_criteriodeseleccion', lcParametros )			endif		endif		Return dodefault( txVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function validar_saldopendiente( txval as variant ) as boolean                                                                                                                                                                                            
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'SELECHEQUEAINSERTAR' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'validar_saldopendiente', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'validar_saldopendiente', lcParametros )			endif		endif		Return dodefault( txVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function valorsugeridocriteriodeseleccion() as void                                                                                                                                                                                                       
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Valorsugeridocriteriodeseleccion', lcParametros )
	EndFunc

	*-----------------------------------------------------------------------------------------
	function valorsugeridofwcriteriodeseleccion() as void                                                                                                                                                                                                     
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Valorsugeridofwcriteriodeseleccion', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	protected function cargarchequesdetercerospendientes() as void                                                                                                                                                                                            
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Cargarchequesdetercerospendientes', lcParametros )
	EndFunc
