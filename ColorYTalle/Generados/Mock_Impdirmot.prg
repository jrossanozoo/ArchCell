...
define class Mock_impdirmot as MockBase of MockBase.prg
	...
	cCaracteres = ""                                                                                                                                                                                                                                          
	cContexto = ""                                                                                                                                                                                                                                            
	cDescripcion =  "Imputación de motivo a cuenta contable"                                                                                                                                                                                                  
	...
	*-----------------------------------------------------------------------------------------
	function f_motivo_desde_access() as variant                                                                                                                                                                                                               
		lnParametros = pcount()lcParametros = ''		lnIdClase = _screen.Mocks.BuscarMock( 'IMPDIRMOT' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'f_motivo_desde_access', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'f_motivo_desde_access', lcParametros )			endif		endif		if this.ldestroy		else			if This.lInstanciarSubEntidadaDemanda				if ( !vartype( this.F_motivo_desde ) = 'O' or isnull( this.F_motivo_desde ) )					this.F_motivo_desde = _Screen.zoo.instanciarentidad( 'Motivo' )					this.F_motivo_desde.lEsSubEntidad = .t.					this.enlazar( 'F_motivo_desde.EventoObtenerLogueo', 'inyectarLogueo' )					this.enlazar( 'F_motivo_desde.EventoObtenerInformacion', 'inyectarInformacion' )				endif				if !this.lBuscandoCodigo					if this.F_motivo_desde.CODIGO # this.F_motivo_desde_PK						this.F_motivo_desde.CODIGO = this.F_motivo_desde_PK					endif				endif			endif		endif		return this.F_motivo_desde
	EndFunc

	*-----------------------------------------------------------------------------------------
	function f_motivo_desde_pk_assign( txval as variant ) as void                                                                                                                                                                                             
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'IMPDIRMOT' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'f_motivo_desde_pk_assign', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'f_motivo_desde_pk_assign', lcParametros )			endif		endif		Local lxValOld As Variant		lxValOld = this.F_motivo_desde_PK		local lxVal as Variant		local loSugerido as Object				lxVal = txVal		lxVal = iif( len( lxVal ) < 3, padr( lxVal, 3 ), lxVal )		this.F_motivo_desde_PK = lxVal		dodefault( lxVal )
	EndFunc

	*-----------------------------------------------------------------------------------------
	function f_motivo_hasta_access() as variant                                                                                                                                                                                                               
		lnParametros = pcount()lcParametros = ''		lnIdClase = _screen.Mocks.BuscarMock( 'IMPDIRMOT' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'f_motivo_hasta_access', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'f_motivo_hasta_access', lcParametros )			endif		endif		if this.ldestroy		else			if This.lInstanciarSubEntidadaDemanda				if ( !vartype( this.F_motivo_hasta ) = 'O' or isnull( this.F_motivo_hasta ) )					this.F_motivo_hasta = _Screen.zoo.instanciarentidad( 'Motivo' )					this.F_motivo_hasta.lEsSubEntidad = .t.					this.enlazar( 'F_motivo_hasta.EventoObtenerLogueo', 'inyectarLogueo' )					this.enlazar( 'F_motivo_hasta.EventoObtenerInformacion', 'inyectarInformacion' )				endif				if !this.lBuscandoCodigo					if this.F_motivo_hasta.CODIGO # this.F_motivo_hasta_PK						this.F_motivo_hasta.CODIGO = this.F_motivo_hasta_PK					endif				endif			endif		endif		return this.F_motivo_hasta
	EndFunc

	*-----------------------------------------------------------------------------------------
	function f_motivo_hasta_pk_assign( txval as variant ) as void                                                                                                                                                                                             
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'IMPDIRMOT' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'f_motivo_hasta_pk_assign', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'f_motivo_hasta_pk_assign', lcParametros )			endif		endif		Local lxValOld As Variant		lxValOld = this.F_motivo_hasta_PK		local lxVal as Variant		local loSugerido as Object				lxVal = txVal		lxVal = iif( len( lxVal ) < 3, padr( lxVal, 3 ), lxVal )		this.F_motivo_hasta_PK = lxVal		dodefault( lxVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function setear_f_motivo_desde( txval as variant ) as void                                                                                                                                                                                                
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'IMPDIRMOT' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'setear_f_motivo_desde', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'setear_f_motivo_desde', lcParametros )			endif		endif		this.F_motivo_desde.CODIGO = txVal		dodefault( txVal )
	EndFunc

	*-----------------------------------------------------------------------------------------
	function setear_f_motivo_hasta( txval as variant ) as void                                                                                                                                                                                                
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'IMPDIRMOT' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'setear_f_motivo_hasta', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'setear_f_motivo_hasta', lcParametros )			endif		endif		this.F_motivo_hasta.CODIGO = txVal		dodefault( txVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function validar_f_motivo_desde( txval as variant, txvalold as variant ) as boolean                                                                                                                                                                       
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1, lxParam2lnParametros = pcount()lxParam1 = txvallxParam2 = txvaloldlcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'IMPDIRMOT' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'validar_f_motivo_desde', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'validar_f_motivo_desde', lcParametros )			endif		endif		Return This.CargaManual() and dodefault( txVal, txValOld )
	EndFunc

	*-----------------------------------------------------------------------------------------
	function validar_f_motivo_hasta( txval as variant, txvalold as variant ) as boolean                                                                                                                                                                       
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1, lxParam2lnParametros = pcount()lxParam1 = txvallxParam2 = txvaloldlcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'IMPDIRMOT' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'validar_f_motivo_hasta', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'validar_f_motivo_hasta', lcParametros )			endif		endif		Return This.CargaManual() and dodefault( txVal, txValOld )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function validarf_motivo_desde() as boolean                                                                                                                                                                                                               
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Validarf_motivo_desde', lcParametros )
	EndFunc

	*-----------------------------------------------------------------------------------------
	function validarf_motivo_hasta() as boolean                                                                                                                                                                                                               
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Validarf_motivo_hasta', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function valorsugeridof_motivo_desde() as void                                                                                                                                                                                                            
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Valorsugeridof_motivo_desde', lcParametros )
	EndFunc

	*-----------------------------------------------------------------------------------------
	function valorsugeridof_motivo_hasta() as void                                                                                                                                                                                                            
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Valorsugeridof_motivo_hasta', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function valorsugeridofwf_motivo_hasta() as void                                                                                                                                                                                                          
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Valorsugeridofwf_motivo_hasta', lcParametros )
	EndFunc
