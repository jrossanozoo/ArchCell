...
define class Din_ItemTICKETNOTADECREDITOImpuestosdetalle as ItemIMPUESTOS of ItemIMPUESTOS.prg
	...
	*--------------------------------------------------------------------------------------------------------
	function Inicializar() as boolean
		...
		oICol = goServicios.SaltosDeCampoYValoresSugeridos.ObtenerValorSugeridoDeUnaEntidadDetalle( 'TICKETNOTADECREDITO', 'Impuestosdetalle')
	...
	*--------------------------------------------------------------------------------------------------------
	function Montodeimpuestointerno_Assign( txVal as variant ) as void
		...
		lxValOld = this.Montodeimpuestointerno
		local lxVal as Variant
		lxVal = txVal

		if This.CargaManual()
			if this.Validar_Montodeimpuestointerno( lxVal )
				this.EventoAntesDeSetear( This, 'Montodeimpuestointerno', lxValOld, lxVal  )
				this.Setear_Montodeimpuestointerno( lxVal )
				this.EventoDespuesDeSetear( This, 'Montodeimpuestointerno', lxValOld, lxVal  )
				If Not (lxVal == lxValOld) And This.PuedeEjecutarCambioSumarizado()
					this.AcumularSumarizado( .t., 'Montodeimpuestointerno', lxVal, lxValOld)
				EndIf
				If lxValOld != lxVal
					If This.PuedeEjecutarHaCambiado()
						this.HaCambiado( 'Montodeimpuestointerno', This )
					Endif
				EndIf
			EndIf
		Else
	...
	*--------------------------------------------------------------------------------------------------------
	function Montodeimpuestointernosindescuento_Assign( txVal as variant ) as void
		...
		Local lxValOld As Variant
		lxValOld = this.Montodeimpuestointernosindescuento
		local lxVal as Variant
		lxVal = txVal

		if This.CargaManual()
			if this.Validar_Montodeimpuestointernosindescuento( lxVal )
				this.EventoAntesDeSetear( This, 'Montodeimpuestointernosindescuento', lxValOld, lxVal  )
				this.Setear_Montodeimpuestointernosindescuento( lxVal )
				this.EventoDespuesDeSetear( This, 'Montodeimpuestointernosindescuento', lxValOld, lxVal  )
				If Not (lxVal == lxValOld) And This.PuedeEjecutarCambioSumarizado()
					this.AcumularSumarizado( .t., 'Montodeimpuestointernosindescuento', lxVal, lxValOld)
				EndIf
				If lxValOld != lxVal
					If This.PuedeEjecutarHaCambiado()
						this.HaCambiado( 'Montodeimpuestointernosindescuento', This )
					Endif
				EndIf
			EndIf
		Else
	...
	*--------------------------------------------------------------------------------------------------------
	function Setear_Montodeimpuestointerno( txVal as variant ) as void
		...
		this.Montodeimpuestointerno = txVal
		dodefault( txVal )
	...
	*--------------------------------------------------------------------------------------------------------
	function Setear_Montodeimpuestointernosindescuento( txVal as variant ) as void
		...
		this.Montodeimpuestointernosindescuento = txVal
		dodefault( txVal )
