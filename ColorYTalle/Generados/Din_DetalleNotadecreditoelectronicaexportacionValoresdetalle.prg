...
define class Din_DetalleNOTADECREDITOELECTRONICAEXPORTACIONValoresdetalle as DetalleValoresventa of DetalleValoresventa.Prg
	...
	*--------------------------------------------------------------------------------------------------------
	function oItem_Access() as variant
		if !this.ldestroy and ( !vartype( this.oItem ) = 'O' or isnull( this.oItem ) )
			this.oItem = _Screen.zoo.crearobjeto( 'Din_ItemNOTADECREDITOELECTRONICAEXPORTACIONValoresdetalle' )
		...
		for lni = 1 to this.Count
			...
			if vartype( this.Item[lni].RECARGOMONTO) = 'N'
				...
				if lnLargoEntero <= 10
					...
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud entera del valor del atributo RECARGOMONTO del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform(lnLargoEntero) + ' Máxima:10), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
			...
			if vartype( this.Item[lni].MONTODESREC) = 'N'
				...
				if lnLargoEntero <= 10
					...
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud entera del valor del atributo MONTODESREC del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform(lnLargoEntero) + ' Máxima:10), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
