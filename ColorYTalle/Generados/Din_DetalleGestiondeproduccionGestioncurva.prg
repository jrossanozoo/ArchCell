...
define class Din_DetalleGESTIONDEPRODUCCIONGestioncurva as DetalleBaseproduccion of DetalleBaseproduccion.Prg
	...
	Protected Function _CargarItem( tnItem as integer )
		...
		With this.oItem
			Try
				...
				.CantProducida = this.Item[ tnItem ].CantProducida
				llVacio = llVacio and this.VerificarSiElAtributoEstaVacio( 'CantProducida', .CantProducida )
	...
	*--------------------------------------------------------------------------------------------------------
	protected function AplanarItem() as Object
		...
		with loRetorno
			...
			.CantProducida = this.oItem.CantProducida
	...
	*--------------------------------------------------------------------------------------------------------
	...
	with this
		if .ValidarItem()
			...
			.Item[ lnItem ].CantProducida = .oItem.CantProducida
	...
	*--------------------------------------------------------------------------------------------------------
	function oItem_Access() as variant
		if !this.ldestroy and ( !vartype( this.oItem ) = 'O' or isnull( this.oItem ) )
			this.oItem = _Screen.zoo.crearobjeto( 'ItemGESTIONDEPRODUCCIONGestioncurva' )
	...
	*--------------------------------------------------------------------------------------------------------
	Function Cargar() as Void
		...
		select c_Gestioncurva
		lcFiltro = this.ObtenerFiltro()
		scan for &lcFiltro
			loItem = createobject('ItemAuxiliar')

			loItem.Codigo = c_Gestioncurva.Codigo
			loItem.Comportamiento = c_Gestioncurva.Comportamiento
			loItem.IdItemInsumo = c_Gestioncurva.IdItemInsumo
			loItem.NROITEM = c_Gestioncurva.NROITEM
			loItem.Articulo_PK = c_Gestioncurva.Articulo
			loItem.ArticuloDetalle = c_Gestioncurva.ArticuloDetalle
			loItem.ColorM_PK = c_Gestioncurva.ColorM
			loItem.ColorMDetalle = c_Gestioncurva.ColorMDetalle
			loItem.TalleM_PK = c_Gestioncurva.TalleM
			loItem.TalleMDetalle = c_Gestioncurva.TalleMDetalle
			loItem.Insumo_PK = c_Gestioncurva.Insumo
			loItem.InsumoDetalle = c_Gestioncurva.InsumoDetalle
			loItem.Color_PK = c_Gestioncurva.Color
			loItem.ColorDetalle = c_Gestioncurva.ColorDetalle
			loItem.Talle_PK = c_Gestioncurva.Talle
			loItem.TalleDetalle = c_Gestioncurva.TalleDetalle
			loItem.Partida = c_Gestioncurva.Partida
			loItem.CantProducida = c_Gestioncurva.CantProducida
			loItem.CantDescarte = c_Gestioncurva.CantDescarte
			loItem.NroItem = lnI 
	...
	*-----------------------------------------------------------------------------------------
	...
	local lnLargoEntero as integer, lnValorDelDecimal as integer, lnLargoDecimal as integer, llRetorno as boolean, lnI as Integer
		...
		for lni = 1 to this.Count
			...
			if vartype( this.Item[lni].CANTPRODUCIDA) = 'N'
				lnLargoEntero = len( transform( int( this.Item[lni].CANTPRODUCIDA ) ) ) 
				lnValorDelDecimal = this.Item[lni].CANTPRODUCIDA - int( this.Item[lni].CANTPRODUCIDA )
				lnLargoDecimal = iif( lnValorDelDecimal = 0, 0, len( transform( lnValorDelDecimal ) ) - 2 )
				if lnLargoEntero <= 8
				else
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud entera del valor del atributo CANTPRODUCIDA del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform(lnLargoEntero) + ' Máxima:8), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
				this.AgregarInformacion( 'El tipo de dato para el atributo CANTPRODUCIDA del item ' + transform( lni ) + ' no es el correcto, en el detalle ' + this.cEtiqueta + '.')
			Endif
	...
	*--------------------------------------------------------------------------------------------------------
	function CopiarItemaItem( toItemOrigen as Object, toItemDestino as Object ) as Object
		...
		toItemDestino.Cantproducida = toItemOrigen.Cantproducida
