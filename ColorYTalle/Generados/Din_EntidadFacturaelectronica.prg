...
define class Din_EntidadFACTURAELECTRONICA as EntColorYTalle_ComprobanteElectronicoNacional of EntColorYTalle_ComprobanteElectronicoNacional.prg
	...
	*--------------------------------------------------------------------------------------------------------
	Protected Function _Buscar() As void
		...
		if empty( This.&lcAtributo )
			...
			lcMensaje = 'El dato buscado TIPO DE COMPROBANTE ' + transform( This.TipoComprobante ) + ', LETRA ' + transform( This.Letra ) + ', PUNTO DE VENTA ' + transform( This.PuntoDeVenta ) + ', NÚMERO ' + transform( This.Numero ) + ', SECUENCIA ' + transform( This.Secuencia ) + ' de la entidad ' + upper( this.cDescripcion ) + ' no existe.'
		else
	...
	*--------------------------------------------------------------------------------------------------------
	function ObtenerFuncionalidades() as String
		return '<ANULABLE><VENTAS><WEBHOOK><CONVALORES><FISCAL><CE><PROMO><NOLISTAGENERICO><PICKING><INTERVINIENTE><VALIDACENTRALIZADOR><VALORCIERRE><DESCUENTOAUTOMATICO><ACTUALIZAEMAILCLIENTE><MIPYME:54><LISTARMODIFICACION>'
	Endfunc
	*--------------------------------------------------------------------------------------------------------
	...
	*--------------------------------------------------------------------------------------------------------
	function Inicializar() as Void
		...
		this.lWHIngresar = goServicios.WebHook.TieneQueMandar( 'FACTURAELECTRONICA', 'INGRESAR' )
		this.lWHModificar = goServicios.WebHook.TieneQueMandar( 'FACTURAELECTRONICA', 'MODIFICAR' )
		this.lWHEliminar = goServicios.WebHook.TieneQueMandar( 'FACTURAELECTRONICA', 'ELIMINAR' )
	endfunc
	...
	function Monedacomprobante_PK_Assign( txVal as variant ) as void
		...
		if this.Validar_Monedacomprobante( lxVal, lxValOld )
			if ( this.lHabilitarMonedacomprobante_PK or this.lEstaSeteandoValorSugerido or lxValOld == lxVal )
				...
				goServicios.Errores.LevantarExcepcion( "No se puede cambiar este valor (Entidad: Factura electrónica de venta - Atributo: Monedacomprobante)" )
			endif 
			If lxValOld != lxVal
				this.EventoCambioParticipantePromocion()
			endif
		...
		with this
			...
			Catch to loError
				...
				if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
					...
					loEx.AgregarInformacion( "Se produjo un error al intentar asignar el valor sugerido " + lcExpresionValorSugeridoDefinidoPorElUsuario + " del atributo Cliente para la entidad Factura electrónica de venta." + chr( 10 ) + "Verifique el valor especificado en la entidad Comportamiento de atributos." + chr( 10 ) + "El tipo de dato del atributo es caracter." )
