...
define class Din_DetallePREPARACIONDEMERCADERIAFacturadetalle as DetalleColorytalle_articulosventa of DetalleColorytalle_articulosventa.Prg
	...
	cEtiqueta = 'Artículos del pedido'
		...
		with this
			...
			if this.lEsNavegacion
				if select( 'c_Facturadetalle' ) > 0 and reccount( 'c_Facturadetalle' ) > 0
					sum Afe_saldo, Ajusteporredondeoconimpuestos, Ajusteporredondeosinimpuestos, Bruto, Monto, Montodescuento, Montoporcentajedescuentoconimpuesto, Neto, Tienerestriccion;
						 to ;
						 lnSum_Afe_saldo, lnSum_Ajusteporredondeoconimpuestos, lnSum_Ajusteporredondeosinimpuestos, lnSum_Bruto, lnSum_Monto, lnSum_Montodescuento, lnSum_Montoporcentajedescuentoconimpuesto, lnSum_Neto, lnSum_Tienerestriccion
					sum Cantidad to lnSum_Cantidad for !IIF(THIS.LESNAVEGACION,c_Facturadetalle.ARTICULO=THIS.CARTICULOSENIA,c_Facturadetalle.ARTICULO_PK=THIS.CARTICULOSENIA)
				endif
					...
					if .oItem.NroItem = lnItem
						...
						with loItem 
							if !empty(.ARTICULO_PK)
								...
								if !IIF(THIS.LESNAVEGACION,.ARTICULO=THIS.CARTICULOSENIA,.ARTICULO_PK=THIS.CARTICULOSENIA)
				...
				if .oItem.ValidarExistenciaCamposFijos() and .oItem.NroItem<=this.count
					...
					with loItem 
						...
						lnSum_Ajusteporredondeosinimpuestos = lnSum_Ajusteporredondeosinimpuestos + .Ajusteporredondeosinimpuestos
						lnSum_Bruto = lnSum_Bruto + .Bruto
						if !IIF(THIS.LESNAVEGACION,this.oItem.ARTICULO=THIS.CARTICULOSENIA,this.oItem.ARTICULO_PK=THIS.CARTICULOSENIA)
							lnSum_Cantidad = lnSum_Cantidad + .Cantidad
						endif
						lnSum_Impuestos = lnSum_Impuestos + .Impuestos
						lnSum_Monto = lnSum_Monto + .Monto
	...
	*--------------------------------------------------------------------------------------------------------
	function oItem_Access() as variant
		if !this.ldestroy and ( !vartype( this.oItem ) = 'O' or isnull( this.oItem ) )
			this.oItem = _Screen.zoo.crearobjeto( 'Din_ItemPREPARACIONDEMERCADERIAFacturadetalle' )
	...
	*--------------------------------------------------------------------------------------------------------
	Function Cargar() as Void
		...
		scan for &lcFiltro
			...
			loItem.Articulo_CondicionIvaVentas = c_Facturadetalle.Articulo_CondicionIvaVentas
			loItem.Articulo_PorcentajeIvaVentas = c_Facturadetalle.Articulo_PorcentajeIvaVentas
			...
			loItem.PrecioSinImpuestosSinRedondear = c_Facturadetalle.PrecioSinImpuestosSinRedondear
			loItem.PrecioConImpuestosSinRedondear = c_Facturadetalle.PrecioConImpuestosSinRedondear
			loItem.PrecioSinRedondear = c_Facturadetalle.PrecioSinRedondear
			...
			loItem.CodigoGTIN = c_Facturadetalle.CodigoGTIN
			loItem.TasaImpuestoInterno = c_Facturadetalle.TasaImpuestoInterno
			loItem.MontoImpuestoInterno = c_Facturadetalle.MontoImpuestoInterno
			loItem.RestringirDescuentos = c_Facturadetalle.RestringirDescuentos
			loItem.Afe_Codigo = c_Facturadetalle.Afe_Codigo
			loItem.TieneRestriccion = c_Facturadetalle.TieneRestriccion
			loItem.CodigoMotivoDevolucion_PK = c_Facturadetalle.CodigoMotivoDevolucion
			loItem.CodigoMotivoDescuento_PK = c_Facturadetalle.CodigoMotivoDescuento
			loItem.CodigoAutorizacionDJCP = c_Facturadetalle.CodigoAutorizacionDJCP
			loItem.Comportamiento = c_Facturadetalle.Comportamiento
			loItem.IdKit = c_Facturadetalle.IdKit
			loItem.afe_SaldoLiquidacion = c_Facturadetalle.afe_SaldoLiquidacion
			loItem.NumeroGiftCard_PK = c_Facturadetalle.NumeroGiftCard
			...
			loItem.MontoProrrateoImpuestoInterno = c_Facturadetalle.MontoProrrateoImpuestoInterno
			loItem.Talle_PK = c_Facturadetalle.Talle
			loItem.TalleDetalle = c_Facturadetalle.TalleDetalle
	...
	*--------------------------------------------------------------------------------------------------------
	function Actualizar( tcClave as string ) as Void
		...
		if this.oItem.TieneDisponible()
			if this.oItem.Comportamiento = 4
				if this.oItem.TieneDisponibleDeParticipantes()
					this.EventoAgregarKits( this.oItem )
				else
					if goParametros.Felino.Generales.PermitirPasarSinStock
						This.EventoNoHayStock( this.oItem.oCompStock.oColErroresParticipantes )
						dodefault( tcClave )
						this.Sumarizar()
					else
						goServicios.Errores.LevantarExcepcion( this.oItem.oCompStock.oColErroresParticipantes )
					endif
				endif
			else
				dodefault( tcClave )
	...
	*--------------------------------------------------------------------------------------------------------
	...
	endfunc
	*--------------------------------------------------------------------------------------------------------
	function EventoAgregarKits( tcKit as String, tnCantidad as Number, tnPrecio as Number, tnDescuento as Number, tnMontoDescuento as Number ) as Void
	endfunc
