...
define class Mock_retirodeefectivo as MockBase of MockBase.prg
	...
	*-----------------------------------------------------------------------------------------
	function montoaentregar_assign( txval as variant ) as void                                                                                                                                                                                                
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'RETIRODEEFECTIVO' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'montoaentregar_assign', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'montoaentregar_assign', lcParametros )			endif		endif		Local lxValOld As Variant		lxValOld = this.Montoaentregar		local lxVal as Variant		lxVal = txVal		if This.CargaManual()			if this.Validar_Montoaentregar( lxVal )				this.Setear_Montoaentregar( lxVal )			EndIf		Else			This.Setear_Montoaentregar( lxVal )		EndIf		dodefault( lxVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function setear_montoaentregar( txval as variant ) as void                                                                                                                                                                                                
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'RETIRODEEFECTIVO' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'setear_montoaentregar', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'setear_montoaentregar', lcParametros )			endif		endif		this.Montoaentregar = txVal		dodefault( txVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function validar_montoaentregar( txval as variant ) as boolean                                                                                                                                                                                            
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'RETIRODEEFECTIVO' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'validar_montoaentregar', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'validar_montoaentregar', lcParametros )			endif		endif		Return dodefault( txVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function validarmonto( tnmontomax ) as string                                                                                                                                                                                                             
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		local lxParam1
	...
	*-----------------------------------------------------------------------------------------
	function valorsugeridomontoaentregar() as void                                                                                                                                                                                                            
		...
		return this.ObtenerResultado( 'Valorsugeridomontoaentregar', lcParametros )
	EndFunc
