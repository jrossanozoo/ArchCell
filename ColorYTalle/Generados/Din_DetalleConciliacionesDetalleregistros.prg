...
define class Din_DetalleCONCILIACIONESDetalleregistros as detalle of detalle.prg

	Sum_Importe = 0

	cAtributosUnicidad = 'Registro_PK'
	nCantidadItems = 999
	...
	*--------------------------------------------------------------------------------------------------------
	function oItem_Access() as variant
		if !this.ldestroy and ( !vartype( this.oItem ) = 'O' or isnull( this.oItem ) )
			this.oItem = _Screen.zoo.crearobjeto( 'ItemCONCILIACIONESDetalleregistros' )
	...
	*--------------------------------------------------------------------------------------------------------
	Function Cargar() as Void
		...
		scan for &lcFiltro
			...
			loItem.Registro_PK = c_Detalleregistros.Registro
			loItem.NroItem = lnI 
	...
	*--------------------------------------------------------------------------------------------------------
	function ValidarExistenciaCamposFijosItemPlano( tnItem as integer ) as Boolean
		...
		if  empty( this.item[ tnItem ].Registro_PK ) 
			llRetorno = .F.
		endif 
		return llRetorno
	...
	*-----------------------------------------------------------------------------------------
	...
	local lnLargoEntero as integer, lnValorDelDecimal as integer, lnLargoDecimal as integer, llRetorno as boolean, lnI as Integer
		...
		for lni = 1 to this.Count
			...
			if vartype( this.Item[lni].REGISTRO_PK) = 'C'
				if len( alltrim( this.Item[lni].REGISTRO_PK )) <= 20
				else
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud del valor del atributo REGISTRO del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform( len( alltrim( this.Item[lni].REGISTRO_PK ))) + ' Máxima:20), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
