...
define class Mock_filtrarcomprobantesainsertar as MockBase of MockBase.prg
	...
	cClase = 'filtrarcomprobantesainsertar'
	...
	*-----------------------------------------------------------------------------------------
	function numerocomprobantedesde_assign( txval as variant ) as void                                                                                                                                                                                        
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'FILTRARCOMPROBANTESAINSERTAR' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'numerocomprobantedesde_assign', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'numerocomprobantedesde_assign', lcParametros )			endif		endif		Local lxValOld As Variant		lxValOld = this.Numerocomprobantedesde		local lxVal as Variant		lxVal = txVal		if This.CargaManual()			if this.Validar_Numerocomprobantedesde( lxVal )				this.Setear_Numerocomprobantedesde( lxVal )			EndIf		Else			This.Setear_Numerocomprobantedesde( lxVal )		EndIf		dodefault( lxVal )
	EndFunc

	*-----------------------------------------------------------------------------------------
	function numerocomprobantehasta_assign( txval as variant ) as void                                                                                                                                                                                        
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'FILTRARCOMPROBANTESAINSERTAR' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'numerocomprobantehasta_assign', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'numerocomprobantehasta_assign', lcParametros )			endif		endif		Local lxValOld As Variant		lxValOld = this.Numerocomprobantehasta		local lxVal as Variant		lxVal = txVal		if This.CargaManual()			if this.Validar_Numerocomprobantehasta( lxVal )				this.Setear_Numerocomprobantehasta( lxVal )			EndIf		Else			This.Setear_Numerocomprobantehasta( lxVal )		EndIf		dodefault( lxVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function setear_numerocomprobantedesde( txval as variant ) as void                                                                                                                                                                                        
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'FILTRARCOMPROBANTESAINSERTAR' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'setear_numerocomprobantedesde', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'setear_numerocomprobantedesde', lcParametros )			endif		endif		this.Numerocomprobantedesde = txVal		dodefault( txVal )
	EndFunc

	*-----------------------------------------------------------------------------------------
	function setear_numerocomprobantehasta( txval as variant ) as void                                                                                                                                                                                        
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'FILTRARCOMPROBANTESAINSERTAR' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'setear_numerocomprobantehasta', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'setear_numerocomprobantehasta', lcParametros )			endif		endif		this.Numerocomprobantehasta = txVal		dodefault( txVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function validar_numerocomprobantedesde( txval as variant ) as boolean                                                                                                                                                                                    
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'FILTRARCOMPROBANTESAINSERTAR' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'validar_numerocomprobantedesde', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'validar_numerocomprobantedesde', lcParametros )			endif		endif		Return dodefault( txVal )
	EndFunc

	*-----------------------------------------------------------------------------------------
	function validar_numerocomprobantehasta( txval as variant ) as boolean                                                                                                                                                                                    
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'FILTRARCOMPROBANTESAINSERTAR' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'validar_numerocomprobantehasta', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'validar_numerocomprobantehasta', lcParametros )			endif		endif		Return dodefault( txVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function valorsugeridofwnumerocomprobantehasta() as void                                                                                                                                                                                                  
		...
		return this.ObtenerResultado( 'Valorsugeridofwnumerocomprobantehasta', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function valorsugeridonumerocomprobantedesde() as void                                                                                                                                                                                                    
		...
		return this.ObtenerResultado( 'Valorsugeridonumerocomprobantedesde', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function valorsugeridonumerocomprobantehasta() as void                                                                                                                                                                                                    
		...
		return this.ObtenerResultado( 'Valorsugeridonumerocomprobantehasta', lcParametros )
	EndFunc
