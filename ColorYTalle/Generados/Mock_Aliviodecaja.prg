...
define class Mock_aliviodecaja as MockBase of MockBase.prg
	...
	*-----------------------------------------------------------------------------------------
	function cambiosdetalledetallealivio() as void                                                                                                                                                                                                            
		...
		return this.ObtenerResultado( 'Cambiosdetalledetallealivio', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function setear_tipoalivio( txval as variant ) as void                                                                                                                                                                                                    
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'ALIVIODECAJA' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'setear_tipoalivio', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'setear_tipoalivio', lcParametros )			endif		endif		this.Tipoalivio = txVal		dodefault( txVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function tipoalivio_assign( txval as variant ) as void                                                                                                                                                                                                    
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'ALIVIODECAJA' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'tipoalivio_assign', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'tipoalivio_assign', lcParametros )			endif		endif		Local lxValOld As Variant		lxValOld = this.Tipoalivio		local lxVal as Variant		lxVal = txVal		if This.CargaManual()			if this.Validar_Tipoalivio( lxVal )				this.Setear_Tipoalivio( lxVal )			EndIf		Else			This.Setear_Tipoalivio( lxVal )		EndIf		dodefault( lxVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function validar_tipoalivio( txval as variant ) as boolean                                                                                                                                                                                                
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'ALIVIODECAJA' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'validar_tipoalivio', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'validar_tipoalivio', lcParametros )			endif		endif		Return dodefault( txVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function validaritemsegunbase( toitem as object, tcbasededatos) as boolean                                                                                                                                                                                
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		local lxParam1, lxParam2
		...
		return this.ObtenerResultado( 'Validaritemsegunbase', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function validarunicidaddecajasporbase() as boolean                                                                                                                                                                                                       
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Validarunicidaddecajasporbase', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function valorsugeridotipoalivio() as void                                                                                                                                                                                                                
		...
		return this.ObtenerResultado( 'Valorsugeridotipoalivio', lcParametros )
	EndFunc
