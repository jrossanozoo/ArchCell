...
define class Din_EntidadFinaldeproduccion_REST as ServicioRestOperacionesEntidad of ServicioRestOperacionesEntidad.prg

	cNamespaceDTOs = "ZooLogicSA.OrganicServiciosREST.Colorytalle.Generados.DTO.Finaldeproduccion"
	cClaseResponse = this.cNamespaceDTOs + ".FinaldeproduccionResponse"
	cClaseModelo = "FinaldeproduccionModelo"
	cEntidad = "Finaldeproduccion"
	...
	function ObtenerClaseRequest( tcOperacion as String ) as String
		...
		do case
			case tcOperacion == "Nuevo"
				lcRetorno = this.cNamespaceDTOs + "." + "FinaldeproduccionModelo"
			...
			case tcOperacion == "Listar"
				lcRetorno = this.cNamespaceDTOs + "." + "FinaldeproduccionListarRequest"
	...
	*-----------------------------------------------------------------------------------------
	function ObtenerClaseResponse( tcOperacion as String ) as String
		...
		do case
			case tcOperacion == "Listar"
				lcRetorno = this.cNamespaceDTOs + "." + "FinaldeproduccionListarResponse"
			otherwise
				lcRetorno = this.cNamespaceDTOs + "." + "FinaldeproduccionModelo"
		endcase
		return lcRetorno
	...
	*-----------------------------------------------------------------------------------------
	protected function SetearDetalleMovimientoDetalle( toEntidad as Object, toModeloEnRequest as Object ) as Void
		...
		for lnI = 1 to lnCantidad 
			...
			this.SetearAtributoModeloEnEntidad( loItem, "Inventario", toEntidad.MovimientoDetalle.oItem, "Inventario_PK" )
			this.SetearAtributoModeloEnEntidad( loItem, "Inventariodetalle", toEntidad.MovimientoDetalle.oItem, "InventarioDetalle" )
	...
	protected function CargarColeccionModeloMovimientoDetalle( toModeloResponse as Object, toEntidad as Object ) as Void
		...
		with toEntidad
			for lnI = 1 to .MovimientoDetalle.Count
				loItem = _Screen.DotNetBridge.CrearObjeto( this.cNamespaceDTOs + ".ItemArticfinaldeprod" )
				this.SetearAtributoString( loItem, "Codigo", .MovimientoDetalle.Item(lnI).Codigo)
				this.SetearAtributoString( loItem, "Inventario", .MovimientoDetalle.Item(lnI).Inventario_PK)
				this.SetearAtributoString( loItem, "InventarioDetalle", .MovimientoDetalle.Item(lnI).InventarioDetalle)
	...
	*-----------------------------------------------------------------------------------------
	protected function ObtenerFiltroSegunModeloRequest( toModeloRequest as Object ) as String
		...
		lcFiltro = this.AgregarCondicionAFiltro( lcFiltro, toModeloRequest, "Ordendeproduccion", "ORDENPROD")
	...
	*-----------------------------------------------------------------------------------------
	protected function ObtenerBusquedaSegunModeloRequest( toRequest as Object ) as String
		...
		if !isnull( lcExpresionBusqueda )
			lcRetorno = iif( !empty( lcRetorno ), lcRetorno + " OR ", "" ) + "Ordenprod LIKE '%" + lcExpresionBusqueda + "%'"
