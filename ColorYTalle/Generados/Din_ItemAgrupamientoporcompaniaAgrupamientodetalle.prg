...
define class Din_ItemAGRUPAMIENTOPORCOMPANIAAgrupamientodetalle as ItemActivo of ItemActivo.prg
	...
	*--------------------------------------------------------------------------------------------------------
	function Inicializar() as boolean
		...
		oICol = goServicios.SaltosDeCampoYValoresSugeridos.ObtenerValorSugeridoDeUnaEntidadDetalle( 'AGRUPAMIENTOPORCOMPANIA', 'Agrupamientodetalle')
	...
	*--------------------------------------------------------------------------------------------------------
	function Agrupamiento_PK_Assign( txVal as variant ) as void
		...
		lxVal = txVal
		lxVal = iif( len( lxVal ) < 61, padr( lxVal, 61 ), lxVal )
		...
		if This.CargaManual()
			if this.Validar_Incluye( lxVal )
				...
				this.EventoDespuesDeSetear( This, 'Incluye', lxValOld, lxVal  )
					...
					goServicios.Errores.LevantarExcepcion( "No se puede cambiar este valor (Entidad: Agrupamientos de bases de datos por compania - Atributo: Incluye)" )
				endif 
			EndIf
		Else
			This.Setear_Incluye( lxVal ) 
		...
		if this.ldestroy
			...
			if This.lInstanciarSubEntidadaDemanda
				if ( !vartype( this.Agrupamiento ) = 'O' or isnull( this.Agrupamiento ) )
					this.Agrupamiento = _Screen.zoo.instanciarentidad( 'Agrupamientoporcompania' )
	...
	*--------------------------------------------------------------------------------------------------------
	Function ValorSugeridoIncluye() as void
		...
		with this
			lcExpresionValorSugeridoDefinidoPorElUsuario = goServicios.SaltosDeCampoYValoresSugeridos.ObtenerValorSugerido( "AGRUPAMIENTOPORCOMPANIA", "Agrupamientodetalle", "Incluye" )
	...
	*--------------------------------------------------------------------------------------------------------
	function ObtenerNombre() as String
		return [ITEMAGRUPAMIENTOPORCOMP]
	endfunc
