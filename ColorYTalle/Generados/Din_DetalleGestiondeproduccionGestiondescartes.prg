...
define class Din_DetalleGESTIONDEPRODUCCIONGestiondescartes as DetalleBaseproduccion of DetalleBaseproduccion.Prg
	...
	Protected Function _CargarItem( tnItem as integer )
		...
		With this.oItem
			Try
				...
				.MotDescarte_PK = this.Item[ tnItem ].MotDescarte_PK
				llVacio = llVacio and this.VerificarSiElAtributoEstaVacio( 'MotDescarte_PK', .MotDescarte_PK )
			
				.MotDescarteDetalle = this.Item[ tnItem ].MotDescarteDetalle
				llVacio = llVacio and this.VerificarSiElAtributoEstaVacio( 'MotDescarteDetalle', .MotDescarteDetalle )
			
				.InventarioDest_PK = this.Item[ tnItem ].InventarioDest_PK
				llVacio = llVacio and this.VerificarSiElAtributoEstaVacio( 'InventarioDest_PK', .InventarioDest_PK )
			
				.InventarioDestDetalle = this.Item[ tnItem ].InventarioDestDetalle
				llVacio = llVacio and this.VerificarSiElAtributoEstaVacio( 'InventarioDestDetalle', .InventarioDestDetalle )
			
				.CantDescarte = this.Item[ tnItem ].CantDescarte
				llVacio = llVacio and this.VerificarSiElAtributoEstaVacio( 'CantDescarte', .CantDescarte )
	...
	*--------------------------------------------------------------------------------------------------------
	protected function AplanarItem() as Object
		...
		with loRetorno
			...
			.MotDescarte_PK = this.oItem.MotDescarte_PK
			.MotDescarteDetalle = this.oItem.MotDescarteDetalle
			.InventarioDest_PK = this.oItem.InventarioDest_PK
			.InventarioDestDetalle = this.oItem.InventarioDestDetalle
			.CantDescarte = this.oItem.CantDescarte
	...
	*--------------------------------------------------------------------------------------------------------
	...
	with this
		if .ValidarItem()
			...
			.Item[ lnItem ].MotDescarte_PK = .oItem.MotDescarte_PK
			.Item[ lnItem ].MotDescarteDetalle = .oItem.MotDescarteDetalle
			.Item[ lnItem ].InventarioDest_PK = .oItem.InventarioDest_PK
			.Item[ lnItem ].InventarioDestDetalle = .oItem.InventarioDestDetalle
			.Item[ lnItem ].CantDescarte = .oItem.CantDescarte
		else
			goServicios.Errores.LevantarExcepcion( .ObtenerInformacion() )
		endif
	endwith
	...
	*--------------------------------------------------------------------------------------------------------
	function oItem_Access() as variant
		if !this.ldestroy and ( !vartype( this.oItem ) = 'O' or isnull( this.oItem ) )
			this.oItem = _Screen.zoo.crearobjeto( 'ItemGESTIONDEPRODUCCIONGestiondescartes' )
	...
	*--------------------------------------------------------------------------------------------------------
	Function Cargar() as Void
		...
		select c_Gestiondescartes
		lcFiltro = this.ObtenerFiltro()
		scan for &lcFiltro
			loItem = createobject('ItemAuxiliar')

			loItem.Codigo = c_Gestiondescartes.Codigo
			loItem.Comportamiento = c_Gestiondescartes.Comportamiento
			loItem.IdItemInsumo = c_Gestiondescartes.IdItemInsumo
			loItem.NROITEM = c_Gestiondescartes.NROITEM
			loItem.Articulo_PK = c_Gestiondescartes.Articulo
			loItem.ArticuloDetalle = c_Gestiondescartes.ArticuloDetalle
			loItem.ColorM_PK = c_Gestiondescartes.ColorM
			loItem.ColorMDetalle = c_Gestiondescartes.ColorMDetalle
			loItem.TalleM_PK = c_Gestiondescartes.TalleM
			loItem.TalleMDetalle = c_Gestiondescartes.TalleMDetalle
			loItem.Insumo_PK = c_Gestiondescartes.Insumo
			loItem.InsumoDetalle = c_Gestiondescartes.InsumoDetalle
			loItem.Color_PK = c_Gestiondescartes.Color
			loItem.ColorDetalle = c_Gestiondescartes.ColorDetalle
			loItem.Talle_PK = c_Gestiondescartes.Talle
			loItem.TalleDetalle = c_Gestiondescartes.TalleDetalle
			loItem.Partida = c_Gestiondescartes.Partida
			loItem.MotDescarte_PK = c_Gestiondescartes.MotDescarte
			loItem.MotDescarteDetalle = c_Gestiondescartes.MotDescarteDetalle
			loItem.InventarioDest_PK = c_Gestiondescartes.InventarioDest
			loItem.InventarioDestDetalle = c_Gestiondescartes.InventarioDestDetalle
			loItem.CantDescarte = c_Gestiondescartes.CantDescarte
			loItem.NroItem = lnI 
	...
	*-----------------------------------------------------------------------------------------
	...
	local lnLargoEntero as integer, lnValorDelDecimal as integer, lnLargoDecimal as integer, llRetorno as boolean, lnI as Integer
		...
		for lni = 1 to this.Count
			...
			if vartype( this.Item[lni].MOTDESCARTE_PK) = 'C'
				if len( alltrim( this.Item[lni].MOTDESCARTE_PK )) <= 15
				else
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud del valor del atributo MOTDESCARTE del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform( len( alltrim( this.Item[lni].MOTDESCARTE_PK ))) + ' Máxima:15), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
				this.AgregarInformacion( 'El tipo de dato para el atributo MOTDESCARTE del item ' + transform( lni ) + ' no es el correcto, en el detalle ' + this.cEtiqueta + '.')
			Endif
			
			if vartype( this.Item[lni].MOTDESCARTEDETALLE) = 'C'
				if len( alltrim( this.Item[lni].MOTDESCARTEDETALLE )) <= 254
				else
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud del valor del atributo MOTDESCARTEDETALLE del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform( len( alltrim( this.Item[lni].MOTDESCARTEDETALLE ))) + ' Máxima:254), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
				this.AgregarInformacion( 'El tipo de dato para el atributo MOTDESCARTEDETALLE del item ' + transform( lni ) + ' no es el correcto, en el detalle ' + this.cEtiqueta + '.')
			Endif
			
			if vartype( this.Item[lni].INVENTARIODEST_PK) = 'C'
				if len( alltrim( this.Item[lni].INVENTARIODEST_PK )) <= 15
				else
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud del valor del atributo INVENTARIODEST del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform( len( alltrim( this.Item[lni].INVENTARIODEST_PK ))) + ' Máxima:15), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
				this.AgregarInformacion( 'El tipo de dato para el atributo INVENTARIODEST del item ' + transform( lni ) + ' no es el correcto, en el detalle ' + this.cEtiqueta + '.')
			Endif
			
			if vartype( this.Item[lni].INVENTARIODESTDETALLE) = 'C'
				if len( alltrim( this.Item[lni].INVENTARIODESTDETALLE )) <= 100
				else
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud del valor del atributo INVENTARIODESTDETALLE del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform( len( alltrim( this.Item[lni].INVENTARIODESTDETALLE ))) + ' Máxima:100), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
				this.AgregarInformacion( 'El tipo de dato para el atributo INVENTARIODESTDETALLE del item ' + transform( lni ) + ' no es el correcto, en el detalle ' + this.cEtiqueta + '.')
			Endif
			
			if vartype( this.Item[lni].CANTDESCARTE) = 'N'
				lnLargoEntero = len( transform( int( this.Item[lni].CANTDESCARTE ) ) ) 
				lnValorDelDecimal = this.Item[lni].CANTDESCARTE - int( this.Item[lni].CANTDESCARTE )
				lnLargoDecimal = iif( lnValorDelDecimal = 0, 0, len( transform( lnValorDelDecimal ) ) - 2 )
				if lnLargoEntero <= 8
				else
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud entera del valor del atributo CANTDESCARTE del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform(lnLargoEntero) + ' Máxima:8), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
				this.AgregarInformacion( 'El tipo de dato para el atributo CANTDESCARTE del item ' + transform( lni ) + ' no es el correcto, en el detalle ' + this.cEtiqueta + '.')
			Endif
		EndFor
		Return llRetorno 
	Endfunc
	...
	*--------------------------------------------------------------------------------------------------------
	function CopiarItemaItem( toItemOrigen as Object, toItemDestino as Object ) as Object
		...
		toItemDestino.Motdescarte_PK = toItemOrigen.Motdescarte_PK
		toItemDestino.Motdescartedetalle = toItemOrigen.Motdescartedetalle
		toItemDestino.Inventariodest_PK = toItemOrigen.Inventariodest_PK
		toItemDestino.Inventariodestdetalle = toItemOrigen.Inventariodestdetalle
		toItemDestino.Cantdescarte = toItemOrigen.Cantdescarte
	endfunc
