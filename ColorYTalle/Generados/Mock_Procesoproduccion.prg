...
define class Mock_procesoproduccion as MockBase of MockBase.prg
	...
	cCaracteres = ""                                                                                                                                                                                                                                          
	cContexto = ""                                                                                                                                                                                                                                            
	cDescripcion =  "Procesos de producción"                                                                                                                                                                                                                  
	...
	*-----------------------------------------------------------------------------------------
	function cambiosdetalleprocesomaquina() as void                                                                                                                                                                                                           
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Cambiosdetalleprocesomaquina', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function procesomaquina_access() as variant                                                                                                                                                                                                               
		lnParametros = pcount()lcParametros = ''		lnIdClase = _screen.Mocks.BuscarMock( 'PROCESOPRODUCCION' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'procesomaquina_access', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'procesomaquina_access', lcParametros )			endif		endif		if !this.ldestroy and ( !vartype( this.Procesomaquina ) = 'O' or isnull( this.Procesomaquina ) )			this.Procesomaquina = this.crearobjeto( 'Din_DetalleProcesoproduccionProcesomaquina' )			this.Procesomaquina.inicializar()			this.enlazar( 'Procesomaquina.EventoObtenerLogueo', 'inyectarLogueo' )			this.enlazar( 'Procesomaquina.EventoObtenerInformacion', 'inyectarInformacion' )			bindevent( this.Procesomaquina, 'CambioSumarizado', this, 'CambiosDetalleProcesomaquina', 1)			this.enlazar( 'Procesomaquina.EventoAdvertirLimitePorDiseno', 'EventoAdvertirLimitePorDiseno')			this.enlazar( 'Procesomaquina.EventoCancelarCargaLimitePorDiseno', 'EventoCancelarCargaLimitePorDiseno')		endif		return this.Procesomaquina
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function rubro_access() as variant                                                                                                                                                                                                                        
		lnParametros = pcount()lcParametros = ''		lnIdClase = _screen.Mocks.BuscarMock( 'PROCESOPRODUCCION' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'rubro_access', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'rubro_access', lcParametros )			endif		endif		if this.ldestroy		else			if This.lInstanciarSubEntidadaDemanda				if ( !vartype( this.Rubro ) = 'O' or isnull( this.Rubro ) )					this.Rubro = _Screen.zoo.instanciarentidad( 'Rubroproduccion' )					this.Rubro.lEsSubEntidad = .t.					this.enlazar( 'Rubro.EventoObtenerLogueo', 'inyectarLogueo' )					this.enlazar( 'Rubro.EventoObtenerInformacion', 'inyectarInformacion' )				endif				if !this.lBuscandoCodigo					if this.Rubro.CODIGO # this.Rubro_PK						this.Rubro.CODIGO = this.Rubro_PK					endif				endif			endif		endif		return this.Rubro
	EndFunc

	*-----------------------------------------------------------------------------------------
	function rubro_pk_assign( txval as variant ) as void                                                                                                                                                                                                      
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'PROCESOPRODUCCION' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'rubro_pk_assign', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'rubro_pk_assign', lcParametros )			endif		endif		Local lxValOld As Variant		lxValOld = this.Rubro_PK		local lxVal as Variant		local loSugerido as Object				lxVal = txVal		lxVal = iif( len( lxVal ) < 15, padr( lxVal, 15 ), lxVal )		this.Rubro_PK = lxVal		if this.Validar_Rubro( lxVal, lxValOld )			This.Setear_Rubro( lxVal )		endif		dodefault( lxVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function setear_rubro( txval as variant ) as void                                                                                                                                                                                                         
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'PROCESOPRODUCCION' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'setear_rubro', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'setear_rubro', lcParametros )			endif		endif		this.Rubro.CODIGO = txVal		dodefault( txVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function validar_rubro( txval as variant, txvalold as variant ) as boolean                                                                                                                                                                                
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1, lxParam2lnParametros = pcount()lxParam1 = txvallxParam2 = txvaloldlcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'PROCESOPRODUCCION' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'validar_rubro', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'validar_rubro', lcParametros )			endif		endif		Return This.CargaManual() and dodefault( txVal, txValOld )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function validarrubro() as boolean                                                                                                                                                                                                                        
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Validarrubro', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function valorsugeridorubro() as void                                                                                                                                                                                                                     
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Valorsugeridorubro', lcParametros )
	EndFunc
