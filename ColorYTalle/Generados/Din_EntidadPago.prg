...
define class Din_EntidadPAGO as entidad of entidad.prg
	...
	*--------------------------------------------------------------------------------------------------------
	function ValidacionDetalles() as Boolean
		...
		llRetorno = dodefault()
		llRetorno = This.ValidarUnDetalle( this.OrdenDePago_PagoDetalle, 'Detalle' ) and llRetorno
		llRetorno = This.ValidarUnDetalle( this.OrdenDePago_ValoresDetalle, 'Valor' ) and llRetorno
		llRetorno = This.ValidarUnDetalle( this.OrdenDePago_ImpuestosComprobante, 'Impuesto' ) and llRetorno
		return llRetorno
	endfunc
	...
	function ValidarOrdenDePago_Proveedor() as boolean
		...
		if empty( this.OrdenDePago_Proveedor_PK )
			...
			If this.OrdenDePago_Proveedor.oAD.ConsultarPorClavePrimaria()
				This.OrdenDePago_Proveedor.oDesactivador.ValidarEstadoActivacion( this.OrdenDePago_Proveedor_PK , goLibrerias.Valorvacioseguntipo( vartype(this.OrdenDePago_Proveedor_PK ) ), this.lnuevo, this.ledicion )
			Else
	...
	*--------------------------------------------------------------------------------------------------------
	function ValidarOrdenDePago_MonedaComprobante() as boolean
		...
		if empty( this.OrdenDePago_MonedaComprobante_PK )
			this.AgregarInformacion( 'Debe cargar el campo Moneda', 9005, 'OrdenDePago_MonedaComprobante' )
			llRetorno = .F.
		else
	...
	*--------------------------------------------------------------------------------------------------------
	function obtenerAtributosObligatorios() as ZooColeccion
		...
		loAtributosObligatorios.add( 'ORDENDEPAGO_MONEDACOMPROBANTE' )
	...
	*--------------------------------------------------------------------------------------------------------
	function ObtenerFuncionalidades() as String
		return '<ANULABLE><COMPRAS><TRANSMODOSEGUROCEN><TRANSMODOSEGURODB><NOIMPO><NOEXPO><LISTARMODIFICACION><WEBHOOK>'
	Endfunc
	*--------------------------------------------------------------------------------------------------------
	...
	*--------------------------------------------------------------------------------------------------------
	function Inicializar() as Void
		...
		this.lWHIngresar = goServicios.WebHook.TieneQueMandar( 'PAGO', 'INGRESAR' )
		this.lWHModificar = goServicios.WebHook.TieneQueMandar( 'PAGO', 'MODIFICAR' )
		this.lWHEliminar = goServicios.WebHook.TieneQueMandar( 'PAGO', 'ELIMINAR' )
	endfunc
			...
			if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
				if !empty( lcExpresionValorSugeridoDefinidoPorElUsuario )
					try
						...
						loEx.Grabar( loError )
						loEx.AgregarInformacion( iif( type( "loError.UserValue.Message" ) == "C", loError.UserValue.Message, loError.Message ) )
						loEx.AgregarInformacion( "Se produjo un error al intentar asignar el valor sugerido " + lcExpresionValorSugeridoDefinidoPorElUsuario + " del atributo Fecha de pago para la entidad Pago." + chr( 10 ) + "Verifique el valor especificado en la entidad Comportamiento de atributos." + chr( 10 ) + "El tipo de dato del atributo es fecha." )
						goServicios.Errores.LevantarExcepcion( loEx )
					endtry
				endif
			else
	...
	*--------------------------------------------------------------------------------------------------------
	Function ValorSugeridoObservacion() as void
		...
		lcExpresionValorSugeridoDefinidoPorElUsuario = null
		with this
			lcExpresionValorSugeridoDefinidoPorElUsuario = goServicios.SaltosDeCampoYValoresSugeridos.ObtenerValorSugerido( "PAGO", "", "Observacion" )
			if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
				if !empty( lcExpresionValorSugeridoDefinidoPorElUsuario )
					try
						lvValorSugeridoDefinidoPorElUsuario = &lcExpresionValorSugeridoDefinidoPorElUsuario
