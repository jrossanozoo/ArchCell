...
define class Mock_impdirope as MockBase of MockBase.prg
	...
	cCaracteres = ""                                                                                                                                                                                                                                          
	cContexto = ""                                                                                                                                                                                                                                            
	cDescripcion =  "Imputación de operadoras de tarjetas"                                                                                                                                                                                                    
	...
	f_Operadora_Desde = null                                                                                                                                                                                                                                  
	f_Operadora_Desde_PK = ""                                                                                                                                                                                                                                 
	f_Operadora_Hasta = null                                                                                                                                                                                                                                  
	f_Operadora_Hasta_PK = ""                                                                                                                                                                                                                                 
	...
	*-----------------------------------------------------------------------------------------
	function f_operadora_desde_access() as variant                                                                                                                                                                                                            
		lnParametros = pcount()lcParametros = ''		lnIdClase = _screen.Mocks.BuscarMock( 'IMPDIROPE' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'f_operadora_desde_access', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'f_operadora_desde_access', lcParametros )			endif		endif		if this.ldestroy		else			if This.lInstanciarSubEntidadaDemanda				if ( !vartype( this.F_operadora_desde ) = 'O' or isnull( this.F_operadora_desde ) )					this.F_operadora_desde = _Screen.zoo.instanciarentidad( 'Operadoradetarjeta' )					this.F_operadora_desde.lEsSubEntidad = .t.					this.enlazar( 'F_operadora_desde.EventoObtenerLogueo', 'inyectarLogueo' )					this.enlazar( 'F_operadora_desde.EventoObtenerInformacion', 'inyectarInformacion' )				endif				if !this.lBuscandoCodigo					if this.F_operadora_desde.CODIGO # this.F_operadora_desde_PK						this.F_operadora_desde.CODIGO = this.F_operadora_desde_PK					endif				endif			endif		endif		return this.F_operadora_desde
	EndFunc

	*-----------------------------------------------------------------------------------------
	function f_operadora_desde_pk_assign( txval as variant ) as void                                                                                                                                                                                          
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'IMPDIROPE' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'f_operadora_desde_pk_assign', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'f_operadora_desde_pk_assign', lcParametros )			endif		endif		Local lxValOld As Variant		lxValOld = this.F_operadora_desde_PK		local lxVal as Variant		local loSugerido as Object				lxVal = txVal		lxVal = iif( len( lxVal ) < 15, padr( lxVal, 15 ), lxVal )		this.F_operadora_desde_PK = lxVal		dodefault( lxVal )
	EndFunc

	*-----------------------------------------------------------------------------------------
	function f_operadora_hasta_access() as variant                                                                                                                                                                                                            
		lnParametros = pcount()lcParametros = ''		lnIdClase = _screen.Mocks.BuscarMock( 'IMPDIROPE' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'f_operadora_hasta_access', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'f_operadora_hasta_access', lcParametros )			endif		endif		if this.ldestroy		else			if This.lInstanciarSubEntidadaDemanda				if ( !vartype( this.F_operadora_hasta ) = 'O' or isnull( this.F_operadora_hasta ) )					this.F_operadora_hasta = _Screen.zoo.instanciarentidad( 'Operadoradetarjeta' )					this.F_operadora_hasta.lEsSubEntidad = .t.					this.enlazar( 'F_operadora_hasta.EventoObtenerLogueo', 'inyectarLogueo' )					this.enlazar( 'F_operadora_hasta.EventoObtenerInformacion', 'inyectarInformacion' )				endif				if !this.lBuscandoCodigo					if this.F_operadora_hasta.CODIGO # this.F_operadora_hasta_PK						this.F_operadora_hasta.CODIGO = this.F_operadora_hasta_PK					endif				endif			endif		endif		return this.F_operadora_hasta
	EndFunc

	*-----------------------------------------------------------------------------------------
	function f_operadora_hasta_pk_assign( txval as variant ) as void                                                                                                                                                                                          
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'IMPDIROPE' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'f_operadora_hasta_pk_assign', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'f_operadora_hasta_pk_assign', lcParametros )			endif		endif		Local lxValOld As Variant		lxValOld = this.F_operadora_hasta_PK		local lxVal as Variant		local loSugerido as Object				lxVal = txVal		lxVal = iif( len( lxVal ) < 15, padr( lxVal, 15 ), lxVal )		this.F_operadora_hasta_PK = lxVal		dodefault( lxVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function setear_f_operadora_desde( txval as variant ) as void                                                                                                                                                                                             
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'IMPDIROPE' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'setear_f_operadora_desde', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'setear_f_operadora_desde', lcParametros )			endif		endif		this.F_operadora_desde.CODIGO = txVal		dodefault( txVal )
	EndFunc

	*-----------------------------------------------------------------------------------------
	function setear_f_operadora_hasta( txval as variant ) as void                                                                                                                                                                                             
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'IMPDIROPE' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'setear_f_operadora_hasta', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'setear_f_operadora_hasta', lcParametros )			endif		endif		this.F_operadora_hasta.CODIGO = txVal		dodefault( txVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function validar_f_operadora_desde( txval as variant, txvalold as variant ) as boolean                                                                                                                                                                    
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1, lxParam2lnParametros = pcount()lxParam1 = txvallxParam2 = txvaloldlcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'IMPDIROPE' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'validar_f_operadora_desde', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'validar_f_operadora_desde', lcParametros )			endif		endif		Return This.CargaManual() and dodefault( txVal, txValOld )
	EndFunc

	*-----------------------------------------------------------------------------------------
	function validar_f_operadora_hasta( txval as variant, txvalold as variant ) as boolean                                                                                                                                                                    
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1, lxParam2lnParametros = pcount()lxParam1 = txvallxParam2 = txvaloldlcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'IMPDIROPE' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'validar_f_operadora_hasta', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'validar_f_operadora_hasta', lcParametros )			endif		endif		Return This.CargaManual() and dodefault( txVal, txValOld )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function validarf_operadora_desde() as boolean                                                                                                                                                                                                            
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Validarf_operadora_desde', lcParametros )
	EndFunc

	*-----------------------------------------------------------------------------------------
	function validarf_operadora_hasta() as boolean                                                                                                                                                                                                            
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Validarf_operadora_hasta', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function valorsugeridof_operadora_desde() as void                                                                                                                                                                                                         
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Valorsugeridof_operadora_desde', lcParametros )
	EndFunc

	*-----------------------------------------------------------------------------------------
	function valorsugeridof_operadora_hasta() as void                                                                                                                                                                                                         
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Valorsugeridof_operadora_hasta', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function valorsugeridofwf_operadora_hasta() as void                                                                                                                                                                                                       
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Valorsugeridofwf_operadora_hasta', lcParametros )
	EndFunc
