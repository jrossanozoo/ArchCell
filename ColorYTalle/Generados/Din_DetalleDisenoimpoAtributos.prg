...
define class Din_DetalleDISENOIMPOAtributos as detalle of detalle.prg
	...
	Protected Function _CargarItem( tnItem as integer )
		...
		With this.oItem
			Try
				...
				.Condicion_PK = this.Item[ tnItem ].Condicion_PK
				llVacio = llVacio and this.VerificarSiElAtributoEstaVacio( 'Condicion_PK', .Condicion_PK )
			
				.SubArea = this.Item[ tnItem ].SubArea
				llVacio = llVacio and this.VerificarSiElAtributoEstaVacio( 'SubArea', .SubArea )
	...
	*--------------------------------------------------------------------------------------------------------
	protected function AplanarItem() as Object
		...
		with loRetorno
			...
			.Condicion_PK = this.oItem.Condicion_PK
			.SubArea = this.oItem.SubArea
	...
	*--------------------------------------------------------------------------------------------------------
	...
	with this
		if .ValidarItem()
			...
			.Item[ lnItem ].Condicion_PK = .oItem.Condicion_PK
			.Item[ lnItem ].SubArea = .oItem.SubArea
		else
			goServicios.Errores.LevantarExcepcion( .ObtenerInformacion() )
		endif
	endwith
	...
	*--------------------------------------------------------------------------------------------------------
	function oItem_Access() as variant
		if !this.ldestroy and ( !vartype( this.oItem ) = 'O' or isnull( this.oItem ) )
			this.oItem = _Screen.zoo.crearobjeto( 'Din_ItemDISENOIMPOAtributos' )
	...
	*--------------------------------------------------------------------------------------------------------
	Function Cargar() as Void
		...
		scan for &lcFiltro
			...
			loItem.Condicion_PK = c_Atributos.Condicion
			loItem.SubArea = c_Atributos.SubArea
			loItem.NroItem = lnI 
	...
	*-----------------------------------------------------------------------------------------
	...
	local lnLargoEntero as integer, lnValorDelDecimal as integer, lnLargoDecimal as integer, llRetorno as boolean, lnI as Integer
		...
		for lni = 1 to this.Count
			...
			if vartype( this.Item[lni].CONDICION_PK) = 'C'
				if len( alltrim( this.Item[lni].CONDICION_PK )) <= 40
				else
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud del valor del atributo CONDICION del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform( len( alltrim( this.Item[lni].CONDICION_PK ))) + ' Máxima:40), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
			...
			if vartype( this.Item[lni].SUBAREA) = 'C'
				if len( alltrim( this.Item[lni].SUBAREA )) <= 20
				else
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud del valor del atributo SUBAREA del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform( len( alltrim( this.Item[lni].SUBAREA ))) + ' Máxima:20), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
				this.AgregarInformacion( 'El tipo de dato para el atributo SUBAREA del item ' + transform( lni ) + ' no es el correcto, en el detalle ' + this.cEtiqueta + '.')
			Endif
		EndFor
		Return llRetorno 
	Endfunc
	...
	*--------------------------------------------------------------------------------------------------------
	function CopiarItemaItem( toItemOrigen as Object, toItemDestino as Object ) as Object
		...
		toItemDestino.Condicion_PK = toItemOrigen.Condicion_PK
		toItemDestino.Subarea = toItemOrigen.Subarea
	endfunc
