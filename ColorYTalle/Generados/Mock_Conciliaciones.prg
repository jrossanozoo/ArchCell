...
define class Mock_conciliaciones as MockBase of MockBase.prg
	...
	cAtributoVendedor = ''                                                                                                                                                                                                                                    
	cCaracteres = ""                                                                                                                                                                                                                                          
	cComprobante = "CONCILIACION"                                                                                                                                                                                                                             
	cContexto = ""                                                                                                                                                                                                                                            
	cDescripcion =  "Conciliaciones bancarias"                                                                                                                                                                                                                
	...
	oColaboradorCuentaBancaria = null                                                                                                                                                                                                                         
	oColeccionVS = null                                                                                                                                                                                                                                       
	oColeccionVSFW = null                                                                                                                                                                                                                                     
	...
	*-----------------------------------------------------------------------------------------
	function ajustarfiltrobusqueda( tctabla as string, tccampoctabanc as string, tccampocodigo as string, tctabledet as string, tccampoent as string, toobjetobusqueda as object ) as void                                                                    
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		local lxParam1, lxParam2, lxParam3, lxParam4, lxParam5, lxParam6

		lnParametros = pcount()
		lxParam1 = tctabla
		lxParam2 = tccampoctabanc
		lxParam3 = tccampocodigo
		lxParam4 = tctabledet
		lxParam5 = tccampoent
		lxParam6 = toobjetobusqueda
	...
	*-----------------------------------------------------------------------------------------
	function ajustarfiltrobusquedamovimiento( toobjetobusqueda as object ) as void                                                                                                                                                                            
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		local lxParam1
		...
		return this.ObtenerResultado( 'Ajustarfiltrobusquedamovimiento', lcParametros )
	EndFunc

	*-----------------------------------------------------------------------------------------
	function ajustarfiltrobusquedaregistro( toobjetobusqueda as object ) as void                                                                                                                                                                              
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		local lxParam1
		...
		return this.ObtenerResultado( 'Ajustarfiltrobusquedaregistro', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function cuentabancaria_pk_assign( txval ) as void                                                                                                                                                                                                        
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'CONCILIACIONES' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'cuentabancaria_pk_assign', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'cuentabancaria_pk_assign', lcParametros )			endif		endif		local lcCuentaBancariaOld as String		lcCuentaBancariaOld = this.CuentaBancaria_pk		dodefault( txVal )		if this.CuentaBancaria_PK != lcCuentaBancariaOld			this.DetalleMovimientos.Limpiar()			this.DetalleRegistros.Limpiar()		endif		this.lHabilitarDetalleMovimientos = !empty( this.CuentaBancaria_PK )		this.lHabilitarDetalleRegistros = !empty( this.CuentaBancaria_PK )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function movimientofueconciliado( tcmovimiento as string ) as boolean                                                                                                                                                                                     
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		local lxParam1

		lnParametros = pcount()
		lxParam1 = tcmovimiento
		...
		return this.ObtenerResultado( 'Movimientofueconciliado', lcParametros )
	EndFunc

	*-----------------------------------------------------------------------------------------
	function nmargendeerrorporredondeo_access() as void                                                                                                                                                                                                       
		lnParametros = pcount()lcParametros = ''		lnIdClase = _screen.Mocks.BuscarMock( 'CONCILIACIONES' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'nmargendeerrorporredondeo_access', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'nmargendeerrorporredondeo_access', lcParametros )			endif		endif		local lnValor as Number		if type( "this.nMargenDeErrorPorRedondeo" ) != "N"			lnValor = goServicios.Parametros.Felino.Fondos.MargenDeErrorPorRedondeoEnConciliacionBancaria			this.nMargenDeErrorPorRedondeo = abs( lnValor )		endif		return this.nMargenDeErrorPorRedondeo
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function obtenerdatosregistros() as void                                                                                                                                                                                                                  
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Obtenerdatosregistros', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function ocolaboradorcuentabancaria_access() as void                                                                                                                                                                                                      
		lnParametros = pcount()lcParametros = ''		lnIdClase = _screen.Mocks.BuscarMock( 'CONCILIACIONES' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'ocolaboradorcuentabancaria_access', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'ocolaboradorcuentabancaria_access', lcParametros )			endif		endif		if !this.ldestroy and vartype( this.oColaboradorCuentaBancaria ) != 'O'			this.oColaboradorCuentaBancaria = _Screen.zoo.CrearObjeto( "ColaboradorCuentaBancaria" )		endif		return this.oColaboradorCuentaBancaria
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function omovimiento_access() as void                                                                                                                                                                                                                     
		lnParametros = pcount()lcParametros = ''		lnIdClase = _screen.Mocks.BuscarMock( 'CONCILIACIONES' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'omovimiento_access', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'omovimiento_access', lcParametros )			endif		endif		if !this.ldestroy and vartype( this.oMovimiento ) != 'O'			this.oMovimiento = _Screen.zoo.InstanciarEntidad( "MovimientoBancario" )		endif		return this.oMovimiento
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function oregistro_access() as void                                                                                                                                                                                                                       
		lnParametros = pcount()lcParametros = ''		lnIdClase = _screen.Mocks.BuscarMock( 'CONCILIACIONES' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'oregistro_access', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'oregistro_access', lcParametros )			endif		endif		if !this.ldestroy and vartype( this.oRegistro ) != 'O'			this.oRegistro = _Screen.zoo.InstanciarEntidad( "RegistroDeCuenta" )		endif		return this.oRegistro
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function refrescardetallemovimientos() as void                                                                                                                                                                                                            
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Refrescardetallemovimientos', lcParametros )
	EndFunc

	*-----------------------------------------------------------------------------------------
	function refrescardetalleregistros() as void                                                                                                                                                                                                              
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Refrescardetalleregistros', lcParametros )
	EndFunc

	*-----------------------------------------------------------------------------------------
	function registrofueconciliado( tcregistro as string ) as void                                                                                                                                                                                            
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		local lxParam1
		...
		return this.ObtenerResultado( 'Registrofueconciliado', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function validardiferenciadeimportes( tnmovimientoimporte as number, tnregistroimporte as number ) as boolean                                                                                                                                             
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		local lxParam1, lxParam2

		lnParametros = pcount()
		lxParam1 = tnmovimientoimporte
		lxParam2 = tnregistroimporte
		...
		return this.ObtenerResultado( 'Validardiferenciadeimportes', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function validarmovimientosyregistros() as boolean                                                                                                                                                                                                        
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Validarmovimientosyregistros', lcParametros )
	EndFunc
