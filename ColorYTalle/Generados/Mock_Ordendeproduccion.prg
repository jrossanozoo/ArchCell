...
define class Mock_ordendeproduccion as MockBase of MockBase.prg
	...
	cCaracteres = ""                                                                                                                                                                                                                                          
	cContexto = ""                                                                                                                                                                                                                                            
	cDescripcion =  "Orden de producción"                                                                                                                                                                                                                     
	...
	*-----------------------------------------------------------------------------------------
	function armarordenenbaseamodeloycurva( tcvarianteprincipal as string, tcvariantesecundaria as string ) as boolean                                                                                                                                        
		...
		lnParametros = pcount()
		lxParam1 = tcvarianteprincipal
		lxParam2 = tcvariantesecundaria
		...
		return this.ObtenerResultado( 'Armarordenenbaseamodeloycurva', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function blanco1_assign( txval as variant ) as void                                                                                                                                                                                                       
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'ORDENDEPRODUCCION' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'blanco1_assign', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'blanco1_assign', lcParametros )			endif		endif		Local lxValOld As Variant		lxValOld = this.Blanco1		local lxVal as Variant		lxVal = txVal		if This.CargaManual()			if this.Validar_Blanco1( lxVal )				this.Setear_Blanco1( lxVal )			EndIf		Else			This.Setear_Blanco1( lxVal )		EndIf		dodefault( lxVal )
	EndFunc

	*-----------------------------------------------------------------------------------------
	function blanco2_assign( txval as variant ) as void                                                                                                                                                                                                       
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'ORDENDEPRODUCCION' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'blanco2_assign', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'blanco2_assign', lcParametros )			endif		endif		Local lxValOld As Variant		lxValOld = this.Blanco2		local lxVal as Variant		lxVal = txVal		if This.CargaManual()			if this.Validar_Blanco2( lxVal )				this.Setear_Blanco2( lxVal )			EndIf		Else			This.Setear_Blanco2( lxVal )		EndIf		dodefault( lxVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function cambiosdetalleordencurva() as void                                                                                                                                                                                                               
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Cambiosdetalleordencurva', lcParametros )
	EndFunc

	*-----------------------------------------------------------------------------------------
	function cambiosdetalleordeninsumos() as void                                                                                                                                                                                                             
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Cambiosdetalleordeninsumos', lcParametros )
	EndFunc

	*-----------------------------------------------------------------------------------------
	function cambiosdetalleordenprocesos() as void                                                                                                                                                                                                            
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Cambiosdetalleordenprocesos', lcParametros )
	EndFunc

	*-----------------------------------------------------------------------------------------
	function cambiosdetalleordensalidas() as void                                                                                                                                                                                                             
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Cambiosdetalleordensalidas', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function curvadeproduccion_access() as variant                                                                                                                                                                                                            
		lnParametros = pcount()lcParametros = ''		lnIdClase = _screen.Mocks.BuscarMock( 'ORDENDEPRODUCCION' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'curvadeproduccion_access', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'curvadeproduccion_access', lcParametros )			endif		endif		if this.ldestroy		else			if This.lInstanciarSubEntidadaDemanda				if ( !vartype( this.Curvadeproduccion ) = 'O' or isnull( this.Curvadeproduccion ) )					this.Curvadeproduccion = _Screen.zoo.instanciarentidad( 'Curvadeproduccion' )					this.Curvadeproduccion.lEsSubEntidad = .t.					this.enlazar( 'Curvadeproduccion.EventoObtenerLogueo', 'inyectarLogueo' )					this.enlazar( 'Curvadeproduccion.EventoObtenerInformacion', 'inyectarInformacion' )				endif				if !this.lBuscandoCodigo					if this.Curvadeproduccion.CODIGO # this.Curvadeproduccion_PK						this.Curvadeproduccion.CODIGO = this.Curvadeproduccion_PK					endif				endif			endif		endif		return this.Curvadeproduccion
	EndFunc

	*-----------------------------------------------------------------------------------------
	function curvadeproduccion_pk_assign( txval as variant ) as void                                                                                                                                                                                          
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'ORDENDEPRODUCCION' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'curvadeproduccion_pk_assign', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'curvadeproduccion_pk_assign', lcParametros )			endif		endif		Local lxValOld As Variant		lxValOld = this.Curvadeproduccion_PK		local lxVal as Variant		local loSugerido as Object				lxVal = txVal		lxVal = iif( len( lxVal ) < 15, padr( lxVal, 15 ), lxVal )		this.Curvadeproduccion_PK = lxVal		if this.Validar_Curvadeproduccion( lxVal, lxValOld )			This.Setear_Curvadeproduccion( lxVal )		endif		dodefault( lxVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function eventoconfirmarcambio( tcmensajedeconfirmacion as string, tctitulo as string ) as void                                                                                                                                                           
		...
		lnParametros = pcount()
		lxParam1 = tcmensajedeconfirmacion
		lxParam2 = tctitulo
		...
		return this.ObtenerResultado( 'Eventoconfirmarcambio', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function eventorefrescargrilla( tcdetalle as string ) as void                                                                                                                                                                                             
		...
		return this.ObtenerResultado( 'Eventorefrescargrilla', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function ordencurva_access() as variant                                                                                                                                                                                                                   
		lnParametros = pcount()lcParametros = ''		lnIdClase = _screen.Mocks.BuscarMock( 'ORDENDEPRODUCCION' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'ordencurva_access', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'ordencurva_access', lcParametros )			endif		endif		if !this.ldestroy and ( !vartype( this.Ordencurva ) = 'O' or isnull( this.Ordencurva ) )			this.Ordencurva = this.crearobjeto( 'DetalleOrdendeproduccionOrdencurva' )			this.Ordencurva.inicializar()			this.enlazar( 'Ordencurva.EventoObtenerLogueo', 'inyectarLogueo' )			this.enlazar( 'Ordencurva.EventoObtenerInformacion', 'inyectarInformacion' )			bindevent( this.Ordencurva, 'CambioSumarizado', this, 'CambiosDetalleOrdencurva', 1)			this.enlazar( 'Ordencurva.EventoAdvertirLimitePorDiseno', 'EventoAdvertirLimitePorDiseno')			this.enlazar( 'Ordencurva.EventoCancelarCargaLimitePorDiseno', 'EventoCancelarCargaLimitePorDiseno')			this.Ordencurva.lVerificarLimitesEnDisenoImpresion = .t.		endif		return this.Ordencurva
	EndFunc

	*-----------------------------------------------------------------------------------------
	function ordeninsumos_access() as variant                                                                                                                                                                                                                 
		lnParametros = pcount()lcParametros = ''		lnIdClase = _screen.Mocks.BuscarMock( 'ORDENDEPRODUCCION' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'ordeninsumos_access', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'ordeninsumos_access', lcParametros )			endif		endif		if !this.ldestroy and ( !vartype( this.Ordeninsumos ) = 'O' or isnull( this.Ordeninsumos ) )			this.Ordeninsumos = this.crearobjeto( 'DetalleOrdendeproduccionOrdeninsumos' )			this.Ordeninsumos.inicializar()			this.enlazar( 'Ordeninsumos.EventoObtenerLogueo', 'inyectarLogueo' )			this.enlazar( 'Ordeninsumos.EventoObtenerInformacion', 'inyectarInformacion' )			bindevent( this.Ordeninsumos, 'CambioSumarizado', this, 'CambiosDetalleOrdeninsumos', 1)			this.enlazar( 'Ordeninsumos.EventoAdvertirLimitePorDiseno', 'EventoAdvertirLimitePorDiseno')			this.enlazar( 'Ordeninsumos.EventoCancelarCargaLimitePorDiseno', 'EventoCancelarCargaLimitePorDiseno')			this.Ordeninsumos.lVerificarLimitesEnDisenoImpresion = .t.		endif		return this.Ordeninsumos
	EndFunc

	*-----------------------------------------------------------------------------------------
	function ordenprocesos_access() as variant                                                                                                                                                                                                                
		lnParametros = pcount()lcParametros = ''		lnIdClase = _screen.Mocks.BuscarMock( 'ORDENDEPRODUCCION' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'ordenprocesos_access', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'ordenprocesos_access', lcParametros )			endif		endif		if !this.ldestroy and ( !vartype( this.Ordenprocesos ) = 'O' or isnull( this.Ordenprocesos ) )			this.Ordenprocesos = this.crearobjeto( 'DetalleOrdendeproduccionOrdenprocesos' )			this.Ordenprocesos.inicializar()			this.enlazar( 'Ordenprocesos.EventoObtenerLogueo', 'inyectarLogueo' )			this.enlazar( 'Ordenprocesos.EventoObtenerInformacion', 'inyectarInformacion' )			bindevent( this.Ordenprocesos, 'CambioSumarizado', this, 'CambiosDetalleOrdenprocesos', 1)			this.enlazar( 'Ordenprocesos.EventoAdvertirLimitePorDiseno', 'EventoAdvertirLimitePorDiseno')			this.enlazar( 'Ordenprocesos.EventoCancelarCargaLimitePorDiseno', 'EventoCancelarCargaLimitePorDiseno')			this.Ordenprocesos.lVerificarLimitesEnDisenoImpresion = .t.		endif		return this.Ordenprocesos
	EndFunc

	*-----------------------------------------------------------------------------------------
	function ordensalidas_access() as variant                                                                                                                                                                                                                 
		lnParametros = pcount()lcParametros = ''		lnIdClase = _screen.Mocks.BuscarMock( 'ORDENDEPRODUCCION' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'ordensalidas_access', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'ordensalidas_access', lcParametros )			endif		endif		if !this.ldestroy and ( !vartype( this.Ordensalidas ) = 'O' or isnull( this.Ordensalidas ) )			this.Ordensalidas = this.crearobjeto( 'DetalleOrdendeproduccionOrdensalidas' )			this.Ordensalidas.inicializar()			this.enlazar( 'Ordensalidas.EventoObtenerLogueo', 'inyectarLogueo' )			this.enlazar( 'Ordensalidas.EventoObtenerInformacion', 'inyectarInformacion' )			bindevent( this.Ordensalidas, 'CambioSumarizado', this, 'CambiosDetalleOrdensalidas', 1)			this.enlazar( 'Ordensalidas.EventoAdvertirLimitePorDiseno', 'EventoAdvertirLimitePorDiseno')			this.enlazar( 'Ordensalidas.EventoCancelarCargaLimitePorDiseno', 'EventoCancelarCargaLimitePorDiseno')			this.Ordensalidas.lVerificarLimitesEnDisenoImpresion = .t.		endif		return this.Ordensalidas
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function productofinal_access() as variant                                                                                                                                                                                                                
		lnParametros = pcount()lcParametros = ''		lnIdClase = _screen.Mocks.BuscarMock( 'ORDENDEPRODUCCION' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'productofinal_access', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'productofinal_access', lcParametros )			endif		endif		if this.ldestroy		else			if This.lInstanciarSubEntidadaDemanda				if ( !vartype( this.Productofinal ) = 'O' or isnull( this.Productofinal ) )					this.Productofinal = _Screen.zoo.instanciarentidad( 'Articulo' )					this.Productofinal.lEsSubEntidad = .t.					this.enlazar( 'Productofinal.EventoObtenerLogueo', 'inyectarLogueo' )					this.enlazar( 'Productofinal.EventoObtenerInformacion', 'inyectarInformacion' )				endif				if !this.lBuscandoCodigo					if this.Productofinal.CODIGO # this.Productofinal_PK						this.Productofinal.CODIGO = this.Productofinal_PK					endif				endif			endif		endif		return this.Productofinal
	EndFunc

	*-----------------------------------------------------------------------------------------
	function productofinal_pk_assign( txval as variant ) as void                                                                                                                                                                                              
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'ORDENDEPRODUCCION' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'productofinal_pk_assign', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'productofinal_pk_assign', lcParametros )			endif		endif		Local lxValOld As Variant		lxValOld = this.Productofinal_PK		local lxVal as Variant		local loSugerido as Object				if txVal = lxValOld or empty( txVal )			lxVal = txVal		else			this.lBuscandoCodigo = .t.			if !this.lEsSubEntidad and ( this.EsNuevo() or this.EsEdicion() )				lxVal = This.Productofinal.oComportamientoCodigoSugerido.ObtenerPKaAsignar( txVal, This.Productofinal )			else				lxVal = txVal			endif			this.lBuscandoCodigo = .f.		endif		lxVal = iif( len( lxVal ) < 15, padr( lxVal, 15 ), lxVal )		this.Productofinal_PK = lxVal		if this.Validar_Productofinal( lxVal, lxValOld )			This.Setear_Productofinal( lxVal )		endif		dodefault( lxVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function quitarcurvadeorden( tcvarianteprincipal as string, tcvariantesecundaria as string ) as void                                                                                                                                                      
		...
		return this.ObtenerResultado( 'Quitarcurvadeorden', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function setear_blanco1( txval as variant ) as void                                                                                                                                                                                                       
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'ORDENDEPRODUCCION' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'setear_blanco1', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'setear_blanco1', lcParametros )			endif		endif		this.Blanco1 = txVal		dodefault( txVal )
	EndFunc

	*-----------------------------------------------------------------------------------------
	function setear_blanco2( txval as variant ) as void                                                                                                                                                                                                       
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'ORDENDEPRODUCCION' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'setear_blanco2', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'setear_blanco2', lcParametros )			endif		endif		this.Blanco2 = txVal		dodefault( txVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function setearfiltrobuscadormodelodeproduccion( tobusqueda as object ) as void                                                                                                                                                                           
		...
		return this.ObtenerResultado( 'Setearfiltrobuscadormodelodeproduccion', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function validar_blanco1( txval as variant ) as boolean                                                                                                                                                                                                   
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'ORDENDEPRODUCCION' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'validar_blanco1', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'validar_blanco1', lcParametros )			endif		endif		Return dodefault( txVal )
	EndFunc

	*-----------------------------------------------------------------------------------------
	function validar_blanco2( txval as variant ) as boolean                                                                                                                                                                                                   
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'ORDENDEPRODUCCION' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'validar_blanco2', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'validar_blanco2', lcParametros )			endif		endif		Return dodefault( txVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function validar_productofinal( txval as variant, txvalold as variant ) as boolean                                                                                                                                                                        
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1, lxParam2lnParametros = pcount()lxParam1 = txvallxParam2 = txvaloldlcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'ORDENDEPRODUCCION' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'validar_productofinal', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'validar_productofinal', lcParametros )			endif		endif		Return This.CargaManual() and dodefault( txVal, txValOld )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function validarcurvadeproduccion() as boolean                                                                                                                                                                                                            
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Validarcurvadeproduccion', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function validarproductofinal() as boolean                                                                                                                                                                                                                
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Validarproductofinal', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function valorsugeridocurvadeproduccion() as void                                                                                                                                                                                                         
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Valorsugeridocurvadeproduccion', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function valorsugeridoproductofinal() as void                                                                                                                                                                                                             
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Valorsugeridoproductofinal', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	protected function armarordendemodeloespecificosincurva() as void                                                                                                                                                                                         
		...
		return this.ObtenerResultado( 'Armarordendemodeloespecificosincurva', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	protected function esmodeloconcurvaespecifica() as boolean                                                                                                                                                                                                
		...
		return this.ObtenerResultado( 'Esmodeloconcurvaespecifica', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	protected function esordencongestioniniciada() as boolean                                                                                                                                                                                                 
		...
		return this.ObtenerResultado( 'Esordencongestioniniciada', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	protected function llenarcurvadeproduccion( togrilla as collection, tomodelo as object ) as void                                                                                                                                                          
		...
		return this.ObtenerResultado( 'Llenarcurvadeproduccion', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	protected function llenarinsumosdeproduccion( togrilla as collection, tomodelo as object ) as void                                                                                                                                                        
		...
		return this.ObtenerResultado( 'Llenarinsumosdeproduccion', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	protected function llenarprocesosdeproduccion( togrilla as collection, tomodelo as object ) as void                                                                                                                                                       
		...
		return this.ObtenerResultado( 'Llenarprocesosdeproduccion', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	protected function llenarsalidasdeproduccion( togrilla as collection, tomodelo as object ) as void                                                                                                                                                        
		...
		return this.ObtenerResultado( 'Llenarsalidasdeproduccion', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	protected function obtenercantidadporcombinacionencurva( tccurva as string, tccolor as string, tctalle as string ) as integer                                                                                                                             
		...
		return this.ObtenerResultado( 'Obtenercantidadporcombinacionencurva', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	protected function obtenerultimoproceso( tomodelo as object) as string                                                                                                                                                                                    
		...
		return this.ObtenerResultado( 'Obtenerultimoproceso', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	protected function validarcurvasingenericos() as boolean                                                                                                                                                                                                  
		...
		return this.ObtenerResultado( 'Validarcurvasingenericos', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	protected function validarultimoprocesounico() as boolean                                                                                                                                                                                                 
		...
		return this.ObtenerResultado( 'Validarultimoprocesounico', lcParametros )
	EndFunc
