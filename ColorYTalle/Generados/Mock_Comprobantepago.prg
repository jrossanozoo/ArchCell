...
define class Mock_comprobantepago as MockBase of MockBase.prg
	...
	cComprobante = ""                                                                                                                                                                                                                                         
	cComprobante = "COMPROBANTEPAGO"                                                                                                                                                                                                                          
	cContexto = ""                                                                                                                                                                                                                                            
	cDescripcion =  "Otros pagos"                                                                                                                                                                                                                             
	cDescripcionDeValorSugeridoParaVuelto = ""                                                                                                                                                                                                                
	cDescripcionSingular = ""                                                                                                                                                                                                                                 
	cDescuentoAnterior = 0                                                                                                                                                                                                                                    
	cDetalleComprobante = "ComprobantePagoDetalle"                                                                                                                                                                                                            
	...
	*-----------------------------------------------------------------------------------------
	function cambiosdetallecomprobantepagodetalle() as void                                                                                                                                                                                                   
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Cambiosdetallecomprobantepagodetalle', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function comprobantepagodetalle_access() as variant                                                                                                                                                                                                       
		lnParametros = pcount()lcParametros = ''		lnIdClase = _screen.Mocks.BuscarMock( 'COMPROBANTEPAGO' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'comprobantepagodetalle_access', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'comprobantepagodetalle_access', lcParametros )			endif		endif		if !this.ldestroy and ( !vartype( this.Comprobantepagodetalle ) = 'O' or isnull( this.Comprobantepagodetalle ) )			this.Comprobantepagodetalle = this.crearobjeto( 'DetalleComprobantepagoComprobantepagodetalle' )			this.Comprobantepagodetalle.inicializar()			this.enlazar( 'Comprobantepagodetalle.EventoObtenerLogueo', 'inyectarLogueo' )			this.enlazar( 'Comprobantepagodetalle.EventoObtenerInformacion', 'inyectarInformacion' )			bindevent( this.Comprobantepagodetalle, 'CambioSumarizado', this, 'CambiosDetalleComprobantepagodetalle', 1)			this.enlazar( 'Comprobantepagodetalle.EventoAdvertirLimitePorDiseno', 'EventoAdvertirLimitePorDiseno')			this.enlazar( 'Comprobantepagodetalle.EventoCancelarCargaLimitePorDiseno', 'EventoCancelarCargaLimitePorDiseno')			this.Comprobantepagodetalle.lVerificarLimitesEnDisenoImpresion = .t.		endif		return this.Comprobantepagodetalle
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function fechacompro_assign( txval as variant ) as void                                                                                                                                                                                                   
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'COMPROBANTEPAGO' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'fechacompro_assign', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'fechacompro_assign', lcParametros )			endif		endif		Local lxValOld As Variant		lxValOld = this.Fechacompro		local lxVal as Variant		lxVal = txVal		if This.CargaManual()			if this.Validar_Fechacompro( lxVal )				this.ValidarDominio_Fechacomprobante( lxVal )				this.Setear_Fechacompro( lxVal )			EndIf		Else			This.Setear_Fechacompro( lxVal )		EndIf		dodefault( lxVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function referencia_assign( txval as variant ) as void                                                                                                                                                                                                    
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'COMPROBANTEPAGO' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'referencia_assign', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'referencia_assign', lcParametros )			endif		endif		Local lxValOld As Variant		lxValOld = this.Referencia		local lxVal as Variant		lxVal = txVal		if This.CargaManual()			if this.Validar_Referencia( lxVal )				this.Setear_Referencia( lxVal )			EndIf		Else			This.Setear_Referencia( lxVal )		EndIf		dodefault( lxVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function setear_fechacompro( txval as variant ) as void                                                                                                                                                                                                   
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'COMPROBANTEPAGO' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'setear_fechacompro', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'setear_fechacompro', lcParametros )			endif		endif		this.Fechacompro = txVal		dodefault( txVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function setear_letra( txval ) as variant                                                                                                                                                                                                                 
	...
	*-----------------------------------------------------------------------------------------
	function setear_referencia( txval as variant ) as void                                                                                                                                                                                                    
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'COMPROBANTEPAGO' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'setear_referencia', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'setear_referencia', lcParametros )			endif		endif		this.Referencia = txVal		dodefault( txVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function validar_fechacompro( txval as variant ) as boolean                                                                                                                                                                                               
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'COMPROBANTEPAGO' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'validar_fechacompro', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'validar_fechacompro', lcParametros )			endif		endif		Return dodefault( txVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function validar_referencia( txval as variant ) as boolean                                                                                                                                                                                                
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'COMPROBANTEPAGO' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'validar_referencia', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'validar_referencia', lcParametros )			endif		endif		Return dodefault( txVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function valorsugeridofechacompro() as void                                                                                                                                                                                                               
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Valorsugeridofechacompro', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function valorsugeridofwfechacompro() as void                                                                                                                                                                                                             
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Valorsugeridofwfechacompro', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function valorsugeridoreferencia() as void                                                                                                                                                                                                                
		...
		return this.ObtenerResultado( 'Valorsugeridoreferencia', lcParametros )
	EndFunc
