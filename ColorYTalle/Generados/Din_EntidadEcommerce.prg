...
define class Din_EntidadECOMMERCE as entidad of entidad.prg
	...
	*--------------------------------------------------------------------------------------------------------
	function Equivaloresdetalle_Access() as variant
		if !this.ldestroy and ( !vartype( this.Equivaloresdetalle ) = 'O' or isnull( this.Equivaloresdetalle ) )
			this.Equivaloresdetalle = this.crearobjeto( 'Din_DetalleEcommerceEquivaloresdetalle' )
			this.Equivaloresdetalle.inicializar()
			this.enlazar( 'Equivaloresdetalle.EventoObtenerLogueo', 'inyectarLogueo' )
			this.enlazar( 'Equivaloresdetalle.EventoObtenerInformacion', 'inyectarInformacion' )
			bindevent( this.Equivaloresdetalle, 'CambioSumarizado', this, 'CambiosDetalleEquivaloresdetalle', 1) 
			this.enlazar( 'Equivaloresdetalle.EventoAdvertirLimitePorDiseno', 'EventoAdvertirLimitePorDiseno') 
			this.enlazar( 'Equivaloresdetalle.EventoCancelarCargaLimitePorDiseno', 'EventoCancelarCargaLimitePorDiseno') 
			this.Equivaloresdetalle.lVerificarLimitesEnDisenoImpresion = .t.
		endif
		return this.Equivaloresdetalle
	endfunc
	...
	*--------------------------------------------------------------------------------------------------------
	function SetearEstadoNuevoEnItems() as Void
		...
		this.EquiValoresDetalle.oItem.lNuevo = this.lNuevo
	endfunc
	...
	*--------------------------------------------------------------------------------------------------------
	function SetearEstadoEdicionEnItems() as Void
		...
		this.EquiValoresDetalle.oItem.lEdicion = this.lEdicion
	endfunc
	...
	*--------------------------------------------------------------------------------------------------------
	function CambiosDetalleEquivaloresdetalle() as void
		dodefault()
	endfunc
	...
	*--------------------------------------------------------------------------------------------------------
	...
	local lnLargoDecimal as integer, lnLargoEntero as Integer, lnLargoDecimal as integer, llRetorno as boolean 
		...
		if ( vartype( this.UsaEquiValores) = 'L' )
		else
			This.AgregarInformacion( 'El tipo de dato para el atributo USAEQUIVALORES no es el correcto.')
			llRetorno = .F.
		Endif
	...
	*--------------------------------------------------------------------------------------------------------
	function ValidacionBasica() as boolean
		...
		With This
			...
			llRetorno = .ValidarConctalle() and llRetorno
	...
	*--------------------------------------------------------------------------------------------------------
	function ValidacionDetalles() as Boolean
		...
		llRetorno = This.ValidarUnDetalle( this.EquiValoresDetalle, 'Equivalencias de valores' ) and llRetorno
		return llRetorno
	endfunc
	...
	function ValidarClienteDefault() as boolean
		...
		if empty( this.ClienteDefault_PK )
			...
			If this.ClienteDefault.oAD.ConsultarPorClavePrimaria()
				This.ClienteDefault.oDesactivador.ValidarEstadoActivacion( this.ClienteDefault_PK , goLibrerias.Valorvacioseguntipo( vartype(this.ClienteDefault_PK ) ), this.lnuevo, this.ledicion )
			Else
	...
	*--------------------------------------------------------------------------------------------------------
	function ValidarConcTalle() as boolean
		local llRetorno as boolean
		llRetorno = dodefault()
		if empty( this.ConcTalle_PK )
		else
			If this.ConcTalle.oAD.ConsultarPorClavePrimaria()
			Else
				this.AgregarInformacion( 'El dato buscado ' + alltrim(transform( this.ConcTalle_PK ))+ ' de la entidad ' +  upper( this.ConcTalle.cDescripcion ) + ' no existe.')
				llRetorno = .F.
			endif
		endif
		Return llRetorno
	...
	*--------------------------------------------------------------------------------------------------------
	function ObtenerFuncionalidades() as String
		return '<IMPOGENERICA><EXPOGENERICA>'
	Endfunc
	...
	*--------------------------------------------------------------------------------------------------------
	function Concenvio_PK_Assign( txVal as variant ) as void
		...
		if txVal = lxValOld or empty( txVal )
			...
			this.lBuscandoCodigo = .t.
			if !this.lEsSubEntidad and ( this.EsNuevo() or this.EsEdicion() )
				lxVal = This.Concenvio.oComportamientoCodigoSugerido.ObtenerPKaAsignar( txVal, This.Concenvio )
			else
				lxVal = txVal
			endif
			this.lBuscandoCodigo = .f.
	...
	*--------------------------------------------------------------------------------------------------------
	function Conccolor_PK_Assign( txVal as variant ) as void
		...
		if txVal = lxValOld or empty( txVal )
			...
			this.lBuscandoCodigo = .t.
			if !this.lEsSubEntidad and ( this.EsNuevo() or this.EsEdicion() )
				lxVal = This.Conccolor.oComportamientoCodigoSugerido.ObtenerPKaAsignar( txVal, This.Conccolor )
			else
				lxVal = txVal
			endif
			this.lBuscandoCodigo = .f.
	...
	*--------------------------------------------------------------------------------------------------------
	function Conctalle_PK_Assign( txVal as variant ) as void

		Local lxValOld As Variant
		lxValOld = this.Conctalle_PK
		local lxVal as Variant
		local loSugerido as Object
		...
		if txVal = lxValOld or empty( txVal )
			...
			this.lBuscandoCodigo = .t.
			if !this.lEsSubEntidad and ( this.EsNuevo() or this.EsEdicion() )
				lxVal = This.Conctalle.oComportamientoCodigoSugerido.ObtenerPKaAsignar( txVal, This.Conctalle )
			else
				lxVal = txVal
			endif
			this.lBuscandoCodigo = .f.
		...
		lxVal = iif( len( lxVal ) < 5, padr( lxVal, 5 ), lxVal )
		this.Conctalle_PK = lxVal

		if this.Validar_Conctalle( lxVal, lxValOld )
	...
	*--------------------------------------------------------------------------------------------------------
	function Concarticulo_PK_Assign( txVal as variant ) as void
		...
		if txVal = lxValOld or empty( txVal )
			...
			this.lBuscandoCodigo = .t.
			if !this.lEsSubEntidad and ( this.EsNuevo() or this.EsEdicion() )
				lxVal = This.Concarticulo.oComportamientoCodigoSugerido.ObtenerPKaAsignar( txVal, This.Concarticulo )
			else
				lxVal = txVal
			endif
			this.lBuscandoCodigo = .f.
	...
	*--------------------------------------------------------------------------------------------------------
	function Validar_Clientedefault( txVal as variant, txValOld as variant ) as Boolean
		...
		Return This.CargaManual() and dodefault( txVal, txValOld ) and This.Clientedefault.oDesactivador.ValidarEstadoActivacion( txVal, txValOld, this.lnuevo, this.ledicion) 
	...
	*--------------------------------------------------------------------------------------------------------
	function Validar_Conctalle( txVal as variant, txValOld as variant ) as Boolean
	...
	*--------------------------------------------------------------------------------------------------------
	function Usaequivalores_Assign( txVal as variant ) as void

		Local lxValOld As Variant
		lxValOld = this.Usaequivalores
		local lxVal as Variant
		lxVal = txVal

		if This.CargaManual()
			if this.Validar_Usaequivalores( lxVal )
				this.Setear_Usaequivalores( lxVal )
			EndIf
		Else
			This.Setear_Usaequivalores( lxVal ) 
		EndIf
	...
	*--------------------------------------------------------------------------------------------------------
	function Setear_Usaequivalores( txVal as variant ) as void

		this.Usaequivalores = txVal
		dodefault( txVal )
	...
	*--------------------------------------------------------------------------------------------------------
	function Validar_Usaequivalores( txVal as variant ) as Boolean
	...
	*--------------------------------------------------------------------------------------------------------
	function Conctalle_Access() as variant
		if this.ldestroy
		else
			if This.lInstanciarSubEntidadaDemanda
				if ( !vartype( this.Conctalle ) = 'O' or isnull( this.Conctalle ) )
					this.Conctalle = _Screen.zoo.instanciarentidad( 'Talle' )
					this.Conctalle.lEsSubEntidad = .t.
					this.enlazar( 'Conctalle.EventoObtenerLogueo', 'inyectarLogueo' )
					this.enlazar( 'Conctalle.EventoObtenerInformacion', 'inyectarInformacion' )
				endif
				if !this.lBuscandoCodigo
					if this.Conctalle.CODIGO # this.Conctalle_PK
						this.Conctalle.CODIGO = this.Conctalle_PK
					endif
				endif
			endif
		endif
		return this.Conctalle
	endfunc
		...
		with this
			...
			Catch to loError
				...
				if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
					...
					loEx.AgregarInformacion( "Se produjo un error al intentar asignar el valor sugerido " + lcExpresionValorSugeridoDefinidoPorElUsuario + " del atributo Concepto para envío para la entidad Plataformas." + chr( 10 ) + "Verifique el valor especificado en la entidad Comportamiento de atributos." + chr( 10 ) + "El tipo de dato del atributo es caracter." )
					goServicios.Errores.LevantarExcepcion( loEx )
				endif
			endtry 
		endwith
			...
			try
				...
				if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
					if !empty( lcExpresionValorSugeridoDefinidoPorElUsuario )
						...
						This.EventoEstaSeteandoValorSugerido( 'Conctalle_PK' )
						.Conctalle_PK = lvValorSugeridoDefinidoPorElUsuario
						.Conctalle.CODIGO = .Conctalle_PK
					endif
				endif
			Catch to loError
				.Conctalle_PK=[]
	...
	*--------------------------------------------------------------------------------------------------------
	Function ValorSugeridoUsaequivalores() as void
		local lcExpresionValorSugeridoDefinidoPorElUsuario as String, lvValorSugeridoDefinidoPorElUsuario as Variant, loError as Object, loEx as Object, loExTipoDato as Object
		lcExpresionValorSugeridoDefinidoPorElUsuario = null
		with this
			lcExpresionValorSugeridoDefinidoPorElUsuario = goServicios.SaltosDeCampoYValoresSugeridos.ObtenerValorSugerido( "ECOMMERCE", "", "Usaequivalores" )
			if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
				if !empty( lcExpresionValorSugeridoDefinidoPorElUsuario )
					try
						lvValorSugeridoDefinidoPorElUsuario = &lcExpresionValorSugeridoDefinidoPorElUsuario
						...
						This.EventoEstaSeteandoValorSugerido( 'Usaequivalores' )
						.Usaequivalores = lvValorSugeridoDefinidoPorElUsuario
					catch to loError
						loEx = newobject("zooexception", "zooexception.prg" )
						loEx.Grabar( loError )
						loEx.AgregarInformacion( iif( type( "loError.UserValue.Message" ) == "C", loError.UserValue.Message, loError.Message ) )
						loEx.AgregarInformacion( "Se produjo un error al intentar asignar el valor sugerido " + lcExpresionValorSugeridoDefinidoPorElUsuario + " del atributo Habilitar equivalencias de valores para la entidad Plataformas." + chr( 10 ) + "Verifique el valor especificado en la entidad Comportamiento de atributos." + chr( 10 ) + "El tipo de dato del atributo es lógico." )
						goServicios.Errores.LevantarExcepcion( loEx )
					endtry
				endif
			endif
