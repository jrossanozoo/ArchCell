...
define class Din_DetalleFACTURAELECTRONICADECREDITOPromocionesdetalle as DetallePromociones of DetallePromociones.Prg
	...
	Protected Function _CargarItem( tnItem as integer )
		...
		With this.oItem
			Try
				...
				.Idartsafec = this.Item[ tnItem ].Idartsafec
				llVacio = llVacio and this.VerificarSiElAtributoEstaVacio( 'Idartsafec', .Idartsafec )
	...
	*--------------------------------------------------------------------------------------------------------
	protected function AplanarItem() as Object
		...
		with loRetorno
			...
			.Idartsafec = this.oItem.Idartsafec
	...
	*--------------------------------------------------------------------------------------------------------
	...
	with this
		if .ValidarItem()
			...
			.Item[ lnItem ].Idartsafec = .oItem.Idartsafec
	...
	*--------------------------------------------------------------------------------------------------------
	function oItem_Access() as variant
		if !this.ldestroy and ( !vartype( this.oItem ) = 'O' or isnull( this.oItem ) )
			this.oItem = _Screen.zoo.crearobjeto( 'Din_ItemFACTURAELECTRONICADECREDITOPromocionesdetalle' )
	...
	*--------------------------------------------------------------------------------------------------------
	Function Cargar() as Void
		...
		scan for &lcFiltro
			...
			loItem.Idartsafec = c_Promocionesdetalle.Idartsafec
	...
	*-----------------------------------------------------------------------------------------
	...
	local lnLargoEntero as integer, lnValorDelDecimal as integer, lnLargoDecimal as integer, llRetorno as boolean, lnI as Integer
		...
		for lni = 1 to this.Count
			...
			if vartype( this.Item[lni].IDARTSAFEC) = 'C'
			else
				llRetorno = .F.
				this.AgregarInformacion( 'El tipo de dato para el atributo IDARTSAFEC del item ' + transform( lni ) + ' no es el correcto, en el detalle ' + this.cEtiqueta + '.')
			Endif
			...
			if vartype( this.Item[lni].PROMOCIONDETALLE) = 'C'
				if len( alltrim( this.Item[lni].PROMOCIONDETALLE )) <= 200
				else
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud del valor del atributo PROMOCIONDETALLE del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform( len( alltrim( this.Item[lni].PROMOCIONDETALLE ))) + ' Máxima:200), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
	...
	*--------------------------------------------------------------------------------------------------------
	function CopiarItemaItem( toItemOrigen as Object, toItemDestino as Object ) as Object
		...
		toItemDestino.Idartsafec = toItemOrigen.Idartsafec
