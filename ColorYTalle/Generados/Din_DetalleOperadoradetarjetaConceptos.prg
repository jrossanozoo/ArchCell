		...
		for lni = 1 to this.Count
			...
			if vartype( this.Item[lni].ORDEN) = 'N'
				...
				if lnLargoEntero <= 4
					...
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud entera del valor del atributo ORDEN del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform(lnLargoEntero) + ' Máxima:4), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
