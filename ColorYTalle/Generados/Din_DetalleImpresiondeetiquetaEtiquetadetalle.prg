		...
		for lni = 1 to this.Count
			...
			if vartype( this.Item[lni].NOTA) = 'C'
				if len( alltrim( this.Item[lni].NOTA )) <= 254
					...
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud del valor del atributo NOTA del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform( len( alltrim( this.Item[lni].NOTA ))) + ' Máxima:254), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
