...
define class Din_DetalleORDENDEPRODUCCIONOrdenprocesos as DetalleBaseproduccion of DetalleBaseproduccion.Prg
	...
	*--------------------------------------------------------------------------------------------------------
	function oItem_Access() as variant
		if !this.ldestroy and ( !vartype( this.oItem ) = 'O' or isnull( this.oItem ) )
			this.oItem = _Screen.zoo.crearobjeto( 'ItemORDENDEPRODUCCIONOrdenprocesos' )
	...
	*--------------------------------------------------------------------------------------------------------
	Function Cargar() as Void
		...
		select c_Ordenprocesos
		lcFiltro = this.ObtenerFiltro()
		scan for &lcFiltro
			loItem = createobject('ItemAuxiliar')

			loItem.CodigoOrden = c_Ordenprocesos.CodigoOrden
			loItem.NROITEM = c_Ordenprocesos.NROITEM
			loItem.Proceso_PK = c_Ordenprocesos.Proceso
			loItem.ProcesoDetalle = c_Ordenprocesos.ProcesoDetalle
			loItem.Orden = c_Ordenprocesos.Orden
			loItem.Taller_PK = c_Ordenprocesos.Taller
			loItem.InventarioEntrada_PK = c_Ordenprocesos.InventarioEntrada
			loItem.InventarioSalida_PK = c_Ordenprocesos.InventarioSalida
			loItem.NroItem = lnI 
		...
		for lni = 1 to this.Count
			...
			if vartype( this.Item[lni].ORDEN) = 'N'
				...
				if lnLargoEntero <= 5
					...
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud entera del valor del atributo ORDEN del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform(lnLargoEntero) + ' Máxima:5), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
