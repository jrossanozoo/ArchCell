...
define class Din_EntidadPRECIODEARTICULO as entidad of entidad.prg
	...
	*--------------------------------------------------------------------------------------------------------
	...
	local lnLargoDecimal as integer, lnLargoEntero as Integer, lnLargoDecimal as integer, llRetorno as boolean 
		...
		if ( vartype( this.AgregadoAlPrecio) = 'N' )
			lnLargoEntero  = len( transform( int( this.AGREGADOALPRECIO ) ) ) 
			lnValorDelDecimal = this.AGREGADOALPRECIO - int( this.AGREGADOALPRECIO)
			lnLargoDecimal = iif( lnValorDelDecimal = 0, 0, len( transform( lnValorDelDecimal ) ) - 2 )
			if lnLargoEntero > 13
				This.AgregarInformacion( 'La longitud entera del valor del atributo AGREGADOALPRECIO es mayor a la permitida.' )
				llRetorno = .F.
			Endif
		else
			This.AgregarInformacion( 'El tipo de dato para el atributo AGREGADOALPRECIO no es el correcto.')
			llRetorno = .F.
		Endif
		...
		if ( vartype( this.SobrePrecio) = 'N' )
			lnLargoEntero  = len( transform( int( this.SOBREPRECIO ) ) ) 
			lnValorDelDecimal = this.SOBREPRECIO - int( this.SOBREPRECIO)
			lnLargoDecimal = iif( lnValorDelDecimal = 0, 0, len( transform( lnValorDelDecimal ) ) - 2 )
			if lnLargoEntero > 13
				This.AgregarInformacion( 'La longitud entera del valor del atributo SOBREPRECIO es mayor a la permitida.' )
				llRetorno = .F.
			Endif
		else
			This.AgregarInformacion( 'El tipo de dato para el atributo SOBREPRECIO no es el correcto.')
			llRetorno = .F.
		Endif
		 
		if ( vartype( this.Codigo) = 'C' )
			if len( alltrim( this.CODIGO ) ) > 46
		...
		if ( vartype( this.Bloqueo) = 'L' )
		else
			This.AgregarInformacion( 'El tipo de dato para el atributo BLOQUEO no es el correcto.')
			llRetorno = .F.
		Endif
	...
	*--------------------------------------------------------------------------------------------------------
	function ValidarFechaVigencia() as boolean
		...
		if empty( this.FechaVigencia )
			this.AgregarInformacion( 'Debe cargar el campo Fecha de vigencia', 9005, 'FechaVigencia' )
			llRetorno = .F.
		endif
		return llRetorno
	endfunc
	...
	*--------------------------------------------------------------------------------------------------------
	function ValidarArticulo() as boolean
		...
		if empty( this.Articulo_PK )
			this.AgregarInformacion( 'Debe cargar el campo Artículo', 9005, 'Articulo' )
			llRetorno = .F.
		else
	...
	*--------------------------------------------------------------------------------------------------------
	function ValidarListaDePrecio() as boolean
		...
		if empty( this.ListaDePrecio_PK )
			this.AgregarInformacion( 'Debe cargar el campo Lista de precio', 9005, 'ListaDePrecio' )
			llRetorno = .F.
		else
	...
	*--------------------------------------------------------------------------------------------------------
	function obtenerAtributosObligatorios() as ZooColeccion
		...
		loAtributosObligatorios.add( 'LISTADEPRECIO' )
	...
	*--------------------------------------------------------------------------------------------------------
	Protected Function _Buscar() As void
		...
		if empty( This.&lcAtributo )
			...
			lcMensaje = 'El dato buscado TIMESTAMP ' + transform( This.TimestampAlta ) + ', LISTA DE PRECIO ' + transform( This.ListaDePrecio_Pk ) + ', ARTÍCULO ' + transform( This.Articulo_Pk ) + ', COLOR ' + transform( This.Color_Pk ) + ', TALLE ' + transform( This.Talle_Pk ) + ' de la entidad ' + upper( this.cDescripcion ) + ' no existe.'
		else
	...
	*--------------------------------------------------------------------------------------------------------
	function ObtenerFuncionalidades() as String
		return '<IMPOGENERICA><HABILITARIMPOINSEGURA><AUDITORIA:B><AUDITORIA<MODULOSLISTADO:PQH><NOLOGUEAR><RECEPCIONBULK><IMPOBULKCONVALIDACIONES><SINATRIBUTOSFW><FORZARREST><REALTIME>'
	Endfunc
	*--------------------------------------------------------------------------------------------------------
	...
	*--------------------------------------------------------------------------------------------------------
	function Inicializar() as Void
		...
		this.oAtributosCC.add( "TimestampAlta" )
		this.oAtributosCC.add( "ListaDePrecio_pk" )
		...
		oICol = goServicios.SaltosDeCampoYValoresSugeridos.ObtenerValorSugeridoDeUnaEntidadDetalle( 'PRECIODEARTICULO', '')
