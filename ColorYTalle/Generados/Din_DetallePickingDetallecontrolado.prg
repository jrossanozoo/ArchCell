...
define class Din_DetallePICKINGDetallecontrolado as detalle of detalle.prg
	...
	Protected Function _CargarItem( tnItem as integer )
		...
		With this.oItem
			Try
				...
				.Agregado = this.Item[ tnItem ].Agregado
				llVacio = llVacio and this.VerificarSiElAtributoEstaVacio( 'Agregado', .Agregado )
	...
	*--------------------------------------------------------------------------------------------------------
	...
	with this
		if .ValidarItem()
			...
			.Item[ lnItem ].Agregado = .oItem.Agregado
	...
	*--------------------------------------------------------------------------------------------------------
	function Sumarizar() as boolean
		local lnItem as integer, loItem as Object, lnSum_Afe_saldo as Number, lnSum_Cantidad as Number

		with this
			lnSum_Afe_saldo = 0
			lnSum_Cantidad = 0
			if this.lEsNavegacion
				if select( 'c_Detallecontrolado' ) > 0 and reccount( 'c_Detallecontrolado' ) > 0
					sum Afe_saldo, Cantidad;
						 to ;
						 lnSum_Afe_saldo, lnSum_Cantidad
				endif
	...
	*--------------------------------------------------------------------------------------------------------
	function oItem_Access() as variant
		if !this.ldestroy and ( !vartype( this.oItem ) = 'O' or isnull( this.oItem ) )
			this.oItem = _Screen.zoo.crearobjeto( 'Din_ItemPICKINGDetallecontrolado' )
	...
	*--------------------------------------------------------------------------------------------------------
	Function Cargar() as Void
		...
		select c_Detallecontrolado
		lcFiltro = this.ObtenerFiltro()
		scan for &lcFiltro
			loItem = createobject('ItemAuxiliar')

			loItem.Codigo = c_Detallecontrolado.Codigo
			loItem.Articulo_PK = c_Detallecontrolado.Articulo
			loItem.NoProcesarStock = c_Detallecontrolado.NoProcesarStock
			loItem.Equivalencia_PK = c_Detallecontrolado.Equivalencia
			loItem.Agregado = c_Detallecontrolado.Agregado
			loItem.Comportamiento = c_Detallecontrolado.Comportamiento
			loItem.NROITEM = c_Detallecontrolado.NROITEM
			loItem.ArticuloDetalle = c_Detallecontrolado.ArticuloDetalle
			loItem.Color_PK = c_Detallecontrolado.Color
			loItem.ColorDetalle = c_Detallecontrolado.ColorDetalle
			loItem.Talle_PK = c_Detallecontrolado.Talle
			loItem.Cantidad = c_Detallecontrolado.Cantidad
			loItem.Afe_saldo = c_Detallecontrolado.Afe_saldo
			loItem.NroItem = lnI 
	...
	*-----------------------------------------------------------------------------------------
	...
	local lnLargoEntero as integer, lnValorDelDecimal as integer, lnLargoDecimal as integer, llRetorno as boolean, lnI as Integer
		...
		for lni = 1 to this.Count
			...
			if vartype( this.Item[lni].AGREGADO) = 'L'
			else
				llRetorno = .F.
				this.AgregarInformacion( 'El tipo de dato para el atributo AGREGADO del item ' + transform( lni ) + ' no es el correcto, en el detalle ' + this.cEtiqueta + '.')
			Endif
	...
	*--------------------------------------------------------------------------------------------------------
	function CopiarItemaItem( toItemOrigen as Object, toItemDestino as Object ) as Object
		...
		toItemDestino.Agregado = toItemOrigen.Agregado
