		...
		for lni = 1 to this.Count
			...
			if vartype( this.Item[lni].OPERADORADETARJETA_PK) = 'C'
				if len( alltrim( this.Item[lni].OPERADORADETARJETA_PK )) <= 15
					...
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud del valor del atributo OPERADORADETARJETA del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform( len( alltrim( this.Item[lni].OPERADORADETARJETA_PK ))) + ' Máxima:15), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
			...
			if vartype( this.Item[lni].OPERADORADETARJETADETALLE) = 'C'
				if len( alltrim( this.Item[lni].OPERADORADETARJETADETALLE )) <= 100
					...
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud del valor del atributo OPERADORADETARJETADETALLE del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform( len( alltrim( this.Item[lni].OPERADORADETARJETADETALLE ))) + ' Máxima:100), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
			...
			if vartype( this.Item[lni].NUMEROCOMERCIO) = 'C'
				if len( alltrim( this.Item[lni].NUMEROCOMERCIO )) <= 15
					...
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud del valor del atributo NUMEROCOMERCIO del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform( len( alltrim( this.Item[lni].NUMEROCOMERCIO ))) + ' Máxima:15), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
