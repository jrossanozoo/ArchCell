...
define class Mock_adt_cliente as MockBase of MockBase.prg
	...
	cCaracteres = ""                                                                                                                                                                                                                                          
	cContexto = ""                                                                                                                                                                                                                                            
	cDescripcion =  "Auditoria Clientes"                                                                                                                                                                                                                      
	...
	*-----------------------------------------------------------------------------------------
	function validar_codigo( txval as variant, txvalold as variant ) as boolean                                                                                                                                                                               
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1, lxParam2lnParametros = pcount()lxParam1 = txvallxParam2 = txvaloldlcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'ADT_CLIENTE' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'validar_codigo', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'validar_codigo', lcParametros )			endif		endif		Return This.CargaManual() and dodefault( txVal, txValOld ) and This.Codigo.oDesactivador.ValidarEstadoActivacion( txVal, txValOld, this.lnuevo, this.ledicion)
	EndFunc
