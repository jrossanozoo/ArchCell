...
define class Din_EntidadCAEA as entidad of entidad.prg
	...
	*--------------------------------------------------------------------------------------------------------
	...
	local lnLargoDecimal as integer, lnLargoEntero as Integer, lnLargoDecimal as integer, llRetorno as boolean 
		...
		if ( vartype( this.NumeroCAEA) = 'C' )
			if len( alltrim( this.NUMEROCAEA ) ) > 20
				This.AgregarInformacion( 'La longitud del valor del atributo NUMEROCAEA es mayor a la permitida.' )
				llRetorno = .F.
			Endif
		else
			This.AgregarInformacion( 'El tipo de dato para el atributo NUMEROCAEA no es el correcto.')
			llRetorno = .F.
		Endif
		...
		if ( vartype( this.ComentariosAFIP) = 'C' ) 
		else
			This.AgregarInformacion( 'El tipo de dato para el atributo COMENTARIOSAFIP no es el correcto.')
			llRetorno = .F.
		Endif
		...
		if ( vartype( this.FechaDeProceso) = 'D' )
		else
			This.AgregarInformacion( 'El tipo de dato para el atributo FECHADEPROCESO no es el correcto.')
			llRetorno = .F.
		Endif
	...
	*--------------------------------------------------------------------------------------------------------
	function ValidacionBasica() as boolean
		...
		Endif
		if This.ValidarIngreso( transform( this.NUMEROINTERNO ) )
		else
			llRetorno = .F.
	...
	*--------------------------------------------------------------------------------------------------------
	...
	With This
		...
		If llValidar
			...
			lcAgrupadorDeActividad = lcAgrupadorDeActividad + '<PK:' + transform( This.NUMEROINTERNO ) + '>'
	...
	*--------------------------------------------------------------------------------------------------------
	Protected Function _Buscar() As void
		...
		if empty( This.&lcAtributo )
			...
			lcMensaje = 'El dato buscado ' + alltrim( transform( This.NUMEROINTERNO ) ) + ' de la entidad ' + upper( this.cDescripcion ) + ' no existe.'
		endif
		if llRetorno
		else
	...
	*--------------------------------------------------------------------------------------------------------
	function Inicializar() as Void
		...
		this.oColeccionVSFW.agregar('.ValorSugeridoFWNumerointerno()','Numerointerno')
		this.oColeccionVSFW.agregar('.ValorSugeridoFWPtoventa()','Ptoventa')
		this.oColeccionVSFW.agregar('.ValorSugeridoFWPeriodocaea()','Periodocaea')
		this.oColeccionVSFW.agregar('.ValorSugeridoFWQuincena()','Quincena')
		oICol = goServicios.SaltosDeCampoYValoresSugeridos.ObtenerValorSugeridoDeUnaEntidadDetalle( 'CAEA', '')
	...
	*--------------------------------------------------------------------------------------------------------
	Function ValorSugeridoObservacion() as void
		...
		with this
			lcExpresionValorSugeridoDefinidoPorElUsuario = goServicios.SaltosDeCampoYValoresSugeridos.ObtenerValorSugerido( "CAEA", "", "Observacion" )
	...
	*--------------------------------------------------------------------------------------------------------
	Function ValorSugeridoFWNumerointerno() as void
		with this
			.Numerointerno = this.ObtenerSiguienteNumerico()
		endwith
	endfunc
	...
	*--------------------------------------------------------------------------------------------------------
	Function ValorSugeridoPtoventa() as void
		...
		lcExpresionValorSugeridoDefinidoPorElUsuario = null
		with this
			lcExpresionValorSugeridoDefinidoPorElUsuario = goServicios.SaltosDeCampoYValoresSugeridos.ObtenerValorSugerido( "CAEA", "", "Ptoventa" )
			if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
				if !empty( lcExpresionValorSugeridoDefinidoPorElUsuario )
					try
						lvValorSugeridoDefinidoPorElUsuario = &lcExpresionValorSugeridoDefinidoPorElUsuario
						...
						This.EventoEstaSeteandoValorSugerido( 'Ptoventa' )
						.Ptoventa = lvValorSugeridoDefinidoPorElUsuario
					catch to loError
						loEx = newobject("zooexception", "zooexception.prg" )
						loEx.Grabar( loError )
						loEx.AgregarInformacion( iif( type( "loError.UserValue.Message" ) == "C", loError.UserValue.Message, loError.Message ) )
	...
	*--------------------------------------------------------------------------------------------------------
	Function ValorSugeridoFWPtoventa() as void
		with this
			.Ptoventa = goparametros.felino.gestiondeventas.facturacionelectronica.nacional.puntodeventacaea
		endwith
	endfunc
	...
	*--------------------------------------------------------------------------------------------------------
	Function ValorSugeridoPeriodocaea() as void
		...
		lcExpresionValorSugeridoDefinidoPorElUsuario = null
		with this
			lcExpresionValorSugeridoDefinidoPorElUsuario = goServicios.SaltosDeCampoYValoresSugeridos.ObtenerValorSugerido( "CAEA", "", "Periodocaea" )
			if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
				if !empty( lcExpresionValorSugeridoDefinidoPorElUsuario )
					try
						lvValorSugeridoDefinidoPorElUsuario = &lcExpresionValorSugeridoDefinidoPorElUsuario
						...
						This.EventoEstaSeteandoValorSugerido( 'Periodocaea' )
						.Periodocaea = lvValorSugeridoDefinidoPorElUsuario
					catch to loError
						loEx = newobject("zooexception", "zooexception.prg" )
						loEx.Grabar( loError )
						loEx.AgregarInformacion( iif( type( "loError.UserValue.Message" ) == "C", loError.UserValue.Message, loError.Message ) )
						loEx.AgregarInformacion( "Se produjo un error al intentar asignar el valor sugerido " + lcExpresionValorSugeridoDefinidoPorElUsuario + " del atributo Período de CAEA (YYYY/MM) para la entidad Obtención de CAEA." + chr( 10 ) + "Verifique el valor especificado en la entidad Comportamiento de atributos." + chr( 10 ) + "El tipo de dato del atributo es caracter." )
						goServicios.Errores.LevantarExcepcion( loEx )
					endtry
				endif
			else
				.ValorSugeridoFWPeriodocaea()
			endif
		endwith
	endfunc
	...
	*--------------------------------------------------------------------------------------------------------
	Function ValorSugeridoFWPeriodocaea() as void
		with this
			.Periodocaea = this.SugerirFechaPeriodo()
		endwith
	endfunc
	...
	*--------------------------------------------------------------------------------------------------------
	Function ValorSugeridoQuincena() as void
		...
		lcExpresionValorSugeridoDefinidoPorElUsuario = null
		with this
			lcExpresionValorSugeridoDefinidoPorElUsuario = goServicios.SaltosDeCampoYValoresSugeridos.ObtenerValorSugerido( "CAEA", "", "Quincena" )
			if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
				if !empty( lcExpresionValorSugeridoDefinidoPorElUsuario )
					try
						lvValorSugeridoDefinidoPorElUsuario = &lcExpresionValorSugeridoDefinidoPorElUsuario
						...
						This.EventoEstaSeteandoValorSugerido( 'Quincena' )
						.Quincena = lvValorSugeridoDefinidoPorElUsuario
					catch to loError
						loEx = newobject("zooexception", "zooexception.prg" )
						loEx.Grabar( loError )
						loEx.AgregarInformacion( iif( type( "loError.UserValue.Message" ) == "C", loError.UserValue.Message, loError.Message ) )
						loEx.AgregarInformacion( "Se produjo un error al intentar asignar el valor sugerido " + lcExpresionValorSugeridoDefinidoPorElUsuario + " del atributo Quincena para la entidad Obtención de CAEA." + chr( 10 ) + "Verifique el valor especificado en la entidad Comportamiento de atributos." + chr( 10 ) + "El tipo de dato del atributo es numérico." )
						goServicios.Errores.LevantarExcepcion( loEx )
					endtry
				endif
			else
				.ValorSugeridoFWQuincena()
			endif
		endwith
	endfunc
