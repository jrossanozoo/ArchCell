...
define class Din_DetalleMOVIMIENTOSTOCKAPRODUCCMovimientodetalle as DetalleArticulosproduccion of DetalleArticulosproduccion.Prg
	...
	Protected Function _CargarItem( tnItem as integer )
		...
		With this.oItem
			Try
				...
				.Unidad = this.Item[ tnItem ].Unidad
				llVacio = llVacio and this.VerificarSiElAtributoEstaVacio( 'Unidad', .Unidad )
				...
				.UnidadInsumo = this.Item[ tnItem ].UnidadInsumo
				llVacio = llVacio and this.VerificarSiElAtributoEstaVacio( 'UnidadInsumo', .UnidadInsumo )
			
				.Rinde = this.Item[ tnItem ].Rinde
				llVacio = llVacio and this.VerificarSiElAtributoEstaVacio( 'Rinde', .Rinde )
			
				.CantidadInsumo = this.Item[ tnItem ].CantidadInsumo
				llVacio = llVacio and this.VerificarSiElAtributoEstaVacio( 'CantidadInsumo', .CantidadInsumo )
	...
	*--------------------------------------------------------------------------------------------------------
	protected function AplanarItem() as Object
		...
		with loRetorno
			...
			.UnidadInsumo = this.oItem.UnidadInsumo
			.Rinde = this.oItem.Rinde
			.CantidadInsumo = this.oItem.CantidadInsumo
	...
	*--------------------------------------------------------------------------------------------------------
	...
	with this
		if .ValidarItem()
			...
			.Item[ lnItem ].Unidad = .oItem.Unidad
			...
			.Item[ lnItem ].UnidadInsumo = .oItem.UnidadInsumo
			.Item[ lnItem ].Rinde = .oItem.Rinde
			.Item[ lnItem ].CantidadInsumo = .oItem.CantidadInsumo
		else
			goServicios.Errores.LevantarExcepcion( .ObtenerInformacion() )
		endif
	endwith
	...
	*--------------------------------------------------------------------------------------------------------
	function Sumarizar() as boolean
		local lnItem as integer, loItem as Object, lnSum_Cantidad as Number, lnSum_Cantidadinsumo as Number

		with this
			lnSum_Cantidad = 0
			lnSum_Cantidadinsumo = 0
			if this.lEsNavegacion
				if select( 'c_Movimientodetalle' ) > 0 and reccount( 'c_Movimientodetalle' ) > 0
					sum Cantidad, Cantidadinsumo;
						 to ;
						 lnSum_Cantidad, lnSum_Cantidadinsumo
				endif
					...
					if .oItem.NroItem = lnItem
						...
						with loItem 
							if !empty(.ARTICULO_PK)
								...
								lnSum_Cantidadinsumo = lnSum_Cantidadinsumo + .Cantidadinsumo
							endif
						endwith
					endif
				endfor
			...
			if .Sum_Cantidadinsumo != lnSum_Cantidadinsumo
				.Sum_Cantidadinsumo = lnSum_Cantidadinsumo
				.EventoCambioSum_Cantidadinsumo()
			endif
	...
	*--------------------------------------------------------------------------------------------------------
	function Acumular( tlForzar as Boolean, tcAtributo as String, tnValor as Decimal, tnValorAnt as Decimal ) as Void
		...
		case tcAtributo == 'Cantidadinsumo'
			this.Sum_Cantidadinsumo = this.Sum_Cantidadinsumo - tnValorAnt + tnValor
			this.EventoCambioSum_Cantidadinsumo()
	...
	*--------------------------------------------------------------------------------------------------------
	function oItem_Access() as variant
		if !this.ldestroy and ( !vartype( this.oItem ) = 'O' or isnull( this.oItem ) )
			this.oItem = _Screen.zoo.crearobjeto( 'ItemMOVIMIENTOSTOCKAPRODUCCMovimientodetalle' )
	...
	*--------------------------------------------------------------------------------------------------------
	Function Cargar() as Void
		...
		scan for &lcFiltro
			...
			loItem.Unidad = c_Movimientodetalle.Unidad
			...
			loItem.UnidadInsumo = c_Movimientodetalle.UnidadInsumo
			loItem.Rinde = c_Movimientodetalle.Rinde
			loItem.CantidadInsumo = c_Movimientodetalle.CantidadInsumo
			loItem.NroItem = lnI 
	...
	*-----------------------------------------------------------------------------------------
	...
	local lnLargoEntero as integer, lnValorDelDecimal as integer, lnLargoDecimal as integer, llRetorno as boolean, lnI as Integer
		...
		for lni = 1 to this.Count
			...
			if vartype( this.Item[lni].UNIDAD) = 'C'
				if len( alltrim( this.Item[lni].UNIDAD )) <= 3
				else
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud del valor del atributo UNIDAD del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform( len( alltrim( this.Item[lni].UNIDAD ))) + ' Máxima:3), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
			...
			if vartype( this.Item[lni].UNIDADINSUMO) = 'C'
				if len( alltrim( this.Item[lni].UNIDADINSUMO )) <= 3
				else
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud del valor del atributo UNIDADINSUMO del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform( len( alltrim( this.Item[lni].UNIDADINSUMO ))) + ' Máxima:3), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
				this.AgregarInformacion( 'El tipo de dato para el atributo UNIDADINSUMO del item ' + transform( lni ) + ' no es el correcto, en el detalle ' + this.cEtiqueta + '.')
			Endif
			...
			if vartype( this.Item[lni].CANTIDADINSUMO) = 'N'
				lnLargoEntero = len( transform( int( this.Item[lni].CANTIDADINSUMO ) ) ) 
				lnValorDelDecimal = this.Item[lni].CANTIDADINSUMO - int( this.Item[lni].CANTIDADINSUMO )
				lnLargoDecimal = iif( lnValorDelDecimal = 0, 0, len( transform( lnValorDelDecimal ) ) - 2 )
				if lnLargoEntero <= 8
				else
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud entera del valor del atributo CANTIDADINSUMO del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform(lnLargoEntero) + ' Máxima:8), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
				this.AgregarInformacion( 'El tipo de dato para el atributo CANTIDADINSUMO del item ' + transform( lni ) + ' no es el correcto, en el detalle ' + this.cEtiqueta + '.')
			Endif
		EndFor
		Return llRetorno 
	Endfunc
	...
	*--------------------------------------------------------------------------------------------------------
	function CopiarItemaItem( toItemOrigen as Object, toItemDestino as Object ) as Object
		...
		toItemDestino.Unidad = toItemOrigen.Unidad
		...
		toItemDestino.Unidadinsumo = toItemOrigen.Unidadinsumo
		toItemDestino.Rinde = toItemOrigen.Rinde
		toItemDestino.Cantidadinsumo = toItemOrigen.Cantidadinsumo
	endfunc
