	...
	function Caja_PK_Assign( txVal as variant ) as void
		...
		if this.Validar_Caja( lxVal, lxValOld )
			if ( this.lHabilitarCaja_PK or this.lEstaSeteandoValorSugerido or lxValOld == lxVal )
				...
				goServicios.Errores.LevantarExcepcion( "No se puede cambiar este valor (Entidad: Comprobantes de caja - Atributo: Caja)" )
			endif 
		endif
		dodefault( lxVal )
		...
		if This.CargaManual()
			if this.Validar_Chequeelectronico( lxVal )
				...
				this.EventoDespuesDeSetear( This, 'Chequeelectronico', lxValOld, lxVal  )
					...
					goServicios.Errores.LevantarExcepcion( "No se puede cambiar este valor (Entidad: Comprobantes de caja - Atributo: Chequeelectronico)" )
				endif 
			EndIf
		Else
			This.Setear_Chequeelectronico( lxVal ) 
		...
		if This.CargaManual()
			if this.Validar_Numerointerno( lxVal )
				...
				this.EventoDespuesDeSetear( This, 'Numerointerno', lxValOld, lxVal  )
					...
					goServicios.Errores.LevantarExcepcion( "No se puede cambiar este valor (Entidad: Comprobantes de caja - Atributo: Numerointerno)" )
				endif 
			EndIf
		Else
			This.Setear_Numerointerno( lxVal ) 
		...
		if This.CargaManual()
			if this.Validar_Monto( lxVal )
				...
				this.EventoDespuesDeSetear( This, 'Monto', lxValOld, lxVal  )
					...
					this.Monto = lxValOld
					goServicios.Errores.LevantarExcepcion( "No se puede cambiar este valor (Entidad: Comprobantes de caja - Atributo: Monto)" )
				endif 
			EndIf
		Else
			This.Setear_Monto( lxVal ) 
