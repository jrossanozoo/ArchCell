...
define class Mock_chequepropio as MockBase of MockBase.prg
	...
	*-----------------------------------------------------------------------------------------
	function chequeelectronico_assign( txval as variant ) as void                                                                                                                                                                                             
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'CHEQUEPROPIO' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'chequeelectronico_assign', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'chequeelectronico_assign', lcParametros )			endif		endif		Local lxValOld As Variant		lxValOld = this.Chequeelectronico		local lxVal as Variant		lxVal = txVal		if This.CargaManual()			if this.Validar_Chequeelectronico( lxVal )				if ( this.lHabilitarChequeelectronico or this.lEstaSeteandoValorSugerido or lxValOld == lxVal)					this.Setear_Chequeelectronico( lxVal )				else					this.Chequeelectronico = lxValOld					goServicios.Errores.LevantarExcepcion( "No se puede cambiar este valor (Entidad: Cheque propio - Atributo: Chequeelectronico)" )				endif			EndIf		Else			This.Setear_Chequeelectronico( lxVal )		EndIf		dodefault( lxVal )
	EndFunc

	*-----------------------------------------------------------------------------------------
	function chequera_access() as variant                                                                                                                                                                                                                     
		lnParametros = pcount()lcParametros = ''		lnIdClase = _screen.Mocks.BuscarMock( 'CHEQUEPROPIO' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'chequera_access', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'chequera_access', lcParametros )			endif		endif		if this.ldestroy		else			if This.lInstanciarSubEntidadaDemanda				if ( !vartype( this.Chequera ) = 'O' or isnull( this.Chequera ) )					this.Chequera = _Screen.zoo.instanciarentidad( 'Chequera' )					this.Chequera.lEsSubEntidad = .t.					this.enlazar( 'Chequera.EventoObtenerLogueo', 'inyectarLogueo' )					this.enlazar( 'Chequera.EventoObtenerInformacion', 'inyectarInformacion' )				endif				if !this.lBuscandoCodigo					if this.Chequera.CODIGO # this.Chequera_PK						this.Chequera.CODIGO = this.Chequera_PK					endif				endif			endif		endif		return this.Chequera
	EndFunc

	*-----------------------------------------------------------------------------------------
	function chequera_pk_assign( txval as variant ) as void                                                                                                                                                                                                   
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'CHEQUEPROPIO' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'chequera_pk_assign', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'chequera_pk_assign', lcParametros )			endif		endif		Local lxValOld As Variant		lxValOld = this.Chequera_PK		local lxVal as Variant		local loSugerido as Object				lxVal = txVal		lxVal = iif( len( lxVal ) < 10, padr( lxVal, 10 ), lxVal )		this.Chequera_PK = lxVal		if this.Validar_Chequera( lxVal, lxValOld )			This.Setear_Chequera( lxVal )			If lxValOld != lxVal or empty( this.Entidadfinanciera_PK )				this.Entidadfinanciera_Pk = This.Chequera.CuentaBancaria.SucursalBancaria.EntidadFinanciera_PK			endif			If lxValOld != lxVal or empty( this.Chequeelectronico )				this.Chequeelectronico = This.Chequera.chequeraelectronica			endif		endif		dodefault( lxVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function eventolanzarexcepcionenkontroler() as void                                                                                                                                                                                                       
		...
		return this.ObtenerResultado( 'Eventolanzarexcepcionenkontroler', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function procesardespuesdesetear_chequeraelectronica() as void                                                                                                                                                                                            
		lnParametros = pcount()lcParametros = ''		lnIdClase = _screen.Mocks.BuscarMock( 'CHEQUEPROPIO' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'procesardespuesdesetear_chequeraelectronica', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'procesardespuesdesetear_chequeraelectronica', lcParametros )			endif		endif		this.Numero.txtDato.BackColor = this.txtDato.nBackColorObligSinFoco
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function setear_chequera( txval ) as void                                                                                                                                                                                                                 
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'CHEQUEPROPIO' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'setear_chequera', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'setear_chequera', lcParametros )			endif		endif		dodefault( txVal )		this.Moneda_PK = this.Chequera.CuentaBancaria.MonedaCuenta_PK
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function validar_chequera( txval as variant, txvalold as variant ) as boolean                                                                                                                                                                             
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1, lxParam2lnParametros = pcount()lxParam1 = txvallxParam2 = txvaloldlcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'CHEQUEPROPIO' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'validar_chequera', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'validar_chequera', lcParametros )			endif		endif		Return This.CargaManual() and dodefault( txVal, txValOld )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function validarchequera() as boolean                                                                                                                                                                                                                     
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Validarchequera', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function validardatos( tnnumero as integer, tdfechaemision as date, tdfecha as date, tochequera as entidad of entidad.prg, tccodigo as string , tbtipovalorelectronico as boolean ) as boolean                                                            
		...
		lnParametros = pcount()
		lxParam1 = tnnumero
		lxParam2 = tdfechaemision
		lxParam3 = tdfecha
		lxParam4 = tochequera
		lxParam5 = tccodigo
		lxParam6 = tbtipovalorelectronico
		...
		return this.ObtenerResultado( 'Validardatos', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function valorsugeridochequera() as void                                                                                                                                                                                                                  
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Valorsugeridochequera', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	protected function validarnumerorangosegunchequera( tnnumero as number, tochequera as entidad of entidad.prg ) as boolean                                                                                                                                 
		...
		return this.ObtenerResultado( 'Validarnumerorangosegunchequera', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	protected function validartipochequera( tochequera as entidad of entidad.prg, tbtipovalorelectronico as boolean ) as boolean                                                                                                                              
		...
		lnParametros = pcount()
		lxParam1 = tochequera
		lxParam2 = tbtipovalorelectronico
		...
		return this.ObtenerResultado( 'Validartipochequera', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	protected function validarunicidadnumeroychequera( tcchequera_pk as string, tnnumero as integer, tccodigo) as boolean                                                                                                                                     
		...
		return this.ObtenerResultado( 'Validarunicidadnumeroychequera', lcParametros )
	EndFunc
