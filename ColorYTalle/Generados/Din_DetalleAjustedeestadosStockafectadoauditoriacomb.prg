...
define class Din_DetalleAJUSTEDEESTADOSStockafectadoauditoriacomb as DetalleAjustecombinacionestados of DetalleAjustecombinacionestados.Prg
	...
	Protected Function _CargarItem( tnItem as integer )
		...
		With this.oItem
			Try
				...
				.Situacion = this.Item[ tnItem ].Situacion
				llVacio = llVacio and this.VerificarSiElAtributoEstaVacio( 'Situacion', .Situacion )
	...
	*--------------------------------------------------------------------------------------------------------
	protected function AplanarItem() as Object
		...
		with loRetorno
			...
			.Situacion = this.oItem.Situacion
	...
	*--------------------------------------------------------------------------------------------------------
	...
	with this
		if .ValidarItem()
			...
			.Item[ lnItem ].Situacion = .oItem.Situacion
	...
	function Sumarizar() as boolean
		...
		with this
			...
			if this.lEsNavegacion
				if select( 'c_Stockafectadoauditoriacomb' ) > 0 and reccount( 'c_Stockafectadoauditoriacomb' ) > 0
					sum Cantidad;
						 to ;
						 lnSum_Cantidad
				endif
	...
	*--------------------------------------------------------------------------------------------------------
	function oItem_Access() as variant
		if !this.ldestroy and ( !vartype( this.oItem ) = 'O' or isnull( this.oItem ) )
			this.oItem = _Screen.zoo.crearobjeto( 'Din_ItemAJUSTEDEESTADOSStockafectadoauditoriacomb' )
	...
	*--------------------------------------------------------------------------------------------------------
	Function Cargar() as Void
		...
		select c_Stockafectadoauditoriacomb
		lcFiltro = this.ObtenerFiltro()
		scan for &lcFiltro
			loItem = createobject('ItemAuxiliar')

			loItem.codigo = c_Stockafectadoauditoriacomb.codigo
			loItem.Comportamiento = c_Stockafectadoauditoriacomb.Comportamiento
			loItem.NROITEM = c_Stockafectadoauditoriacomb.NROITEM
			loItem.Situacion = c_Stockafectadoauditoriacomb.Situacion
			loItem.Articulo_PK = c_Stockafectadoauditoriacomb.Articulo
			loItem.ArticuloDetalle = c_Stockafectadoauditoriacomb.ArticuloDetalle
			loItem.Color_PK = c_Stockafectadoauditoriacomb.Color
			loItem.ColorDetalle = c_Stockafectadoauditoriacomb.ColorDetalle
			loItem.Talle_PK = c_Stockafectadoauditoriacomb.Talle
			loItem.TalleDetalle = c_Stockafectadoauditoriacomb.TalleDetalle
			loItem.StockAct = c_Stockafectadoauditoriacomb.StockAct
			loItem.StockProy = c_Stockafectadoauditoriacomb.StockProy
			loItem.Cantidad = c_Stockafectadoauditoriacomb.Cantidad
			loItem.NroItem = lnI 
	...
	*-----------------------------------------------------------------------------------------
	...
	local lnLargoEntero as integer, lnValorDelDecimal as integer, lnLargoDecimal as integer, llRetorno as boolean, lnI as Integer
		...
		for lni = 1 to this.Count
			...
			if vartype( this.Item[lni].SITUACION) = 'C'
				if len( alltrim( this.Item[lni].SITUACION )) <= 40
				else
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud del valor del atributo SITUACION del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform( len( alltrim( this.Item[lni].SITUACION ))) + ' Máxima:40), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
				this.AgregarInformacion( 'El tipo de dato para el atributo SITUACION del item ' + transform( lni ) + ' no es el correcto, en el detalle ' + this.cEtiqueta + '.')
			Endif
	...
	*--------------------------------------------------------------------------------------------------------
	function CopiarItemaItem( toItemOrigen as Object, toItemDestino as Object ) as Object
		...
		toItemDestino.Situacion = toItemOrigen.Situacion
