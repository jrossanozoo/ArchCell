...
define class Mock_datostareaprogramadaecommerce as MockBase of MockBase.prg
	...
	cClase = 'datostareaprogramadaecommerce'
	...
	cCaracteres = ""                                                                                                                                                                                                                                          
	cContexto = ""                                                                                                                                                                                                                                            
	cDescripcion =  "Parámetros tarea programada ecommerce"                                                                                                                                                                                                   
	cDescripcionSingular = ""                                                                                                                                                                                                                                 
	cEvento = ""                                                                                                                                                                                                                                              
	cFormatoFechaNet = ""                                                                                                                                                                                                                                     
	cIdentificadorConexion = ""                                                                                                                                                                                                                               
	cNombre = ""                                                                                                                                                                                                                                              
	cNombre = 'DATOSTAREAPROGRAMADAECOMMERCE'                                                                                                                                                                                                                 
	cPrefijoImportar = ""                                                                                                                                                                                                                                     
	cPrefijoRecibir = ""                                                                                                                                                                                                                                      
	...
	*-----------------------------------------------------------------------------------------
	function cambiosdetalleplataforma() as void                                                                                                                                                                                                               
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Cambiosdetalleplataforma', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function generararchivocondatosdetareaprogramada() as boolean                                                                                                                                                                                             
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Generararchivocondatosdetareaprogramada', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function lhabilitarplataforma_assign( txval as variant ) as void                                                                                                                                                                                          
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'DATOSTAREAPROGRAMADAECOMMERCE' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'lhabilitarplataforma_assign', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'lhabilitarplataforma_assign', lcParametros )			endif		endif		this.lHabilitarPlataforma = txVal		this.Plataforma.habilitarItems( txVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function obtenertiempoconfiguradotareaprogramada() as integer                                                                                                                                                                                             
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Obtenertiempoconfiguradotareaprogramada', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function periodicidad_assign( txval as variant ) as void                                                                                                                                                                                                  
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'DATOSTAREAPROGRAMADAECOMMERCE' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'periodicidad_assign', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'periodicidad_assign', lcParametros )			endif		endif		Local lxValOld As Variant		lxValOld = this.Periodicidad		local lxVal as Variant		lxVal = txVal		if This.CargaManual()			if this.Validar_Periodicidad( lxVal )				if ( this.lHabilitarPeriodicidad or this.lEstaSeteandoValorSugerido or lxValOld == lxVal)					this.Setear_Periodicidad( lxVal )				else					this.Periodicidad = lxValOld					goServicios.Errores.LevantarExcepcion( "No se puede cambiar este valor (Entidad: Parámetros tarea programada ecommerce - Atributo: Periodicidad)" )				endif			EndIf		Else			This.Setear_Periodicidad( lxVal )		EndIf		dodefault( lxVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function plataforma_access() as variant                                                                                                                                                                                                                   
		lnParametros = pcount()lcParametros = ''		lnIdClase = _screen.Mocks.BuscarMock( 'DATOSTAREAPROGRAMADAECOMMERCE' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'plataforma_access', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'plataforma_access', lcParametros )			endif		endif		if !this.ldestroy and ( !vartype( this.Plataforma ) = 'O' or isnull( this.Plataforma ) )			this.Plataforma = this.crearobjeto( 'Din_DetalleDatostareaprogramadaecommercePlataforma' )			this.Plataforma.inicializar()			this.enlazar( 'Plataforma.EventoObtenerLogueo', 'inyectarLogueo' )			this.enlazar( 'Plataforma.EventoObtenerInformacion', 'inyectarInformacion' )			bindevent( this.Plataforma, 'CambioSumarizado', this, 'CambiosDetallePlataforma', 1)			this.enlazar( 'Plataforma.EventoAdvertirLimitePorDiseno', 'EventoAdvertirLimitePorDiseno')			this.enlazar( 'Plataforma.EventoCancelarCargaLimitePorDiseno', 'EventoCancelarCargaLimitePorDiseno')		endif		return this.Plataforma
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function setear_periodicidad( txval as variant ) as void                                                                                                                                                                                                  
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'DATOSTAREAPROGRAMADAECOMMERCE' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'setear_periodicidad', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'setear_periodicidad', lcParametros )			endif		endif		this.Periodicidad = txVal		dodefault( txVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function validar_periodicidad( txval as variant ) as boolean                                                                                                                                                                                              
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'DATOSTAREAPROGRAMADAECOMMERCE' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'validar_periodicidad', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'validar_periodicidad', lcParametros )			endif		endif		Return dodefault( txVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function valorsugeridoperiodicidad() as void                                                                                                                                                                                                              
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Valorsugeridoperiodicidad', lcParametros )
	EndFunc

	*-----------------------------------------------------------------------------------------
	function verificarcarpetascripts() as void                                                                                                                                                                                                                
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Verificarcarpetascripts', lcParametros )
	EndFunc
