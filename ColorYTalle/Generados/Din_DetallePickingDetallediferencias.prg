...
define class Din_DetallePICKINGDetallediferencias as detalle of detalle.prg
	...
	function Sumarizar() as boolean
		...
		with this
			...
			if this.lEsNavegacion
				if select( 'c_Detallediferencias' ) > 0 and reccount( 'c_Detallediferencias' ) > 0
					sum Cantidad;
						 to ;
						 lnSum_Cantidad
				endif
	...
	*--------------------------------------------------------------------------------------------------------
	function oItem_Access() as variant
		if !this.ldestroy and ( !vartype( this.oItem ) = 'O' or isnull( this.oItem ) )
			this.oItem = _Screen.zoo.crearobjeto( 'Din_ItemPICKINGDetallediferencias' )
	...
	*--------------------------------------------------------------------------------------------------------
	Function Cargar() as Void
		...
		select c_Detallediferencias
		lcFiltro = this.ObtenerFiltro()
		scan for &lcFiltro
			loItem = createobject('ItemAuxiliar')

			loItem.Codigo = c_Detallediferencias.Codigo
			loItem.Articulo_PK = c_Detallediferencias.Articulo
			loItem.NoProcesarStock = c_Detallediferencias.NoProcesarStock
			loItem.Equivalencia_PK = c_Detallediferencias.Equivalencia
			loItem.Comportamiento = c_Detallediferencias.Comportamiento
			loItem.NROITEM = c_Detallediferencias.NROITEM
			loItem.ArticuloDetalle = c_Detallediferencias.ArticuloDetalle
			loItem.Color_PK = c_Detallediferencias.Color
			loItem.ColorDetalle = c_Detallediferencias.ColorDetalle
			loItem.Talle_PK = c_Detallediferencias.Talle
			loItem.Cantidad = c_Detallediferencias.Cantidad
			loItem.NroItem = lnI 
