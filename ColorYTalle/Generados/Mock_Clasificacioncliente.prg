...
define class Mock_clasificacioncliente as MockBase of MockBase.prg
	...
	*-----------------------------------------------------------------------------------------
	function requierelimitedeconsumo_assign( txval as variant ) as void                                                                                                                                                                                       
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'CLASIFICACIONCLIENTE' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'requierelimitedeconsumo_assign', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'requierelimitedeconsumo_assign', lcParametros )			endif		endif		Local lxValOld As Variant		lxValOld = this.Requierelimitedeconsumo		local lxVal as Variant		lxVal = txVal		if This.CargaManual()			if this.Validar_Requierelimitedeconsumo( lxVal )				this.Setear_Requierelimitedeconsumo( lxVal )			EndIf		Else			This.Setear_Requierelimitedeconsumo( lxVal )		EndIf		dodefault( lxVal )
	EndFunc

	*-----------------------------------------------------------------------------------------
	function requiereseguridadmodifcliente_assign( txval as variant ) as void                                                                                                                                                                                 
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'CLASIFICACIONCLIENTE' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'requiereseguridadmodifcliente_assign', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'requiereseguridadmodifcliente_assign', lcParametros )			endif		endif		Local lxValOld As Variant		lxValOld = this.Requiereseguridadmodifcliente		local lxVal as Variant		lxVal = txVal		if This.CargaManual()			if this.Validar_Requiereseguridadmodifcliente( lxVal )				this.Setear_Requiereseguridadmodifcliente( lxVal )			EndIf		Else			This.Setear_Requiereseguridadmodifcliente( lxVal )		EndIf		dodefault( lxVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function setear_requierelimitedeconsumo( txval as variant ) as void                                                                                                                                                                                       
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'CLASIFICACIONCLIENTE' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'setear_requierelimitedeconsumo', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'setear_requierelimitedeconsumo', lcParametros )			endif		endif		this.Requierelimitedeconsumo = txVal		dodefault( txVal )
	EndFunc

	*-----------------------------------------------------------------------------------------
	function setear_requiereseguridadmodifcliente( txval as variant ) as void                                                                                                                                                                                 
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'CLASIFICACIONCLIENTE' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'setear_requiereseguridadmodifcliente', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'setear_requiereseguridadmodifcliente', lcParametros )			endif		endif		this.Requiereseguridadmodifcliente = txVal		dodefault( txVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function validar_requierelimitedeconsumo( txval as variant ) as boolean                                                                                                                                                                                   
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'CLASIFICACIONCLIENTE' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'validar_requierelimitedeconsumo', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'validar_requierelimitedeconsumo', lcParametros )			endif		endif		Return dodefault( txVal )
	EndFunc

	*-----------------------------------------------------------------------------------------
	function validar_requiereseguridadmodifcliente( txval as variant ) as boolean                                                                                                                                                                             
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'CLASIFICACIONCLIENTE' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'validar_requiereseguridadmodifcliente', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'validar_requiereseguridadmodifcliente', lcParametros )			endif		endif		Return dodefault( txVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function valorsugeridorequierelimitedeconsumo() as void                                                                                                                                                                                                   
		...
		return this.ObtenerResultado( 'Valorsugeridorequierelimitedeconsumo', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function valorsugeridorequiereseguridadmodifcliente() as void                                                                                                                                                                                             
		...
		return this.ObtenerResultado( 'Valorsugeridorequiereseguridadmodifcliente', lcParametros )
	EndFunc
