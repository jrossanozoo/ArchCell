...
define class Din_DetalleLIQUIDACIONDETALLERLiquidaciontallerproduccion as DetalleProduccioncolorytalle of DetalleProduccioncolorytalle.Prg
	...
	function Sumarizar() as boolean
		...
		with this
			...
			if this.lEsNavegacion
				if select( 'c_Liquidaciontallerproduccion' ) > 0 and reccount( 'c_Liquidaciontallerproduccion' ) > 0
					sum Cantidad, Monto;
						 to ;
	...
	*--------------------------------------------------------------------------------------------------------
	function oItem_Access() as variant
		if !this.ldestroy and ( !vartype( this.oItem ) = 'O' or isnull( this.oItem ) )
			this.oItem = _Screen.zoo.crearobjeto( 'ItemLIQUIDACIONDETALLERLiquidaciontallerproduccion' )
	...
	*--------------------------------------------------------------------------------------------------------
	Function Cargar() as Void
		...
		select c_Liquidaciontallerproduccion
		lcFiltro = this.ObtenerFiltro()
		scan for &lcFiltro
			loItem = createobject('ItemAuxiliar')

			loItem.Codigo = c_Liquidaciontallerproduccion.Codigo
			loItem.Comportamiento = c_Liquidaciontallerproduccion.Comportamiento
			loItem.IdItemInsumo = c_Liquidaciontallerproduccion.IdItemInsumo
			loItem.NROITEM = c_Liquidaciontallerproduccion.NROITEM
			loItem.Proceso_PK = c_Liquidaciontallerproduccion.Proceso
			loItem.ProcesoDetalle = c_Liquidaciontallerproduccion.ProcesoDetalle
			loItem.Semielaborado_PK = c_Liquidaciontallerproduccion.Semielaborado
			loItem.SemielaboradoDetalle = c_Liquidaciontallerproduccion.SemielaboradoDetalle
			loItem.Color_PK = c_Liquidaciontallerproduccion.Color
			loItem.ColorDetalle = c_Liquidaciontallerproduccion.ColorDetalle
			loItem.Talle_PK = c_Liquidaciontallerproduccion.Talle
			loItem.TalleDetalle = c_Liquidaciontallerproduccion.TalleDetalle
			loItem.Cantidad = c_Liquidaciontallerproduccion.Cantidad
			loItem.Articulo_PK = c_Liquidaciontallerproduccion.Articulo
			loItem.ArticuloDetalle = c_Liquidaciontallerproduccion.ArticuloDetalle
			loItem.Costo = c_Liquidaciontallerproduccion.Costo
			loItem.Monto = c_Liquidaciontallerproduccion.Monto
			loItem.NroItem = lnI 
