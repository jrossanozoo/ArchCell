...
define class Din_EntidadMOTIVODATOSADICIONALESCOMPROBANTESA as entidad of entidad.prg

	cNombre = 'MOTIVODATOSADICIONALESCOMPROBANTESA'
	FECHAIMPO = ctod( '  /  /    ' )
	...
	cDescripcion =  [Motivo en datos adicionales de comprobantes A]
	lEntidadEditable =  .t.
	...
	*--------------------------------------------------------------------------------------------------------
	function oAD_Access() as variant
		if !this.ldestroy and ( !vartype( this.oAD ) = 'O' or isnull( this.oAD ) )
			this.oAD = this.crearobjeto( 'Din_EntidadMOTIVODATOSADICIONALESCOMPROBANTESAAD_' + alltrim( _screen.zoo.app.TipoDeBase ))
	...
	*--------------------------------------------------------------------------------------------------------
	...
	local lnLargoDecimal as integer, lnLargoEntero as Integer, lnLargoDecimal as integer, llRetorno as boolean 
		...
		if ( vartype( this.DescripcionCorta) = 'C' )
			if len( alltrim( this.DESCRIPCIONCORTA ) ) > 40
				This.AgregarInformacion( 'La longitud del valor del atributo DESCRIPCIONCORTA es mayor a la permitida.' )
				llRetorno = .F.
			Endif
		else
			This.AgregarInformacion( 'El tipo de dato para el atributo DESCRIPCIONCORTA no es el correcto.')
			llRetorno = .F.
		Endif
	...
	*--------------------------------------------------------------------------------------------------------
	function Inicializar() as Void
		...
		oICol = goServicios.SaltosDeCampoYValoresSugeridos.ObtenerValorSugeridoDeUnaEntidadDetalle( 'MOTIVODATOSADICIONALESCOMPROBANTESA', '')
	...
	*--------------------------------------------------------------------------------------------------------
	function Descripcioncorta_Assign( txVal as variant ) as void

		Local lxValOld As Variant
		lxValOld = this.Descripcioncorta
		local lxVal as Variant
		lxVal = txVal

		if This.CargaManual()
			if this.Validar_Descripcioncorta( lxVal )
				this.Setear_Descripcioncorta( lxVal )
			EndIf
		Else
			This.Setear_Descripcioncorta( lxVal ) 
		EndIf
	...
	*--------------------------------------------------------------------------------------------------------
	function Setear_Descripcioncorta( txVal as variant ) as void

		this.Descripcioncorta = txVal
		dodefault( txVal )
	...
	*--------------------------------------------------------------------------------------------------------
	function Validar_Descripcioncorta( txVal as variant ) as Boolean
	...
	*--------------------------------------------------------------------------------------------------------
	Function ValorSugeridoDescripcion() as void
		...
		with this
			lcExpresionValorSugeridoDefinidoPorElUsuario = goServicios.SaltosDeCampoYValoresSugeridos.ObtenerValorSugerido( "MOTIVODATOSADICIONALESCOMPROBANTESA", "", "Descripcion" )
			if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
				if !empty( lcExpresionValorSugeridoDefinidoPorElUsuario )
					try
						...
						loEx.AgregarInformacion( "Se produjo un error al intentar asignar el valor sugerido " + lcExpresionValorSugeridoDefinidoPorElUsuario + " del atributo Descripción para la entidad Motivo en datos adicionales de comprobantes A." + chr( 10 ) + "Verifique el valor especificado en la entidad Comportamiento de atributos." + chr( 10 ) + "El tipo de dato del atributo es caracter." )
