...
define class Din_EntidadNOTADECREDITO as EntColorytalle_ComprobanteDeVentasConValores of EntColorytalle_ComprobanteDeVentasConValores.prg
	...
	*--------------------------------------------------------------------------------------------------------
	function ObtenerFuncionalidades() as String
		return '<ANULABLE><WEBHOOK><CAI><VENTAS><FISCAL><CONVALORES><NOLISTAGENERICO><VALORCIERRE><DESCUENTOAUTOMATICO><ACTUALIZAEMAILCLIENTE><LISTARMODIFICACION>'
	Endfunc
	*--------------------------------------------------------------------------------------------------------
	...
	*--------------------------------------------------------------------------------------------------------
	function Inicializar() as Void
		...
		this.lWHIngresar = goServicios.WebHook.TieneQueMandar( 'NOTADECREDITO', 'INGRESAR' )
		this.lWHModificar = goServicios.WebHook.TieneQueMandar( 'NOTADECREDITO', 'MODIFICAR' )
		this.lWHEliminar = goServicios.WebHook.TieneQueMandar( 'NOTADECREDITO', 'ELIMINAR' )
	endfunc
	...
	function Monedacomprobante_PK_Assign( txVal as variant ) as void
		...
		if this.Validar_Monedacomprobante( lxVal, lxValOld )
			if ( this.lHabilitarMonedacomprobante_PK or this.lEstaSeteandoValorSugerido or lxValOld == lxVal )
				...
				goServicios.Errores.LevantarExcepcion( "No se puede cambiar este valor (Entidad: Nota de crédito manual de venta - Atributo: Monedacomprobante)" )
			endif 
		endif
		dodefault( lxVal )
		...
		with this
			...
			Catch to loError
				...
				if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
					...
					loEx.AgregarInformacion( "Se produjo un error al intentar asignar el valor sugerido " + lcExpresionValorSugeridoDefinidoPorElUsuario + " del atributo Cliente para la entidad Nota de crédito manual de venta." + chr( 10 ) + "Verifique el valor especificado en la entidad Comportamiento de atributos." + chr( 10 ) + "El tipo de dato del atributo es caracter." )
			...
			if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
				if !empty( lcExpresionValorSugeridoDefinidoPorElUsuario )
					try
						...
						loEx.Grabar( loError )
						loEx.AgregarInformacion( iif( type( "loError.UserValue.Message" ) == "C", loError.UserValue.Message, loError.Message ) )
						loEx.AgregarInformacion( "Se produjo un error al intentar asignar el valor sugerido " + lcExpresionValorSugeridoDefinidoPorElUsuario + " del atributo Letra comprobante asociado para la entidad Nota de crédito manual de venta." + chr( 10 ) + "Verifique el valor especificado en la entidad Comportamiento de atributos." + chr( 10 ) + "El tipo de dato del atributo es caracter." )
						goServicios.Errores.LevantarExcepcion( loEx )
					endtry
				endif
			endif
