...
define class Din_DetalleMERCADERIAENTRANSITOMovimientodetalle as DetalleMovstock of DetalleMovstock.Prg
	...
	*--------------------------------------------------------------------------------------------------------
	function Sumarizar() as boolean
		local lnItem as integer, loItem as Object, lnSum_Cantidad as Number, lnSum_Montodescuento as Number
		...
		with this
			...
			if this.lEsNavegacion
				if select( 'c_Movimientodetalle' ) > 0 and reccount( 'c_Movimientodetalle' ) > 0
					sum Cantidad, Montodescuento;
						 to ;
						 lnSum_Cantidad, lnSum_Montodescuento
				endif
	...
	*--------------------------------------------------------------------------------------------------------
	function oItem_Access() as variant
		if !this.ldestroy and ( !vartype( this.oItem ) = 'O' or isnull( this.oItem ) )
			this.oItem = _Screen.zoo.crearobjeto( 'ItemMERCADERIAENTRANSITOMovimientodetalle' )
	...
	*--------------------------------------------------------------------------------------------------------
	Function Cargar() as Void
		...
		scan for &lcFiltro
			...
			loItem.IdItemArticulos = c_Movimientodetalle.IdItemArticulos
			loItem.Comportamiento = c_Movimientodetalle.Comportamiento
			loItem.PorcentajeIVA = c_Movimientodetalle.PorcentajeIVA
			...
			loItem.PrecioConImpuestos = c_Movimientodetalle.PrecioConImpuestos
			loItem.PrecioSinImpuestos = c_Movimientodetalle.PrecioSinImpuestos
			loItem.Descuento = c_Movimientodetalle.Descuento
			loItem.MontoDescuento = c_Movimientodetalle.MontoDescuento
			loItem.NroItem = lnI 
		...
		for lni = 1 to this.Count
			...
			if vartype( this.Item[lni].PRECIOCONIMPUESTOS) = 'N'
				...
				if lnLargoEntero <= 10
					...
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud entera del valor del atributo PRECIOCONIMPUESTOS del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform(lnLargoEntero) + ' Máxima:10), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
			...
			if vartype( this.Item[lni].PRECIOSINIMPUESTOS) = 'N'
				...
				if lnLargoEntero <= 10
					...
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud entera del valor del atributo PRECIOSINIMPUESTOS del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform(lnLargoEntero) + ' Máxima:10), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
