...
define class Mock_pago as MockBase of MockBase.prg
	...
	cValorPagoACuenta = ""                                                                                                                                                                                                                                    
	cValoresDetalle = "OrdenDePago_ValoresDetalle"                                                                                                                                                                                                            
	...
	*-----------------------------------------------------------------------------------------
	function cambiosdetalleordendepago_impuestoscomprobante() as void                                                                                                                                                                                         
		...
		return this.ObtenerResultado( 'Cambiosdetalleordendepago_impuestoscomprobante', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function cambiosdetalleordendepago_pagodetalle() as void                                                                                                                                                                                                  
		...
		return this.ObtenerResultado( 'Cambiosdetalleordendepago_pagodetalle', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function cambiosdetalleordendepago_valoresdetalle() as void                                                                                                                                                                                               
		...
		return this.ObtenerResultado( 'Cambiosdetalleordendepago_valoresdetalle', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function despuesdeasignarordendepago() as void                                                                                                                                                                                                            
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Despuesdeasignarordendepago', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function eventorefrescardetalles() as void                                                                                                                                                                                                                
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Eventorefrescardetalles', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function obtenernombredecomprobantedeventas( tntipo as integer ) as string                                                                                                                                                                                
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		local lxParam1
	...
	*-----------------------------------------------------------------------------------------
	function ocoleccionoriginalvalores_access() as variant                                                                                                                                                                                                    
		lnParametros = pcount()lcParametros = ''		lnIdClase = _screen.Mocks.BuscarMock( 'PAGO' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'ocoleccionoriginalvalores_access', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'ocoleccionoriginalvalores_access', lcParametros )			endif		endif		if this.ldestroy		else			if !vartype( this.oColeccionOriginalValores ) = 'O' or isnull( this.oColeccionOriginalValores )				this.oColeccionOriginalValores = _Screen.Zoo.CrearObjeto( "ZooColeccion" )			endif		endif		return this.oColeccionOriginalValores
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function setear_ordendepago( txval as variant ) as void                                                                                                                                                                                                   
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'PAGO' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'setear_ordendepago', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'setear_ordendepago', lcParametros )			endif		endif		this.Ordendepago.CODIGO = txVal		dodefault( txVal )
	EndFunc

	*-----------------------------------------------------------------------------------------
	function setear_ordendepago_cotizacion( txval as variant ) as void                                                                                                                                                                                        
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'PAGO' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'setear_ordendepago_cotizacion', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'setear_ordendepago_cotizacion', lcParametros )			endif		endif		this.Ordendepago_cotizacion = txVal		dodefault( txVal )
	EndFunc

	*-----------------------------------------------------------------------------------------
	function setear_ordendepago_fecha( txval as variant ) as void                                                                                                                                                                                             
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'PAGO' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'setear_ordendepago_fecha', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'setear_ordendepago_fecha', lcParametros )			endif		endif		this.Ordendepago_fecha = txVal		dodefault( txVal )
	EndFunc

	*-----------------------------------------------------------------------------------------
	function setear_ordendepago_monedacomprobante( txval as variant ) as void                                                                                                                                                                                 
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'PAGO' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'setear_ordendepago_monedacomprobante', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'setear_ordendepago_monedacomprobante', lcParametros )			endif		endif		this.Ordendepago_monedacomprobante.CODIGO = txVal		dodefault( txVal )
	EndFunc

	*-----------------------------------------------------------------------------------------
	function setear_ordendepago_obs( txval as variant ) as void                                                                                                                                                                                               
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'PAGO' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'setear_ordendepago_obs', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'setear_ordendepago_obs', lcParametros )			endif		endif		this.Ordendepago_obs = txVal		dodefault( txVal )
	EndFunc

	*-----------------------------------------------------------------------------------------
	function setear_ordendepago_proveedor( txval as variant ) as void                                                                                                                                                                                         
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'PAGO' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'setear_ordendepago_proveedor', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'setear_ordendepago_proveedor', lcParametros )			endif		endif		this.Ordendepago_proveedor.CODIGO = txVal		dodefault( txVal )
	EndFunc

	*-----------------------------------------------------------------------------------------
	function setear_ordendepago_total( txval as variant ) as void                                                                                                                                                                                             
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'PAGO' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'setear_ordendepago_total', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'setear_ordendepago_total', lcParametros )			endif		endif		this.Ordendepago_total = txVal		dodefault( txVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function setearfiltrobuscadorordendepago( tobusqueda as object ) as void                                                                                                                                                                                  
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		local lxParam1
		...
		return this.ObtenerResultado( 'Setearfiltrobuscadorordendepago', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function validar_ordendepago( txval as variant, txvalold as variant ) as boolean                                                                                                                                                                          
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1, lxParam2lnParametros = pcount()lxParam1 = txvallxParam2 = txvaloldlcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'PAGO' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'validar_ordendepago', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'validar_ordendepago', lcParametros )			endif		endif		if This.CargaManual()			local lcXml as String, lcMensaje as String			lcMensaje = ""			if !empty( txVal )				lcXml = This.oAd.ObtenerDatosEntidad( "", "OrdenDePago = '" + txVal + "'" )				This.XmlACursor( lcXml, "c_Ver" )				if reccount( "c_ver" ) >= 1					lcMensaje = "Orden de pago afectada por el pago " + c_ver.Letra + " " + ;						 padl( c_ver.PuntoDeVenta, 4, "0" ) + "-" + padl( c_ver.Numero, 10, "0" )				Endif					use in select( "c_ver" )				if !empty( lcMensaje )					this.OrdenDePago_pk = ""					goServicios.Errores.LevantarExcepcion( lcMensaje )						EndIf				This.ValidarOrdenDePagoConValorCuentaCorriente( txVal )			Endif		Endif		Return dodefault( txVal, txValOld )
	EndFunc

	*-----------------------------------------------------------------------------------------
	function validar_ordendepago_cotizacion( txval as variant ) as boolean                                                                                                                                                                                    
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'PAGO' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'validar_ordendepago_cotizacion', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'validar_ordendepago_cotizacion', lcParametros )			endif		endif		Return dodefault( txVal )
	EndFunc

	*-----------------------------------------------------------------------------------------
	function validar_ordendepago_fecha( txval as variant ) as boolean                                                                                                                                                                                         
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'PAGO' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'validar_ordendepago_fecha', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'validar_ordendepago_fecha', lcParametros )			endif		endif		Return dodefault( txVal )
	EndFunc

	*-----------------------------------------------------------------------------------------
	function validar_ordendepago_monedacomprobante( txval as variant, txvalold as variant ) as boolean                                                                                                                                                        
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1, lxParam2lnParametros = pcount()lxParam1 = txvallxParam2 = txvaloldlcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'PAGO' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'validar_ordendepago_monedacomprobante', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'validar_ordendepago_monedacomprobante', lcParametros )			endif		endif		Return This.CargaManual() and dodefault( txVal, txValOld )
	EndFunc

	*-----------------------------------------------------------------------------------------
	function validar_ordendepago_obs( txval as variant ) as boolean                                                                                                                                                                                           
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'PAGO' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'validar_ordendepago_obs', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'validar_ordendepago_obs', lcParametros )			endif		endif		Return dodefault( txVal )
	EndFunc

	*-----------------------------------------------------------------------------------------
	function validar_ordendepago_proveedor( txval as variant, txvalold as variant ) as boolean                                                                                                                                                                
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1, lxParam2lnParametros = pcount()lxParam1 = txvallxParam2 = txvaloldlcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'PAGO' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'validar_ordendepago_proveedor', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'validar_ordendepago_proveedor', lcParametros )			endif		endif		Return This.CargaManual() and dodefault( txVal, txValOld ) and This.Ordendepago_proveedor.oDesactivador.ValidarEstadoActivacion( txVal, txValOld, this.lnuevo, this.ledicion)
	EndFunc

	*-----------------------------------------------------------------------------------------
	function validar_ordendepago_total( txval as variant ) as boolean                                                                                                                                                                                         
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'PAGO' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'validar_ordendepago_total', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'validar_ordendepago_total', lcParametros )			endif		endif		Return dodefault( txVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function validarordendepago() as boolean                                                                                                                                                                                                                  
		...
		return this.ObtenerResultado( 'Validarordendepago', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function validarordendepago_monedacomprobante() as boolean                                                                                                                                                                                                
		...
		return this.ObtenerResultado( 'Validarordendepago_monedacomprobante', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function validarordendepago_proveedor() as boolean                                                                                                                                                                                                        
		...
		return this.ObtenerResultado( 'Validarordendepago_proveedor', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function valorsugeridoordendepago() as void                                                                                                                                                                                                               
		...
		return this.ObtenerResultado( 'Valorsugeridoordendepago', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function valorsugeridorecibo() as void                                                                                                                                                                                                                    
		...
		return this.ObtenerResultado( 'Valorsugeridorecibo', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	protected function inicializaratributosordendepago() as void                                                                                                                                                                                              
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Inicializaratributosordendepago', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	protected function validarordendepagoconvalorcuentacorriente( txval as variant ) as void                                                                                                                                                                  
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		local lxParam1
		...
		return this.ObtenerResultado( 'Validarordendepagoconvalorcuentacorriente', lcParametros )
	EndFunc
