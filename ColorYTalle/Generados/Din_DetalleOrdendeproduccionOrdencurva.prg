...
define class Din_DetalleORDENDEPRODUCCIONOrdencurva as DetalleBaseproduccion of DetalleBaseproduccion.Prg
	...
	Protected Function _CargarItem( tnItem as integer )
		...
		With this.oItem
			Try
				...
				.Producto_PK = this.Item[ tnItem ].Producto_PK
				llVacio = llVacio and this.VerificarSiElAtributoEstaVacio( 'Producto_PK', .Producto_PK )
			
				.ProductoDetalle = this.Item[ tnItem ].ProductoDetalle
				llVacio = llVacio and this.VerificarSiElAtributoEstaVacio( 'ProductoDetalle', .ProductoDetalle )
	...
	*--------------------------------------------------------------------------------------------------------
	protected function AplanarItem() as Object
		...
		with loRetorno
			...
			.Producto_PK = this.oItem.Producto_PK
			.ProductoDetalle = this.oItem.ProductoDetalle
	...
	*--------------------------------------------------------------------------------------------------------
	...
	with this
		if .ValidarItem()
			...
			.Item[ lnItem ].Producto_PK = .oItem.Producto_PK
			.Item[ lnItem ].ProductoDetalle = .oItem.ProductoDetalle
	...
	*--------------------------------------------------------------------------------------------------------
	function Sumarizar() as boolean
		local lnItem as integer, loItem as Object, lnSum_Cantidad as Number, lnSum_Total as Number

		with this
			lnSum_Cantidad = 0
			lnSum_Total = 0
			if this.lEsNavegacion
				if select( 'c_Ordencurva' ) > 0 and reccount( 'c_Ordencurva' ) > 0
					sum Cantidad, Total;
						 to ;
						 lnSum_Cantidad, lnSum_Total
				endif
	...
	*--------------------------------------------------------------------------------------------------------
	function oItem_Access() as variant
		if !this.ldestroy and ( !vartype( this.oItem ) = 'O' or isnull( this.oItem ) )
			this.oItem = _Screen.zoo.crearobjeto( 'ItemORDENDEPRODUCCIONOrdencurva' )
	...
	*--------------------------------------------------------------------------------------------------------
	Function Cargar() as Void
		...
		scan for &lcFiltro
			...
			loItem.CodigoOrden = c_Ordencurva.CodigoOrden
			loItem.NROITEM = c_Ordencurva.NROITEM
			loItem.Producto_PK = c_Ordencurva.Producto
			loItem.ProductoDetalle = c_Ordencurva.ProductoDetalle
			loItem.Color_PK = c_Ordencurva.Color
			loItem.ColorDetalle = c_Ordencurva.ColorDetalle
			loItem.Talle_PK = c_Ordencurva.Talle
			loItem.TalleDetalle = c_Ordencurva.TalleDetalle
			loItem.Cantidad = c_Ordencurva.Cantidad
			loItem.Total = c_Ordencurva.Total
			loItem.NroItem = lnI 
	...
	*--------------------------------------------------------------------------------------------------------
	function ValidarExistenciaCamposFijosItemPlano( tnItem as integer ) as Boolean
		...
		if  empty( this.item[ tnItem ].Producto_PK ) 
			llRetorno = .F.
		endif 
		return llRetorno
	...
	*-----------------------------------------------------------------------------------------
	...
	local lnLargoEntero as integer, lnValorDelDecimal as integer, lnLargoDecimal as integer, llRetorno as boolean, lnI as Integer
		...
		for lni = 1 to this.Count
			...
			if vartype( this.Item[lni].PRODUCTO_PK) = 'C'
				if len( alltrim( this.Item[lni].PRODUCTO_PK )) <= 15
				else
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud del valor del atributo PRODUCTO del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform( len( alltrim( this.Item[lni].PRODUCTO_PK ))) + ' Máxima:15), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
				this.AgregarInformacion( 'El tipo de dato para el atributo PRODUCTO del item ' + transform( lni ) + ' no es el correcto, en el detalle ' + this.cEtiqueta + '.')
			Endif
			
			if vartype( this.Item[lni].PRODUCTODETALLE) = 'C'
				if len( alltrim( this.Item[lni].PRODUCTODETALLE )) <= 100
				else
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud del valor del atributo PRODUCTODETALLE del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform( len( alltrim( this.Item[lni].PRODUCTODETALLE ))) + ' Máxima:100), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
				this.AgregarInformacion( 'El tipo de dato para el atributo PRODUCTODETALLE del item ' + transform( lni ) + ' no es el correcto, en el detalle ' + this.cEtiqueta + '.')
			Endif
			...
			if vartype( this.Item[lni].TOTAL) = 'N'
				...
				if lnLargoEntero <= 8
					...
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud entera del valor del atributo TOTAL del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform(lnLargoEntero) + ' Máxima:8), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
	...
	*--------------------------------------------------------------------------------------------------------
	function CopiarItemaItem( toItemOrigen as Object, toItemDestino as Object ) as Object
		...
		toItemDestino.Producto_PK = toItemOrigen.Producto_PK
		toItemDestino.Productodetalle = toItemOrigen.Productodetalle
