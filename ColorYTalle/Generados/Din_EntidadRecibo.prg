...
define class Din_EntidadRECIBO as EntColorytalle_ComprobanteDeVentasConValores of EntColorytalle_ComprobanteDeVentasConValores.prg
	...
	*--------------------------------------------------------------------------------------------------------
	function ValidacionDetalles() as Boolean
		...
		llRetorno = This.ValidarUnDetalle( this.RetencionesRecibidas, 'Retenciones' ) and llRetorno
		return llRetorno
	endfunc
	...
	*--------------------------------------------------------------------------------------------------------
	function ObtenerFuncionalidades() as String
		return '<ANULABLE><TRANSMODOSEGUROCEN><TRANSMODOSEGURODB><VENTAS><WEBHOOK><FISCAL><NOIMPO><NOEXPO><LISTARMODIFICACION><VALORCIERRE>'
	Endfunc
	*--------------------------------------------------------------------------------------------------------
	...
	*--------------------------------------------------------------------------------------------------------
	function Inicializar() as Void
		...
		this.lWHIngresar = goServicios.WebHook.TieneQueMandar( 'RECIBO', 'INGRESAR' )
		this.lWHModificar = goServicios.WebHook.TieneQueMandar( 'RECIBO', 'MODIFICAR' )
		this.lWHEliminar = goServicios.WebHook.TieneQueMandar( 'RECIBO', 'ELIMINAR' )
	endfunc
	...
	function Monedacomprobante_PK_Assign( txVal as variant ) as void
		...
		if this.Validar_Monedacomprobante( lxVal, lxValOld )
			if ( this.lHabilitarMonedacomprobante_PK or this.lEstaSeteandoValorSugerido or lxValOld == lxVal )
				...
				goServicios.Errores.LevantarExcepcion( "No se puede cambiar este valor (Entidad: Recibo - Atributo: Monedacomprobante)" )
			endif 
		endif
		dodefault( lxVal )
	...
	*--------------------------------------------------------------------------------------------------------
	Function ValorSugeridoMonedacomprobante() as void
		...
		with this
			try
				lcExpresionValorSugeridoDefinidoPorElUsuario = goServicios.SaltosDeCampoYValoresSugeridos.ObtenerValorSugerido( "RECIBO", "", "Monedacomprobante" )
				if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
					if !empty( lcExpresionValorSugeridoDefinidoPorElUsuario )
						lvValorSugeridoDefinidoPorElUsuario = &lcExpresionValorSugeridoDefinidoPorElUsuario
						if vartype( lvValorSugeridoDefinidoPorElUsuario ) # "C"
			...
			Catch to loError
				...
				if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
					...
					loEx.AgregarInformacion( "Se produjo un error al intentar asignar el valor sugerido " + lcExpresionValorSugeridoDefinidoPorElUsuario + " del atributo Moneda para la entidad Recibo." + chr( 10 ) + "Verifique el valor especificado en la entidad Comportamiento de atributos." + chr( 10 ) + "El tipo de dato del atributo es caracter." )
					goServicios.Errores.LevantarExcepcion( loEx )
				endif
			endtry 
		endwith
