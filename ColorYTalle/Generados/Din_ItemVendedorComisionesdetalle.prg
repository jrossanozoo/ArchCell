...
define class Din_ItemVENDEDORComisionesdetalle as ItemActivo of ItemActivo.prg
	...
	*--------------------------------------------------------------------------------------------------------
	function Comision_PK_Assign( txVal as variant ) as void
		...
		If lxValOld != lxVal or empty( this.Comisiondetalle ) 
			this.Comisiondetalle = This.Comision.Descripcion
		endif
	...
	*--------------------------------------------------------------------------------------------------------
	function Comisiondetalle_Assign( txVal as variant ) as void
		...
		Local lxValOld As Variant
		lxValOld = this.Comisiondetalle
		local lxVal as Variant
		lxVal = txVal

		if This.CargaManual()
			if this.Validar_Comisiondetalle( lxVal )
				this.EventoAntesDeSetear( This, 'Comisiondetalle', lxValOld, lxVal  )
				this.Setear_Comisiondetalle( lxVal )
				this.EventoDespuesDeSetear( This, 'Comisiondetalle', lxValOld, lxVal  )
				If lxValOld != lxVal
					If This.PuedeEjecutarHaCambiado()
						this.HaCambiado( 'Comisiondetalle', This )
					Endif
				EndIf
			EndIf
		Else
