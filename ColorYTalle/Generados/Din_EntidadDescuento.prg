...
define class Din_EntidadDESCUENTO as entidad of entidad.prg
	...
	Vendedor_ClasificacionVDesde_PK = []
	Vendedor_ClasificacionVDesde = null
	lHabilitarVendedor_ClasificacionVDesde_PK = .T.
	DiaLu = .F.
	lHabilitarDiaLu = .T.
	Vendedor_ClasificacionVHasta_PK = []
	Vendedor_ClasificacionVHasta = null
	lHabilitarVendedor_ClasificacionVHasta_PK = .T.
	DiaMa = .F.
	lHabilitarDiaMa = .T.
	DiaMi = .F.
	lHabilitarDiaMi = .T.
	...
	cAtributoVendedor = 'VENDHASTA_PK'
...
cCodigoPrevioGuardarComo = ""
	...
	*-----------------------------------------------------------------------------------------
	function RestaurarGenHabilitar() as Void
		...
		this.lHabilitarVendedor_ClasificacionVDesde_PK = .T.
		this.lHabilitarDiaLu = .T.
		this.lHabilitarVendedor_ClasificacionVHasta_PK = .T.
	...
	*--------------------------------------------------------------------------------------------------------
	function ValidacionBasica() as boolean
		...
		With This
			llRetorno = .ValidarModofuncionamiento() and llRetorno
	...
	*--------------------------------------------------------------------------------------------------------
	...
	endfunc
	*--------------------------------------------------------------------------------------------------------
	function ValidarModoFuncionamiento() as boolean
		local llRetorno as boolean
		llRetorno = dodefault()
		if empty( this.ModoFuncionamiento )
			this.AgregarInformacion( 'Debe cargar el campo Modo funcionamiento', 9005, 'ModoFuncionamiento' )
			llRetorno = .F.
		endif
		return llRetorno
	endfunc
	...
	function ValidarVendDesde() as boolean
		...
		if empty( this.VendDesde_PK )
			...
			If this.VendDesde.oAD.ConsultarPorClavePrimaria()
				This.VendDesde.oDesactivador.ValidarEstadoActivacion( this.VendDesde_PK , goLibrerias.Valorvacioseguntipo( vartype(this.VendDesde_PK ) ), this.lnuevo, this.ledicion )
			Else
	...
	function ValidarVendHasta() as boolean
		...
		if empty( this.VendHasta_PK )
			...
			If this.VendHasta.oAD.ConsultarPorClavePrimaria()
				This.VendHasta.oDesactivador.ValidarEstadoActivacion( this.VendHasta_PK , goLibrerias.Valorvacioseguntipo( vartype(this.VendHasta_PK ) ), this.lnuevo, this.ledicion )
			Else
	...
	*--------------------------------------------------------------------------------------------------------
	function ValidarVendedor_ClasificacionVDesde() as boolean
		local llRetorno as boolean
		llRetorno = dodefault()
		if empty( this.Vendedor_ClasificacionVDesde_PK )
		else
			If this.Vendedor_ClasificacionVDesde.oAD.ConsultarPorClavePrimaria()
			Else
				this.AgregarInformacion( 'El dato buscado ' + alltrim(transform( this.Vendedor_ClasificacionVDesde_PK ))+ ' de la entidad ' +  upper( this.Vendedor_ClasificacionVDesde.cDescripcion ) + ' no existe.')
				llRetorno = .F.
			endif
		endif
		Return llRetorno
	...
	*--------------------------------------------------------------------------------------------------------
	function ValidarVendedor_ClasificacionVHasta() as boolean
		local llRetorno as boolean
		llRetorno = dodefault()
		if empty( this.Vendedor_ClasificacionVHasta_PK )
		else
			If this.Vendedor_ClasificacionVHasta.oAD.ConsultarPorClavePrimaria()
			Else
				this.AgregarInformacion( 'El dato buscado ' + alltrim(transform( this.Vendedor_ClasificacionVHasta_PK ))+ ' de la entidad ' +  upper( this.Vendedor_ClasificacionVHasta.cDescripcion ) + ' no existe.')
				llRetorno = .F.
			endif
		endif
		Return llRetorno
	...
	*--------------------------------------------------------------------------------------------------------
	function obtenerAtributosObligatorios() as ZooColeccion
		...
		loAtributosObligatorios.add( 'MODOFUNCIONAMIENTO' )
	...
	*--------------------------------------------------------------------------------------------------------
	function ObtenerFuncionalidades() as String
		return '<EXPOGENERICA><IMPOGENERICA><NOLISTAGENERICO><PUBLICA><LINCE><PUBLICA><DESACTIVABLE><AUDITORIA<MODULOSLISTADO:P><GUARDARCOMO>'
	Endfunc
	*--------------------------------------------------------------------------------------------------------
	...
	*--------------------------------------------------------------------------------------------------------
	function Inicializar() as Void
		...
		this.oColeccionVSFW.agregar('.ValorSugeridoFWMontodesde()','Montodesde')
		...
		this.oColeccionVSFW.agregar('.ValorSugeridoFWVendedor_clasificacionvhasta()','Vendedor_clasificacionvhasta')
	...
	*--------------------------------------------------------------------------------------------------------
	function Vendedor_clasificacionvdesde_PK_Assign( txVal as variant ) as void

		Local lxValOld As Variant
		lxValOld = this.Vendedor_clasificacionvdesde_PK
		local lxVal as Variant
		local loSugerido as Object
		
		lxVal = txVal
		lxVal = iif( len( lxVal ) < 10, padr( lxVal, 10 ), lxVal )
		this.Vendedor_clasificacionvdesde_PK = lxVal
	...
	*--------------------------------------------------------------------------------------------------------
	function Vendedor_clasificacionvhasta_PK_Assign( txVal as variant ) as void

		Local lxValOld As Variant
		lxValOld = this.Vendedor_clasificacionvhasta_PK
		local lxVal as Variant
		local loSugerido as Object
		
		lxVal = txVal
		lxVal = iif( len( lxVal ) < 10, padr( lxVal, 10 ), lxVal )
		this.Vendedor_clasificacionvhasta_PK = lxVal
	...
	*--------------------------------------------------------------------------------------------------------
	function Setear_Vendedor_clasificacionvdesde( txVal as variant ) as void

		this.Vendedor_clasificacionvdesde.CODIGO = txVal
		dodefault( txVal )

	endfunc

	*--------------------------------------------------------------------------------------------------------
	function Setear_Vendedor_clasificacionvhasta( txVal as variant ) as void

		this.Vendedor_clasificacionvhasta.CODIGO = txVal
		dodefault( txVal )
	...
	*--------------------------------------------------------------------------------------------------------
	function Validar_Venddesde( txVal as variant, txValOld as variant ) as Boolean
		...
		Return This.CargaManual() and dodefault( txVal, txValOld ) and This.Venddesde.oDesactivador.ValidarEstadoActivacion( txVal, txValOld, this.lnuevo, this.ledicion) 
	...
	*--------------------------------------------------------------------------------------------------------
	function Validar_Vendhasta( txVal as variant, txValOld as variant ) as Boolean
		...
		Return This.CargaManual() and dodefault( txVal, txValOld ) and This.Vendhasta.oDesactivador.ValidarEstadoActivacion( txVal, txValOld, this.lnuevo, this.ledicion) 

	endfunc

	*--------------------------------------------------------------------------------------------------------
	function Validar_Vendedor_clasificacionvdesde( txVal as variant, txValOld as variant ) as Boolean
	...
	*--------------------------------------------------------------------------------------------------------
	function Validar_Vendedor_clasificacionvhasta( txVal as variant, txValOld as variant ) as Boolean
	...
	*--------------------------------------------------------------------------------------------------------
	function Vendedor_clasificacionvdesde_Access() as variant
		if this.ldestroy
		else
			if This.lInstanciarSubEntidadaDemanda
				if ( !vartype( this.Vendedor_clasificacionvdesde ) = 'O' or isnull( this.Vendedor_clasificacionvdesde ) )
					this.Vendedor_clasificacionvdesde = _Screen.zoo.instanciarentidad( 'Clasificacionvendedor' )
					this.Vendedor_clasificacionvdesde.lEsSubEntidad = .t.
					this.enlazar( 'Vendedor_clasificacionvdesde.EventoObtenerLogueo', 'inyectarLogueo' )
					this.enlazar( 'Vendedor_clasificacionvdesde.EventoObtenerInformacion', 'inyectarInformacion' )
				endif
				if !this.lBuscandoCodigo
					if this.Vendedor_clasificacionvdesde.CODIGO # this.Vendedor_clasificacionvdesde_PK
						this.Vendedor_clasificacionvdesde.CODIGO = this.Vendedor_clasificacionvdesde_PK
					endif
				endif
			endif
		endif
		return this.Vendedor_clasificacionvdesde
	endfunc

	*--------------------------------------------------------------------------------------------------------
	function Vendedor_clasificacionvhasta_Access() as variant
		if this.ldestroy
		else
			if This.lInstanciarSubEntidadaDemanda
				if ( !vartype( this.Vendedor_clasificacionvhasta ) = 'O' or isnull( this.Vendedor_clasificacionvhasta ) )
					this.Vendedor_clasificacionvhasta = _Screen.zoo.instanciarentidad( 'Clasificacionvendedor' )
					this.Vendedor_clasificacionvhasta.lEsSubEntidad = .t.
					this.enlazar( 'Vendedor_clasificacionvhasta.EventoObtenerLogueo', 'inyectarLogueo' )
					this.enlazar( 'Vendedor_clasificacionvhasta.EventoObtenerInformacion', 'inyectarInformacion' )
				endif
				if !this.lBuscandoCodigo
					if this.Vendedor_clasificacionvhasta.CODIGO # this.Vendedor_clasificacionvhasta_PK
						this.Vendedor_clasificacionvhasta.CODIGO = this.Vendedor_clasificacionvhasta_PK
					endif
				endif
			endif
		endif
		return this.Vendedor_clasificacionvhasta
	endfunc
			...
			if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
				if !empty( lcExpresionValorSugeridoDefinidoPorElUsuario )
					try
						...
						loEx.Grabar( loError )
						loEx.AgregarInformacion( iif( type( "loError.UserValue.Message" ) == "C", loError.UserValue.Message, loError.Message ) )
						loEx.AgregarInformacion( "Se produjo un error al intentar asignar el valor sugerido " + lcExpresionValorSugeridoDefinidoPorElUsuario + " del atributo Subtotal con IVA para la entidad Descuentos y recargos." + chr( 10 ) + "Verifique el valor especificado en la entidad Comportamiento de atributos." + chr( 10 ) + "El tipo de dato del atributo es numérico." )
						goServicios.Errores.LevantarExcepcion( loEx )
					endtry
				endif
			else
				.ValorSugeridoFWMontodesde()
			endif
		endwith
	endfunc

	*--------------------------------------------------------------------------------------------------------
	Function ValorSugeridoFWMontodesde() as void
		with this
			.Montodesde = 0
		endwith
	endfunc
			...
			if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
				if !empty( lcExpresionValorSugeridoDefinidoPorElUsuario )
					try
						...
						loEx.Grabar( loError )
						loEx.AgregarInformacion( iif( type( "loError.UserValue.Message" ) == "C", loError.UserValue.Message, loError.Message ) )
						loEx.AgregarInformacion( "Se produjo un error al intentar asignar el valor sugerido " + lcExpresionValorSugeridoDefinidoPorElUsuario + " del atributo Monto sugerido para la entidad Descuentos y recargos." + chr( 10 ) + "Verifique el valor especificado en la entidad Comportamiento de atributos." + chr( 10 ) + "El tipo de dato del atributo es numérico." )
						goServicios.Errores.LevantarExcepcion( loEx )
					endtry
				endif
			endif
			...
			if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
				if !empty( lcExpresionValorSugeridoDefinidoPorElUsuario )
					try
						...
						loEx.Grabar( loError )
						loEx.AgregarInformacion( iif( type( "loError.UserValue.Message" ) == "C", loError.UserValue.Message, loError.Message ) )
						loEx.AgregarInformacion( "Se produjo un error al intentar asignar el valor sugerido " + lcExpresionValorSugeridoDefinidoPorElUsuario + " del atributo Perfil de seguridad hasta para la entidad Descuentos y recargos." + chr( 10 ) + "Verifique el valor especificado en la entidad Comportamiento de atributos." + chr( 10 ) + "El tipo de dato del atributo es caracter." )
						goServicios.Errores.LevantarExcepcion( loEx )
					endtry
				endif
			else
	...
	*--------------------------------------------------------------------------------------------------------
	Function ValorSugeridoVendedor_clasificacionvdesde() as void
		local lcExpresionValorSugeridoDefinidoPorElUsuario as String, lvValorSugeridoDefinidoPorElUsuario as Variant, loError as Object, loEx as Object, loExTipoDato as Object
		lcExpresionValorSugeridoDefinidoPorElUsuario = null
		with this
			try
				lcExpresionValorSugeridoDefinidoPorElUsuario = goServicios.SaltosDeCampoYValoresSugeridos.ObtenerValorSugerido( "DESCUENTO", "", "Vendedor_clasificacionvdesde" )
				if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
					if !empty( lcExpresionValorSugeridoDefinidoPorElUsuario )
						lvValorSugeridoDefinidoPorElUsuario = &lcExpresionValorSugeridoDefinidoPorElUsuario
						if vartype( lvValorSugeridoDefinidoPorElUsuario ) # "C"
						...
						This.EventoEstaSeteandoValorSugerido( 'Vendedor_clasificacionvdesde_PK' )
						.Vendedor_clasificacionvdesde_PK = lvValorSugeridoDefinidoPorElUsuario
						.Vendedor_clasificacionvdesde.CODIGO = .Vendedor_clasificacionvdesde_PK
					endif
				endif
			Catch to loError
				.Vendedor_clasificacionvdesde_PK=[]
				...
				if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
					...
					loEx.AgregarInformacion( "Se produjo un error al intentar asignar el valor sugerido " + lcExpresionValorSugeridoDefinidoPorElUsuario + " del atributo Clasificación vendedor para la entidad Descuentos y recargos." + chr( 10 ) + "Verifique el valor especificado en la entidad Comportamiento de atributos." + chr( 10 ) + "El tipo de dato del atributo es caracter." )
					goServicios.Errores.LevantarExcepcion( loEx )
				endif
			endtry 
		endwith
	...
	*--------------------------------------------------------------------------------------------------------
	Function ValorSugeridoVendedor_clasificacionvhasta() as void
		local lcExpresionValorSugeridoDefinidoPorElUsuario as String, lvValorSugeridoDefinidoPorElUsuario as Variant, loError as Object, loEx as Object, loExTipoDato as Object
		lcExpresionValorSugeridoDefinidoPorElUsuario = null
		with this
			try
				lcExpresionValorSugeridoDefinidoPorElUsuario = goServicios.SaltosDeCampoYValoresSugeridos.ObtenerValorSugerido( "DESCUENTO", "", "Vendedor_clasificacionvhasta" )
				if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
					if !empty( lcExpresionValorSugeridoDefinidoPorElUsuario )
						lvValorSugeridoDefinidoPorElUsuario = &lcExpresionValorSugeridoDefinidoPorElUsuario
						if vartype( lvValorSugeridoDefinidoPorElUsuario ) # "C"
						...
						This.EventoEstaSeteandoValorSugerido( 'Vendedor_clasificacionvhasta_PK' )
						.Vendedor_clasificacionvhasta_PK = lvValorSugeridoDefinidoPorElUsuario
						.Vendedor_clasificacionvhasta.CODIGO = .Vendedor_clasificacionvhasta_PK
					endif
				else
					.ValorSugeridoFWVendedor_clasificacionvhasta()
				endif
			Catch to loError
				.Vendedor_clasificacionvhasta_PK=[]
				loEx = newobject("zooexception", "zooexception.prg" )
				...
				if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
					...
					loEx.AgregarInformacion( "Se produjo un error al intentar asignar el valor sugerido " + lcExpresionValorSugeridoDefinidoPorElUsuario + " del atributo Clasificación vendedor hasta para la entidad Descuentos y recargos." + chr( 10 ) + "Verifique el valor especificado en la entidad Comportamiento de atributos." + chr( 10 ) + "El tipo de dato del atributo es caracter." )
					goServicios.Errores.LevantarExcepcion( loEx )
				endif
			endtry 
		endwith
	...
	*--------------------------------------------------------------------------------------------------------
	Function ValorSugeridoFWVendedor_clasificacionvhasta() as void
		with this
			try
				.Vendedor_clasificacionvhasta_PK = Replicate( "Z",10)
			Catch to loError
				.Vendedor_clasificacionvhasta_PK=[]
				loEx = newobject("zooexception", "zooexception.prg" )
				loEx.Grabar( loError )
		...
		with this
			...
			Catch to loError
				...
				if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
					...
					loEx.AgregarInformacion( "Se produjo un error al intentar asignar el valor sugerido " + lcExpresionValorSugeridoDefinidoPorElUsuario + " del atributo Clasificación cliente hasta para la entidad Descuentos y recargos." + chr( 10 ) + "Verifique el valor especificado en la entidad Comportamiento de atributos." + chr( 10 ) + "El tipo de dato del atributo es caracter." )
					goServicios.Errores.LevantarExcepcion( loEx )
				endif
			endtry 
		endwith
		...
		with this
			...
			Catch to loError
				...
				if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
					...
					loEx.AgregarInformacion( "Se produjo un error al intentar asignar el valor sugerido " + lcExpresionValorSugeridoDefinidoPorElUsuario + " del atributo Tipo cliente hasta para la entidad Descuentos y recargos." + chr( 10 ) + "Verifique el valor especificado en la entidad Comportamiento de atributos." + chr( 10 ) + "El tipo de dato del atributo es caracter." )
					goServicios.Errores.LevantarExcepcion( loEx )
				endif
			endtry 
		endwith
		...
		with this
			...
			Catch to loError
				...
				if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
					...
					loEx.AgregarInformacion( "Se produjo un error al intentar asignar el valor sugerido " + lcExpresionValorSugeridoDefinidoPorElUsuario + " del atributo Categoría cliente hasta para la entidad Descuentos y recargos." + chr( 10 ) + "Verifique el valor especificado en la entidad Comportamiento de atributos." + chr( 10 ) + "El tipo de dato del atributo es caracter." )
					goServicios.Errores.LevantarExcepcion( loEx )
				endif
			endtry 
		endwith
