...
define class Din_DetalleFINALDEPRODUCCIONMovimientodetalle as DetalleArticulosproduccion of DetalleArticulosproduccion.Prg
	...
	Protected Function _CargarItem( tnItem as integer )
		...
		With this.oItem
			Try
				...
				.CantidadLimite = this.Item[ tnItem ].CantidadLimite
				llVacio = llVacio and this.VerificarSiElAtributoEstaVacio( 'CantidadLimite', .CantidadLimite )
				...
				.InventarioDetalle = this.Item[ tnItem ].InventarioDetalle
				llVacio = llVacio and this.VerificarSiElAtributoEstaVacio( 'InventarioDetalle', .InventarioDetalle )
	...
	*--------------------------------------------------------------------------------------------------------
	protected function AplanarItem() as Object
		...
		with loRetorno
			...
			.CantidadLimite = this.oItem.CantidadLimite
			.NROITEM = this.oItem.NROITEM
			.Inventario_PK = this.oItem.Inventario_PK
			.InventarioDetalle = this.oItem.InventarioDetalle
	...
	*--------------------------------------------------------------------------------------------------------
	...
	with this
		if .ValidarItem()
			...
			.Item[ lnItem ].CantidadLimite = .oItem.CantidadLimite
			.Item[ lnItem ].NROITEM = .oItem.NROITEM
			.Item[ lnItem ].Inventario_PK = .oItem.Inventario_PK
			.Item[ lnItem ].InventarioDetalle = .oItem.InventarioDetalle
	...
	*--------------------------------------------------------------------------------------------------------
	function oItem_Access() as variant
		if !this.ldestroy and ( !vartype( this.oItem ) = 'O' or isnull( this.oItem ) )
			this.oItem = _Screen.zoo.crearobjeto( 'Din_ItemFINALDEPRODUCCIONMovimientodetalle' )
	...
	*--------------------------------------------------------------------------------------------------------
	Function Cargar() as Void
		...
		scan for &lcFiltro
			...
			loItem.InventarioDetalle = c_Movimientodetalle.InventarioDetalle
	...
	*-----------------------------------------------------------------------------------------
	...
	local lnLargoEntero as integer, lnValorDelDecimal as integer, lnLargoDecimal as integer, llRetorno as boolean, lnI as Integer
		...
		for lni = 1 to this.Count
			...
			if vartype( this.Item[lni].INVENTARIODETALLE) = 'C'
				if len( alltrim( this.Item[lni].INVENTARIODETALLE )) <= 100
				else
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud del valor del atributo INVENTARIODETALLE del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform( len( alltrim( this.Item[lni].INVENTARIODETALLE ))) + ' Máxima:100), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
				this.AgregarInformacion( 'El tipo de dato para el atributo INVENTARIODETALLE del item ' + transform( lni ) + ' no es el correcto, en el detalle ' + this.cEtiqueta + '.')
			Endif
	...
	*--------------------------------------------------------------------------------------------------------
	function CopiarItemaItem( toItemOrigen as Object, toItemDestino as Object ) as Object
		...
		toItemDestino.Cantidadlimite = toItemOrigen.Cantidadlimite
		toItemDestino.Nroitem = toItemOrigen.Nroitem
		toItemDestino.Inventario_PK = toItemOrigen.Inventario_PK
		toItemDestino.Inventariodetalle = toItemOrigen.Inventariodetalle
