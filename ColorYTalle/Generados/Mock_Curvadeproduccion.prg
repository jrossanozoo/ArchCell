...
define class Mock_curvadeproduccion as MockBase of MockBase.prg
	...
	cContexto = ""                                                                                                                                                                                                                                            
	cCurvaAplicada = ''                                                                                                                                                                                                                                       
	cDescripcion =  "Curva de producción"                                                                                                                                                                                                                     
	cDescripcionSingular = ""                                                                                                                                                                                                                                 
	cEvento = ""                                                                                                                                                                                                                                              
	cFormatoFechaNet = ""                                                                                                                                                                                                                                     
	cIdentificadorConexion = ""                                                                                                                                                                                                                               
	cNombre = ""                                                                                                                                                                                                                                              
	cNombre = 'CURVADEPRODUCCION'                                                                                                                                                                                                                             
	cPaletaAplicada = ''                                                                                                                                                                                                                                      
	cPrefijoImportar = ""                                                                                                                                                                                                                                     
	cPrefijoRecibir = ""                                                                                                                                                                                                                                      
	...
	*-----------------------------------------------------------------------------------------
	function cambiosdetalledetalle() as void                                                                                                                                                                                                                  
		...
		return this.ObtenerResultado( 'Cambiosdetalledetalle', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function eventomensajeconfirmar( tcmensaje as string, tcmensajecancelacion as string, tcatributo as string, txvalold as variant ) as void                                                                                                                 
		...
		return this.ObtenerResultado( 'Eventomensajeconfirmar', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function existecombinacion( tccolor as string, tctalle as string ) as boolean                                                                                                                                                                             
		...
		return this.ObtenerResultado( 'Existecombinacion', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function setear_paletadecolores( txval as variant ) as void                                                                                                                                                                                               
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'CURVADEPRODUCCION' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'setear_paletadecolores', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'setear_paletadecolores', lcParametros )			endif		endif		local lcAnterior as String, lcNuevo as String, loItem as Object, loPaleta as Object, loCurva as Object		dodefault( txVal )		if this.lActualizarDetalle			this.ActualizarDetalleConCombinaciones()		endif
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function validar_paletadecolores( txval as variant, txvalold as variant ) as boolean                                                                                                                                                                      
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1, lxParam2lnParametros = pcount()lxParam1 = txvallxParam2 = txvaloldlcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'CURVADEPRODUCCION' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'validar_paletadecolores', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'validar_paletadecolores', lcParametros )			endif		endif		local llRetorno as Boolean		if this.EsModoEdicion()			try				this.lActualizarDetalle = .t.				if !empty(txVal) and txVal != txValOld and !empty(this.CurvaDeTalles_PK) and this.Detalle.count > 0					this.eventoMensajeConfirmar('Esta cambiando la curva de talles con definiciones cargadas. Se eliminaran las combinaciones y se cargaran las nuevas.', 'Ha cancelar el cambio' )					this.lActualizarDetalle = .f.				endif				llRetorno = dodefault( txVal, txValOld )			catch				this.PaletaDeColores_PK = txValOld			endtry		else			llRetorno = dodefault( txVal, txValOld )		endif		Return llRetorno
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function validarcurvadetalles() as boolean                                                                                                                                                                                                                
		...
		return this.ObtenerResultado( 'Validarcurvadetalles', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function validarpaletadecolores() as boolean                                                                                                                                                                                                              
		...
		return this.ObtenerResultado( 'Validarpaletadecolores', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function valorsugeridocurvadetalles() as void                                                                                                                                                                                                             
		...
		return this.ObtenerResultado( 'Valorsugeridocurvadetalles', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function valorsugeridopaletadecolores() as void                                                                                                                                                                                                           
		...
		return this.ObtenerResultado( 'Valorsugeridopaletadecolores', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	protected function actualizardetalleconcombinaciones() as void                                                                                                                                                                                            
		...
		return this.ObtenerResultado( 'Actualizardetalleconcombinaciones', lcParametros )
	EndFunc
