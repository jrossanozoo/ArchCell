...
define class Din_EntidadUNIDADDEMEDIDA as entidad of entidad.prg

	cNombre = 'UNIDADDEMEDIDA'
	FECHAEXPO = ctod( '  /  /    ' )
	...
	Descripcion = []
	EquivalenciaAfip_PK = 0
	EquivalenciaAfip = null
	Obs = []
	EquivalenciaArba_PK = []
	EquivalenciaArba = null
	lPermiteMinusculasPK = .f.
	cDescripcion =  [Unidad de medida]
	lEntidadEditable =  .t.
	...
	*--------------------------------------------------------------------------------------------------------
	function oAD_Access() as variant
		if !this.ldestroy and ( !vartype( this.oAD ) = 'O' or isnull( this.oAD ) )
			this.oAD = this.crearobjeto( 'Din_EntidadUNIDADDEMEDIDAAD_' + alltrim( _screen.zoo.app.TipoDeBase ))
	...
	*--------------------------------------------------------------------------------------------------------
	function Agrupublidetalle_Access() as variant
		if !this.ldestroy and ( !vartype( this.Agrupublidetalle ) = 'O' or isnull( this.Agrupublidetalle ) )
			this.Agrupublidetalle = this.crearobjeto( 'Din_DetalleUnidaddemedidaAgrupublidetalle' )
	...
	*--------------------------------------------------------------------------------------------------------
	function ValidacionBasica() as boolean
		...
		With This
			...
			llRetorno = .ValidarEquivalenciaarba() and llRetorno
	...
	*--------------------------------------------------------------------------------------------------------
	function ValidarEquivalenciaArba() as boolean
		local llRetorno as boolean
		llRetorno = dodefault()
		if empty( this.EquivalenciaArba_PK )
		else
			If this.EquivalenciaArba.oAD.ConsultarPorClavePrimaria()
			Else
				this.AgregarInformacion( 'El dato buscado ' + alltrim(transform( this.EquivalenciaArba_PK ))+ ' de la entidad ' +  upper( this.EquivalenciaArba.cDescripcion ) + ' no existe.')
				llRetorno = .F.
			endif
		endif
		Return llRetorno
	...
	*--------------------------------------------------------------------------------------------------------
	function ObtenerFuncionalidades() as String
		return '<EXPOGENERICA><IMPOGENERICA><CODIGOSUGERIDO><TRANSFERALTAS><BAJALOGICA><PUBLICA>'
	Endfunc
	...
	*--------------------------------------------------------------------------------------------------------
	function Inicializar() as Void
		...
		oICol = goServicios.SaltosDeCampoYValoresSugeridos.ObtenerValorSugeridoDeUnaEntidadDetalle( 'UNIDADDEMEDIDA', '')
	...
	*--------------------------------------------------------------------------------------------------------
	function Equivalenciaarba_PK_Assign( txVal as variant ) as void

		Local lxValOld As Variant
		lxValOld = this.Equivalenciaarba_PK
		local lxVal as Variant
		local loSugerido as Object
		
		lxVal = txVal
		lxVal = iif( len( lxVal ) < 10, padr( lxVal, 10 ), lxVal )
		this.Equivalenciaarba_PK = lxVal

		if this.Validar_Equivalenciaarba( lxVal, lxValOld )
			This.Setear_Equivalenciaarba( lxVal )
		endif
		dodefault( lxVal )
	...
	*--------------------------------------------------------------------------------------------------------
	function Setear_Equivalenciaarba( txVal as variant ) as void

		this.Equivalenciaarba.CODIGO = txVal
		dodefault( txVal )
	...
	*--------------------------------------------------------------------------------------------------------
	function Validar_Equivalenciaarba( txVal as variant, txValOld as variant ) as Boolean
		...
		if this.ldestroy
			...
			if This.lInstanciarSubEntidadaDemanda
				if ( !vartype( this.Equivalenciaafip ) = 'O' or isnull( this.Equivalenciaafip ) )
					this.Equivalenciaafip = _Screen.zoo.instanciarentidad( 'Unidaddemedidaafip' )
	...
	*--------------------------------------------------------------------------------------------------------
	function Equivalenciaarba_Access() as variant
		if this.ldestroy
		else
			if This.lInstanciarSubEntidadaDemanda
				if ( !vartype( this.Equivalenciaarba ) = 'O' or isnull( this.Equivalenciaarba ) )
					this.Equivalenciaarba = _Screen.zoo.instanciarentidad( 'Unidaddemedidaarba' )
					this.Equivalenciaarba.lEsSubEntidad = .t.
					this.enlazar( 'Equivalenciaarba.EventoObtenerLogueo', 'inyectarLogueo' )
					this.enlazar( 'Equivalenciaarba.EventoObtenerInformacion', 'inyectarInformacion' )
				endif
				if !this.lBuscandoCodigo
					if this.Equivalenciaarba.CODIGO # this.Equivalenciaarba_PK
						this.Equivalenciaarba.CODIGO = this.Equivalenciaarba_PK
					endif
				endif
			endif
		endif
		return this.Equivalenciaarba
	endfunc
	...
	*--------------------------------------------------------------------------------------------------------
	Function ValorSugeridoDescripcion() as void
		...
		with this
			lcExpresionValorSugeridoDefinidoPorElUsuario = goServicios.SaltosDeCampoYValoresSugeridos.ObtenerValorSugerido( "UNIDADDEMEDIDA", "", "Descripcion" )
	...
	*--------------------------------------------------------------------------------------------------------
	Function ValorSugeridoEquivalenciaafip() as void
		...
		with this
			try
				lcExpresionValorSugeridoDefinidoPorElUsuario = goServicios.SaltosDeCampoYValoresSugeridos.ObtenerValorSugerido( "UNIDADDEMEDIDA", "", "Equivalenciaafip" )
				if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
					if !empty( lcExpresionValorSugeridoDefinidoPorElUsuario )
						lvValorSugeridoDefinidoPorElUsuario = &lcExpresionValorSugeridoDefinidoPorElUsuario
	...
	*--------------------------------------------------------------------------------------------------------
	Function ValorSugeridoObs() as void
		...
		lcExpresionValorSugeridoDefinidoPorElUsuario = null
		with this
			lcExpresionValorSugeridoDefinidoPorElUsuario = goServicios.SaltosDeCampoYValoresSugeridos.ObtenerValorSugerido( "UNIDADDEMEDIDA", "", "Obs" )
			if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
				if !empty( lcExpresionValorSugeridoDefinidoPorElUsuario )
					try
						lvValorSugeridoDefinidoPorElUsuario = &lcExpresionValorSugeridoDefinidoPorElUsuario
	...
	*--------------------------------------------------------------------------------------------------------
	Function ValorSugeridoEquivalenciaarba() as void
		local lcExpresionValorSugeridoDefinidoPorElUsuario as String, lvValorSugeridoDefinidoPorElUsuario as Variant, loError as Object, loEx as Object, loExTipoDato as Object
		lcExpresionValorSugeridoDefinidoPorElUsuario = null
		with this
			try
				lcExpresionValorSugeridoDefinidoPorElUsuario = goServicios.SaltosDeCampoYValoresSugeridos.ObtenerValorSugerido( "UNIDADDEMEDIDA", "", "Equivalenciaarba" )
				if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
					if !empty( lcExpresionValorSugeridoDefinidoPorElUsuario )
						lvValorSugeridoDefinidoPorElUsuario = &lcExpresionValorSugeridoDefinidoPorElUsuario
						if vartype( lvValorSugeridoDefinidoPorElUsuario ) # "C"
						...
						This.EventoEstaSeteandoValorSugerido( 'Equivalenciaarba_PK' )
						.Equivalenciaarba_PK = lvValorSugeridoDefinidoPorElUsuario
						.Equivalenciaarba.CODIGO = .Equivalenciaarba_PK
					endif
				endif
			Catch to loError
				.Equivalenciaarba_PK=[]
				...
				if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
					...
					loEx.AgregarInformacion( "Se produjo un error al intentar asignar el valor sugerido " + lcExpresionValorSugeridoDefinidoPorElUsuario + " del atributo Código ARBA para la entidad Unidades de medidas." + chr( 10 ) + "Verifique el valor especificado en la entidad Comportamiento de atributos." + chr( 10 ) + "El tipo de dato del atributo es caracter." )
					goServicios.Errores.LevantarExcepcion( loEx )
				endif
			endtry 
		endwith
