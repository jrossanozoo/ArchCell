...
define class Din_DetalleORDENDEPRODUCCIONOrdeninsumos as DetalleBaseproduccion of DetalleBaseproduccion.Prg
	...
	Protected Function _CargarItem( tnItem as integer )
		...
		With this.oItem
			Try
				...
				.ColorOrdDet = this.Item[ tnItem ].ColorOrdDet
				llVacio = llVacio and this.VerificarSiElAtributoEstaVacio( 'ColorOrdDet', .ColorOrdDet )
	...
	*--------------------------------------------------------------------------------------------------------
	protected function AplanarItem() as Object
		...
		with loRetorno
			...
			.ColorOrdDet = this.oItem.ColorOrdDet
	...
	*--------------------------------------------------------------------------------------------------------
	...
	with this
		if .ValidarItem()
			...
			.Item[ lnItem ].ColorOrdDet = .oItem.ColorOrdDet
	...
	*--------------------------------------------------------------------------------------------------------
	function oItem_Access() as variant
		if !this.ldestroy and ( !vartype( this.oItem ) = 'O' or isnull( this.oItem ) )
			this.oItem = _Screen.zoo.crearobjeto( 'Din_ItemORDENDEPRODUCCIONOrdeninsumos' )
	...
	*--------------------------------------------------------------------------------------------------------
	Function Cargar() as Void
		...
		select c_Ordeninsumos
		lcFiltro = this.ObtenerFiltro()
		scan for &lcFiltro
			loItem = createobject('ItemAuxiliar')

			loItem.CodigoOrden = c_Ordeninsumos.CodigoOrden
			loItem.NROITEM = c_Ordeninsumos.NROITEM
			loItem.Proceso_PK = c_Ordeninsumos.Proceso
			loItem.ColorM_PK = c_Ordeninsumos.ColorM
			loItem.ColorMDetalle = c_Ordeninsumos.ColorMDetalle
			loItem.TalleM_PK = c_Ordeninsumos.TalleM
			loItem.TalleMDetalle = c_Ordeninsumos.TalleMDetalle
			loItem.Insumo_PK = c_Ordeninsumos.Insumo
			loItem.InsumoDetalle = c_Ordeninsumos.InsumoDetalle
			loItem.Color_PK = c_Ordeninsumos.Color
			loItem.ColorOrdDet = c_Ordeninsumos.ColorOrdDet
			loItem.Talle_PK = c_Ordeninsumos.Talle
			loItem.TalleDetalle = c_Ordeninsumos.TalleDetalle
			loItem.UnidadDeMedida_PK = c_Ordeninsumos.UnidadDeMedida
			loItem.Cantidad = c_Ordeninsumos.Cantidad
			loItem.NroItem = lnI 
	...
	*-----------------------------------------------------------------------------------------
	...
	local lnLargoEntero as integer, lnValorDelDecimal as integer, lnLargoDecimal as integer, llRetorno as boolean, lnI as Integer
		...
		for lni = 1 to this.Count
			...
			if vartype( this.Item[lni].COLORORDDET) = 'C'
				if len( alltrim( this.Item[lni].COLORORDDET )) <= 50
				else
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud del valor del atributo COLORORDDET del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform( len( alltrim( this.Item[lni].COLORORDDET ))) + ' Máxima:50), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
				this.AgregarInformacion( 'El tipo de dato para el atributo COLORORDDET del item ' + transform( lni ) + ' no es el correcto, en el detalle ' + this.cEtiqueta + '.')
			Endif
	...
	*--------------------------------------------------------------------------------------------------------
	function CopiarItemaItem( toItemOrigen as Object, toItemDestino as Object ) as Object
		...
		toItemDestino.Colororddet = toItemOrigen.Colororddet
