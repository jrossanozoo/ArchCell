...
define class Din_ItemTICKETFACTURAPromocionesdetalle as ItemPromociones of ItemPromociones.prg
	...
	*--------------------------------------------------------------------------------------------------------
	function Inicializar() as boolean
		...
		oICol = goServicios.SaltosDeCampoYValoresSugeridos.ObtenerValorSugeridoDeUnaEntidadDetalle( 'TICKETFACTURA', 'Promocionesdetalle')
	...
	*--------------------------------------------------------------------------------------------------------
	function Idartsafec_Assign( txVal as variant ) as void
		...
		Local lxValOld As Variant
		lxValOld = this.Idartsafec
		local lxVal as Variant
		lxVal = txVal

		if This.CargaManual()
			if this.Validar_Idartsafec( lxVal )
				this.EventoAntesDeSetear( This, 'Idartsafec', lxValOld, lxVal  )
				this.Setear_Idartsafec( lxVal )
				this.EventoDespuesDeSetear( This, 'Idartsafec', lxValOld, lxVal  )
				If lxValOld != lxVal
					If This.PuedeEjecutarHaCambiado()
						this.HaCambiado( 'Idartsafec', This )
					Endif
				EndIf
			EndIf
		Else
	...
	*--------------------------------------------------------------------------------------------------------
	Function ValorSugeridoPromocion() as void
		...
		with this
			try
				lcExpresionValorSugeridoDefinidoPorElUsuario = goServicios.SaltosDeCampoYValoresSugeridos.ObtenerValorSugerido( "TICKETFACTURA", "Promocionesdetalle", "Promocion" )
