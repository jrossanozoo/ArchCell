...
define class Mock_cuponespendientes as MockBase of MockBase.prg
	...
	cCaracteres = ""                                                                                                                                                                                                                                          
	cContexto = ""                                                                                                                                                                                                                                            
	cDescripcion =  "Detalle de cupones"                                                                                                                                                                                                                      
	...
	*-----------------------------------------------------------------------------------------
	function dispositivopos_access() as variant                                                                                                                                                                                                               
		lnParametros = pcount()lcParametros = ''		lnIdClase = _screen.Mocks.BuscarMock( 'CUPONESPENDIENTES' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'dispositivopos_access', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'dispositivopos_access', lcParametros )			endif		endif		if this.ldestroy		else			if This.lInstanciarSubEntidadaDemanda				if ( !vartype( this.Dispositivopos ) = 'O' or isnull( this.Dispositivopos ) )					this.Dispositivopos = _Screen.zoo.instanciarentidad( 'Pos' )					this.Dispositivopos.lEsSubEntidad = .t.					this.enlazar( 'Dispositivopos.EventoObtenerLogueo', 'inyectarLogueo' )					this.enlazar( 'Dispositivopos.EventoObtenerInformacion', 'inyectarInformacion' )				endif				if !this.lBuscandoCodigo					if this.Dispositivopos.CODIGO # this.Dispositivopos_PK						this.Dispositivopos.CODIGO = this.Dispositivopos_PK					endif				endif			endif		endif		return this.Dispositivopos
	EndFunc

	*-----------------------------------------------------------------------------------------
	function dispositivopos_pk_assign( txval as variant ) as void                                                                                                                                                                                             
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'CUPONESPENDIENTES' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'dispositivopos_pk_assign', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'dispositivopos_pk_assign', lcParametros )			endif		endif		Local lxValOld As Variant		lxValOld = this.Dispositivopos_PK		local lxVal as Variant		local loSugerido as Object				lxVal = txVal		lxVal = iif( len( lxVal ) < 10, padr( lxVal, 10 ), lxVal )		this.Dispositivopos_PK = lxVal		if this.Validar_Dispositivopos( lxVal, lxValOld )			This.Setear_Dispositivopos( lxVal )		endif		dodefault( lxVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function inyectarcuponespendientes( tocuponespendientes as object ) as void                                                                                                                                                                               
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		local lxParam1

		lnParametros = pcount()
		lxParam1 = tocuponespendientes
		...
		return this.ObtenerResultado( 'Inyectarcuponespendientes', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function setear_dispositivopos( txval as variant ) as void                                                                                                                                                                                                
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'CUPONESPENDIENTES' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'setear_dispositivopos', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'setear_dispositivopos', lcParametros )			endif		endif		this.Dispositivopos.CODIGO = txVal		dodefault( txVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function validar_dispositivopos( txval as variant, txvalold as variant ) as boolean                                                                                                                                                                       
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1, lxParam2lnParametros = pcount()lxParam1 = txvallxParam2 = txvaloldlcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'CUPONESPENDIENTES' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'validar_dispositivopos', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'validar_dispositivopos', lcParametros )			endif		endif		Return This.CargaManual() and dodefault( txVal, txValOld )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function validardispositivopos() as boolean                                                                                                                                                                                                               
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Validardispositivopos', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	protected function detectarcambio( tcatributo as string, toitem as object ) as void                                                                                                                                                                       
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		local lxParam1, lxParam2
		...
		return this.ObtenerResultado( 'Detectarcambio', lcParametros )
	EndFunc
