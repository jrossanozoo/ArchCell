...
define class Din_EntidadNOTADEDEBITOELECTRONICAEXPORTACION as Ent_ComprobanteElectronicoExportacion of Ent_ComprobanteElectronicoExportacion.prg
	...
	*--------------------------------------------------------------------------------------------------------
	function ObtenerFuncionalidades() as String
		return '<ANULABLE><WEBHOOK><VENTAS><CONVALORES><FISCAL><CE><NOLISTAGENERICO><VALORCIERRE><DESCUENTOAUTOMATICO><ACTUALIZAEMAILCLIENTE><LISTARMODIFICACION>'
	Endfunc
	*--------------------------------------------------------------------------------------------------------
	...
	*--------------------------------------------------------------------------------------------------------
	function Inicializar() as Void
		...
		this.lWHIngresar = goServicios.WebHook.TieneQueMandar( 'NOTADEDEBITOELECTRONICAEXPORTACION', 'INGRESAR' )
		this.lWHModificar = goServicios.WebHook.TieneQueMandar( 'NOTADEDEBITOELECTRONICAEXPORTACION', 'MODIFICAR' )
		this.lWHEliminar = goServicios.WebHook.TieneQueMandar( 'NOTADEDEBITOELECTRONICAEXPORTACION', 'ELIMINAR' )
	endfunc
	...
	function Monedacomprobante_PK_Assign( txVal as variant ) as void
		...
		if this.Validar_Monedacomprobante( lxVal, lxValOld )
			if ( this.lHabilitarMonedacomprobante_PK or this.lEstaSeteandoValorSugerido or lxValOld == lxVal )
				...
				goServicios.Errores.LevantarExcepcion( "No se puede cambiar este valor (Entidad: Nota de débito electrónica exportación de venta - Atributo: Monedacomprobante)" )
			endif 
		endif
		dodefault( lxVal )
		...
		with this
			...
			Catch to loError
				...
				if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
					...
					loEx.AgregarInformacion( "Se produjo un error al intentar asignar el valor sugerido " + lcExpresionValorSugeridoDefinidoPorElUsuario + " del atributo Cliente para la entidad Nota de débito electrónica exportación de venta." + chr( 10 ) + "Verifique el valor especificado en la entidad Comportamiento de atributos." + chr( 10 ) + "El tipo de dato del atributo es caracter." )
	...
	*--------------------------------------------------------------------------------------------------------
	Function ValorSugeridoMonedacomprobante() as void
		...
		with this
			try
				lcExpresionValorSugeridoDefinidoPorElUsuario = goServicios.SaltosDeCampoYValoresSugeridos.ObtenerValorSugerido( "NOTADEDEBITOELECTRONICAEXPORTACION", "", "Monedacomprobante" )
				if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
					if !empty( lcExpresionValorSugeridoDefinidoPorElUsuario )
						lvValorSugeridoDefinidoPorElUsuario = &lcExpresionValorSugeridoDefinidoPorElUsuario
						if vartype( lvValorSugeridoDefinidoPorElUsuario ) # "C"
	...
	*--------------------------------------------------------------------------------------------------------
	Function ValorSugeridoFWMonedacomprobante() as void
		with this
			try
				.Monedacomprobante_PK = ALLTRIM( goParametros.Felino.GestionDeVentas.FacturacionElectronica.Exportacion.CodigoDeMonedaExtranjeraSugerida )
				.Monedacomprobante.CODIGO = .Monedacomprobante_PK
			Catch to loError
				.Monedacomprobante_PK=[]
				loEx = newobject("zooexception", "zooexception.prg" )
