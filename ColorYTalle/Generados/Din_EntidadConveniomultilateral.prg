...
define class Din_EntidadCONVENIOMULTILATERAL as entidad of entidad.prg
	...
	BASEDEDATOSALTAFW = []
	TIMESTAMP = 0
	ImpuestosConvenio = null
	cAtributoPK = 'Codigo'
	Codigo = []
	Obs = []
	Fecha = ctod( '  /  /    ' )
	lPermiteMinusculasPK = .f.
	cDescripcion =  [Convenio multilateral]
	lEntidadEditable =  .t.
	...
	*--------------------------------------------------------------------------------------------------------
	function oAD_Access() as variant
		if !this.ldestroy and ( !vartype( this.oAD ) = 'O' or isnull( this.oAD ) )
			this.oAD = this.crearobjeto( 'Din_EntidadCONVENIOMULTILATERALAD_' + alltrim( _screen.zoo.app.TipoDeBase ))
	...
	*--------------------------------------------------------------------------------------------------------
	function Impuestosconvenio_Access() as variant
		if !this.ldestroy and ( !vartype( this.Impuestosconvenio ) = 'O' or isnull( this.Impuestosconvenio ) )
			this.Impuestosconvenio = this.crearobjeto( 'DetalleConveniomultilateralImpuestosconvenio' )
			this.Impuestosconvenio.inicializar()
			this.enlazar( 'Impuestosconvenio.EventoObtenerLogueo', 'inyectarLogueo' )
			this.enlazar( 'Impuestosconvenio.EventoObtenerInformacion', 'inyectarInformacion' )
			bindevent( this.Impuestosconvenio, 'CambioSumarizado', this, 'CambiosDetalleImpuestosconvenio', 1) 
			this.enlazar( 'Impuestosconvenio.EventoAdvertirLimitePorDiseno', 'EventoAdvertirLimitePorDiseno') 
			this.enlazar( 'Impuestosconvenio.EventoCancelarCargaLimitePorDiseno', 'EventoCancelarCargaLimitePorDiseno') 
		endif
		return this.Impuestosconvenio
	endfunc

	*--------------------------------------------------------------------------------------------------------
	function SetearEstadoNuevoEnItems() as Void
		dodefault()
		this.ImpuestosConvenio.oItem.lNuevo = this.lNuevo
	endfunc

	*--------------------------------------------------------------------------------------------------------
	function SetearEstadoEdicionEnItems() as Void
		dodefault()
		this.ImpuestosConvenio.oItem.lEdicion = this.lEdicion
	endfunc

	*--------------------------------------------------------------------------------------------------------
	function CambiosDetalleImpuestosconvenio() as void
		dodefault()
	endfunc
	...
	*--------------------------------------------------------------------------------------------------------
	function ValidacionDetalles() as Boolean
		...
		llRetorno = dodefault()
		llRetorno = This.ValidarUnDetalle( this.ImpuestosConvenio, 'Impuestos' ) and llRetorno
		return llRetorno
	endfunc
	...
	*--------------------------------------------------------------------------------------------------------
	function ValidarFecha() as boolean
		...
		llRetorno = dodefault()
		if empty( this.Fecha )
			this.AgregarInformacion( 'Debe cargar el campo Fecha de presentación', 9005, 'Fecha' )
			llRetorno = .F.
		endif
		return llRetorno
	endfunc
	...
	*--------------------------------------------------------------------------------------------------------
	function Inicializar() as Void
		...
		oICol = goServicios.SaltosDeCampoYValoresSugeridos.ObtenerValorSugeridoDeUnaEntidadDetalle( 'CONVENIOMULTILATERAL', '')
	...
	*--------------------------------------------------------------------------------------------------------
	Function ValorSugeridoObs() as void
		...
		with this
			lcExpresionValorSugeridoDefinidoPorElUsuario = goServicios.SaltosDeCampoYValoresSugeridos.ObtenerValorSugerido( "CONVENIOMULTILATERAL", "", "Obs" )
	...
	*--------------------------------------------------------------------------------------------------------
	Function ValorSugeridoFecha() as void
		...
		lcExpresionValorSugeridoDefinidoPorElUsuario = null
		with this
			lcExpresionValorSugeridoDefinidoPorElUsuario = goServicios.SaltosDeCampoYValoresSugeridos.ObtenerValorSugerido( "CONVENIOMULTILATERAL", "", "Fecha" )
			if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
				if !empty( lcExpresionValorSugeridoDefinidoPorElUsuario )
					try
						lvValorSugeridoDefinidoPorElUsuario = &lcExpresionValorSugeridoDefinidoPorElUsuario
						...
						loEx.Grabar( loError )
						loEx.AgregarInformacion( iif( type( "loError.UserValue.Message" ) == "C", loError.UserValue.Message, loError.Message ) )
						loEx.AgregarInformacion( "Se produjo un error al intentar asignar el valor sugerido " + lcExpresionValorSugeridoDefinidoPorElUsuario + " del atributo Fecha de presentación para la entidad Convenio multilateral." + chr( 10 ) + "Verifique el valor especificado en la entidad Comportamiento de atributos." + chr( 10 ) + "El tipo de dato del atributo es fecha." )
						goServicios.Errores.LevantarExcepcion( loEx )
					endtry
				endif
			else
