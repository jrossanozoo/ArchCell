...
define class Din_EntidadIMPVERCC as ent_ImputacionDirectaBase of ent_ImputacionDirectaBase.prg
	...
	*--------------------------------------------------------------------------------------------------------
	...
	local lnLargoDecimal as integer, lnLargoEntero as Integer, lnLargoDecimal as integer, llRetorno as boolean 
		...
		if ( vartype( this.f_BaseAgru_Desde) = 'C' )
			if len( alltrim( this.F_BASEAGRU_DESDE ) ) > 21
		...
		if ( vartype( this.f_BaseAgru_Hasta) = 'C' )
			if len( alltrim( this.F_BASEAGRU_HASTA ) ) > 21
	...
	*--------------------------------------------------------------------------------------------------------
	function Validarf_RazonS_Desde() as boolean
		local llRetorno as boolean
		llRetorno = dodefault()
		if empty( this.f_RazonS_Desde_PK )
		else
			If this.f_RazonS_Desde.oAD.ConsultarPorClavePrimaria()
			Else
				this.AgregarInformacion( 'El dato buscado ' + alltrim(transform( this.f_RazonS_Desde_PK ))+ ' de la entidad ' +  upper( this.f_RazonS_Desde.cDescripcion ) + ' no existe.')
				llRetorno = .F.
			endif
		endif
		Return llRetorno
	...
	*--------------------------------------------------------------------------------------------------------
	function Validarf_RazonS_Hasta() as boolean
		local llRetorno as boolean
		llRetorno = dodefault()
		if empty( this.f_RazonS_Hasta_PK )
		else
			If this.f_RazonS_Hasta.oAD.ConsultarPorClavePrimaria()
			Else
				this.AgregarInformacion( 'El dato buscado ' + alltrim(transform( this.f_RazonS_Hasta_PK ))+ ' de la entidad ' +  upper( this.f_RazonS_Hasta.cDescripcion ) + ' no existe.')
				llRetorno = .F.
			endif
		endif
		Return llRetorno
	...
	*--------------------------------------------------------------------------------------------------------
	Function ValorSugeridoFWF_baseagru_hasta() as void
		with this
			.F_baseagru_hasta = 'ZZZZZZZZZZZZZZZZZZZZZ'
		endwith
	endfunc
			...
			try
				...
				if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
					if !empty( lcExpresionValorSugeridoDefinidoPorElUsuario )
						...
						This.EventoEstaSeteandoValorSugerido( 'F_razons_desde_PK' )
						.F_razons_desde_PK = lvValorSugeridoDefinidoPorElUsuario
						.F_razons_desde.CODIGO = .F_razons_desde_PK
					endif
				endif
			Catch to loError
				.F_razons_desde_PK=[]
	...
	*--------------------------------------------------------------------------------------------------------
	Function ValorSugeridoBasededatos() as void
		...
		lcExpresionValorSugeridoDefinidoPorElUsuario = null
		with this
			lcExpresionValorSugeridoDefinidoPorElUsuario = goServicios.SaltosDeCampoYValoresSugeridos.ObtenerValorSugerido( "IMPVERCC", "", "Basededatos" )
			if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
				if !empty( lcExpresionValorSugeridoDefinidoPorElUsuario )
					try
						lvValorSugeridoDefinidoPorElUsuario = &lcExpresionValorSugeridoDefinidoPorElUsuario
			...
			try
				...
				if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
					if !empty( lcExpresionValorSugeridoDefinidoPorElUsuario )
						...
						This.EventoEstaSeteandoValorSugerido( 'F_razons_hasta_PK' )
						.F_razons_hasta_PK = lvValorSugeridoDefinidoPorElUsuario
						.F_razons_hasta.CODIGO = .F_razons_hasta_PK
					endif
				else
