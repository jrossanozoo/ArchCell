define class ColaboradorComprobanteConRecargo as ColaboradorComprobanteConValores of ColaboradorComprobanteConValores.prg

	#IF .f.
		Local this as ColaboradorComprobanteConRecargo of ColaboradorComprobanteConRecargo.prg
	#ENDIF
	...
	function oComprobante_Access() as Object
		if this.lDestroy
			...
			if ( !vartype( this.oComprobante ) = 'O' or isnull( this.oComprobante ) )
				this.oComprobante = goServicios.Formularios.Procesar( alltrim( goParametros.Felino.GestionDeVentas.Recibo.Recargo.TipoDeComprobante ) )	
			endif
		endif
	...
	*-----------------------------------------------------------------------------------------	
	function GenerarComprobantePorRecargo( tcCliente as string, tnMonto as Integer, tcComprobante as String, toComprobanteAsociado as object ) as VOID 
		local loComprobante as Object, loError as Object, lnMonto as Numeric, llParametroDesgloceEnPrecio as Boolean
		...
		with loComprobante
			...
			with .FacturaDetalle
				.Limpiar()
				.oItem.Articulo_pk = goParametros.Felino.GestionDeVentas.Recibo.Recargo.ArticuloDelComprobante
				.oItem.Cantidad = 1
			...
			with .ValoresDetalle
				.Limpiar()
				.oItem.Valor_pk = goParametros.Felino.GestionDeVentas.Recibo.Recargo.ValorDelComprobante
				.oItem.Monto = loComprobante.total
				.actualizar()
			endwith 	

		 	.zadsfw = .zadsfw + "Comprobante generado por el " + tcComprobante + " - Motivo: Recibo con recargo por financiación de tarjeta"
		 	.grabar()
	...
	*-----------------------------------------------------------------------------------------
	function AnularComprobanteRecargo() as Void
