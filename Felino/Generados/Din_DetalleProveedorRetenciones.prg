...
define class Din_DetallePROVEEDORRetenciones as detalle of detalle.prg
	...
	Protected Function _CargarItem( tnItem as integer )
		...
		With this.oItem
			Try
				...
				.SedeExtraJuris = this.Item[ tnItem ].SedeExtraJuris
				llVacio = llVacio and this.VerificarSiElAtributoEstaVacio( 'SedeExtraJuris', .SedeExtraJuris )
	...
	*--------------------------------------------------------------------------------------------------------
	protected function AplanarItem() as Object
		...
		with loRetorno
			...
			.SedeExtraJuris = this.oItem.SedeExtraJuris
	...
	*--------------------------------------------------------------------------------------------------------
	...
	with this
		if .ValidarItem()
			...
			.Item[ lnItem ].SedeExtraJuris = .oItem.SedeExtraJuris
		else
			goServicios.Errores.LevantarExcepcion( .ObtenerInformacion() )
		endif
	endwith
	...
	*--------------------------------------------------------------------------------------------------------
	function oItem_Access() as variant
		if !this.ldestroy and ( !vartype( this.oItem ) = 'O' or isnull( this.oItem ) )
			this.oItem = _Screen.zoo.crearobjeto( 'ItemPROVEEDORRetenciones' )
	...
	*--------------------------------------------------------------------------------------------------------
	Function Cargar() as Void
		...
		select c_Retenciones
		lcFiltro = this.ObtenerFiltro()
		scan for &lcFiltro
			loItem = createobject('ItemAuxiliar')

			loItem.codigo = c_Retenciones.codigo
			loItem.NROITEM = c_Retenciones.NROITEM
			loItem.Jurisdiccion_PK = c_Retenciones.Jurisdiccion
			loItem.JurisdiccionDetalle = c_Retenciones.JurisdiccionDetalle
			loItem.Resolucion = c_Retenciones.Resolucion
			loItem.Porcentaje = c_Retenciones.Porcentaje
			loItem.FechaExpiracion = goLibrerias.ObtenerFechaFormateada( c_Retenciones.FechaExpiracion )
			loItem.Excluido = c_Retenciones.Excluido
			loItem.SedeExtraJuris = c_Retenciones.SedeExtraJuris
			loItem.NroItem = lnI 
	...
	*-----------------------------------------------------------------------------------------
	...
	local lnLargoEntero as integer, lnValorDelDecimal as integer, lnLargoDecimal as integer, llRetorno as boolean, lnI as Integer
		...
		for lni = 1 to this.Count
			...
			if vartype( this.Item[lni].SEDEEXTRAJURIS) = 'L'
			else
				llRetorno = .F.
				this.AgregarInformacion( 'El tipo de dato para el atributo SEDEEXTRAJURIS del item ' + transform( lni ) + ' no es el correcto, en el detalle ' + this.cEtiqueta + '.')
			Endif
		EndFor
		Return llRetorno 
	Endfunc
	...
	*--------------------------------------------------------------------------------------------------------
	function CopiarItemaItem( toItemOrigen as Object, toItemDestino as Object ) as Object
		...
		toItemDestino.Sedeextrajuris = toItemOrigen.Sedeextrajuris
	endfunc
