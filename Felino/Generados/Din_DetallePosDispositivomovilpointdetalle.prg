		...
		for lni = 1 to this.Count
			...
			if vartype( this.Item[lni].DISPOSITIVOMOVIL) = 'C'
				if len( alltrim( this.Item[lni].DISPOSITIVOMOVIL )) <= 40
					...
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud del valor del atributo DISPOSITIVOMOVIL del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform( len( alltrim( this.Item[lni].DISPOSITIVOMOVIL ))) + ' Máxima:40), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
