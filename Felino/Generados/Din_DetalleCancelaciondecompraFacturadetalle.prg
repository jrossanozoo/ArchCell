...
define class Din_DetalleCANCELACIONDECOMPRAFacturadetalle as DetalleArticuloscompra of DetalleArticuloscompra.Prg
	...
	Protected Function _CargarItem( tnItem as integer )
		...
		With this.oItem
			Try
				...
				.Afe_NumInt = this.Item[ tnItem ].Afe_NumInt
				llVacio = llVacio and this.VerificarSiElAtributoEstaVacio( 'Afe_NumInt', .Afe_NumInt )
	...
	*--------------------------------------------------------------------------------------------------------
	protected function AplanarItem() as Object
		...
		with loRetorno
			...
			.Afe_NumInt = this.oItem.Afe_NumInt
	...
	*--------------------------------------------------------------------------------------------------------
	...
	with this
		if .ValidarItem()
			...
			.Item[ lnItem ].Afe_NumInt = .oItem.Afe_NumInt
	...
	*--------------------------------------------------------------------------------------------------------
	function oItem_Access() as variant
		if !this.ldestroy and ( !vartype( this.oItem ) = 'O' or isnull( this.oItem ) )
			this.oItem = _Screen.zoo.crearobjeto( 'Din_ItemCANCELACIONDECOMPRAFacturadetalle' )
	...
	*--------------------------------------------------------------------------------------------------------
	Function Cargar() as Void
		...
		scan for &lcFiltro
			...
			loItem.Afe_NumInt = c_Facturadetalle.Afe_NumInt
		...
		for lni = 1 to this.Count
			...
			if vartype( this.Item[lni].MONTOPORCENTAJEDESCUENTOCONIMPUESTO) = 'N'
				...
				if lnLargoEntero <= 6
					...
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud entera del valor del atributo MONTOPORCENTAJEDESCUENTOCONIMPUESTO del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform(lnLargoEntero) + ' Máxima:6), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
			...
			if vartype( this.Item[lni].MONTOIVA) = 'N'
				...
				if lnLargoEntero <= 13
					...
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud entera del valor del atributo MONTOIVA del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform(lnLargoEntero) + ' Máxima:13), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
			...
			if vartype( this.Item[lni].MONTOPORCENTAJEDESCUENTOSINIMPUESTO) = 'N'
				...
				if lnLargoEntero <= 13
					...
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud entera del valor del atributo MONTOPORCENTAJEDESCUENTOSINIMPUESTO del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform(lnLargoEntero) + ' Máxima:13), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
			...
			if vartype( this.Item[lni].MONTODESCUENTOCONIMPUESTOS) = 'N'
				...
				if lnLargoEntero <= 13
					...
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud entera del valor del atributo MONTODESCUENTOCONIMPUESTOS del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform(lnLargoEntero) + ' Máxima:13), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
			...
			if vartype( this.Item[lni].MONTODESCUENTOSINIMPUESTOS) = 'N'
				...
				if lnLargoEntero <= 13
					...
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud entera del valor del atributo MONTODESCUENTOSINIMPUESTOS del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform(lnLargoEntero) + ' Máxima:13), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
			...
			if vartype( this.Item[lni].AFE_NUMINT) = 'N'
				lnLargoEntero = len( transform( int( this.Item[lni].AFE_NUMINT ) ) ) 
				lnValorDelDecimal = this.Item[lni].AFE_NUMINT - int( this.Item[lni].AFE_NUMINT )
				lnLargoDecimal = iif( lnValorDelDecimal = 0, 0, len( transform( lnValorDelDecimal ) ) - 2 )
				if lnLargoEntero <= 10
				else
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud entera del valor del atributo AFE_NUMINT del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform(lnLargoEntero) + ' Máxima:10), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
				this.AgregarInformacion( 'El tipo de dato para el atributo AFE_NUMINT del item ' + transform( lni ) + ' no es el correcto, en el detalle ' + this.cEtiqueta + '.')
			Endif
	...
	*--------------------------------------------------------------------------------------------------------
	function CopiarItemaItem( toItemOrigen as Object, toItemDestino as Object ) as Object
		...
		toItemDestino.Afe_numint = toItemOrigen.Afe_numint
