		...
		for lni = 1 to this.Count
			...
			if vartype( this.Item[lni].IMPORTENETO) = 'N'
				...
				if lnLargoEntero <= 13
					...
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud entera del valor del atributo IMPORTENETO del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform(lnLargoEntero) + ' Máxima:13), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
			...
			if vartype( this.Item[lni].IMPORTETOTAL) = 'N'
				...
				if lnLargoEntero <= 14
					...
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud entera del valor del atributo IMPORTETOTAL del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform(lnLargoEntero) + ' Máxima:14), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
			...
			if vartype( this.Item[lni].PROCESADO) = 'C'
				if len( alltrim( this.Item[lni].PROCESADO )) <= 15
					...
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud del valor del atributo PROCESADO del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform( len( alltrim( this.Item[lni].PROCESADO ))) + ' Máxima:15), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
