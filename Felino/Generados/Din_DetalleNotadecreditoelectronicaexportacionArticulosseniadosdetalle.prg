...
define class Din_DetalleNOTADECREDITOELECTRONICAEXPORTACIONArticulosseniadosdetalle as DetalleArticulosseniados of DetalleArticulosseniados.Prg
	...
	*--------------------------------------------------------------------------------------------------------
	function oItem_Access() as variant
		if !this.ldestroy and ( !vartype( this.oItem ) = 'O' or isnull( this.oItem ) )
			this.oItem = _Screen.zoo.crearobjeto( 'Din_ItemNOTADECREDITOELECTRONICAEXPORTACIONArticulosseniadosdetalle' )
	...
	*--------------------------------------------------------------------------------------------------------
	Function Cargar() as Void
		...
		scan for &lcFiltro
			...
			loItem.Material_PK = c_Articulosseniadosdetalle.Material
			...
			loItem.Talle = c_Articulosseniadosdetalle.Talle
		...
		for lni = 1 to this.Count
			...
			if vartype( this.Item[lni].PRECIOUNITARIO) = 'N'
				...
				if lnLargoEntero <= 6
					...
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud entera del valor del atributo PRECIOUNITARIO del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform(lnLargoEntero) + ' Máxima:6), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
			...
			if vartype( this.Item[lni].CODIGOAUTORIZACIONDJCP) = 'C'
				if len( alltrim( this.Item[lni].CODIGOAUTORIZACIONDJCP )) <= 10
				else
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud del valor del atributo CODIGOAUTORIZACIONDJCP del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform( len( alltrim( this.Item[lni].CODIGOAUTORIZACIONDJCP ))) + ' Máxima:10), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
			...
			if vartype( this.Item[lni].PRECIO) = 'N'
				...
				if lnLargoEntero <= 6
					...
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud entera del valor del atributo PRECIO del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform(lnLargoEntero) + ' Máxima:6), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
