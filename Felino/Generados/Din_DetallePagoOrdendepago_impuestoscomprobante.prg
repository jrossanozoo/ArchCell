...
define class Din_DetallePAGOOrdendepago_impuestoscomprobante as detalle of detalle.prg
	...
	function Sumarizar() as boolean
		...
		with this
			...
			if this.lEsNavegacion
				if select( 'c_Ordendepago_impuestoscomprobante' ) > 0 and reccount( 'c_Ordendepago_impuestoscomprobante' ) > 0
					sum Monto;
						 to ;
						 lnSum_Monto
				endif
	...
	*--------------------------------------------------------------------------------------------------------
	function oItem_Access() as variant
		if !this.ldestroy and ( !vartype( this.oItem ) = 'O' or isnull( this.oItem ) )
			this.oItem = _Screen.zoo.crearobjeto( 'Din_ItemPAGOOrdendepago_impuestoscomprobante' )
	...
	*--------------------------------------------------------------------------------------------------------
	Function Cargar() as Void
		...
		select c_Ordendepago_impuestoscomprobante
		lcFiltro = this.ObtenerFiltro()
		scan for &lcFiltro
			loItem = createobject('ItemAuxiliar')

			loItem.Codigo = c_Ordendepago_impuestoscomprobante.Codigo
			loItem.NROITEM = c_Ordendepago_impuestoscomprobante.NROITEM
			loItem.CodImp_PK = c_Ordendepago_impuestoscomprobante.CodImp
			loItem.CodImpDetalle = c_Ordendepago_impuestoscomprobante.CodImpDetalle
			loItem.Monto = c_Ordendepago_impuestoscomprobante.Monto
			loItem.NroItem = lnI 
