...
define class Din_EntidadPADRONIIBB as entidad of entidad.prg
	...
	*--------------------------------------------------------------------------------------------------------
	...
	local lnLargoDecimal as integer, lnLargoEntero as Integer, lnLargoDecimal as integer, llRetorno as boolean 
		...
		if ( vartype( this.FECHAEXPIRACION) = 'D' )
		else
			This.AgregarInformacion( 'El tipo de dato para el atributo FECHAEXPIRACION no es el correcto.')
			llRetorno = .F.
		Endif
		 
		if ( vartype( this.CODIGOCLIENTE) = 'C' )
			if len( alltrim( this.CODIGOCLIENTE ) ) > 5
				This.AgregarInformacion( 'La longitud del valor del atributo CODIGOCLIENTE es mayor a la permitida.' )
				llRetorno = .F.
			Endif
		else
			This.AgregarInformacion( 'El tipo de dato para el atributo CODIGOCLIENTE no es el correcto.')
			llRetorno = .F.
		Endif
		...
		if ( vartype( this.NROITEM) = 'N' )
			lnLargoEntero  = len( transform( int( this.NROITEM ) ) ) 
			lnValorDelDecimal = this.NROITEM - int( this.NROITEM)
			lnLargoDecimal = iif( lnValorDelDecimal = 0, 0, len( transform( lnValorDelDecimal ) ) - 2 )
			if lnLargoEntero > 5
				This.AgregarInformacion( 'La longitud entera del valor del atributo NROITEM es mayor a la permitida.' )
				llRetorno = .F.
			Endif
		else
			This.AgregarInformacion( 'El tipo de dato para el atributo NROITEM no es el correcto.')
			llRetorno = .F.
		Endif
	...
	*--------------------------------------------------------------------------------------------------------
	Protected Function _Buscar() As void
		...
		if empty( This.&lcAtributo )
			...
			lcMensaje = 'El dato buscado . ' + transform( This.CODIGOCLIENTE ) + ', JURISDICCION ' + transform( This.JURISDICCION_Pk ) + ' de la entidad ' + upper( this.cDescripcion ) + ' no existe.'
		else
	...
	*--------------------------------------------------------------------------------------------------------
	function Inicializar() as Void
		...
		this.oAtributosCC.add( "CODIGOCLIENTE" )
		this.oAtributosCC.add( "JURISDICCION_pk" )
		this.oColeccionVS = _screen.zoo.CrearObjeto( 'zoocoleccion' )
		this.oColeccionVSFW = _screen.zoo.CrearObjeto( 'zoocoleccion' )
		this.oColeccionVSFW.agregar('.ValorSugeridoFWNroitem()','Nroitem')
		oICol = goServicios.SaltosDeCampoYValoresSugeridos.ObtenerValorSugeridoDeUnaEntidadDetalle( 'PADRONIIBB', '')
