...
define class Din_EntidadIMPDIRIMP as ent_ImputacionDirectaBase of ent_ImputacionDirectaBase.prg
	...
	*--------------------------------------------------------------------------------------------------------
	...
	local lnLargoDecimal as integer, lnLargoEntero as Integer, lnLargoDecimal as integer, llRetorno as boolean 
		...
		if ( vartype( this.f_Impuesto_Desde) = 'C' )
			if len( alltrim( this.F_IMPUESTO_DESDE ) ) > 10
				This.AgregarInformacion( 'La longitud del valor del atributo F_IMPUESTO_DESDE es mayor a la permitida.' )
				llRetorno = .F.
			Endif
		else
			This.AgregarInformacion( 'El tipo de dato para el atributo F_IMPUESTO_DESDE no es el correcto.')
			llRetorno = .F.
		Endif
		 
		if ( vartype( this.f_Impuesto_Hasta) = 'C' )
			if len( alltrim( this.F_IMPUESTO_HASTA ) ) > 10
				This.AgregarInformacion( 'La longitud del valor del atributo F_IMPUESTO_HASTA es mayor a la permitida.' )
				llRetorno = .F.
			Endif
		else
			This.AgregarInformacion( 'El tipo de dato para el atributo F_IMPUESTO_HASTA no es el correcto.')
			llRetorno = .F.
		Endif
		 
		if ( vartype( this.f_TipoImp_Desde) = 'C' )
			if len( alltrim( this.F_TIPOIMP_DESDE ) ) > 10
				This.AgregarInformacion( 'La longitud del valor del atributo F_TIPOIMP_DESDE es mayor a la permitida.' )
				llRetorno = .F.
			Endif
		else
			This.AgregarInformacion( 'El tipo de dato para el atributo F_TIPOIMP_DESDE no es el correcto.')
			llRetorno = .F.
		Endif
		...
		if ( vartype( this.f_TipoImp_Hasta) = 'C' )
			if len( alltrim( this.F_TIPOIMP_HASTA ) ) > 10
				This.AgregarInformacion( 'La longitud del valor del atributo F_TIPOIMP_HASTA es mayor a la permitida.' )
				llRetorno = .F.
			Endif
		else
			This.AgregarInformacion( 'El tipo de dato para el atributo F_TIPOIMP_HASTA no es el correcto.')
			llRetorno = .F.
		Endif
		 
		if ( vartype( this.f_Jurisdiccion_Desde) = 'C' )
			if len( alltrim( this.F_JURISDICCION_DESDE ) ) > 8
				This.AgregarInformacion( 'La longitud del valor del atributo F_JURISDICCION_DESDE es mayor a la permitida.' )
				llRetorno = .F.
			Endif
		else
			This.AgregarInformacion( 'El tipo de dato para el atributo F_JURISDICCION_DESDE no es el correcto.')
			llRetorno = .F.
		Endif
		 
		if ( vartype( this.f_Jurisdiccion_Hasta) = 'C' )
			if len( alltrim( this.F_JURISDICCION_HASTA ) ) > 8
				This.AgregarInformacion( 'La longitud del valor del atributo F_JURISDICCION_HASTA es mayor a la permitida.' )
				llRetorno = .F.
			Endif
		else
			This.AgregarInformacion( 'El tipo de dato para el atributo F_JURISDICCION_HASTA no es el correcto.')
			llRetorno = .F.
		Endif
			...
			if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
				if !empty( lcExpresionValorSugeridoDefinidoPorElUsuario )
					try
						...
						This.EventoEstaSeteandoValorSugerido( 'F_impuesto_desde' )
						.F_impuesto_desde = lvValorSugeridoDefinidoPorElUsuario
					catch to loError
						loEx = newobject("zooexception", "zooexception.prg" )
						loEx.Grabar( loError )
						loEx.AgregarInformacion( iif( type( "loError.UserValue.Message" ) == "C", loError.UserValue.Message, loError.Message ) )
			...
			if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
				if !empty( lcExpresionValorSugeridoDefinidoPorElUsuario )
					try
						...
						This.EventoEstaSeteandoValorSugerido( 'F_impuesto_hasta' )
						.F_impuesto_hasta = lvValorSugeridoDefinidoPorElUsuario
					catch to loError
						loEx = newobject("zooexception", "zooexception.prg" )
						loEx.Grabar( loError )
						loEx.AgregarInformacion( iif( type( "loError.UserValue.Message" ) == "C", loError.UserValue.Message, loError.Message ) )
			...
			if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
				if !empty( lcExpresionValorSugeridoDefinidoPorElUsuario )
					try
						...
						This.EventoEstaSeteandoValorSugerido( 'F_tipoimp_desde' )
						.F_tipoimp_desde = lvValorSugeridoDefinidoPorElUsuario
					catch to loError
						loEx = newobject("zooexception", "zooexception.prg" )
						loEx.Grabar( loError )
						loEx.AgregarInformacion( iif( type( "loError.UserValue.Message" ) == "C", loError.UserValue.Message, loError.Message ) )
			...
			if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
				if !empty( lcExpresionValorSugeridoDefinidoPorElUsuario )
					try
						...
						This.EventoEstaSeteandoValorSugerido( 'F_tipoimp_hasta' )
						.F_tipoimp_hasta = lvValorSugeridoDefinidoPorElUsuario
					catch to loError
						loEx = newobject("zooexception", "zooexception.prg" )
						loEx.Grabar( loError )
						loEx.AgregarInformacion( iif( type( "loError.UserValue.Message" ) == "C", loError.UserValue.Message, loError.Message ) )
			...
			if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
				if !empty( lcExpresionValorSugeridoDefinidoPorElUsuario )
					try
						...
						This.EventoEstaSeteandoValorSugerido( 'F_jurisdiccion_desde' )
						.F_jurisdiccion_desde = lvValorSugeridoDefinidoPorElUsuario
					catch to loError
						loEx = newobject("zooexception", "zooexception.prg" )
						loEx.Grabar( loError )
						loEx.AgregarInformacion( iif( type( "loError.UserValue.Message" ) == "C", loError.UserValue.Message, loError.Message ) )
			...
			if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
				if !empty( lcExpresionValorSugeridoDefinidoPorElUsuario )
					try
						...
						This.EventoEstaSeteandoValorSugerido( 'F_jurisdiccion_hasta' )
						.F_jurisdiccion_hasta = lvValorSugeridoDefinidoPorElUsuario
					catch to loError
						loEx = newobject("zooexception", "zooexception.prg" )
						loEx.Grabar( loError )
						loEx.AgregarInformacion( iif( type( "loError.UserValue.Message" ) == "C", loError.UserValue.Message, loError.Message ) )
