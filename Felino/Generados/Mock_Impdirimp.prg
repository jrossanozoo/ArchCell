...
define class Mock_impdirimp as MockBase of MockBase.prg
	...
	f_Impuesto_Desde = ""                                                                                                                                                                                                                                     
	f_Impuesto_Hasta = ""                                                                                                                                                                                                                                     
	f_Jurisdiccion_Desde = ""                                                                                                                                                                                                                                 
	f_Jurisdiccion_Hasta = ""                                                                                                                                                                                                                                 
	f_TipoImp_Desde = ""                                                                                                                                                                                                                                      
	f_TipoImp_Hasta = ""                                                                                                                                                                                                                                      
	...
	*-----------------------------------------------------------------------------------------
	function f_impuesto_desde_assign( txval as variant ) as void                                                                                                                                                                                              
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'IMPDIRIMP' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'f_impuesto_desde_assign', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'f_impuesto_desde_assign', lcParametros )			endif		endif		Local lxValOld As Variant		lxValOld = this.F_impuesto_desde		local lxVal as Variant		lxVal = txVal		if This.CargaManual()			if this.Validar_F_impuesto_desde( lxVal )				this.Setear_F_impuesto_desde( lxVal )			EndIf		Else			This.Setear_F_impuesto_desde( lxVal )		EndIf		dodefault( lxVal )
	EndFunc

	*-----------------------------------------------------------------------------------------
	function f_impuesto_hasta_assign( txval as variant ) as void                                                                                                                                                                                              
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'IMPDIRIMP' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'f_impuesto_hasta_assign', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'f_impuesto_hasta_assign', lcParametros )			endif		endif		Local lxValOld As Variant		lxValOld = this.F_impuesto_hasta		local lxVal as Variant		lxVal = txVal		if This.CargaManual()			if this.Validar_F_impuesto_hasta( lxVal )				this.Setear_F_impuesto_hasta( lxVal )			EndIf		Else			This.Setear_F_impuesto_hasta( lxVal )		EndIf		dodefault( lxVal )
	EndFunc

	*-----------------------------------------------------------------------------------------
	function f_jurisdiccion_desde_assign( txval as variant ) as void                                                                                                                                                                                          
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'IMPDIRIMP' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'f_jurisdiccion_desde_assign', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'f_jurisdiccion_desde_assign', lcParametros )			endif		endif		Local lxValOld As Variant		lxValOld = this.F_jurisdiccion_desde		local lxVal as Variant		lxVal = txVal		if This.CargaManual()			if this.Validar_F_jurisdiccion_desde( lxVal )				this.Setear_F_jurisdiccion_desde( lxVal )			EndIf		Else			This.Setear_F_jurisdiccion_desde( lxVal )		EndIf		dodefault( lxVal )
	EndFunc

	*-----------------------------------------------------------------------------------------
	function f_jurisdiccion_hasta_assign( txval as variant ) as void                                                                                                                                                                                          
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'IMPDIRIMP' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'f_jurisdiccion_hasta_assign', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'f_jurisdiccion_hasta_assign', lcParametros )			endif		endif		Local lxValOld As Variant		lxValOld = this.F_jurisdiccion_hasta		local lxVal as Variant		lxVal = txVal		if This.CargaManual()			if this.Validar_F_jurisdiccion_hasta( lxVal )				this.Setear_F_jurisdiccion_hasta( lxVal )			EndIf		Else			This.Setear_F_jurisdiccion_hasta( lxVal )		EndIf		dodefault( lxVal )
	EndFunc

	*-----------------------------------------------------------------------------------------
	function f_tipoimp_desde_assign( txval as variant ) as void                                                                                                                                                                                               
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'IMPDIRIMP' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'f_tipoimp_desde_assign', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'f_tipoimp_desde_assign', lcParametros )			endif		endif		Local lxValOld As Variant		lxValOld = this.F_tipoimp_desde		local lxVal as Variant		lxVal = txVal		if This.CargaManual()			if this.Validar_F_tipoimp_desde( lxVal )				this.Setear_F_tipoimp_desde( lxVal )			EndIf		Else			This.Setear_F_tipoimp_desde( lxVal )		EndIf		dodefault( lxVal )
	EndFunc

	*-----------------------------------------------------------------------------------------
	function f_tipoimp_hasta_assign( txval as variant ) as void                                                                                                                                                                                               
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'IMPDIRIMP' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'f_tipoimp_hasta_assign', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'f_tipoimp_hasta_assign', lcParametros )			endif		endif		Local lxValOld As Variant		lxValOld = this.F_tipoimp_hasta		local lxVal as Variant		lxVal = txVal		if This.CargaManual()			if this.Validar_F_tipoimp_hasta( lxVal )				this.Setear_F_tipoimp_hasta( lxVal )			EndIf		Else			This.Setear_F_tipoimp_hasta( lxVal )		EndIf		dodefault( lxVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function validar_f_impuesto_desde( txval as variant ) as boolean                                                                                                                                                                                          
	...
	*-----------------------------------------------------------------------------------------
	function validar_f_impuesto_hasta( txval as variant ) as boolean                                                                                                                                                                                          
	...
	*-----------------------------------------------------------------------------------------
	function validar_f_jurisdiccion_desde( txval as variant ) as boolean                                                                                                                                                                                      
	...
	*-----------------------------------------------------------------------------------------
	function validar_f_jurisdiccion_hasta( txval as variant ) as boolean                                                                                                                                                                                      
	...
	*-----------------------------------------------------------------------------------------
	function validar_f_tipoimp_desde( txval as variant ) as boolean                                                                                                                                                                                           
	...
	*-----------------------------------------------------------------------------------------
	function validar_f_tipoimp_hasta( txval as variant ) as boolean                                                                                                                                                                                           
