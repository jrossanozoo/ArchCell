...
define class Mock_deudadecheques as MockBase of MockBase.prg
	...
	*-----------------------------------------------------------------------------------------
	function moneda_assign( txval as variant ) as void                                                                                                                                                                                                        
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'DEUDADECHEQUES' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'moneda_assign', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'moneda_assign', lcParametros )			endif		endif		Local lxValOld As Variant		lxValOld = this.Moneda		local lxVal as Variant		lxVal = txVal		if This.CargaManual()			if this.Validar_Moneda( lxVal )				this.Setear_Moneda( lxVal )			EndIf		Else			This.Setear_Moneda( lxVal )		EndIf		dodefault( lxVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function validaridglobal() as boolean                                                                                                                                                                                                                     
		...
		return this.ObtenerResultado( 'Validaridglobal', lcParametros )
	EndFunc
