...
define class Din_DetalleREDONDEODEPRECIOSDetredondeoporentero as detalle of detalle.prg
	...
	*--------------------------------------------------------------------------------------------------------
	function oItem_Access() as variant
		if !this.ldestroy and ( !vartype( this.oItem ) = 'O' or isnull( this.oItem ) )
			this.oItem = _Screen.zoo.crearobjeto( 'ItemREDONDEODEPRECIOSDetredondeoporentero' )
	...
	*--------------------------------------------------------------------------------------------------------
	Function Cargar() as Void
		...
		select c_Detredondeoporentero
		lcFiltro = this.ObtenerFiltro()
		scan for &lcFiltro
			loItem = createobject('ItemAuxiliar')

			loItem.Codigo = c_Detredondeoporentero.Codigo
			loItem.NROITEM = c_Detredondeoporentero.NROITEM
			loItem.TerminacionCaracter = c_Detredondeoporentero.TerminacionCaracter
			loItem.Desde = c_Detredondeoporentero.Desde
			loItem.Hasta = c_Detredondeoporentero.Hasta
			loItem.DesdePrecio = c_Detredondeoporentero.DesdePrecio
			loItem.HastaPrecio = c_Detredondeoporentero.HastaPrecio
			loItem.Terminacion = c_Detredondeoporentero.Terminacion
			loItem.NroItem = lnI 
	...
	local lnLargoEntero as integer, lnValorDelDecimal as integer, lnLargoDecimal as integer, llRetorno as boolean, lnI as Integer
		...
		for lni = 1 to this.Count
			...
			if vartype( this.Item[lni].TERMINACIONCARACTER) = 'C'
				if len( alltrim( this.Item[lni].TERMINACIONCARACTER )) <= 16
				else
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud del valor del atributo TERMINACIONCARACTER del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform( len( alltrim( this.Item[lni].TERMINACIONCARACTER ))) + ' Máxima:16), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
			...
			if vartype( this.Item[lni].DESDE) = 'N'
				...
				if lnLargoEntero <= 16
					...
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud entera del valor del atributo DESDE del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform(lnLargoEntero) + ' Máxima:16), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
			...
			if vartype( this.Item[lni].HASTA) = 'N'
				...
				if lnLargoEntero <= 16
					...
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud entera del valor del atributo HASTA del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform(lnLargoEntero) + ' Máxima:16), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
			...
			if vartype( this.Item[lni].DESDEPRECIO) = 'N'
				...
				if lnLargoEntero <= 14
					...
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud entera del valor del atributo DESDEPRECIO del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform(lnLargoEntero) + ' Máxima:14), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
			...
			if vartype( this.Item[lni].HASTAPRECIO) = 'N'
				...
				if lnLargoEntero <= 14
					...
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud entera del valor del atributo HASTAPRECIO del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform(lnLargoEntero) + ' Máxima:14), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
			...
			if vartype( this.Item[lni].TERMINACION) = 'N'
				...
				if lnLargoEntero <= 16
					...
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud entera del valor del atributo TERMINACION del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform(lnLargoEntero) + ' Máxima:16), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
