...
define class Din_DetalleORDENDEPAGOOrdendepagodetalle as detalle of detalle.prg
	...
	nCantidadItems = 999
	cNombre = 'Ordendepagodetalle'
	cEtiqueta = 'Ordendepagodetalle'
	...
	Protected Function _CargarItem( tnItem as integer )
		...
		With this.oItem
			Try
				...
				.TotalComprobanteConImpuestos = this.Item[ tnItem ].TotalComprobanteConImpuestos
				llVacio = llVacio and this.VerificarSiElAtributoEstaVacio( 'TotalComprobanteConImpuestos', .TotalComprobanteConImpuestos )
			
				.TotalComprobanteSinImpuestos = this.Item[ tnItem ].TotalComprobanteSinImpuestos
				llVacio = llVacio and this.VerificarSiElAtributoEstaVacio( 'TotalComprobanteSinImpuestos', .TotalComprobanteSinImpuestos )
			
				.TotalComprobanteGravado = this.Item[ tnItem ].TotalComprobanteGravado
				llVacio = llVacio and this.VerificarSiElAtributoEstaVacio( 'TotalComprobanteGravado', .TotalComprobanteGravado )
			
				.TotalComprobanteIva = this.Item[ tnItem ].TotalComprobanteIva
				llVacio = llVacio and this.VerificarSiElAtributoEstaVacio( 'TotalComprobanteIva', .TotalComprobanteIva )
				...
				.Emision = this.Item[ tnItem ].Emision
				llVacio = llVacio and this.VerificarSiElAtributoEstaVacio( 'Emision', .Emision )
	...
	*--------------------------------------------------------------------------------------------------------
	protected function AplanarItem() as Object
		...
		with loRetorno
			...
			.TotalComprobanteConImpuestos = this.oItem.TotalComprobanteConImpuestos
			.TotalComprobanteSinImpuestos = this.oItem.TotalComprobanteSinImpuestos
			.TotalComprobanteGravado = this.oItem.TotalComprobanteGravado
			.TotalComprobanteIva = this.oItem.TotalComprobanteIva
			.CuentaParaImpresion = this.oItem.CuentaParaImpresion
			.NROITEM = this.oItem.NROITEM
			.Emision = this.oItem.Emision
	...
	*--------------------------------------------------------------------------------------------------------
	...
	with this
		if .ValidarItem()
			...
			.Item[ lnItem ].TotalComprobanteConImpuestos = .oItem.TotalComprobanteConImpuestos
			.Item[ lnItem ].TotalComprobanteSinImpuestos = .oItem.TotalComprobanteSinImpuestos
			.Item[ lnItem ].TotalComprobanteGravado = .oItem.TotalComprobanteGravado
			.Item[ lnItem ].TotalComprobanteIva = .oItem.TotalComprobanteIva
			.Item[ lnItem ].CuentaParaImpresion = .oItem.CuentaParaImpresion
			.Item[ lnItem ].NROITEM = .oItem.NROITEM
			.Item[ lnItem ].Emision = .oItem.Emision
	...
	function Sumarizar() as boolean
		...
		with this
			...
			if this.lEsNavegacion
				if select( 'c_Ordendepagodetalle' ) > 0 and reccount( 'c_Ordendepagodetalle' ) > 0
					sum Monto, Saldoaux;
						 to ;
	...
	*--------------------------------------------------------------------------------------------------------
	function oItem_Access() as variant
		if !this.ldestroy and ( !vartype( this.oItem ) = 'O' or isnull( this.oItem ) )
			this.oItem = _Screen.zoo.crearobjeto( 'ItemORDENDEPAGOOrdendepagodetalle' )
	...
	*--------------------------------------------------------------------------------------------------------
	Function Cargar() as Void
		...
		select c_Ordendepagodetalle
		lcFiltro = this.ObtenerFiltro()
		scan for &lcFiltro
			loItem = createobject('ItemAuxiliar')

			loItem.Fecha = goLibrerias.ObtenerFechaFormateada( c_Ordendepagodetalle.Fecha )
			loItem.Codigo = c_Ordendepagodetalle.Codigo
			loItem.Letra = c_Ordendepagodetalle.Letra
			loItem.PuntoDeVenta = c_Ordendepagodetalle.PuntoDeVenta
			loItem.NumeroDeComprobante = c_Ordendepagodetalle.NumeroDeComprobante
			loItem.TipoDeComprobante = c_Ordendepagodetalle.TipoDeComprobante
			loItem.Valor_PK = c_Ordendepagodetalle.Valor
			loItem.Tipo = c_Ordendepagodetalle.Tipo
			loItem.EsPagoACuenta = c_Ordendepagodetalle.EsPagoACuenta
			loItem.CodigoComprobante = c_Ordendepagodetalle.CodigoComprobante
			loItem.Caja_PK = c_Ordendepagodetalle.Caja
			loItem.TotalComprobanteConImpuestos = c_Ordendepagodetalle.TotalComprobanteConImpuestos
			loItem.TotalComprobanteSinImpuestos = c_Ordendepagodetalle.TotalComprobanteSinImpuestos
			loItem.TotalComprobanteGravado = c_Ordendepagodetalle.TotalComprobanteGravado
			loItem.TotalComprobanteIva = c_Ordendepagodetalle.TotalComprobanteIva
			loItem.NROITEM = c_Ordendepagodetalle.NROITEM
			loItem.Emision = goLibrerias.ObtenerFechaFormateada( c_Ordendepagodetalle.Emision )
			loItem.Vencimiento = goLibrerias.ObtenerFechaFormateada( c_Ordendepagodetalle.Vencimiento )
			loItem.Descripcion = c_Ordendepagodetalle.Descripcion
			loItem.SaldoAux = c_Ordendepagodetalle.SaldoAux
			loItem.Monto = c_Ordendepagodetalle.Monto
			loItem.Anotacion = c_Ordendepagodetalle.Anotacion
			loItem.NroItem = lnI 
	...
	*--------------------------------------------------------------------------------------------------------
	function ValidarExistenciaCamposFijosItemPlano( tnItem as integer ) as Boolean
		...
		if  empty( this.item[ tnItem ].Vencimiento ) 
			llRetorno = .F.
		endif 
		return llRetorno
	...
	*-----------------------------------------------------------------------------------------
	...
	local lnLargoEntero as integer, lnValorDelDecimal as integer, lnLargoDecimal as integer, llRetorno as boolean, lnI as Integer
		...
		for lni = 1 to this.Count
			...
			if vartype( this.Item[lni].TOTALCOMPROBANTECONIMPUESTOS) = 'N'
				lnLargoEntero = len( transform( int( this.Item[lni].TOTALCOMPROBANTECONIMPUESTOS ) ) ) 
				lnValorDelDecimal = this.Item[lni].TOTALCOMPROBANTECONIMPUESTOS - int( this.Item[lni].TOTALCOMPROBANTECONIMPUESTOS )
				lnLargoDecimal = iif( lnValorDelDecimal = 0, 0, len( transform( lnValorDelDecimal ) ) - 2 )
				if lnLargoEntero <= 13
				else
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud entera del valor del atributo TOTALCOMPROBANTECONIMPUESTOS del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform(lnLargoEntero) + ' Máxima:13), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
				this.AgregarInformacion( 'El tipo de dato para el atributo TOTALCOMPROBANTECONIMPUESTOS del item ' + transform( lni ) + ' no es el correcto, en el detalle ' + this.cEtiqueta + '.')
			Endif
			
			if vartype( this.Item[lni].TOTALCOMPROBANTESINIMPUESTOS) = 'N'
				lnLargoEntero = len( transform( int( this.Item[lni].TOTALCOMPROBANTESINIMPUESTOS ) ) ) 
				lnValorDelDecimal = this.Item[lni].TOTALCOMPROBANTESINIMPUESTOS - int( this.Item[lni].TOTALCOMPROBANTESINIMPUESTOS )
				lnLargoDecimal = iif( lnValorDelDecimal = 0, 0, len( transform( lnValorDelDecimal ) ) - 2 )
				if lnLargoEntero <= 13
				else
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud entera del valor del atributo TOTALCOMPROBANTESINIMPUESTOS del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform(lnLargoEntero) + ' Máxima:13), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
				this.AgregarInformacion( 'El tipo de dato para el atributo TOTALCOMPROBANTESINIMPUESTOS del item ' + transform( lni ) + ' no es el correcto, en el detalle ' + this.cEtiqueta + '.')
			Endif
			
			if vartype( this.Item[lni].TOTALCOMPROBANTEGRAVADO) = 'N'
				lnLargoEntero = len( transform( int( this.Item[lni].TOTALCOMPROBANTEGRAVADO ) ) ) 
				lnValorDelDecimal = this.Item[lni].TOTALCOMPROBANTEGRAVADO - int( this.Item[lni].TOTALCOMPROBANTEGRAVADO )
				lnLargoDecimal = iif( lnValorDelDecimal = 0, 0, len( transform( lnValorDelDecimal ) ) - 2 )
				if lnLargoEntero <= 13
				else
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud entera del valor del atributo TOTALCOMPROBANTEGRAVADO del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform(lnLargoEntero) + ' Máxima:13), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
				this.AgregarInformacion( 'El tipo de dato para el atributo TOTALCOMPROBANTEGRAVADO del item ' + transform( lni ) + ' no es el correcto, en el detalle ' + this.cEtiqueta + '.')
			Endif
			
			if vartype( this.Item[lni].TOTALCOMPROBANTEIVA) = 'N'
				lnLargoEntero = len( transform( int( this.Item[lni].TOTALCOMPROBANTEIVA ) ) ) 
				lnValorDelDecimal = this.Item[lni].TOTALCOMPROBANTEIVA - int( this.Item[lni].TOTALCOMPROBANTEIVA )
				lnLargoDecimal = iif( lnValorDelDecimal = 0, 0, len( transform( lnValorDelDecimal ) ) - 2 )
				if lnLargoEntero <= 13
				else
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud entera del valor del atributo TOTALCOMPROBANTEIVA del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform(lnLargoEntero) + ' Máxima:13), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
				this.AgregarInformacion( 'El tipo de dato para el atributo TOTALCOMPROBANTEIVA del item ' + transform( lni ) + ' no es el correcto, en el detalle ' + this.cEtiqueta + '.')
			Endif
			...
			if inlist(vartype( this.Item[lni].EMISION),'D','T')
			else
				llRetorno = .F.
				this.AgregarInformacion( 'El tipo de dato para el atributo EMISION del item ' + transform( lni ) + ' no es el correcto, en el detalle ' + this.cEtiqueta + '.')
			Endif
			...
			if vartype( this.Item[lni].SALDOAUX) = 'N'
				...
				if lnLargoEntero <= 14
					...
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud entera del valor del atributo SALDOAUX del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform(lnLargoEntero) + ' Máxima:14), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
			...
			if vartype( this.Item[lni].MONTO) = 'N'
				...
				if lnLargoEntero <= 14
					...
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud entera del valor del atributo MONTO del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform(lnLargoEntero) + ' Máxima:14), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
	...
	*--------------------------------------------------------------------------------------------------------
	function CopiarItemaItem( toItemOrigen as Object, toItemDestino as Object ) as Object
		...
		toItemDestino.Totalcomprobanteconimpuestos = toItemOrigen.Totalcomprobanteconimpuestos
		toItemDestino.Totalcomprobantesinimpuestos = toItemOrigen.Totalcomprobantesinimpuestos
		toItemDestino.Totalcomprobantegravado = toItemOrigen.Totalcomprobantegravado
		toItemDestino.Totalcomprobanteiva = toItemOrigen.Totalcomprobanteiva
		toItemDestino.Cuentaparaimpresion = toItemOrigen.Cuentaparaimpresion
		toItemDestino.Nroitem = toItemOrigen.Nroitem
		toItemDestino.Emision = toItemOrigen.Emision
