...
define class Din_ItemAJUSTEDEESTADOSStockafectadoauditoriacomb as ItemActivo of ItemActivo.prg
	...
	*--------------------------------------------------------------------------------------------------------
	function Inicializar() as boolean
		...
		oICol = goServicios.SaltosDeCampoYValoresSugeridos.ObtenerValorSugeridoDeUnaEntidadDetalle( 'AJUSTEDEESTADOS', 'Stockafectadoauditoriacomb')
	...
	*--------------------------------------------------------------------------------------------------------
	function Situacion_Assign( txVal as variant ) as void
		...
		Local lxValOld As Variant
		lxValOld = this.Situacion
		local lxVal as Variant
		lxVal = txVal

		if This.CargaManual()
			if this.Validar_Situacion( lxVal )
				this.EventoAntesDeSetear( This, 'Situacion', lxValOld, lxVal  )
				this.Setear_Situacion( lxVal )
				this.EventoDespuesDeSetear( This, 'Situacion', lxValOld, lxVal  )
				If lxValOld != lxVal
					If This.PuedeEjecutarHaCambiado()
						this.HaCambiado( 'Situacion', This )
					Endif
				EndIf
			EndIf
		Else
