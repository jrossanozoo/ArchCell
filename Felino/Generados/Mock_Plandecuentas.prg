...
define class Mock_plandecuentas as MockBase of MockBase.prg
	...
	cCaracteres = ""                                                                                                                                                                                                                                          
	cContexto = ""                                                                                                                                                                                                                                            
	cDescripcion =  "Plan de cuentas contables"                                                                                                                                                                                                               
	...
	*-----------------------------------------------------------------------------------------
	function indicesinflacion_assign( txval as variant ) as void                                                                                                                                                                                              
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'PLANDECUENTAS' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'indicesinflacion_assign', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'indicesinflacion_assign', lcParametros )			endif		endif		Local lxValOld As Variant		lxValOld = this.Indicesinflacion		local lxVal as Variant		lxVal = txVal		if This.CargaManual()			if this.Validar_Indicesinflacion( lxVal )				if ( this.lHabilitarIndicesinflacion or this.lEstaSeteandoValorSugerido or alltrim( lxValOld ) == alltrim( lxVal ))					this.Setear_Indicesinflacion( lxVal )				else					this.Indicesinflacion = lxValOld					goServicios.Errores.LevantarExcepcion( "No se puede cambiar este valor (Entidad: Plan de cuentas contables - Atributo: Indicesinflacion)" )				endif			EndIf		Else			This.Setear_Indicesinflacion( lxVal )		EndIf		dodefault( lxVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function validar_indicesinflacion( txval as variant ) as boolean                                                                                                                                                                                          
