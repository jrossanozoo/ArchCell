...
define class Mock_nuevoentregaonline as MockBase of MockBase.prg
	...
	cClase = 'nuevoentregaonline'

	
	AutocompletarDetalle = .F.                                                                                                                                                                                                                                
	BASEDEDATOSALTAFW = ""                                                                                                                                                                                                                                    
	...
	cCaracteres = ""                                                                                                                                                                                                                                          
	cContexto = ""                                                                                                                                                                                                                                            
	cDescripcion =  "Nuevo Remito de venta en base a Pendientes de entrega"                                                                                                                                                                                   
	cDescripcionSingular = ""                                                                                                                                                                                                                                 
	cEvento = ""                                                                                                                                                                                                                                              
	cFormatoFechaNet = ""                                                                                                                                                                                                                                     
	cIdentificadorConexion = ""                                                                                                                                                                                                                               
	cNombre = ""                                                                                                                                                                                                                                              
	cNombre = 'NUEVOENTREGAONLINE'                                                                                                                                                                                                                            
	cPrefijoImportar = ""                                                                                                                                                                                                                                     
	cPrefijoRecibir = ""                                                                                                                                                                                                                                      
	...
	*-----------------------------------------------------------------------------------------
	function autocompletardetalle_assign( txval as variant ) as void                                                                                                                                                                                          
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'NUEVOENTREGAONLINE' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'autocompletardetalle_assign', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'autocompletardetalle_assign', lcParametros )			endif		endif		Local lxValOld As Variant		lxValOld = this.Autocompletardetalle		local lxVal as Variant		lxVal = txVal		if This.CargaManual()			if this.Validar_Autocompletardetalle( lxVal )				this.Setear_Autocompletardetalle( lxVal )			EndIf		Else			This.Setear_Autocompletardetalle( lxVal )		EndIf		dodefault( lxVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function inyectarorigen( tokontroler as object ) as void                                                                                                                                                                                                  
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		local lxParam1

		lnParametros = pcount()
		lxParam1 = tokontroler
		...
		return this.ObtenerResultado( 'Inyectarorigen', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function setear_autocompletardetalle( txval as variant ) as void                                                                                                                                                                                          
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'NUEVOENTREGAONLINE' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'setear_autocompletardetalle', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'setear_autocompletardetalle', lcParametros )			endif		endif		this.Autocompletardetalle = txVal		dodefault( txVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function validar_autocompletardetalle( txval as variant ) as boolean                                                                                                                                                                                      
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'NUEVOENTREGAONLINE' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'validar_autocompletardetalle', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'validar_autocompletardetalle', lcParametros )			endif		endif		Return dodefault( txVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function valorsugeridoautocompletardetalle() as void                                                                                                                                                                                                      
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Valorsugeridoautocompletardetalle', lcParametros )
	EndFunc
