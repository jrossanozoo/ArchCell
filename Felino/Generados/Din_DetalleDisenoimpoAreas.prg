...
define class Din_DetalleDISENOIMPOAreas as detalle of detalle.prg
	...
	Protected Function _CargarItem( tnItem as integer )
		...
		With this.oItem
			Try
				...
				.deteccionDetalleValor = this.Item[ tnItem ].deteccionDetalleValor
				llVacio = llVacio and this.VerificarSiElAtributoEstaVacio( 'deteccionDetalleValor', .deteccionDetalleValor )
			
				.CargaParcial = this.Item[ tnItem ].CargaParcial
				llVacio = llVacio and this.VerificarSiElAtributoEstaVacio( 'CargaParcial', .CargaParcial )
	...
	*--------------------------------------------------------------------------------------------------------
	protected function AplanarItem() as Object
		...
		with loRetorno
			...
			.deteccionDetalleValor = this.oItem.deteccionDetalleValor
			.CargaParcial = this.oItem.CargaParcial
	...
	*--------------------------------------------------------------------------------------------------------
	...
	with this
		if .ValidarItem()
			...
			.Item[ lnItem ].deteccionDetalleValor = .oItem.deteccionDetalleValor
			.Item[ lnItem ].CargaParcial = .oItem.CargaParcial
		else
			goServicios.Errores.LevantarExcepcion( .ObtenerInformacion() )
		endif
	endwith
	...
	*--------------------------------------------------------------------------------------------------------
	function oItem_Access() as variant
		if !this.ldestroy and ( !vartype( this.oItem ) = 'O' or isnull( this.oItem ) )
			this.oItem = _Screen.zoo.crearobjeto( 'Din_ItemDISENOIMPOAreas' )
	...
	*--------------------------------------------------------------------------------------------------------
	Function Cargar() as Void
		...
		scan for &lcFiltro
			...
			loItem.deteccionDetalleValor = c_Areas.deteccionDetalleValor
			loItem.CargaParcial = c_Areas.CargaParcial
			loItem.NroItem = lnI 
	...
	*-----------------------------------------------------------------------------------------
	...
	local lnLargoEntero as integer, lnValorDelDecimal as integer, lnLargoDecimal as integer, llRetorno as boolean, lnI as Integer
		...
		for lni = 1 to this.Count
			...
			if vartype( this.Item[lni].DETECCIONDETALLEVALOR) = 'C'
				if len( alltrim( this.Item[lni].DETECCIONDETALLEVALOR )) <= 20
				else
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud del valor del atributo DETECCIONDETALLEVALOR del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform( len( alltrim( this.Item[lni].DETECCIONDETALLEVALOR ))) + ' Máxima:20), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
				this.AgregarInformacion( 'El tipo de dato para el atributo DETECCIONDETALLEVALOR del item ' + transform( lni ) + ' no es el correcto, en el detalle ' + this.cEtiqueta + '.')
			Endif
			
			if vartype( this.Item[lni].CARGAPARCIAL) = 'L'
			else
				llRetorno = .F.
				this.AgregarInformacion( 'El tipo de dato para el atributo CARGAPARCIAL del item ' + transform( lni ) + ' no es el correcto, en el detalle ' + this.cEtiqueta + '.')
			Endif
		EndFor
		Return llRetorno 
	Endfunc
	...
	*--------------------------------------------------------------------------------------------------------
	function CopiarItemaItem( toItemOrigen as Object, toItemDestino as Object ) as Object
		...
		toItemDestino.Detecciondetallevalor = toItemOrigen.Detecciondetallevalor
		toItemDestino.Cargaparcial = toItemOrigen.Cargaparcial
	endfunc
