...
define class Din_DetallePAGOOrdendepago_pagodetalle as detalle of detalle.prg
	...
	*--------------------------------------------------------------------------------------------------------
	function Sumarizar() as boolean
		local lnItem as integer, loItem as Object, lnSum_Monto as Number, lnSum_Recargosinpercepciones as Number, lnSum_Saldoaux as Number
		...
		with this
			...
			if this.lEsNavegacion
				if select( 'c_Ordendepago_pagodetalle' ) > 0 and reccount( 'c_Ordendepago_pagodetalle' ) > 0
					sum Monto, Saldoaux;
						 to ;
	...
	*--------------------------------------------------------------------------------------------------------
	function oItem_Access() as variant
		if !this.ldestroy and ( !vartype( this.oItem ) = 'O' or isnull( this.oItem ) )
			this.oItem = _Screen.zoo.crearobjeto( 'ItemPAGOOrdendepago_pagodetalle' )
	...
	*--------------------------------------------------------------------------------------------------------
	Function Cargar() as Void
		...
		select c_Ordendepago_pagodetalle
		lcFiltro = this.ObtenerFiltro()
		scan for &lcFiltro
			loItem = createobject('ItemAuxiliar')

			loItem.Fecha = goLibrerias.ObtenerFechaFormateada( c_Ordendepago_pagodetalle.Fecha )
			loItem.Codigo = c_Ordendepago_pagodetalle.Codigo
			loItem.Letra = c_Ordendepago_pagodetalle.Letra
			loItem.PuntoDeVenta = c_Ordendepago_pagodetalle.PuntoDeVenta
			loItem.NumeroDeComprobante = c_Ordendepago_pagodetalle.NumeroDeComprobante
			loItem.TipoDeComprobante = c_Ordendepago_pagodetalle.TipoDeComprobante
			loItem.Valor_PK = c_Ordendepago_pagodetalle.Valor
			loItem.Tipo = c_Ordendepago_pagodetalle.Tipo
			loItem.EsPagoACuenta = c_Ordendepago_pagodetalle.EsPagoACuenta
			loItem.CodigoComprobante = c_Ordendepago_pagodetalle.CodigoComprobante
			loItem.Caja_PK = c_Ordendepago_pagodetalle.Caja
			loItem.NROITEM = c_Ordendepago_pagodetalle.NROITEM
			loItem.Emision = goLibrerias.ObtenerFechaFormateada( c_Ordendepago_pagodetalle.Emision )
			loItem.Vencimiento = goLibrerias.ObtenerFechaFormateada( c_Ordendepago_pagodetalle.Vencimiento )
			loItem.Descripcion = c_Ordendepago_pagodetalle.Descripcion
			loItem.SaldoAux = c_Ordendepago_pagodetalle.SaldoAux
			loItem.Monto = c_Ordendepago_pagodetalle.Monto
			loItem.Anotacion = c_Ordendepago_pagodetalle.Anotacion
			loItem.NroItem = lnI 
