...
define class Din_DetalleNOTADECREDITOELECTRONICAEXPORTACIONImpuestoscomprobante as detalle of detalle.prg
	...
	*--------------------------------------------------------------------------------------------------------
	function oItem_Access() as variant
		if !this.ldestroy and ( !vartype( this.oItem ) = 'O' or isnull( this.oItem ) )
			this.oItem = _Screen.zoo.crearobjeto( 'Din_ItemNOTADECREDITOELECTRONICAEXPORTACIONImpuestoscomprobante' )
		...
		for lni = 1 to this.Count
			...
			if vartype( this.Item[lni].PORCENTAJE) = 'N'
				...
				if lnLargoEntero <= 6
					...
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud entera del valor del atributo PORCENTAJE del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform(lnLargoEntero) + ' Máxima:6), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
			...
			if vartype( this.Item[lni].MINIMONOIMPONIBLE) = 'N'
				...
				if lnLargoEntero <= 6
					...
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud entera del valor del atributo MINIMONOIMPONIBLE del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform(lnLargoEntero) + ' Máxima:6), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
