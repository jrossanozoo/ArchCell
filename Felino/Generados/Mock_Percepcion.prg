...
define class Mock_percepcion as MockBase of MockBase.prg
	...
	*-----------------------------------------------------------------------------------------
	function minimoimponible_assign( txval as variant ) as void                                                                                                                                                                                               
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'PERCEPCION' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'minimoimponible_assign', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'minimoimponible_assign', lcParametros )			endif		endif		Local lxValOld As Variant		lxValOld = this.Minimoimponible		local lxVal as Variant		lxVal = txVal		if This.CargaManual()			if this.Validar_Minimoimponible( lxVal )				this.Setear_Minimoimponible( lxVal )			EndIf		Else			This.Setear_Minimoimponible( lxVal )		EndIf		dodefault( lxVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function setear_minimoimponible( txval as variant ) as void                                                                                                                                                                                               
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'PERCEPCION' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'setear_minimoimponible', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'setear_minimoimponible', lcParametros )			endif		endif		this.Minimoimponible = txVal		dodefault( txVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function validar_minimoimponible( txval as variant ) as boolean                                                                                                                                                                                           
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'PERCEPCION' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'validar_minimoimponible', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'validar_minimoimponible', lcParametros )			endif		endif		Return dodefault( txVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function validarminimoimponible() as boolean                                                                                                                                                                                                              
		...
		return this.ObtenerResultado( 'Validarminimoimponible', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function validarporcentaje() as boolean                                                                                                                                                                                                                   
		...
		return this.ObtenerResultado( 'Validarporcentaje', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function valorsugeridominimoimponible() as void                                                                                                                                                                                                           
		...
		return this.ObtenerResultado( 'Valorsugeridominimoimponible', lcParametros )
	EndFunc
