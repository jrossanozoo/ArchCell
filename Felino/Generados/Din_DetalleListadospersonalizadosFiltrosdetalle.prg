		...
		for lni = 1 to this.Count
			...
			if vartype( this.Item[lni].DIASHACIAATRAS) = 'N'
				...
				if lnLargoEntero <= 2
					...
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud entera del valor del atributo DIASHACIAATRAS del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform(lnLargoEntero) + ' Máxima:2), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
