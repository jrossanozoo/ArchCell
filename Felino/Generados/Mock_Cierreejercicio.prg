...
define class Mock_cierreejercicio as MockBase of MockBase.prg
	...
	*-----------------------------------------------------------------------------------------
	function indice_assign( txval as variant ) as void                                                                                                                                                                                                        
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'CIERREEJERCICIO' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'indice_assign', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'indice_assign', lcParametros )			endif		endif		Local lxValOld As Variant		lxValOld = this.Indice		local lxVal as Variant		lxVal = txVal		if This.CargaManual()			if this.Validar_Indice( lxVal )				if ( this.lHabilitarIndice or this.lEstaSeteandoValorSugerido or alltrim( lxValOld ) == alltrim( lxVal ))					this.Setear_Indice( lxVal )				else					this.Indice = lxValOld					goServicios.Errores.LevantarExcepcion( "No se puede cambiar este valor (Entidad: Cierre de ejercicio - Atributo: Indice)" )				endif			EndIf		Else			This.Setear_Indice( lxVal )		EndIf		dodefault( lxVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function validar_indice( txval as variant ) as boolean                                                                                                                                                                                                    
