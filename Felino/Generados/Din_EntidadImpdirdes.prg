...
define class Din_EntidadIMPDIRDES as ent_ImputacionDirectaBase of ent_ImputacionDirectaBase.prg
	...
	*--------------------------------------------------------------------------------------------------------
	...
	local lnLargoDecimal as integer, lnLargoEntero as Integer, lnLargoDecimal as integer, llRetorno as boolean 
		...
		if ( vartype( this.f_Destino_Desde) = 'C' )
			if len( alltrim( this.F_DESTINO_DESDE ) ) > 10
				This.AgregarInformacion( 'La longitud del valor del atributo F_DESTINO_DESDE es mayor a la permitida.' )
				llRetorno = .F.
			Endif
		else
			This.AgregarInformacion( 'El tipo de dato para el atributo F_DESTINO_DESDE no es el correcto.')
			llRetorno = .F.
		Endif
		 
		if ( vartype( this.f_Destino_Hasta) = 'C' )
			if len( alltrim( this.F_DESTINO_HASTA ) ) > 10
				This.AgregarInformacion( 'La longitud del valor del atributo F_DESTINO_HASTA es mayor a la permitida.' )
				llRetorno = .F.
			Endif
		else
			This.AgregarInformacion( 'El tipo de dato para el atributo F_DESTINO_HASTA no es el correcto.')
			llRetorno = .F.
		Endif
			...
			if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
				if !empty( lcExpresionValorSugeridoDefinidoPorElUsuario )
					try
						...
						This.EventoEstaSeteandoValorSugerido( 'F_destino_desde' )
						.F_destino_desde = lvValorSugeridoDefinidoPorElUsuario
					catch to loError
						loEx = newobject("zooexception", "zooexception.prg" )
						loEx.Grabar( loError )
						loEx.AgregarInformacion( iif( type( "loError.UserValue.Message" ) == "C", loError.UserValue.Message, loError.Message ) )
			...
			if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
				if !empty( lcExpresionValorSugeridoDefinidoPorElUsuario )
					try
						...
						This.EventoEstaSeteandoValorSugerido( 'F_destino_hasta' )
						.F_destino_hasta = lvValorSugeridoDefinidoPorElUsuario
					catch to loError
						loEx = newobject("zooexception", "zooexception.prg" )
						loEx.Grabar( loError )
						loEx.AgregarInformacion( iif( type( "loError.UserValue.Message" ) == "C", loError.UserValue.Message, loError.Message ) )
