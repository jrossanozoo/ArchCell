...
define class Din_DetalleREDONDEODEPRECIOSDetprueba as detalle of detalle.prg
	...
	*--------------------------------------------------------------------------------------------------------
	function oItem_Access() as variant
		if !this.ldestroy and ( !vartype( this.oItem ) = 'O' or isnull( this.oItem ) )
			this.oItem = _Screen.zoo.crearobjeto( 'ItemREDONDEODEPRECIOSDetprueba' )
	...
	*--------------------------------------------------------------------------------------------------------
	Function Cargar() as Void
		...
		scan for &lcFiltro
			...
			loItem.Codigo = c_Detprueba.Codigo
			loItem.NROITEM = c_Detprueba.NROITEM
			loItem.Precio = c_Detprueba.Precio
			loItem.Resultado = c_Detprueba.Resultado
			loItem.NroItem = lnI 
	...
	*--------------------------------------------------------------------------------------------------------
	function ValidarExistenciaCamposFijosItemPlano( tnItem as integer ) as Boolean
		...
		if  empty( this.item[ tnItem ].Precio ) 
			llRetorno = .F.
		endif 
		return llRetorno
		...
		for lni = 1 to this.Count
			...
			if vartype( this.Item[lni].PRECIO) = 'N'
				...
				if lnLargoEntero <= 14
					...
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud entera del valor del atributo PRECIO del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform(lnLargoEntero) + ' Máxima:14), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
			...
			if vartype( this.Item[lni].RESULTADO) = 'N'
				lnLargoEntero = len( transform( int( this.Item[lni].RESULTADO ) ) ) 
				lnValorDelDecimal = this.Item[lni].RESULTADO - int( this.Item[lni].RESULTADO )
				lnLargoDecimal = iif( lnValorDelDecimal = 0, 0, len( transform( lnValorDelDecimal ) ) - 2 )
				if lnLargoEntero <= 14
				else
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud entera del valor del atributo RESULTADO del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform(lnLargoEntero) + ' Máxima:14), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
