...
define class Din_DetalleREQUERIMIENTODECOMPRAImpuestosdetalle as DetalleImpuestos of DetalleImpuestos.Prg
	...
	*--------------------------------------------------------------------------------------------------------
	function Sumarizar() as boolean
		local lnItem as integer, loItem as Object, lnSum_Montodeiva as Number, lnSum_Montodeivasindescuento as Number, lnSum_Montonogravado as Number, lnSum_Montonogravadosindescuento as Number
		...
		with this
			...
			if this.lEsNavegacion
				if select( 'c_Impuestosdetalle' ) > 0 and reccount( 'c_Impuestosdetalle' ) > 0
					sum Montodeiva, Montodeivasindescuento, Montonogravado, Montonogravadosindescuento;
						 to ;
						 lnSum_Montodeiva, lnSum_Montodeivasindescuento, lnSum_Montonogravado, lnSum_Montonogravadosindescuento
				endif
	...
	*--------------------------------------------------------------------------------------------------------
	function oItem_Access() as variant
		if !this.ldestroy and ( !vartype( this.oItem ) = 'O' or isnull( this.oItem ) )
			this.oItem = _Screen.zoo.crearobjeto( 'Din_ItemREQUERIMIENTODECOMPRAImpuestosdetalle' )
		...
		for lni = 1 to this.Count
			...
			if vartype( this.Item[lni].MONTODEIVA) = 'N'
				...
				if lnLargoEntero <= 10
					...
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud entera del valor del atributo MONTODEIVA del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform(lnLargoEntero) + ' Máxima:10), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
			...
			if vartype( this.Item[lni].MONTODEIVASINDESCUENTO) = 'N'
				...
				if lnLargoEntero <= 10
					...
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud entera del valor del atributo MONTODEIVASINDESCUENTO del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform(lnLargoEntero) + ' Máxima:10), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
			...
			if vartype( this.Item[lni].MONTONOGRAVADO) = 'N'
				...
				if lnLargoEntero <= 10
					...
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud entera del valor del atributo MONTONOGRAVADO del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform(lnLargoEntero) + ' Máxima:10), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
			...
			if vartype( this.Item[lni].MONTONOGRAVADOSINDESCUENTO) = 'N'
				...
				if lnLargoEntero <= 10
					...
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud entera del valor del atributo MONTONOGRAVADOSINDESCUENTO del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform(lnLargoEntero) + ' Máxima:10), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
