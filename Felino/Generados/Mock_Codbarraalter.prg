...
define class Mock_codbarraalter as MockBase of MockBase.prg
	...
	cCaracteres = ""                                                                                                                                                                                                                                          
	cContexto = ""                                                                                                                                                                                                                                            
	cDescripcion =  "Código de barra alternativo"                                                                                                                                                                                                             
	...
	lHabilitarCodBarDetalle = .T.                                                                                                                                                                                                                             
	lHabilitarControlaUnicidad = .T.                                                                                                                                                                                                                          
	...
	*-----------------------------------------------------------------------------------------
	function cambiosdetallecodbardetalle() as void                                                                                                                                                                                                            
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Cambiosdetallecodbardetalle', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function codbardetalle_access() as variant                                                                                                                                                                                                                
		lnParametros = pcount()lcParametros = ''		lnIdClase = _screen.Mocks.BuscarMock( 'CODBARRAALTER' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'codbardetalle_access', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'codbardetalle_access', lcParametros )			endif		endif		if !this.ldestroy and ( !vartype( this.Codbardetalle ) = 'O' or isnull( this.Codbardetalle ) )			this.Codbardetalle = this.crearobjeto( 'DetalleCodbarraalterCodbardetalle' )			this.Codbardetalle.inicializar()			this.enlazar( 'Codbardetalle.EventoObtenerLogueo', 'inyectarLogueo' )			this.enlazar( 'Codbardetalle.EventoObtenerInformacion', 'inyectarInformacion' )			bindevent( this.Codbardetalle, 'CambioSumarizado', this, 'CambiosDetalleCodbardetalle', 1)			this.enlazar( 'Codbardetalle.EventoAdvertirLimitePorDiseno', 'EventoAdvertirLimitePorDiseno')			this.enlazar( 'Codbardetalle.EventoCancelarCargaLimitePorDiseno', 'EventoCancelarCargaLimitePorDiseno')			this.Codbardetalle.lVerificarLimitesEnDisenoImpresion = .t.		endif		return this.Codbardetalle
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function controlaunicidad_assign( txval as variant ) as void                                                                                                                                                                                              
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'CODBARRAALTER' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'controlaunicidad_assign', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'controlaunicidad_assign', lcParametros )			endif		endif		Local lxValOld As Variant		lxValOld = this.Controlaunicidad		local lxVal as Variant		lxVal = txVal		if This.CargaManual()			if this.Validar_Controlaunicidad( lxVal )				if ( this.lHabilitarControlaunicidad or this.lEstaSeteandoValorSugerido or lxValOld == lxVal)					this.Setear_Controlaunicidad( lxVal )				else					this.Controlaunicidad = lxValOld					goServicios.Errores.LevantarExcepcion( "No se puede cambiar este valor (Entidad: Código de barra alternativo - Atributo: Controlaunicidad)" )				endif			EndIf		Else			This.Setear_Controlaunicidad( lxVal )		EndIf		dodefault( lxVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function existeatributoctrolunicidadendetalle() as boolean                                                                                                                                                                                                
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Existeatributoctrolunicidadendetalle', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function habilitado_assign( txval as variant ) as void                                                                                                                                                                                                    
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'CODBARRAALTER' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'habilitado_assign', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'habilitado_assign', lcParametros )			endif		endif		Local lxValOld As Variant		lxValOld = this.Habilitado		local lxVal as Variant		lxVal = txVal		if This.CargaManual()			if this.Validar_Habilitado( lxVal )				this.Setear_Habilitado( lxVal )			EndIf		Else			This.Setear_Habilitado( lxVal )		EndIf		dodefault( lxVal )
	EndFunc

	*-----------------------------------------------------------------------------------------
	function habilitarcombocontrolaunicidad() as void                                                                                                                                                                                                         
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Habilitarcombocontrolaunicidad', lcParametros )
	EndFunc

	*-----------------------------------------------------------------------------------------
	function habilitarcontrolessecuencial( txval as variant ) as void                                                                                                                                                                                         
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		local lxParam1
		...
		return this.ObtenerResultado( 'Habilitarcontrolessecuencial', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function lhabilitarcodbardetalle_assign( txval as variant ) as void                                                                                                                                                                                       
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'CODBARRAALTER' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'lhabilitarcodbardetalle_assign', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'lhabilitarcodbardetalle_assign', lcParametros )			endif		endif		this.lHabilitarCodBarDetalle = txVal		this.CodBarDetalle.habilitarItems( txVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function setear_controlaunicidad( txval as variant ) as void                                                                                                                                                                                              
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'CODBARRAALTER' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'setear_controlaunicidad', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'setear_controlaunicidad', lcParametros )			endif		endif		this.Controlaunicidad = txVal		dodefault( txVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function setear_habilitado( txval as variant ) as void                                                                                                                                                                                                    
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'CODBARRAALTER' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'setear_habilitado', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'setear_habilitado', lcParametros )			endif		endif		this.Habilitado = txVal		dodefault( txVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function setear_utilizasecuencial( txval as variant ) as void                                                                                                                                                                                             
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'CODBARRAALTER' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'setear_utilizasecuencial', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'setear_utilizasecuencial', lcParametros )			endif		endif		this.Utilizasecuencial = txVal		dodefault( txVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function utilizasecuencial_assign( txval as variant ) as void                                                                                                                                                                                             
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'CODBARRAALTER' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'utilizasecuencial_assign', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'utilizasecuencial_assign', lcParametros )			endif		endif		Local lxValOld As Variant		lxValOld = this.Utilizasecuencial		local lxVal as Variant		lxVal = txVal		if This.CargaManual()			if this.Validar_Utilizasecuencial( lxVal )				this.Setear_Utilizasecuencial( lxVal )			EndIf		Else			This.Setear_Utilizasecuencial( lxVal )		EndIf		dodefault( lxVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function validar_controlaunicidad( txval as variant ) as boolean                                                                                                                                                                                          
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'CODBARRAALTER' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'validar_controlaunicidad', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'validar_controlaunicidad', lcParametros )			endif		endif		Return dodefault( txVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function validar_habilitado( txval as variant ) as boolean                                                                                                                                                                                                
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'CODBARRAALTER' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'validar_habilitado', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'validar_habilitado', lcParametros )			endif		endif		Return dodefault( txVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function validar_utilizasecuencial( txval as variant ) as boolean                                                                                                                                                                                         
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'CODBARRAALTER' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'validar_utilizasecuencial', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'validar_utilizasecuencial', lcParametros )			endif		endif		Return dodefault( txVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function valorsugeridocontrolaunicidad() as void                                                                                                                                                                                                          
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Valorsugeridocontrolaunicidad', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function valorsugeridofwhabilitado() as void                                                                                                                                                                                                              
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Valorsugeridofwhabilitado', lcParametros )
	EndFunc

	*-----------------------------------------------------------------------------------------
	function valorsugeridohabilitado() as void                                                                                                                                                                                                                
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Valorsugeridohabilitado', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function valorsugeridoutilizasecuencial() as void                                                                                                                                                                                                         
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Valorsugeridoutilizasecuencial', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function verificarcombocontrolaunicidadcompleto() as void                                                                                                                                                                                                 
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Verificarcombocontrolaunicidadcompleto', lcParametros )
	EndFunc
