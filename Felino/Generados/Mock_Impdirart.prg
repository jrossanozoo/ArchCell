...
define class Mock_impdirart as MockBase of MockBase.prg
	...
	*-----------------------------------------------------------------------------------------
	function prioridadccosto_assign( txval as variant ) as void                                                                                                                                                                                               
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'IMPDIRART' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'prioridadccosto_assign', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'prioridadccosto_assign', lcParametros )			endif		endif		Local lxValOld As Variant		lxValOld = this.Prioridadccosto		local lxVal as Variant		lxVal = txVal		if This.CargaManual()			if this.Validar_Prioridadccosto( lxVal )				this.ValidarDominio_Numericononegativo( lxVal )				this.Setear_Prioridadccosto( lxVal )			EndIf		Else			This.Setear_Prioridadccosto( lxVal )		EndIf		dodefault( lxVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function setear_prioridadccosto( txval as variant ) as void                                                                                                                                                                                               
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'IMPDIRART' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'setear_prioridadccosto', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'setear_prioridadccosto', lcParametros )			endif		endif		this.Prioridadccosto = txVal		dodefault( txVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function validar_prioridadccosto( txval as variant ) as boolean                                                                                                                                                                                           
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'IMPDIRART' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'validar_prioridadccosto', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'validar_prioridadccosto', lcParametros )			endif		endif		Return dodefault( txVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function valorsugeridoprioridadccosto() as void                                                                                                                                                                                                           
		...
		return this.ObtenerResultado( 'Valorsugeridoprioridadccosto', lcParametros )
	EndFunc
