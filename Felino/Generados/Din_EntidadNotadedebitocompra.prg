...
define class Din_EntidadNOTADEDEBITOCOMPRA as Ent_ComprobanteDeComprasConValores of Ent_ComprobanteDeComprasConValores.prg
	...
	*--------------------------------------------------------------------------------------------------------
	function ValidarTipoComprobanteRG1361() as boolean
		...
		if empty( this.TipoComprobanteRG1361 )
			this.AgregarInformacion( 'Debe cargar el campo Tipo nota débito', 9005, 'TipoComprobanteRG1361' )
			llRetorno = .F.
		endif
		return llRetorno
	endfunc
	...
	*--------------------------------------------------------------------------------------------------------
	function ValidarLetra() as boolean
		...
		llRetorno = dodefault()
		if empty( this.Letra )
			this.AgregarInformacion( 'Debe cargar el campo Número Nota Débito', 9005, 'Letra' )
			llRetorno = .F.
		endif
		return llRetorno
	endfunc
	...
	*--------------------------------------------------------------------------------------------------------
	function ObtenerFuncionalidades() as String
		return '<ANULABLE><COMPRAS><CONVALORES><FISCAL><NOIMPO><NOEXPO>'
	Endfunc
	*--------------------------------------------------------------------------------------------------------
			...
			if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
				if !empty( lcExpresionValorSugeridoDefinidoPorElUsuario )
					try
						...
						loEx.Grabar( loError )
						loEx.AgregarInformacion( iif( type( "loError.UserValue.Message" ) == "C", loError.UserValue.Message, loError.Message ) )
						loEx.AgregarInformacion( "Se produjo un error al intentar asignar el valor sugerido " + lcExpresionValorSugeridoDefinidoPorElUsuario + " del atributo Tipo nota débito para la entidad Nota de débito de compra." + chr( 10 ) + "Verifique el valor especificado en la entidad Comportamiento de atributos." + chr( 10 ) + "El tipo de dato del atributo es numérico." )
						goServicios.Errores.LevantarExcepcion( loEx )
					endtry
				endif
			endif
