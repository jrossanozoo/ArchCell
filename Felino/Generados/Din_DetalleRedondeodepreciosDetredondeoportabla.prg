		...
		for lni = 1 to this.Count
			...
			if vartype( this.Item[lni].REDONDEAREN) = 'N'
				...
				if lnLargoEntero <= 14
					...
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud entera del valor del atributo REDONDEAREN del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform(lnLargoEntero) + ' Máxima:14), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
