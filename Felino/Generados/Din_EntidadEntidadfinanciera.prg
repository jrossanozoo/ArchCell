...
define class Din_EntidadENTIDADFINANCIERA as entidad of entidad.prg
	...
	cDescripcion =  [Entidad financiera]
	lEntidadEditable =  .t.
		...
		if This.CargaManual()
			if this.Validar_Rut( lxVal )
				if ( this.lHabilitarRut or this.lEstaSeteandoValorSugerido or alltrim( lxValOld ) == alltrim( lxVal ))
					...
					this.Rut = lxValOld
					goServicios.Errores.LevantarExcepcion( "No se puede cambiar este valor (Entidad: Entidad financiera - Atributo: Rut)" )
				endif 
			EndIf
		Else
			This.Setear_Rut( lxVal ) 
