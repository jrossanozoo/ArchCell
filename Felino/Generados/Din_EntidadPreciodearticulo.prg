...
define class Din_EntidadPRECIODEARTICULO as entidad of entidad.prg
	...
	*--------------------------------------------------------------------------------------------------------
	function ValidarArticulo() as boolean
		...
		if empty( this.Articulo_PK )
			this.AgregarInformacion( 'Debe cargar el campo Articulo', 9005, 'Articulo' )
			llRetorno = .F.
		else
	...
	*--------------------------------------------------------------------------------------------------------
	Protected Function _Buscar() As void
		...
		if empty( This.&lcAtributo )
			...
			lcMensaje = 'El dato buscado TIMESTAMP ' + transform( This.TimestampAlta ) + ', LISTA DE PRECIO ' + transform( This.ListaDePrecio_Pk ) + ', ARTICULO ' + transform( This.Articulo_Pk ) + ', COLOR ' + transform( This.Color_Pk ) + ', TALLE ' + transform( This.Talle ) + ' de la entidad ' + upper( this.cDescripcion ) + ' no existe.'
		else
	...
	*--------------------------------------------------------------------------------------------------------
	function ObtenerFuncionalidades() as String
		return '<AUDITORIA:B><MODULOSLISTADO:B><NOIMPO><NOEXPO><INSERTCONTINUO><SINATRIBUTOSFW>'
	Endfunc
	*--------------------------------------------------------------------------------------------------------
