...
define class Mock_asociacionarticulosoperacionesafip as MockBase of MockBase.prg
	...
	cClase = 'asociacionarticulosoperacionesafip'
	...
	cCaracteres = ""                                                                                                                                                                                                                                          
	cContexto = ""                                                                                                                                                                                                                                            
	cDescripcion =  "AFIP - Asociación de artículos y tipos de operación"                                                                                                                                                                                     
	cDescripcionSingular = ""                                                                                                                                                                                                                                 
	cEvento = ""                                                                                                                                                                                                                                              
	cFormatoFechaNet = ""                                                                                                                                                                                                                                     
	cIdentificadorConexion = ""                                                                                                                                                                                                                               
	cNombre = ""                                                                                                                                                                                                                                              
	cNombre = 'ASOCIACIONARTICULOSOPERACIONESAFIP'                                                                                                                                                                                                            
	cPrefijoImportar = ""                                                                                                                                                                                                                                     
	cPrefijoRecibir = ""                                                                                                                                                                                                                                      
	...
	*-----------------------------------------------------------------------------------------
	function articuloconoperacionesasociadas( lccodigoarticulo as string ) as boolean                                                                                                                                                                         
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		local lxParam1

		lnParametros = pcount()
		lxParam1 = lccodigoarticulo
		...
		return this.ObtenerResultado( 'Articuloconoperacionesasociadas', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function operacioncompra_access() as variant                                                                                                                                                                                                              
		lnParametros = pcount()lcParametros = ''		lnIdClase = _screen.Mocks.BuscarMock( 'ASOCIACIONARTICULOSOPERACIONESAFIP' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'operacioncompra_access', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'operacioncompra_access', lcParametros )			endif		endif		if this.ldestroy		else			if This.lInstanciarSubEntidadaDemanda				if ( !vartype( this.Operacioncompra ) = 'O' or isnull( this.Operacioncompra ) )					this.Operacioncompra = _Screen.zoo.instanciarentidad( 'Tipodeoperacionafip' )					this.Operacioncompra.lEsSubEntidad = .t.					this.enlazar( 'Operacioncompra.EventoObtenerLogueo', 'inyectarLogueo' )					this.enlazar( 'Operacioncompra.EventoObtenerInformacion', 'inyectarInformacion' )				endif				if !this.lBuscandoCodigo					if this.Operacioncompra.CODIGO # this.Operacioncompra_PK						this.Operacioncompra.CODIGO = this.Operacioncompra_PK					endif				endif			endif		endif		return this.Operacioncompra
	EndFunc

	*-----------------------------------------------------------------------------------------
	function operacioncompra_pk_assign( txval as variant ) as void                                                                                                                                                                                            
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'ASOCIACIONARTICULOSOPERACIONESAFIP' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'operacioncompra_pk_assign', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'operacioncompra_pk_assign', lcParametros )			endif		endif		Local lxValOld As Variant		lxValOld = this.Operacioncompra_PK		local lxVal as Variant		local loSugerido as Object				lxVal = txVal		lxVal = iif( len( lxVal ) < 2, padr( lxVal, 2 ), lxVal )		this.Operacioncompra_PK = lxVal		if this.Validar_Operacioncompra( lxVal, lxValOld )			This.Setear_Operacioncompra( lxVal )		endif		dodefault( lxVal )
	EndFunc

	*-----------------------------------------------------------------------------------------
	function operacionventa_access() as variant                                                                                                                                                                                                               
		lnParametros = pcount()lcParametros = ''		lnIdClase = _screen.Mocks.BuscarMock( 'ASOCIACIONARTICULOSOPERACIONESAFIP' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'operacionventa_access', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'operacionventa_access', lcParametros )			endif		endif		if this.ldestroy		else			if This.lInstanciarSubEntidadaDemanda				if ( !vartype( this.Operacionventa ) = 'O' or isnull( this.Operacionventa ) )					this.Operacionventa = _Screen.zoo.instanciarentidad( 'Tipodeoperacionafip' )					this.Operacionventa.lEsSubEntidad = .t.					this.enlazar( 'Operacionventa.EventoObtenerLogueo', 'inyectarLogueo' )					this.enlazar( 'Operacionventa.EventoObtenerInformacion', 'inyectarInformacion' )				endif				if !this.lBuscandoCodigo					if this.Operacionventa.CODIGO # this.Operacionventa_PK						this.Operacionventa.CODIGO = this.Operacionventa_PK					endif				endif			endif		endif		return this.Operacionventa
	EndFunc

	*-----------------------------------------------------------------------------------------
	function operacionventa_pk_assign( txval as variant ) as void                                                                                                                                                                                             
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'ASOCIACIONARTICULOSOPERACIONESAFIP' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'operacionventa_pk_assign', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'operacionventa_pk_assign', lcParametros )			endif		endif		Local lxValOld As Variant		lxValOld = this.Operacionventa_PK		local lxVal as Variant		local loSugerido as Object				lxVal = txVal		lxVal = iif( len( lxVal ) < 2, padr( lxVal, 2 ), lxVal )		this.Operacionventa_PK = lxVal		if this.Validar_Operacionventa( lxVal, lxValOld )			This.Setear_Operacionventa( lxVal )		endif		dodefault( lxVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function setear_operacioncompra( txval as variant ) as void                                                                                                                                                                                               
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'ASOCIACIONARTICULOSOPERACIONESAFIP' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'setear_operacioncompra', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'setear_operacioncompra', lcParametros )			endif		endif		this.Operacioncompra.CODIGO = txVal		dodefault( txVal )
	EndFunc

	*-----------------------------------------------------------------------------------------
	function setear_operacionventa( txval as variant ) as void                                                                                                                                                                                                
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'ASOCIACIONARTICULOSOPERACIONESAFIP' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'setear_operacionventa', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'setear_operacionventa', lcParametros )			endif		endif		this.Operacionventa.CODIGO = txVal		dodefault( txVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function validar_operacioncompra( txval as variant, txvalold as variant ) as boolean                                                                                                                                                                      
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1, lxParam2lnParametros = pcount()lxParam1 = txvallxParam2 = txvaloldlcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'ASOCIACIONARTICULOSOPERACIONESAFIP' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'validar_operacioncompra', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'validar_operacioncompra', lcParametros )			endif		endif		Return This.CargaManual() and dodefault( txVal, txValOld )
	EndFunc

	*-----------------------------------------------------------------------------------------
	function validar_operacionventa( txval as variant, txvalold as variant ) as boolean                                                                                                                                                                       
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1, lxParam2lnParametros = pcount()lxParam1 = txvallxParam2 = txvaloldlcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'ASOCIACIONARTICULOSOPERACIONESAFIP' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'validar_operacionventa', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'validar_operacionventa', lcParametros )			endif		endif		Return This.CargaManual() and dodefault( txVal, txValOld )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function validaroperacioncompra() as boolean                                                                                                                                                                                                              
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Validaroperacioncompra', lcParametros )
	EndFunc

	*-----------------------------------------------------------------------------------------
	function validaroperacionventa() as boolean                                                                                                                                                                                                               
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Validaroperacionventa', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function valorsugeridooperacioncompra() as void                                                                                                                                                                                                           
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Valorsugeridooperacioncompra', lcParametros )
	EndFunc

	*-----------------------------------------------------------------------------------------
	function valorsugeridooperacionventa() as void                                                                                                                                                                                                            
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Valorsugeridooperacionventa', lcParametros )
	EndFunc
