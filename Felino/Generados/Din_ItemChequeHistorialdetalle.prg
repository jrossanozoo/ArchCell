...
define class Din_ItemCHEQUEHistorialdetalle as ItemActivo of ItemActivo.prg
	...
	*--------------------------------------------------------------------------------------------------------
	function Inicializar() as boolean
		...
		oICol = goServicios.SaltosDeCampoYValoresSugeridos.ObtenerValorSugeridoDeUnaEntidadDetalle( 'CHEQUE', 'Historialdetalle')
	...
	*--------------------------------------------------------------------------------------------------------
	function Destinodedescarga_PK_Assign( txVal as variant ) as void
		...
		lxValOld = this.Destinodedescarga_PK
		local lxVal as Variant
		local loSugerido as Object
		
		lxVal = txVal
		lxVal = iif( len( lxVal ) < 10, padr( lxVal, 10 ), lxVal )
		this.Destinodedescarga_PK = lxVal

		if this.Validar_Destinodedescarga( lxVal, lxValOld )
			this.EventoAntesDeSetear( This, 'Destinodedescarga_Pk', lxValOld, lxVal  )
			This.Setear_Destinodedescarga( lxVal )
			this.EventoDespuesDeSetear( This, 'Destinodedescarga_Pk', lxValOld, lxVal  )
			If lxValOld != lxVal
				If This.PuedeEjecutarHaCambiado()
					this.HaCambiado( 'Destinodedescarga_Pk', This )
				Endif
			EndIf
		endif
		dodefault( lxVal )
	...
	*--------------------------------------------------------------------------------------------------------
	function Setear_Destinodedescarga( txVal as variant ) as void
		...
		this.Destinodedescarga.CODIGO = txVal
		dodefault( txVal )
	...
	*--------------------------------------------------------------------------------------------------------
	function Destinodedescargadescripcion_Assign( txVal as variant ) as void
		...
		Local lxValOld As Variant
		lxValOld = this.Destinodedescargadescripcion
		local lxVal as Variant
		lxVal = txVal

		if This.CargaManual()
			if this.Validar_Destinodedescargadescripcion( lxVal )
				this.EventoAntesDeSetear( This, 'Destinodedescargadescripcion', lxValOld, lxVal  )
				this.Setear_Destinodedescargadescripcion( lxVal )
				this.EventoDespuesDeSetear( This, 'Destinodedescargadescripcion', lxValOld, lxVal  )
				If lxValOld != lxVal
					If This.PuedeEjecutarHaCambiado()
						this.HaCambiado( 'Destinodedescargadescripcion', This )
					Endif
				EndIf
			EndIf
		Else
	...
	*--------------------------------------------------------------------------------------------------------
	function Setear_Destinodedescargadescripcion( txVal as variant ) as void
		...
		this.Destinodedescargadescripcion = txVal
		dodefault( txVal )
	...
	function Destinodedescarga_Access() as variant
		if this.ldestroy
			...
			if This.lInstanciarSubEntidadaDemanda
				if ( !vartype( this.Destinodedescarga ) = 'O' or isnull( this.Destinodedescarga ) )
					this.Destinodedescarga = _Screen.zoo.instanciarentidad( 'Destinodescargacheques' )
					this.Destinodedescarga.lEsSubEntidad = .t.
					this.enlazar( 'Destinodedescarga.EventoObtenerLogueo', 'inyectarLogueo' )
					this.enlazar( 'Destinodedescarga.EventoObtenerInformacion', 'inyectarInformacion' )
				endif
				if !this.lBuscandoCodigo
					if this.Destinodedescarga.CODIGO # this.Destinodedescarga_PK
						this.Destinodedescarga.CODIGO = this.Destinodedescarga_PK
					endif
				endif
			endif
		endif
		return this.Destinodedescarga
	endfunc
	...
	*--------------------------------------------------------------------------------------------------------
	Function ValorSugeridoConcepto() as void
		...
		with this
			try
				lcExpresionValorSugeridoDefinidoPorElUsuario = goServicios.SaltosDeCampoYValoresSugeridos.ObtenerValorSugerido( "CHEQUE", "Historialdetalle", "Concepto" )
	...
	*--------------------------------------------------------------------------------------------------------
	Function ValorSugeridoCajaestado() as void
		...
		lcExpresionValorSugeridoDefinidoPorElUsuario = null
		with this
			lcExpresionValorSugeridoDefinidoPorElUsuario = goServicios.SaltosDeCampoYValoresSugeridos.ObtenerValorSugerido( "CHEQUE", "Historialdetalle", "Cajaestado" )
			if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
				if !empty( lcExpresionValorSugeridoDefinidoPorElUsuario )
					try
						lvValorSugeridoDefinidoPorElUsuario = &lcExpresionValorSugeridoDefinidoPorElUsuario
						...
						This.EventoEstaSeteandoValorSugerido( 'Cajaestado' )
						.Cajaestado = lvValorSugeridoDefinidoPorElUsuario
					catch to loError
						loEx = newobject("zooexception", "zooexception.prg" )
						loEx.Grabar( loError )
						loEx.AgregarInformacion( iif( type( "loError.UserValue.Message" ) == "C", loError.UserValue.Message, loError.Message ) )
						loEx.AgregarInformacion( "Se produjo un error al intentar asignar el valor sugerido " + lcExpresionValorSugeridoDefinidoPorElUsuario + " del atributo Caja para el detalle Historial de interacciones." + chr( 10 ) + "Verifique el valor especificado en la entidad Comportamiento de atributos." + chr( 10 ) + "El tipo de dato del atributo es numérico." )
						goServicios.Errores.LevantarExcepcion( loEx )
					endtry
				endif
			endif
