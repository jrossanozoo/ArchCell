...
define class Din_EntidadIMPDIRVAL as ent_ImputacionDirectaBase of ent_ImputacionDirectaBase.prg
	...
	*--------------------------------------------------------------------------------------------------------
	...
	local lnLargoDecimal as integer, lnLargoEntero as Integer, lnLargoDecimal as integer, llRetorno as boolean 
		...
		if ( vartype( this.f_Valor_Desde) = 'C' )
			if len( alltrim( this.F_VALOR_DESDE ) ) > 5
				This.AgregarInformacion( 'La longitud del valor del atributo F_VALOR_DESDE es mayor a la permitida.' )
				llRetorno = .F.
			Endif
		else
			This.AgregarInformacion( 'El tipo de dato para el atributo F_VALOR_DESDE no es el correcto.')
			llRetorno = .F.
		Endif
		 
		if ( vartype( this.f_Valor_Hasta) = 'C' )
			if len( alltrim( this.F_VALOR_HASTA ) ) > 5
				This.AgregarInformacion( 'La longitud del valor del atributo F_VALOR_HASTA es mayor a la permitida.' )
				llRetorno = .F.
			Endif
		else
			This.AgregarInformacion( 'El tipo de dato para el atributo F_VALOR_HASTA no es el correcto.')
			llRetorno = .F.
		Endif
		 
		if ( vartype( this.f_Moneda_Desde) = 'C' )
			if len( alltrim( this.F_MONEDA_DESDE ) ) > 10
				This.AgregarInformacion( 'La longitud del valor del atributo F_MONEDA_DESDE es mayor a la permitida.' )
				llRetorno = .F.
			Endif
		else
			This.AgregarInformacion( 'El tipo de dato para el atributo F_MONEDA_DESDE no es el correcto.')
			llRetorno = .F.
		Endif
		...
		if ( vartype( this.f_Moneda_Hasta) = 'C' )
			if len( alltrim( this.F_MONEDA_HASTA ) ) > 10
				This.AgregarInformacion( 'La longitud del valor del atributo F_MONEDA_HASTA es mayor a la permitida.' )
				llRetorno = .F.
			Endif
		else
			This.AgregarInformacion( 'El tipo de dato para el atributo F_MONEDA_HASTA no es el correcto.')
			llRetorno = .F.
		Endif
		 
		if ( vartype( this.f_OperadoraDeTarjeta_Desde) = 'C' )
			if len( alltrim( this.F_OPERADORADETARJETA_DESDE ) ) > 15
				This.AgregarInformacion( 'La longitud del valor del atributo F_OPERADORADETARJETA_DESDE es mayor a la permitida.' )
				llRetorno = .F.
			Endif
		else
			This.AgregarInformacion( 'El tipo de dato para el atributo F_OPERADORADETARJETA_DESDE no es el correcto.')
			llRetorno = .F.
		Endif
		 
		if ( vartype( this.f_OperadoraDeTarjeta_Hasta) = 'C' )
			if len( alltrim( this.F_OPERADORADETARJETA_HASTA ) ) > 15
				This.AgregarInformacion( 'La longitud del valor del atributo F_OPERADORADETARJETA_HASTA es mayor a la permitida.' )
				llRetorno = .F.
			Endif
		else
			This.AgregarInformacion( 'El tipo de dato para el atributo F_OPERADORADETARJETA_HASTA no es el correcto.')
			llRetorno = .F.
		Endif
			...
			if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
				if !empty( lcExpresionValorSugeridoDefinidoPorElUsuario )
					try
						...
						This.EventoEstaSeteandoValorSugerido( 'F_valor_desde' )
						.F_valor_desde = lvValorSugeridoDefinidoPorElUsuario
					catch to loError
						loEx = newobject("zooexception", "zooexception.prg" )
						loEx.Grabar( loError )
						loEx.AgregarInformacion( iif( type( "loError.UserValue.Message" ) == "C", loError.UserValue.Message, loError.Message ) )
			...
			if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
				if !empty( lcExpresionValorSugeridoDefinidoPorElUsuario )
					try
						...
						This.EventoEstaSeteandoValorSugerido( 'F_valor_hasta' )
						.F_valor_hasta = lvValorSugeridoDefinidoPorElUsuario
					catch to loError
						loEx = newobject("zooexception", "zooexception.prg" )
						loEx.Grabar( loError )
						loEx.AgregarInformacion( iif( type( "loError.UserValue.Message" ) == "C", loError.UserValue.Message, loError.Message ) )
			...
			if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
				if !empty( lcExpresionValorSugeridoDefinidoPorElUsuario )
					try
						...
						This.EventoEstaSeteandoValorSugerido( 'F_moneda_desde' )
						.F_moneda_desde = lvValorSugeridoDefinidoPorElUsuario
					catch to loError
						loEx = newobject("zooexception", "zooexception.prg" )
						loEx.Grabar( loError )
						loEx.AgregarInformacion( iif( type( "loError.UserValue.Message" ) == "C", loError.UserValue.Message, loError.Message ) )
			...
			if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
				if !empty( lcExpresionValorSugeridoDefinidoPorElUsuario )
					try
						...
						This.EventoEstaSeteandoValorSugerido( 'F_moneda_hasta' )
						.F_moneda_hasta = lvValorSugeridoDefinidoPorElUsuario
					catch to loError
						loEx = newobject("zooexception", "zooexception.prg" )
						loEx.Grabar( loError )
						loEx.AgregarInformacion( iif( type( "loError.UserValue.Message" ) == "C", loError.UserValue.Message, loError.Message ) )
			...
			if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
				if !empty( lcExpresionValorSugeridoDefinidoPorElUsuario )
					try
						...
						This.EventoEstaSeteandoValorSugerido( 'F_operadoradetarjeta_desde' )
						.F_operadoradetarjeta_desde = lvValorSugeridoDefinidoPorElUsuario
					catch to loError
						loEx = newobject("zooexception", "zooexception.prg" )
						loEx.Grabar( loError )
						loEx.AgregarInformacion( iif( type( "loError.UserValue.Message" ) == "C", loError.UserValue.Message, loError.Message ) )
			...
			if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
				if !empty( lcExpresionValorSugeridoDefinidoPorElUsuario )
					try
						...
						This.EventoEstaSeteandoValorSugerido( 'F_operadoradetarjeta_hasta' )
						.F_operadoradetarjeta_hasta = lvValorSugeridoDefinidoPorElUsuario
					catch to loError
						loEx = newobject("zooexception", "zooexception.prg" )
						loEx.Grabar( loError )
						loEx.AgregarInformacion( iif( type( "loError.UserValue.Message" ) == "C", loError.UserValue.Message, loError.Message ) )
	...
	*--------------------------------------------------------------------------------------------------------
	Function ValorSugeridoFWF_operadoradetarjeta_hasta() as void
		with this
			.F_operadoradetarjeta_hasta = 'ZZZZZZZZZZZZZZZ'
		endwith
	endfunc
