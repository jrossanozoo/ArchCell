...
define class Din_EntidadFACTURAAGRUPADA as Ent_ComprobanteAgrupado of Ent_ComprobanteAgrupado.prg
	...
	*--------------------------------------------------------------------------------------------------------
	function ObtenerFuncionalidades() as String
		return '<ANULABLE><VENTAS><CAI><CONVALORES><FISCAL><PROMO><NOLISTAGENERICO><PICKING><INTERVINIENTE><VALIDACENTRALIZADOR><VALORCIERRE><DESCUENTOAUTOMATICO><ACTUALIZAEMAILCLIENTE><MIPYME:54><AGRUPACOMPROBANTES:1;2;27><MIPYMEORIGINAL:27><CF>'
	Endfunc
	*--------------------------------------------------------------------------------------------------------
	...
	*--------------------------------------------------------------------------------------------------------
	function Validar_Cliente( txVal as variant, txValOld as variant ) as Boolean
		...
		Return This.CargaManual() and dodefault( txVal, txValOld ) and This.ValidarAtributoParticipanteDePromo( 'Cliente_Pk', txVal, txValOld )
			...
			if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
				if !empty( lcExpresionValorSugeridoDefinidoPorElUsuario )
					try
						...
						loEx.Grabar( loError )
						loEx.AgregarInformacion( iif( type( "loError.UserValue.Message" ) == "C", loError.UserValue.Message, loError.Message ) )
						loEx.AgregarInformacion( "Se produjo un error al intentar asignar el valor sugerido " + lcExpresionValorSugeridoDefinidoPorElUsuario + " del atributo Vto pago para la entidad Factura." + chr( 10 ) + "Verifique el valor especificado en la entidad Comportamiento de atributos." + chr( 10 ) + "El tipo de dato del atributo es fecha." )
						goServicios.Errores.LevantarExcepcion( loEx )
					endtry
				endif
			else
