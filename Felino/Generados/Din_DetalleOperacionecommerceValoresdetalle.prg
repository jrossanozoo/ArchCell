		...
		for lni = 1 to this.Count
			...
			if vartype( this.Item[lni].RECIBIDO) = 'N'
				...
				if lnLargoEntero <= 18
					...
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud entera del valor del atributo RECIBIDO del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform(lnLargoEntero) + ' Máxima:18), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
