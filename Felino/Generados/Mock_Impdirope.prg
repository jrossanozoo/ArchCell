...
define class Mock_impdirope as MockBase of MockBase.prg
	...
	*-----------------------------------------------------------------------------------------
	function f_operadora_desde_assign( txval as variant ) as void                                                                                                                                                                                             
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'IMPDIROPE' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'f_operadora_desde_assign', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'f_operadora_desde_assign', lcParametros )			endif		endif		Local lxValOld As Variant		lxValOld = this.F_operadora_desde		local lxVal as Variant		lxVal = txVal		if This.CargaManual()			if this.Validar_F_operadora_desde( lxVal )				this.Setear_F_operadora_desde( lxVal )			EndIf		Else			This.Setear_F_operadora_desde( lxVal )		EndIf		dodefault( lxVal )
	EndFunc

	*-----------------------------------------------------------------------------------------
	function f_operadora_hasta_assign( txval as variant ) as void                                                                                                                                                                                             
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'IMPDIROPE' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'f_operadora_hasta_assign', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'f_operadora_hasta_assign', lcParametros )			endif		endif		Local lxValOld As Variant		lxValOld = this.F_operadora_hasta		local lxVal as Variant		lxVal = txVal		if This.CargaManual()			if this.Validar_F_operadora_hasta( lxVal )				this.Setear_F_operadora_hasta( lxVal )			EndIf		Else			This.Setear_F_operadora_hasta( lxVal )		EndIf		dodefault( lxVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function validar_f_operadora_desde( txval as variant ) as boolean                                                                                                                                                                                         
	...
	*-----------------------------------------------------------------------------------------
	function validar_f_operadora_hasta( txval as variant ) as boolean                                                                                                                                                                                         
