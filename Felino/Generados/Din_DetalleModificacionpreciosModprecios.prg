...
define class Din_DetalleMODIFICACIONPRECIOSModprecios as detalle of detalle.prg
	...
	*--------------------------------------------------------------------------------------------------------
	Function Cargar() as Void
		...
		scan for &lcFiltro
			...
			loItem.Talle = c_Modprecios.Talle
		...
		for lni = 1 to this.Count
			...
			if vartype( this.Item[lni].PRECIODELISTA) = 'N'
				...
				if lnLargoEntero <= 6
					...
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud entera del valor del atributo PRECIODELISTA del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform(lnLargoEntero) + ' Máxima:6), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
			...
			if vartype( this.Item[lni].PRECIOACT) = 'N'
				...
				if lnLargoEntero <= 6
					...
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud entera del valor del atributo PRECIOACT del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform(lnLargoEntero) + ' Máxima:6), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
