		...
		for lni = 1 to this.Count
			...
			if vartype( this.Item[lni].CENTRODECOSTODETALLE) = 'C'
				if len( alltrim( this.Item[lni].CENTRODECOSTODETALLE )) <= 150
					...
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud del valor del atributo CENTRODECOSTODETALLE del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform( len( alltrim( this.Item[lni].CENTRODECOSTODETALLE ))) + ' Máxima:150), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
