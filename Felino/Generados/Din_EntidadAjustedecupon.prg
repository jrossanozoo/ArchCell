...
define class Din_EntidadAJUSTEDECUPON as entidad of entidad.prg
	...
	*--------------------------------------------------------------------------------------------------------
	...
	local lnLargoDecimal as integer, lnLargoEntero as Integer, lnLargoDecimal as integer, llRetorno as boolean 
		...
		if ( vartype( this.NumeroFactura) = 'N' )
			lnLargoEntero  = len( transform( int( this.NUMEROFACTURA ) ) ) 
			lnValorDelDecimal = this.NUMEROFACTURA - int( this.NUMEROFACTURA)
			lnLargoDecimal = iif( lnValorDelDecimal = 0, 0, len( transform( lnValorDelDecimal ) ) - 2 )
			if lnLargoEntero > 8
				This.AgregarInformacion( 'La longitud entera del valor del atributo NUMEROFACTURA es mayor a la permitida.' )
				llRetorno = .F.
			Endif
		else
			This.AgregarInformacion( 'El tipo de dato para el atributo NUMEROFACTURA no es el correcto.')
			llRetorno = .F.
		Endif
		 
		if ( vartype( this.FactTipo) = 'N' )
			lnLargoEntero  = len( transform( int( this.FACTTIPO ) ) ) 
			lnValorDelDecimal = this.FACTTIPO - int( this.FACTTIPO)
			lnLargoDecimal = iif( lnValorDelDecimal = 0, 0, len( transform( lnValorDelDecimal ) ) - 2 )
			if lnLargoEntero > 2
				This.AgregarInformacion( 'La longitud entera del valor del atributo FACTTIPO es mayor a la permitida.' )
				llRetorno = .F.
			Endif
		else
			This.AgregarInformacion( 'El tipo de dato para el atributo FACTTIPO no es el correcto.')
			llRetorno = .F.
		Endif
		...
		if ( vartype( this.PuntoDeVentaFactura) = 'N' )
			lnLargoEntero  = len( transform( int( this.PUNTODEVENTAFACTURA ) ) ) 
			lnValorDelDecimal = this.PUNTODEVENTAFACTURA - int( this.PUNTODEVENTAFACTURA)
			lnLargoDecimal = iif( lnValorDelDecimal = 0, 0, len( transform( lnValorDelDecimal ) ) - 2 )
			if lnLargoEntero > 4
				This.AgregarInformacion( 'La longitud entera del valor del atributo PUNTODEVENTAFACTURA es mayor a la permitida.' )
				llRetorno = .F.
			Endif
		else
			This.AgregarInformacion( 'El tipo de dato para el atributo PUNTODEVENTAFACTURA no es el correcto.')
			llRetorno = .F.
		Endif
		...
		if ( vartype( this.MontoAjuste) = 'N' )
			lnLargoEntero  = len( transform( int( this.MONTOAJUSTE ) ) ) 
			lnValorDelDecimal = this.MONTOAJUSTE - int( this.MONTOAJUSTE)
			lnLargoDecimal = iif( lnValorDelDecimal = 0, 0, len( transform( lnValorDelDecimal ) ) - 2 )
			if lnLargoEntero > 13
				This.AgregarInformacion( 'La longitud entera del valor del atributo MONTOAJUSTE es mayor a la permitida.' )
				llRetorno = .F.
			Endif
		else
			This.AgregarInformacion( 'El tipo de dato para el atributo MONTOAJUSTE no es el correcto.')
			llRetorno = .F.
		Endif
		...
		if ( vartype( this.IdFactura) = 'C' )
			if len( alltrim( this.IDFACTURA ) ) > 38
				This.AgregarInformacion( 'La longitud del valor del atributo IDFACTURA es mayor a la permitida.' )
				llRetorno = .F.
			Endif
		else
			This.AgregarInformacion( 'El tipo de dato para el atributo IDFACTURA no es el correcto.')
			llRetorno = .F.
		Endif
		 
		if ( vartype( this.IdCupon) = 'C' )
			if len( alltrim( this.IDCUPON ) ) > 38
				This.AgregarInformacion( 'La longitud del valor del atributo IDCUPON es mayor a la permitida.' )
				llRetorno = .F.
			Endif
		else
			This.AgregarInformacion( 'El tipo de dato para el atributo IDCUPON no es el correcto.')
			llRetorno = .F.
		Endif
		 
		if ( vartype( this.NotaDeCredito) = 'C' )
			if len( alltrim( this.NOTADECREDITO ) ) > 38
				This.AgregarInformacion( 'La longitud del valor del atributo NOTADECREDITO es mayor a la permitida.' )
				llRetorno = .F.
			Endif
		else
			This.AgregarInformacion( 'El tipo de dato para el atributo NOTADECREDITO no es el correcto.')
			llRetorno = .F.
		Endif
		 
		if ( vartype( this.IdCliente) = 'C' )
			if len( alltrim( this.IDCLIENTE ) ) > 10
		...
		if ( vartype( this.Vendedor) = 'C' )
			if len( alltrim( this.VENDEDOR ) ) > 10
				This.AgregarInformacion( 'La longitud del valor del atributo VENDEDOR es mayor a la permitida.' )
				llRetorno = .F.
			Endif
		else
			This.AgregarInformacion( 'El tipo de dato para el atributo VENDEDOR no es el correcto.')
			llRetorno = .F.
		Endif
		 
		if ( vartype( this.LetraFactura) = 'C' )
			if len( alltrim( this.LETRAFACTURA ) ) > 1
				This.AgregarInformacion( 'La longitud del valor del atributo LETRAFACTURA es mayor a la permitida.' )
				llRetorno = .F.
			Endif
		else
			This.AgregarInformacion( 'El tipo de dato para el atributo LETRAFACTURA no es el correcto.')
			llRetorno = .F.
		Endif
	...
	*--------------------------------------------------------------------------------------------------------
	function ValidacionBasica() as boolean
		...
		With This
			llRetorno = .ValidarIdcupon() and llRetorno
	...
	*--------------------------------------------------------------------------------------------------------
	function ValidarIdCupon() as boolean
		...
		llRetorno = dodefault()
		if empty( this.IdCupon )
			this.AgregarInformacion( 'Debe cargar el campo Id cupón', 9005, 'IdCupon' )
			llRetorno = .F.
		endif
		return llRetorno
	endfunc
	...
	*--------------------------------------------------------------------------------------------------------
	function obtenerAtributosObligatorios() as ZooColeccion
		...
		loAtributosObligatorios.add( 'IDCUPON' )
	...
	*--------------------------------------------------------------------------------------------------------
	Protected Function _Buscar() As void
		...
		if empty( This.&lcAtributo )
			...
			lcMensaje = 'El dato buscado ID CUPÓN ' + transform( This.IdCupon ) + ' de la entidad ' + upper( this.cDescripcion ) + ' no existe.'
		else
	...
	*--------------------------------------------------------------------------------------------------------
	function Inicializar() as Void
		...
		this.oAtributosCC.add( "IdCupon" )
		this.oColeccionVS = _screen.zoo.CrearObjeto( 'zoocoleccion' )
		this.oColeccionVSFW = _screen.zoo.CrearObjeto( 'zoocoleccion' )
		oICol = goServicios.SaltosDeCampoYValoresSugeridos.ObtenerValorSugeridoDeUnaEntidadDetalle( 'AJUSTEDECUPON', '')
