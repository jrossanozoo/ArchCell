		...
		for lni = 1 to this.Count
			...
			if vartype( this.Item[lni].TALONARIO) = 'C'
				if len( alltrim( this.Item[lni].TALONARIO )) <= 35
					...
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud del valor del atributo TALONARIO del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform( len( alltrim( this.Item[lni].TALONARIO ))) + ' Máxima:35), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
			...
			if vartype( this.Item[lni].ENTIDAD) = 'C'
				if len( alltrim( this.Item[lni].ENTIDAD )) <= 150
					...
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud del valor del atributo ENTIDAD del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform( len( alltrim( this.Item[lni].ENTIDAD ))) + ' Máxima:150), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
