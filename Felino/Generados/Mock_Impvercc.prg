...
define class Mock_impvercc as MockBase of MockBase.prg
	...
	*-----------------------------------------------------------------------------------------
	function f_razons_desde_assign( txval as variant ) as void                                                                                                                                                                                                
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'IMPVERCC' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'f_razons_desde_assign', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'f_razons_desde_assign', lcParametros )			endif		endif		Local lxValOld As Variant		lxValOld = this.F_razons_desde		local lxVal as Variant		lxVal = txVal		if This.CargaManual()			if this.Validar_F_razons_desde( lxVal )				this.Setear_F_razons_desde( lxVal )			EndIf		Else			This.Setear_F_razons_desde( lxVal )		EndIf		dodefault( lxVal )
	EndFunc

	*-----------------------------------------------------------------------------------------
	function f_razons_hasta_assign( txval as variant ) as void                                                                                                                                                                                                
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'IMPVERCC' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'f_razons_hasta_assign', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'f_razons_hasta_assign', lcParametros )			endif		endif		Local lxValOld As Variant		lxValOld = this.F_razons_hasta		local lxVal as Variant		lxVal = txVal		if This.CargaManual()			if this.Validar_F_razons_hasta( lxVal )				this.Setear_F_razons_hasta( lxVal )			EndIf		Else			This.Setear_F_razons_hasta( lxVal )		EndIf		dodefault( lxVal )
	EndFunc

	*-----------------------------------------------------------------------------------------
	function f_sucursal_desde_assign( txval as variant ) as void                                                                                                                                                                                              
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'IMPVERCC' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'f_sucursal_desde_assign', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'f_sucursal_desde_assign', lcParametros )			endif		endif		Local lxValOld As Variant		lxValOld = this.F_sucursal_desde		local lxVal as Variant		lxVal = txVal		if This.CargaManual()			if this.Validar_F_sucursal_desde( lxVal )				this.Setear_F_sucursal_desde( lxVal )			EndIf		Else			This.Setear_F_sucursal_desde( lxVal )		EndIf		dodefault( lxVal )
	EndFunc

	*-----------------------------------------------------------------------------------------
	function f_sucursal_hasta_assign( txval as variant ) as void                                                                                                                                                                                              
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'IMPVERCC' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'f_sucursal_hasta_assign', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'f_sucursal_hasta_assign', lcParametros )			endif		endif		Local lxValOld As Variant		lxValOld = this.F_sucursal_hasta		local lxVal as Variant		lxVal = txVal		if This.CargaManual()			if this.Validar_F_sucursal_hasta( lxVal )				this.Setear_F_sucursal_hasta( lxVal )			EndIf		Else			This.Setear_F_sucursal_hasta( lxVal )		EndIf		dodefault( lxVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function validar_f_razons_desde( txval as variant ) as boolean                                                                                                                                                                                            
	...
	*-----------------------------------------------------------------------------------------
	function validar_f_razons_hasta( txval as variant ) as boolean                                                                                                                                                                                            
	...
	*-----------------------------------------------------------------------------------------
	function validar_f_sucursal_desde( txval as variant ) as boolean                                                                                                                                                                                          
	...
	*-----------------------------------------------------------------------------------------
	function validar_f_sucursal_hasta( txval as variant ) as boolean                                                                                                                                                                                          
	...
	*-----------------------------------------------------------------------------------------
	function valorsugeridof_baseagru_desde() as void                                                                                                                                                                                                          
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Valorsugeridof_baseagru_desde', lcParametros )
	EndFunc
