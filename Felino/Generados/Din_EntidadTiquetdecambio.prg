...
define class Din_EntidadTIQUETDECAMBIO as entidad of entidad.prg
	...
	*--------------------------------------------------------------------------------------------------------
	function oAD_Access() as variant
		if !this.ldestroy and ( !vartype( this.oAD ) = 'O' or isnull( this.oAD ) )
			this.oAD = this.crearobjeto( 'Din_EntidadTIQUETDECAMBIOAD_' + alltrim( _screen.zoo.app.TipoDeBase ))
	...
	*--------------------------------------------------------------------------------------------------------
	function SetearEstadoNuevoEnItems() as Void
		dodefault()
		this.DetalleComprobante.oItem.lNuevo = this.lNuevo
	endfunc

	*--------------------------------------------------------------------------------------------------------
	function SetearEstadoEdicionEnItems() as Void
		dodefault()
		this.DetalleComprobante.oItem.lEdicion = this.lEdicion
	endfunc
	...
	*--------------------------------------------------------------------------------------------------------
	...
	local lnLargoDecimal as integer, lnLargoEntero as Integer, lnLargoDecimal as integer, llRetorno as boolean 
		...
		if ( vartype( this.NroArt) = 'C' )
			if len( alltrim( this.NROART ) ) > 5
				This.AgregarInformacion( 'La longitud del valor del atributo NROART es mayor a la permitida.' )
				llRetorno = .F.
			Endif
		else
			This.AgregarInformacion( 'El tipo de dato para el atributo NROART no es el correcto.')
			llRetorno = .F.
		Endif
		 
		if ( vartype( this.CantArt) = 'C' )
			if len( alltrim( this.CANTART ) ) > 10
				This.AgregarInformacion( 'La longitud del valor del atributo CANTART es mayor a la permitida.' )
				llRetorno = .F.
			Endif
		else
			This.AgregarInformacion( 'El tipo de dato para el atributo CANTART no es el correcto.')
			llRetorno = .F.
		Endif
	...
	*--------------------------------------------------------------------------------------------------------
	function ValidacionDetalles() as Boolean
		...
		llRetorno = dodefault()
		llRetorno = This.ValidarUnDetalle( this.DetalleComprobante, 'Artículos para cambio' ) and llRetorno
		return llRetorno
	endfunc
	...
	*--------------------------------------------------------------------------------------------------------
	Protected Function _Buscar() As void
		...
		if empty( This.&lcAtributo )
			...
			lcMensaje = 'El dato buscado TIPO DE COMPROBANTE ' + transform( This.TipoComprobante ) + ', LETRA ' + transform( This.Letra ) + ', PTO. VENTA ' + transform( This.PuntoDeVenta ) + ', NÚMERO ' + transform( This.Numero ) + ', NÚMERO DE ARTÍCULOS ' + transform( This.NroArt ) + ', CANTIDAD DE ARTÍCULO ' + transform( This.CantArt ) + ' de la entidad ' + upper( this.cDescripcion ) + ' no existe.'
		else
	...
	*--------------------------------------------------------------------------------------------------------
	function Inicializar() as Void
		...
		this.oAtributosCC.add( "NroArt" )
		this.oAtributosCC.add( "CantArt" )
		this.oColeccionVS = _screen.zoo.CrearObjeto( 'zoocoleccion' )
		this.oColeccionVSFW = _screen.zoo.CrearObjeto( 'zoocoleccion' )
		oICol = goServicios.SaltosDeCampoYValoresSugeridos.ObtenerValorSugeridoDeUnaEntidadDetalle( 'TIQUETDECAMBIO', '')
