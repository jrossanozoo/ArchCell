...
define class Mock_vendedor as MockBase of MockBase.prg
	...
	CodigoPostal = ""                                                                                                                                                                                                                                         
	Comision = 0                                                                                                                                                                                                                                              
	ComisionesDetalle = null                                                                                                                                                                                                                                  
	Departamento = ""                                                                                                                                                                                                                                         
	DescMaxDetalleFacturas = 0                                                                                                                                                                                                                                
	DescMaxTotalFacturas = 0                                                                                                                                                                                                                                  
	...
	*-----------------------------------------------------------------------------------------
	function cambiosdetallecomisionesdetalle() as void                                                                                                                                                                                                        
		...
		return this.ObtenerResultado( 'Cambiosdetallecomisionesdetalle', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function comision_assign( txval as variant ) as void                                                                                                                                                                                                      
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'VENDEDOR' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'comision_assign', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'comision_assign', lcParametros )			endif		endif		Local lxValOld As Variant		lxValOld = this.Comision		local lxVal as Variant		lxVal = txVal		if This.CargaManual()			if this.Validar_Comision( lxVal )				this.Setear_Comision( lxVal )			EndIf		Else			This.Setear_Comision( lxVal )		EndIf		dodefault( lxVal )
	EndFunc

	*-----------------------------------------------------------------------------------------
	function comisionesdetalle_access() as variant                                                                                                                                                                                                            
		lnParametros = pcount()lcParametros = ''		lnIdClase = _screen.Mocks.BuscarMock( 'VENDEDOR' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'comisionesdetalle_access', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'comisionesdetalle_access', lcParametros )			endif		endif		if !this.ldestroy and ( !vartype( this.Comisionesdetalle ) = 'O' or isnull( this.Comisionesdetalle ) )			this.Comisionesdetalle = this.crearobjeto( 'DetalleVendedorComisionesdetalle' )			this.Comisionesdetalle.inicializar()			this.enlazar( 'Comisionesdetalle.EventoObtenerLogueo', 'inyectarLogueo' )			this.enlazar( 'Comisionesdetalle.EventoObtenerInformacion', 'inyectarInformacion' )			bindevent( this.Comisionesdetalle, 'CambioSumarizado', this, 'CambiosDetalleComisionesdetalle', 1)			this.enlazar( 'Comisionesdetalle.EventoAdvertirLimitePorDiseno', 'EventoAdvertirLimitePorDiseno')			this.enlazar( 'Comisionesdetalle.EventoCancelarCargaLimitePorDiseno', 'EventoCancelarCargaLimitePorDiseno')			this.Comisionesdetalle.lVerificarLimitesEnDisenoImpresion = .t.		endif		return this.Comisionesdetalle
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function descmaxdetallefacturas_assign( txval as variant ) as void                                                                                                                                                                                        
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'VENDEDOR' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'descmaxdetallefacturas_assign', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'descmaxdetallefacturas_assign', lcParametros )			endif		endif		Local lxValOld As Variant		lxValOld = this.Descmaxdetallefacturas		local lxVal as Variant		lxVal = txVal		if This.CargaManual()			if this.Validar_Descmaxdetallefacturas( lxVal )				this.Setear_Descmaxdetallefacturas( lxVal )			EndIf		Else			This.Setear_Descmaxdetallefacturas( lxVal )		EndIf		dodefault( lxVal )
	EndFunc

	*-----------------------------------------------------------------------------------------
	function descmaxtotalfacturas_assign( txval as variant ) as void                                                                                                                                                                                          
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'VENDEDOR' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'descmaxtotalfacturas_assign', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'descmaxtotalfacturas_assign', lcParametros )			endif		endif		Local lxValOld As Variant		lxValOld = this.Descmaxtotalfacturas		local lxVal as Variant		lxVal = txVal		if This.CargaManual()			if this.Validar_Descmaxtotalfacturas( lxVal )				this.Setear_Descmaxtotalfacturas( lxVal )			EndIf		Else			This.Setear_Descmaxtotalfacturas( lxVal )		EndIf		dodefault( lxVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function eventoalasignarcodigodecomisioninexistente() as void                                                                                                                                                                                             
		...
		return this.ObtenerResultado( 'Eventoalasignarcodigodecomisioninexistente', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function fechaegreso_assign( txval as variant ) as void                                                                                                                                                                                                   
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'VENDEDOR' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'fechaegreso_assign', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'fechaegreso_assign', lcParametros )			endif		endif		Local lxValOld As Variant		lxValOld = this.Fechaegreso		local lxVal as Variant		lxVal = txVal		if This.CargaManual()			if this.Validar_Fechaegreso( lxVal )				this.ValidarDominio_Fechacalendario( lxVal )				this.Setear_Fechaegreso( lxVal )			EndIf		Else			This.Setear_Fechaegreso( lxVal )		EndIf		dodefault( lxVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function objetivomensual_assign( txval as variant ) as void                                                                                                                                                                                               
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'VENDEDOR' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'objetivomensual_assign', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'objetivomensual_assign', lcParametros )			endif		endif		Local lxValOld As Variant		lxValOld = this.Objetivomensual		local lxVal as Variant		lxVal = txVal		if This.CargaManual()			if this.Validar_Objetivomensual( lxVal )				this.Setear_Objetivomensual( lxVal )			EndIf		Else			This.Setear_Objetivomensual( lxVal )		EndIf		dodefault( lxVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function setear_comision( txval as variant ) as void                                                                                                                                                                                                      
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'VENDEDOR' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'setear_comision', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'setear_comision', lcParametros )			endif		endif		this.Comision = txVal		dodefault( txVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function setear_descmaxdetallefacturas( txval as variant ) as void                                                                                                                                                                                        
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'VENDEDOR' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'setear_descmaxdetallefacturas', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'setear_descmaxdetallefacturas', lcParametros )			endif		endif		this.Descmaxdetallefacturas = txVal		dodefault( txVal )
	EndFunc

	*-----------------------------------------------------------------------------------------
	function setear_descmaxtotalfacturas( txval as variant ) as void                                                                                                                                                                                          
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'VENDEDOR' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'setear_descmaxtotalfacturas', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'setear_descmaxtotalfacturas', lcParametros )			endif		endif		this.Descmaxtotalfacturas = txVal		dodefault( txVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function setear_fechaegreso( txval as variant ) as void                                                                                                                                                                                                   
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'VENDEDOR' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'setear_fechaegreso', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'setear_fechaegreso', lcParametros )			endif		endif		this.Fechaegreso = txVal		dodefault( txVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function setear_objetivomensual( txval as variant ) as void                                                                                                                                                                                               
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'VENDEDOR' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'setear_objetivomensual', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'setear_objetivomensual', lcParametros )			endif		endif		this.Objetivomensual = txVal		dodefault( txVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function validar_comision( txval as variant ) as boolean                                                                                                                                                                                                  
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'VENDEDOR' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'validar_comision', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'validar_comision', lcParametros )			endif		endif		Return dodefault( txVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function validar_descmaxdetallefacturas( txval as variant ) as boolean                                                                                                                                                                                    
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'VENDEDOR' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'validar_descmaxdetallefacturas', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'validar_descmaxdetallefacturas', lcParametros )			endif		endif		Return dodefault( txVal )
	EndFunc

	*-----------------------------------------------------------------------------------------
	function validar_descmaxtotalfacturas( txval as variant ) as boolean                                                                                                                                                                                      
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'VENDEDOR' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'validar_descmaxtotalfacturas', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'validar_descmaxtotalfacturas', lcParametros )			endif		endif		Return dodefault( txVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function validar_fechaegreso( txval as variant ) as boolean                                                                                                                                                                                               
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'VENDEDOR' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'validar_fechaegreso', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'validar_fechaegreso', lcParametros )			endif		endif		Return dodefault( txVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function validar_objetivomensual( txval as variant ) as boolean                                                                                                                                                                                           
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'VENDEDOR' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'validar_objetivomensual', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'validar_objetivomensual', lcParametros )			endif		endif		Return dodefault( txVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function valorsugeridocomision() as void                                                                                                                                                                                                                  
		...
		return this.ObtenerResultado( 'Valorsugeridocomision', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function valorsugeridofechaegreso() as void                                                                                                                                                                                                               
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Valorsugeridofechaegreso', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function valorsugeridoobjetivomensual() as void                                                                                                                                                                                                           
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Valorsugeridoobjetivomensual', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	protected function cargaratributosvirtualesendetalle( todetalle as object ) as void                                                                                                                                                                       
		...
		return this.ObtenerResultado( 'Cargaratributosvirtualesendetalle', lcParametros )
	EndFunc
