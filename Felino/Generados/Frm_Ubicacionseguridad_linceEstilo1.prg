...
define class Frm_Ubicacionseguridad_linceEstilo1 as Ubicacionseguridad_lince of Ubicacionseguridad_lince.vcx
	ColControles = null
	...
	*-----------------------------------------------------------------------------------------
	...
	function SeteosVisuales() as boolean
		...
		************* ubicacionseguridad_lince/78d1cq8rw/Form
		...
		with this
			...
			.HEIGHT = 181
			.LEFT = 249
			.NAME = [ubicacionseguridad_lince]
			.TOP = 201
			.WIDTH = 509
			.WINDOWTYPE = 0

			************* Lblbase1/Lblbase/Label
			if type( '.Lblbase1' ) # 'O'
				.newobject( 'Lblbase1', 'LBLBASE', 'LBLBASE.VCX' )
			endif
			...
			************* txtRuta/Txtbox/Textbox
			if type( '.txtRuta' ) # 'O'
				.newobject( 'txtRuta', 'TXTBOX', 'TXTBASE.VCX' )
			endif
			...
			************* Cmdbase1/Cmdbase/Commandbutton
			if type( '.Cmdbase1' ) # 'O'
				.newobject( 'Cmdbase1', 'CMDBASE', 'CMDBASE.VCX' )
			endif
			...
			with .OBOTONERA
				...
				************* CMDACEPTAR/Botonconfoco/Commandbutton
				...
				with .CMDACEPTAR
					...
					.FONTBOLD = .F.
					.FONTITALIC = .F.
					.FONTNAME = [DIN MITTELSCHRIFT STD]
					.FONTSIZE = 14
					.FORECOLOR = 0
					.HEIGHT = 27
					.LEFT = 301
