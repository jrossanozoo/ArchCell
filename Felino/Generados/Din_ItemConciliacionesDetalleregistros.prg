...
define class Din_ItemCONCILIACIONESDetalleregistros as ItemActivo of ItemActivo.prg
	...
	*--------------------------------------------------------------------------------------------------------
	function Inicializar() as boolean
		...
		oICol = goServicios.SaltosDeCampoYValoresSugeridos.ObtenerValorSugeridoDeUnaEntidadDetalle( 'CONCILIACIONES', 'Detalleregistros')
	...
	function Registro_PK_Assign( txVal as variant ) as void
		...
		if this.Validar_Registro( lxVal, lxValOld )
			if ( this.lHabilitarRegistro_PK or this.lEstaSeteandoValorSugerido or lxValOld == lxVal )
				...
				goServicios.Errores.LevantarExcepcion( "No se puede cambiar este valor (Entidad: Conciliaciones - Atributo: Registro)" )
			endif 
		endif
		...
		if This.CargaManual()
			if this.Validar_Nroitem( lxVal )
				...
				this.EventoDespuesDeSetear( This, 'Nroitem', lxValOld, lxVal  )
					...
					goServicios.Errores.LevantarExcepcion( "No se puede cambiar este valor (Entidad: Conciliaciones - Atributo: Nroitem)" )
				endif 
			EndIf
		Else
			This.Setear_Nroitem( lxVal ) 
		...
		if This.CargaManual()
			if this.Validar_Fecha( lxVal )
				...
				this.EventoDespuesDeSetear( This, 'Fecha', lxValOld, lxVal  )
					...
					this.Fecha = lxValOld
					goServicios.Errores.LevantarExcepcion( "No se puede cambiar este valor (Entidad: Conciliaciones - Atributo: Fecha)" )
				endif 
			EndIf
		Else
			This.Setear_Fecha( lxVal ) 
		...
		if This.CargaManual()
			if this.Validar_Fechadeoperacion( lxVal )
				...
				this.EventoDespuesDeSetear( This, 'Fechadeoperacion', lxValOld, lxVal  )
					...
					goServicios.Errores.LevantarExcepcion( "No se puede cambiar este valor (Entidad: Conciliaciones - Atributo: Fechadeoperacion)" )
				endif 
			EndIf
		Else
			This.Setear_Fechadeoperacion( lxVal ) 
		...
		if This.CargaManual()
			if this.Validar_Comprobante( lxVal )
				...
				this.EventoDespuesDeSetear( This, 'Comprobante', lxValOld, lxVal  )
					...
					goServicios.Errores.LevantarExcepcion( "No se puede cambiar este valor (Entidad: Conciliaciones - Atributo: Comprobante)" )
				endif 
			EndIf
		Else
			This.Setear_Comprobante( lxVal ) 
		...
		if This.CargaManual()
			if this.Validar_Tipovalor( lxVal )
				...
				this.EventoDespuesDeSetear( This, 'Tipovalor', lxValOld, lxVal  )
					...
					goServicios.Errores.LevantarExcepcion( "No se puede cambiar este valor (Entidad: Conciliaciones - Atributo: Tipovalor)" )
				endif 
			EndIf
		Else
			This.Setear_Tipovalor( lxVal ) 
		...
		if This.CargaManual()
			if this.Validar_Importe( lxVal )
				...
				this.EventoDespuesDeSetear( This, 'Importe', lxValOld, lxVal  )
					...
					goServicios.Errores.LevantarExcepcion( "No se puede cambiar este valor (Entidad: Conciliaciones - Atributo: Importe)" )
				endif 
			EndIf
		Else
			This.Setear_Importe( lxVal ) 
	...
	*--------------------------------------------------------------------------------------------------------
	Function ValorSugeridoRegistro() as void
		...
		with this
			try
				lcExpresionValorSugeridoDefinidoPorElUsuario = goServicios.SaltosDeCampoYValoresSugeridos.ObtenerValorSugerido( "CONCILIACIONES", "Detalleregistros", "Registro" )
				if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
					if !empty( lcExpresionValorSugeridoDefinidoPorElUsuario )
						...
						This.EventoEstaSeteandoValorSugerido( 'Registro_PK' )
						.Registro_PK = lvValorSugeridoDefinidoPorElUsuario
						.Registro.CODIGO = .Registro_PK
					endif
				endif
			Catch to loError
				.Registro_PK=[]
				...
				if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
					...
					loEx.AgregarInformacion( "Se produjo un error al intentar asignar el valor sugerido " + lcExpresionValorSugeridoDefinidoPorElUsuario + " del atributo Registro para el detalle Registros conciliables." + chr( 10 ) + "Verifique el valor especificado en la entidad Comportamiento de atributos." + chr( 10 ) + "El tipo de dato del atributo es caracter." )
	...
	*--------------------------------------------------------------------------------------------------------
	function ObtenerNombre() as String
		return [ITEMREGCONCILIABLES]
	endfunc

	*--------------------------------------------------------------------------------------------------------
	function PuedeEjecutarCambioSumarizado() as boolean
		local llRetorno as Boolean
		llRetorno = !empty(this.REGISTRO_PK) and dodefault()
		return llRetorno
	endfunc
