...
define class Mock_expoagrupadas as MockBase of MockBase.prg
	...
	cCaracteres = ""                                                                                                                                                                                                                                          
	cContexto = ""                                                                                                                                                                                                                                            
	cDescripcion =  "Configuración de exportaciones agrupadas"                                                                                                                                                                                                
	...
	cPrefijoRecibir = ""                                                                                                                                                                                                                                      
	cSufijoHooks = ""                                                                                                                                                                                                                                         
	cSufijoHooks = "ExportacionAgrupada"                                                                                                                                                                                                                      
	...
	*-----------------------------------------------------------------------------------------
	function cambiosdetalledisenos() as void                                                                                                                                                                                                                  
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Cambiosdetalledisenos', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function disenos_access() as variant                                                                                                                                                                                                                      
		lnParametros = pcount()lcParametros = ''		lnIdClase = _screen.Mocks.BuscarMock( 'EXPOAGRUPADAS' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'disenos_access', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'disenos_access', lcParametros )			endif		endif		if !this.ldestroy and ( !vartype( this.Disenos ) = 'O' or isnull( this.Disenos ) )			this.Disenos = this.crearobjeto( 'Din_DetalleExpoagrupadasDisenos' )			this.Disenos.inicializar()			this.enlazar( 'Disenos.EventoObtenerLogueo', 'inyectarLogueo' )			this.enlazar( 'Disenos.EventoObtenerInformacion', 'inyectarInformacion' )			bindevent( this.Disenos, 'CambioSumarizado', this, 'CambiosDetalleDisenos', 1)			this.enlazar( 'Disenos.EventoAdvertirLimitePorDiseno', 'EventoAdvertirLimitePorDiseno')			this.enlazar( 'Disenos.EventoCancelarCargaLimitePorDiseno', 'EventoCancelarCargaLimitePorDiseno')		endif		return this.Disenos
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function filtrafecha_assign( txval as variant ) as void                                                                                                                                                                                                   
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'EXPOAGRUPADAS' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'filtrafecha_assign', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'filtrafecha_assign', lcParametros )			endif		endif		Local lxValOld As Variant		lxValOld = this.Filtrafecha		local lxVal as Variant		lxVal = txVal		if This.CargaManual()			if this.Validar_Filtrafecha( lxVal )				this.Setear_Filtrafecha( lxVal )			EndIf		Else			This.Setear_Filtrafecha( lxVal )		EndIf		dodefault( lxVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function setear_filtrafecha( txval as variant ) as void                                                                                                                                                                                                   
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'EXPOAGRUPADAS' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'setear_filtrafecha', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'setear_filtrafecha', lcParametros )			endif		endif		this.Filtrafecha = txVal		dodefault( txVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function validar_filtrafecha( txval as variant ) as boolean                                                                                                                                                                                               
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer, ;		lnIdMetodo as Integer, lnIdClase as Integer, lxParam1lnParametros = pcount()lxParam1 = txvallcParametros = ''		for lni = 1 to lnParametros			lcParam = 'lxParam' + alltrim(str(lni))			lcParametros = lcParametros + iif( !empty(lcParametros),',','') + iif( vartype( &lcParam ) = 'O' or isnull(&lcParam), "'*OBJETO'", goLibrerias.ValorAString( &lcParam ) )		endfor		lnIdClase = _screen.Mocks.BuscarMock( 'EXPOAGRUPADAS' )		if lnIdClase > 0			lnIdMetodo =  _screen.Mocks.BuscarMetodo( lnIdClase, 'validar_filtrafecha', lcParametros )			if lnIdMetodo > 0				return this.ObtenerResultado( 'validar_filtrafecha', lcParametros )			endif		endif		Return dodefault( txVal )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	function valorsugeridofiltrafecha() as void                                                                                                                                                                                                               
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		...
		return this.ObtenerResultado( 'Valorsugeridofiltrafecha', lcParametros )
	EndFunc
	...
	*-----------------------------------------------------------------------------------------
	protected function obteneritem( tccursor as string ) as itemarbol                                                                                                                                                                                         
		local lcParametros as string, lcParam as string, lni as integer, lnParametros as integer
		local lxParam1
