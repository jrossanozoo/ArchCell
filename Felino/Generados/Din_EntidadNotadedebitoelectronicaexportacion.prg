...
define class Din_EntidadNOTADEDEBITOELECTRONICAEXPORTACION as Ent_ComprobanteElectronicoExportacion of Ent_ComprobanteElectronicoExportacion.prg
	...
	cDescripcion =  [Nota de débito electrónica exportación de venta]
	lEntidadEditable =  .t.
	lTieneVendedorComoClaveForanea = .t.
	cAtributoVendedor = 'VENDEDOR_PK'
	oAtributosCC =  Null
	oCompFacturacionElectronicaExportacion = null
	...
	*--------------------------------------------------------------------------------------------------------
	function oAD_Access() as variant
		if !this.ldestroy and ( !vartype( this.oAD ) = 'O' or isnull( this.oAD ) )
			this.oAD = this.crearobjeto( 'Din_EntidadNOTADEDEBITOELECTRONICAEXPORTACIONAD_' + alltrim( _screen.zoo.app.TipoDeBase ))
	...
	*--------------------------------------------------------------------------------------------------------
	function Impuestosdetalle_Access() as variant
		if !this.ldestroy and ( !vartype( this.Impuestosdetalle ) = 'O' or isnull( this.Impuestosdetalle ) )
			this.Impuestosdetalle = this.crearobjeto( 'Din_DetalleNotadedebitoelectronicaexportacionImpuestosdetalle' )
	...
	*--------------------------------------------------------------------------------------------------------
	function Compafec_Access() as variant
		if !this.ldestroy and ( !vartype( this.Compafec ) = 'O' or isnull( this.Compafec ) )
			this.Compafec = this.crearobjeto( 'Din_DetalleNotadedebitoelectronicaexportacionCompafec' )
	...
	*--------------------------------------------------------------------------------------------------------
	function Impuestoscomprobante_Access() as variant
		if !this.ldestroy and ( !vartype( this.Impuestoscomprobante ) = 'O' or isnull( this.Impuestoscomprobante ) )
			this.Impuestoscomprobante = this.crearobjeto( 'Din_DetalleNotadedebitoelectronicaexportacionImpuestoscomprobante' )
	...
	*--------------------------------------------------------------------------------------------------------
	function Facturadetalle_Access() as variant
		if !this.ldestroy and ( !vartype( this.Facturadetalle ) = 'O' or isnull( this.Facturadetalle ) )
			this.Facturadetalle = this.crearobjeto( 'Din_DetalleNotadedebitoelectronicaexportacionFacturadetalle' )
	...
	*--------------------------------------------------------------------------------------------------------
	function Valoresdetalle_Access() as variant
		if !this.ldestroy and ( !vartype( this.Valoresdetalle ) = 'O' or isnull( this.Valoresdetalle ) )
			this.Valoresdetalle = this.crearobjeto( 'Din_DetalleNotadedebitoelectronicaexportacionValoresdetalle' )
	...
	*--------------------------------------------------------------------------------------------------------
	function Articulosseniadosdetalle_Access() as variant
		if !this.ldestroy and ( !vartype( this.Articulosseniadosdetalle ) = 'O' or isnull( this.Articulosseniadosdetalle ) )
			this.Articulosseniadosdetalle = this.crearobjeto( 'Din_DetalleNotadedebitoelectronicaexportacionArticulosseniadosdetalle' )
	...
	*--------------------------------------------------------------------------------------------------------
	...
	local lnLargoDecimal as integer, lnLargoEntero as Integer, lnLargoDecimal as integer, llRetorno as boolean 
		...
		if ( vartype( this.NoCalculaPercepcion) = 'L' )
		else
			This.AgregarInformacion( 'El tipo de dato para el atributo NOCALCULAPERCEPCION no es el correcto.')
			llRetorno = .F.
		Endif
		...
		if ( vartype( this.MonedaComprobante) = 'C' )
			if len( alltrim( this.MONEDACOMPROBANTE ) ) > 10
				This.AgregarInformacion( 'La longitud del valor del atributo MONEDACOMPROBANTE es mayor a la permitida.' )
				llRetorno = .F.
			Endif
		else
			This.AgregarInformacion( 'El tipo de dato para el atributo MONEDACOMPROBANTE no es el correcto.')
			llRetorno = .F.
		Endif
		...
		if ( vartype( this.COTIZACIONDESPACHO) = 'N' )
			lnLargoEntero  = len( transform( int( this.COTIZACIONDESPACHO ) ) ) 
			lnValorDelDecimal = this.COTIZACIONDESPACHO - int( this.COTIZACIONDESPACHO)
			lnLargoDecimal = iif( lnValorDelDecimal = 0, 0, len( transform( lnValorDelDecimal ) ) - 2 )
			if lnLargoEntero > 11
				This.AgregarInformacion( 'La longitud entera del valor del atributo COTIZACIONDESPACHO es mayor a la permitida.' )
				llRetorno = .F.
			Endif
		else
			This.AgregarInformacion( 'El tipo de dato para el atributo COTIZACIONDESPACHO no es el correcto.')
			llRetorno = .F.
		Endif
	...
	*--------------------------------------------------------------------------------------------------------
	function VotacionCambioEstadoNUEVO( tcEstado as string ) as Boolean
		*!*Se ejecuta antes de hacer el cambio de estado
		...
		llVotacion = llVotacion and this.oCompFacturacionElectronicaExportacion.votarCambioEstadoNUEVO( tcEstado )
	...
	*--------------------------------------------------------------------------------------------------------
	function VotacionCambioEstadoELIMINAR( tcEstado as string ) as Boolean
		*!*Se ejecuta antes de hacer el cambio de estado
		local llVotacion as boolean
		llVotacion = dodefault()
		llVotacion = llVotacion and this.oCompFacturacionElectronicaExportacion.votarCambioEstadoELIMINAR( tcEstado )
	...
	*--------------------------------------------------------------------------------------------------------
	function VotacionCambioEstadoMODIFICAR( tcEstado as string ) as Boolean
		*!*Se ejecuta antes de hacer el cambio de estado
		local llVotacion as boolean
		llVotacion = dodefault()
		llVotacion = llVotacion and this.oCompFacturacionElectronicaExportacion.votarCambioEstadoMODIFICAR( tcEstado )
	...
	*--------------------------------------------------------------------------------------------------------
	function VotacionCambioEstadoCANCELAR( tcEstado as string ) as Boolean
		*!*Se ejecuta antes de hacer el cambio de estado
		local llVotacion as boolean
		llVotacion = dodefault()
		llVotacion = llVotacion and this.oCompFacturacionElectronicaExportacion.votarCambioEstadoCANCELAR( tcEstado )
	...
	*--------------------------------------------------------------------------------------------------------
	function VotacionCambioEstadoGRABAR( tcEstado as string ) as Boolean
		*!*Se ejecuta antes de hacer el cambio de estado
		local llVotacion as boolean
		llVotacion = dodefault()
		llVotacion = llVotacion and this.oCompFacturacionElectronicaExportacion.votarCambioEstadoGRABAR( tcEstado )
	...
	*--------------------------------------------------------------------------------------------------------
	function VotacionCambioEstadoANULAR( tcEstado as string ) as Boolean
		*!*Se ejecuta antes de hacer el cambio de estado
		local llVotacion as boolean
		llVotacion = dodefault()
		llVotacion = llVotacion and this.oCompFacturacionElectronicaExportacion.votarCambioEstadoANULAR( tcEstado )
	...
	*--------------------------------------------------------------------------------------------------------
	function ValidacionBasica() as boolean
		...
		With This
			...
			llRetorno = .ValidarIncoterms() and llRetorno
			llRetorno = .ValidarListadeprecios() and llRetorno
			llRetorno = .ValidarCotizaciondespacho() and llRetorno
	...
	*--------------------------------------------------------------------------------------------------------
	function ActualizarEstado() as void
		...
		this.oCompFacturacionElectronicaExportacion.lNuevo = this.lNuevo
		this.oCompFacturacionElectronicaExportacion.lEdicion = this.lEdicion
		this.oCompFacturacionElectronicaExportacion.lEliminar = this.lEliminar
	...
	*--------------------------------------------------------------------------------------------------------
	function ValidarCOTIZACIONDESPACHO() as boolean
		local llRetorno as boolean
		llRetorno = dodefault()
		if empty( this.COTIZACIONDESPACHO )
			this.AgregarInformacion( 'Debe cargar el campo Cotización despacho', 9005, 'COTIZACIONDESPACHO' )
			llRetorno = .F.
		endif
		return llRetorno
	endfunc
	...
	*--------------------------------------------------------------------------------------------------------
	function ValidarIncoterms() as boolean
		local llRetorno as boolean
		llRetorno = dodefault()
		if empty( this.Incoterms_PK )
			this.AgregarInformacion( 'Debe cargar el campo Incoterms', 9005, 'Incoterms' )
			llRetorno = .F.
		else
			If this.Incoterms.oAD.ConsultarPorClavePrimaria()
			Else
				this.AgregarInformacion( 'El dato buscado ' + alltrim(transform( this.Incoterms_PK ))+ ' de la entidad ' +  upper( this.Incoterms.cDescripcion ) + ' no existe.')
				llRetorno = .F.
			endif
		endif
		Return llRetorno
	...
	*--------------------------------------------------------------------------------------------------------
	function obtenerAtributosObligatorios() as ZooColeccion
		...
		loAtributosObligatorios.add( 'INCOTERMS' )
		loAtributosObligatorios.add( 'LISTADEPRECIOS' )
		loAtributosObligatorios.add( 'COTIZACIONDESPACHO' )
	...
	*--------------------------------------------------------------------------------------------------------
	function SetearColeccionSentenciasAnterior_NUEVO() as Void
		...
		dodefault()
		this.oCompFacturacionElectronicaExportacion.SetearColeccionSentenciasAnterior_NUEVO()
	...
	*--------------------------------------------------------------------------------------------------------
	function SetearColeccionSentenciasAnterior_MODIFICAR() as Void
		...
		dodefault()
		this.oCompFacturacionElectronicaExportacion.SetearColeccionSentenciasAnterior_MODIFICAR()
	...
	*--------------------------------------------------------------------------------------------------------
	function SetearColeccionSentenciasAnterior_ANULAR() as Void
		...
		dodefault()
		this.oCompFacturacionElectronicaExportacion.SetearColeccionSentenciasAnterior_ANULAR()
	...
	*--------------------------------------------------------------------------------------------------------
	function SetearColeccionSentenciasAnterior_ELIMINAR() as Void
		...
		dodefault()
		this.oCompFacturacionElectronicaExportacion.SetearColeccionSentenciasAnterior_ELIMINAR()
	...
	*--------------------------------------------------------------------------------------------------------
	function ObtenerFuncionalidades() as String
		return '<ANULABLE><VENTAS><CONVALORES><FISCAL><CE><VALORCIERRE><DESCUENTOAUTOMATICO><ACTUALIZAEMAILCLIENTE>'
	Endfunc
	*--------------------------------------------------------------------------------------------------------
	...
	*--------------------------------------------------------------------------------------------------------
	function Inicializar() as Void
		...
		this.oProveedorAtributosAnulacion = newobject( "din_entidadNOTADEDEBITOELECTRONICAEXPORTACIONAtributosAnulacion", "din_entidadNOTADEDEBITOELECTRONICAEXPORTACIONAtributosAnulacion.prg" )
		...
		this.oColeccionVSFW.agregar('.ValorSugeridoFWIncoterms()','Incoterms')
		...
		oICol = goServicios.SaltosDeCampoYValoresSugeridos.ObtenerValorSugeridoDeUnaEntidadDetalle( 'NOTADEDEBITOELECTRONICAEXPORTACION', '')
	...
	function Caja_PK_Assign( txVal as variant ) as void
		...
		if this.Validar_Caja( lxVal, lxValOld )
			if ( this.lHabilitarCaja_PK or this.lEstaSeteandoValorSugerido or lxValOld == lxVal )
				...
				goServicios.Errores.LevantarExcepcion( "No se puede cambiar este valor (Entidad: Nota de débito electrónica exportación de venta - Atributo: Caja)" )
			endif 
		endif
		dodefault( lxVal )
	...
	function Cliente_PK_Assign( txVal as variant ) as void
		...
		if this.Validar_Cliente( lxVal, lxValOld )
			...
			If lxValOld != lxVal or empty( this.Clientedescripcion ) 
				...
				goServicios.Errores.LevantarExcepcion( "No se puede cambiar este valor (Entidad: Nota de débito electrónica exportación de venta - Atributo: Cliente)" )
			endif 
	...
	*--------------------------------------------------------------------------------------------------------
	function Incoterms_PK_Assign( txVal as variant ) as void

		Local lxValOld As Variant
		lxValOld = this.Incoterms_PK
		local lxVal as Variant
		local loSugerido as Object
		
		lxVal = txVal
		lxVal = iif( len( lxVal ) < 3, padr( lxVal, 3 ), lxVal )
		this.Incoterms_PK = lxVal

		if this.Validar_Incoterms( lxVal, lxValOld )
			This.Setear_Incoterms( lxVal )
		endif
		dodefault( lxVal )
	...
	function Listadeprecios_PK_Assign( txVal as variant ) as void
		...
		if this.Validar_Listadeprecios( lxVal, lxValOld )
			...
			If lxValOld != lxVal or empty( this.Condicioniva ) 
				...
				goServicios.Errores.LevantarExcepcion( "No se puede cambiar este valor (Entidad: Nota de débito electrónica exportación de venta - Atributo: Listadeprecios)" )
			endif 
		endif
		dodefault( lxVal )
	...
	*--------------------------------------------------------------------------------------------------------
	function Setear_Incoterms( txVal as variant ) as void

		this.Incoterms.CODIGO = txVal
		dodefault( txVal )
	...
	*--------------------------------------------------------------------------------------------------------
	function Validar_Incoterms( txVal as variant, txValOld as variant ) as Boolean
	...
	*--------------------------------------------------------------------------------------------------------
	function Nocalculapercepcion_Assign( txVal as variant ) as void

		Local lxValOld As Variant
		lxValOld = this.Nocalculapercepcion
		local lxVal as Variant
		lxVal = txVal

		if This.CargaManual()
			if this.Validar_Nocalculapercepcion( lxVal )
				this.Setear_Nocalculapercepcion( lxVal )
			EndIf
		Else
			This.Setear_Nocalculapercepcion( lxVal ) 
		EndIf
	...
	*--------------------------------------------------------------------------------------------------------
	function Monedacomprobante_Assign( txVal as variant ) as void

		Local lxValOld As Variant
		lxValOld = this.Monedacomprobante
		local lxVal as Variant
		lxVal = txVal

		if This.CargaManual()
			if this.Validar_Monedacomprobante( lxVal )
		...
		if This.CargaManual()
			if this.Validar_Fechavtosenia( lxVal )
				if ( this.lHabilitarFechavtosenia or this.lEstaSeteandoValorSugerido or lxValOld == lxVal)
					...
					goServicios.Errores.LevantarExcepcion( "No se puede cambiar este valor (Entidad: Nota de débito electrónica exportación de venta - Atributo: Fechavtosenia)" )
				endif 
			EndIf
		Else
			This.Setear_Fechavtosenia( lxVal ) 
	...
	*--------------------------------------------------------------------------------------------------------
	function Cotizaciondespacho_Assign( txVal as variant ) as void

		Local lxValOld As Variant
		lxValOld = this.Cotizaciondespacho
		local lxVal as Variant
		lxVal = txVal

		if This.CargaManual()
			if this.Validar_Cotizaciondespacho( lxVal )
				this.ValidarDominio_Numericononegativo( lxVal )
				this.Setear_Cotizaciondespacho( lxVal )
			EndIf
		Else
			This.Setear_Cotizaciondespacho( lxVal ) 
		EndIf
	...
	*--------------------------------------------------------------------------------------------------------
	function Setear_Nocalculapercepcion( txVal as variant ) as void

		this.Nocalculapercepcion = txVal
		dodefault( txVal )
	...
	*--------------------------------------------------------------------------------------------------------
	function Setear_Cotizaciondespacho( txVal as variant ) as void

		this.Cotizaciondespacho = txVal
		dodefault( txVal )
	...
	*--------------------------------------------------------------------------------------------------------
	function Validar_Nocalculapercepcion( txVal as variant ) as Boolean
	...
	*--------------------------------------------------------------------------------------------------------
	function Validar_Monedacomprobante( txVal as variant ) as Boolean
	...
	*--------------------------------------------------------------------------------------------------------
	function Validar_Cotizaciondespacho( txVal as variant ) as Boolean
	...
	*--------------------------------------------------------------------------------------------------------
	function Incoterms_Access() as variant
		if this.ldestroy
		else
			if This.lInstanciarSubEntidadaDemanda
				if ( !vartype( this.Incoterms ) = 'O' or isnull( this.Incoterms ) )
					this.Incoterms = _Screen.zoo.instanciarentidad( 'Incoterms' )
					this.Incoterms.lEsSubEntidad = .t.
					this.enlazar( 'Incoterms.EventoObtenerLogueo', 'inyectarLogueo' )
					this.enlazar( 'Incoterms.EventoObtenerInformacion', 'inyectarInformacion' )
				endif
				if !this.lBuscandoCodigo
					if this.Incoterms.CODIGO # this.Incoterms_PK
						this.Incoterms.CODIGO = this.Incoterms_PK
					endif
				endif
			endif
		endif
	...
	*--------------------------------------------------------------------------------------------------------
	function oCompFacturacionelectronicaexportacion_Access() as variant
		if !this.ldestroy and ( !vartype( this.oCompFacturacionelectronicaexportacion ) = 'O' or isnull( this.oCompFacturacionelectronicaexportacion ) )
			this.oCompFacturacionelectronicaexportacion = _Screen.zoo.InstanciarComponente( 'ComponenteFacturacionelectronicaexportacion' )
		this.oCompFacturacionelectronicaexportacion.Inicializar()
		this.enlazar( 'oCompFacturacionelectronicaexportacion.EventoObtenerLogueo', 'inyectarLogueo' )
		this.enlazar( 'oCompFacturacionelectronicaexportacion.EventoObtenerInformacion', 'inyectarInformacion' )
		endif
		return this.oCompFacturacionelectronicaexportacion
	endfunc
	...
	*--------------------------------------------------------------------------------------------------------
	Function ValorSugeridoFecha() as void
		...
		with this
			lcExpresionValorSugeridoDefinidoPorElUsuario = goServicios.SaltosDeCampoYValoresSugeridos.ObtenerValorSugerido( "NOTADEDEBITOELECTRONICAEXPORTACION", "", "Fecha" )
	...
	*--------------------------------------------------------------------------------------------------------
	Function ValorSugeridoCliente() as void
		...
		with this
			try
				lcExpresionValorSugeridoDefinidoPorElUsuario = goServicios.SaltosDeCampoYValoresSugeridos.ObtenerValorSugerido( "NOTADEDEBITOELECTRONICAEXPORTACION", "", "Cliente" )
				if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
					if !empty( lcExpresionValorSugeridoDefinidoPorElUsuario )
						lvValorSugeridoDefinidoPorElUsuario = &lcExpresionValorSugeridoDefinidoPorElUsuario
	...
	*--------------------------------------------------------------------------------------------------------
	Function ValorSugeridoObs() as void
		...
		lcExpresionValorSugeridoDefinidoPorElUsuario = null
		with this
			lcExpresionValorSugeridoDefinidoPorElUsuario = goServicios.SaltosDeCampoYValoresSugeridos.ObtenerValorSugerido( "NOTADEDEBITOELECTRONICAEXPORTACION", "", "Obs" )
			if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
				if !empty( lcExpresionValorSugeridoDefinidoPorElUsuario )
					try
						lvValorSugeridoDefinidoPorElUsuario = &lcExpresionValorSugeridoDefinidoPorElUsuario
	...
	*--------------------------------------------------------------------------------------------------------
	Function ValorSugeridoVendedor() as void
		...
		with this
			try
				lcExpresionValorSugeridoDefinidoPorElUsuario = goServicios.SaltosDeCampoYValoresSugeridos.ObtenerValorSugerido( "NOTADEDEBITOELECTRONICAEXPORTACION", "", "Vendedor" )
				if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
					if !empty( lcExpresionValorSugeridoDefinidoPorElUsuario )
						lvValorSugeridoDefinidoPorElUsuario = &lcExpresionValorSugeridoDefinidoPorElUsuario
						if vartype( lvValorSugeridoDefinidoPorElUsuario ) # "C"
	...
	*--------------------------------------------------------------------------------------------------------
	Function ValorSugeridoLetra() as void
		...
		lcExpresionValorSugeridoDefinidoPorElUsuario = null
		with this
			lcExpresionValorSugeridoDefinidoPorElUsuario = goServicios.SaltosDeCampoYValoresSugeridos.ObtenerValorSugerido( "NOTADEDEBITOELECTRONICAEXPORTACION", "", "Letra" )
			if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
				if !empty( lcExpresionValorSugeridoDefinidoPorElUsuario )
					try
						lvValorSugeridoDefinidoPorElUsuario = &lcExpresionValorSugeridoDefinidoPorElUsuario
	...
	*--------------------------------------------------------------------------------------------------------
	Function ValorSugeridoClientedescripcion() as void
		...
		lcExpresionValorSugeridoDefinidoPorElUsuario = null
		with this
			lcExpresionValorSugeridoDefinidoPorElUsuario = goServicios.SaltosDeCampoYValoresSugeridos.ObtenerValorSugerido( "NOTADEDEBITOELECTRONICAEXPORTACION", "", "Clientedescripcion" )
			if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
				if !empty( lcExpresionValorSugeridoDefinidoPorElUsuario )
					try
						lvValorSugeridoDefinidoPorElUsuario = &lcExpresionValorSugeridoDefinidoPorElUsuario
	...
	*--------------------------------------------------------------------------------------------------------
	Function ValorSugeridoPuntodeventa() as void
		...
		lcExpresionValorSugeridoDefinidoPorElUsuario = null
		with this
			lcExpresionValorSugeridoDefinidoPorElUsuario = goServicios.SaltosDeCampoYValoresSugeridos.ObtenerValorSugerido( "NOTADEDEBITOELECTRONICAEXPORTACION", "", "Puntodeventa" )
			if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
				if !empty( lcExpresionValorSugeridoDefinidoPorElUsuario )
					try
						lvValorSugeridoDefinidoPorElUsuario = &lcExpresionValorSugeridoDefinidoPorElUsuario
	...
	*--------------------------------------------------------------------------------------------------------
	Function ValorSugeridoNumero() as void
		...
		lcExpresionValorSugeridoDefinidoPorElUsuario = null
		with this
			lcExpresionValorSugeridoDefinidoPorElUsuario = goServicios.SaltosDeCampoYValoresSugeridos.ObtenerValorSugerido( "NOTADEDEBITOELECTRONICAEXPORTACION", "", "Numero" )
			if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
				if !empty( lcExpresionValorSugeridoDefinidoPorElUsuario )
					try
						lvValorSugeridoDefinidoPorElUsuario = &lcExpresionValorSugeridoDefinidoPorElUsuario
	...
	*--------------------------------------------------------------------------------------------------------
	Function ValorSugeridoIncoterms() as void
		local lcExpresionValorSugeridoDefinidoPorElUsuario as String, lvValorSugeridoDefinidoPorElUsuario as Variant, loError as Object, loEx as Object, loExTipoDato as Object
		lcExpresionValorSugeridoDefinidoPorElUsuario = null
		with this
			try
				lcExpresionValorSugeridoDefinidoPorElUsuario = goServicios.SaltosDeCampoYValoresSugeridos.ObtenerValorSugerido( "NOTADEDEBITOELECTRONICAEXPORTACION", "", "Incoterms" )
				if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
					if !empty( lcExpresionValorSugeridoDefinidoPorElUsuario )
						lvValorSugeridoDefinidoPorElUsuario = &lcExpresionValorSugeridoDefinidoPorElUsuario
						if vartype( lvValorSugeridoDefinidoPorElUsuario ) # "C"
						...
						This.EventoEstaSeteandoValorSugerido( 'Incoterms_PK' )
						.Incoterms_PK = lvValorSugeridoDefinidoPorElUsuario
						.Incoterms.CODIGO = .Incoterms_PK
					endif
				else
					.ValorSugeridoFWIncoterms()
				endif
			Catch to loError
				.Incoterms_PK=[]
				loEx = newobject("zooexception", "zooexception.prg" )
				...
				if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
					...
					loEx.AgregarInformacion( "Se produjo un error al intentar asignar el valor sugerido " + lcExpresionValorSugeridoDefinidoPorElUsuario + " del atributo Incoterms para la entidad Nota de débito electrónica exportación." + chr( 10 ) + "Verifique el valor especificado en la entidad Comportamiento de atributos." + chr( 10 ) + "El tipo de dato del atributo es caracter." )
					goServicios.Errores.LevantarExcepcion( loEx )
				endif
			endtry 
		endwith
	...
	*--------------------------------------------------------------------------------------------------------
	Function ValorSugeridoFWIncoterms() as void
		with this
			try
				.Incoterms_PK = goParametros.Felino.GestionDeVentas.FacturacionElectronica.Exportacion.Incoterms
				.Incoterms.CODIGO = .Incoterms_PK
			Catch to loError
				.Incoterms_PK=[]
				loEx = newobject("zooexception", "zooexception.prg" )
	...
	*--------------------------------------------------------------------------------------------------------
	Function ValorSugeridoFWMonedacomprobante() as void
		with this
			.Monedacomprobante = goParametros.Felino.GestionDeVentas.FacturacionElectronica.Exportacion.CodigoDeMonedaExtranjeraSugerida
		endwith
	endfunc
	...
	*--------------------------------------------------------------------------------------------------------
	Function ValorSugeridoListadeprecios() as void
		...
		with this
			try
				lcExpresionValorSugeridoDefinidoPorElUsuario = goServicios.SaltosDeCampoYValoresSugeridos.ObtenerValorSugerido( "NOTADEDEBITOELECTRONICAEXPORTACION", "", "Listadeprecios" )
				if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
					if !empty( lcExpresionValorSugeridoDefinidoPorElUsuario )
						lvValorSugeridoDefinidoPorElUsuario = &lcExpresionValorSugeridoDefinidoPorElUsuario
						if vartype( lvValorSugeridoDefinidoPorElUsuario ) # "C"
	...
	*--------------------------------------------------------------------------------------------------------
	Function ValorSugeridoFWListadeprecios() as void
		with this
			try
				.Listadeprecios_PK = goParametros.Felino.GestionDeVentas.FacturacionElectronica.Exportacion.ListaDePreciosSugerida
				.Listadeprecios.CODIGO = .Listadeprecios_PK
			Catch to loError
				.Listadeprecios_PK=[]
				loEx = newobject("zooexception", "zooexception.prg" )
	...
	*--------------------------------------------------------------------------------------------------------
	Function ValorSugeridoEmail() as void
		...
		lcExpresionValorSugeridoDefinidoPorElUsuario = null
		with this
			lcExpresionValorSugeridoDefinidoPorElUsuario = goServicios.SaltosDeCampoYValoresSugeridos.ObtenerValorSugerido( "NOTADEDEBITOELECTRONICAEXPORTACION", "", "Email" )
			if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
				if !empty( lcExpresionValorSugeridoDefinidoPorElUsuario )
					try
						lvValorSugeridoDefinidoPorElUsuario = &lcExpresionValorSugeridoDefinidoPorElUsuario
	...
	*--------------------------------------------------------------------------------------------------------
	Function ValorSugeridoFechavtosenia() as void
		...
		lcExpresionValorSugeridoDefinidoPorElUsuario = null
		with this
			lcExpresionValorSugeridoDefinidoPorElUsuario = goServicios.SaltosDeCampoYValoresSugeridos.ObtenerValorSugerido( "NOTADEDEBITOELECTRONICAEXPORTACION", "", "Fechavtosenia" )
			if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
				if !empty( lcExpresionValorSugeridoDefinidoPorElUsuario )
					try
						lvValorSugeridoDefinidoPorElUsuario = &lcExpresionValorSugeridoDefinidoPorElUsuario
	...
	*--------------------------------------------------------------------------------------------------------
	Function ValorSugeridoPorcentajedescuento() as void
		...
		lcExpresionValorSugeridoDefinidoPorElUsuario = null
		with this
			lcExpresionValorSugeridoDefinidoPorElUsuario = goServicios.SaltosDeCampoYValoresSugeridos.ObtenerValorSugerido( "NOTADEDEBITOELECTRONICAEXPORTACION", "", "Porcentajedescuento" )
			if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
				if !empty( lcExpresionValorSugeridoDefinidoPorElUsuario )
					try
						lvValorSugeridoDefinidoPorElUsuario = &lcExpresionValorSugeridoDefinidoPorElUsuario
	...
	*--------------------------------------------------------------------------------------------------------
	Function ValorSugeridoMontodescuento3() as void
		...
		lcExpresionValorSugeridoDefinidoPorElUsuario = null
		with this
			lcExpresionValorSugeridoDefinidoPorElUsuario = goServicios.SaltosDeCampoYValoresSugeridos.ObtenerValorSugerido( "NOTADEDEBITOELECTRONICAEXPORTACION", "", "Montodescuento3" )
			if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
				if !empty( lcExpresionValorSugeridoDefinidoPorElUsuario )
					try
						lvValorSugeridoDefinidoPorElUsuario = &lcExpresionValorSugeridoDefinidoPorElUsuario
	...
	*--------------------------------------------------------------------------------------------------------
	Function ValorSugeridoCotizaciondespacho() as void
		local lcExpresionValorSugeridoDefinidoPorElUsuario as String, lvValorSugeridoDefinidoPorElUsuario as Variant, loError as Object, loEx as Object, loExTipoDato as Object
		lcExpresionValorSugeridoDefinidoPorElUsuario = null
		with this
			lcExpresionValorSugeridoDefinidoPorElUsuario = goServicios.SaltosDeCampoYValoresSugeridos.ObtenerValorSugerido( "NOTADEDEBITOELECTRONICAEXPORTACION", "", "Cotizaciondespacho" )
			if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
				if !empty( lcExpresionValorSugeridoDefinidoPorElUsuario )
					try
						lvValorSugeridoDefinidoPorElUsuario = &lcExpresionValorSugeridoDefinidoPorElUsuario
						...
						This.EventoEstaSeteandoValorSugerido( 'Cotizaciondespacho' )
						.Cotizaciondespacho = lvValorSugeridoDefinidoPorElUsuario
					catch to loError
						loEx = newobject("zooexception", "zooexception.prg" )
						loEx.Grabar( loError )
						loEx.AgregarInformacion( iif( type( "loError.UserValue.Message" ) == "C", loError.UserValue.Message, loError.Message ) )
						loEx.AgregarInformacion( "Se produjo un error al intentar asignar el valor sugerido " + lcExpresionValorSugeridoDefinidoPorElUsuario + " del atributo Cotización despacho para la entidad Nota de débito electrónica exportación." + chr( 10 ) + "Verifique el valor especificado en la entidad Comportamiento de atributos." + chr( 10 ) + "El tipo de dato del atributo es numérico." )
						goServicios.Errores.LevantarExcepcion( loEx )
					endtry
				endif
			endif
	...
	*--------------------------------------------------------------------------------------------------------
	Function ValorSugeridoTipocpterelacionado() as void
		...
		lcExpresionValorSugeridoDefinidoPorElUsuario = null
		with this
			lcExpresionValorSugeridoDefinidoPorElUsuario = goServicios.SaltosDeCampoYValoresSugeridos.ObtenerValorSugerido( "NOTADEDEBITOELECTRONICAEXPORTACION", "", "Tipocpterelacionado" )
			if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
				if !empty( lcExpresionValorSugeridoDefinidoPorElUsuario )
					try
						lvValorSugeridoDefinidoPorElUsuario = &lcExpresionValorSugeridoDefinidoPorElUsuario
						...
						loEx.Grabar( loError )
						loEx.AgregarInformacion( iif( type( "loError.UserValue.Message" ) == "C", loError.UserValue.Message, loError.Message ) )
						loEx.AgregarInformacion( "Se produjo un error al intentar asignar el valor sugerido " + lcExpresionValorSugeridoDefinidoPorElUsuario + " del atributo Tipo comprobante asociado para la entidad Nota de débito electrónica exportación." + chr( 10 ) + "Verifique el valor especificado en la entidad Comportamiento de atributos." + chr( 10 ) + "El tipo de dato del atributo es numérico." )
						goServicios.Errores.LevantarExcepcion( loEx )
					endtry
				endif
			endif
	...
	*--------------------------------------------------------------------------------------------------------
	Function ValorSugeridoLetracpterelacionado() as void
		...
		lcExpresionValorSugeridoDefinidoPorElUsuario = null
		with this
			lcExpresionValorSugeridoDefinidoPorElUsuario = goServicios.SaltosDeCampoYValoresSugeridos.ObtenerValorSugerido( "NOTADEDEBITOELECTRONICAEXPORTACION", "", "Letracpterelacionado" )
			if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
				if !empty( lcExpresionValorSugeridoDefinidoPorElUsuario )
					try
						lvValorSugeridoDefinidoPorElUsuario = &lcExpresionValorSugeridoDefinidoPorElUsuario
	...
	*--------------------------------------------------------------------------------------------------------
	Function ValorSugeridoPuntodeventacpterelacionado() as void
		...
		lcExpresionValorSugeridoDefinidoPorElUsuario = null
		with this
			lcExpresionValorSugeridoDefinidoPorElUsuario = goServicios.SaltosDeCampoYValoresSugeridos.ObtenerValorSugerido( "NOTADEDEBITOELECTRONICAEXPORTACION", "", "Puntodeventacpterelacionado" )
			if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
				if !empty( lcExpresionValorSugeridoDefinidoPorElUsuario )
					try
						lvValorSugeridoDefinidoPorElUsuario = &lcExpresionValorSugeridoDefinidoPorElUsuario
	...
	*--------------------------------------------------------------------------------------------------------
	Function ValorSugeridoNumerocpterelacionado() as void
		...
		lcExpresionValorSugeridoDefinidoPorElUsuario = null
		with this
			lcExpresionValorSugeridoDefinidoPorElUsuario = goServicios.SaltosDeCampoYValoresSugeridos.ObtenerValorSugerido( "NOTADEDEBITOELECTRONICAEXPORTACION", "", "Numerocpterelacionado" )
			if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
				if !empty( lcExpresionValorSugeridoDefinidoPorElUsuario )
					try
						lvValorSugeridoDefinidoPorElUsuario = &lcExpresionValorSugeridoDefinidoPorElUsuario
						...
						loEx.Grabar( loError )
						loEx.AgregarInformacion( iif( type( "loError.UserValue.Message" ) == "C", loError.UserValue.Message, loError.Message ) )
						loEx.AgregarInformacion( "Se produjo un error al intentar asignar el valor sugerido " + lcExpresionValorSugeridoDefinidoPorElUsuario + " del atributo Número comprobante asociado para la entidad Nota de débito electrónica exportación." + chr( 10 ) + "Verifique el valor especificado en la entidad Comportamiento de atributos." + chr( 10 ) + "El tipo de dato del atributo es numérico." )
						goServicios.Errores.LevantarExcepcion( loEx )
					endtry
				endif
			endif
	...
	*--------------------------------------------------------------------------------------------------------
	Function ValorSugeridoFechacpterelacionado() as void
		...
		lcExpresionValorSugeridoDefinidoPorElUsuario = null
		with this
			lcExpresionValorSugeridoDefinidoPorElUsuario = goServicios.SaltosDeCampoYValoresSugeridos.ObtenerValorSugerido( "NOTADEDEBITOELECTRONICAEXPORTACION", "", "Fechacpterelacionado" )
			if !isnull( lcExpresionValorSugeridoDefinidoPorElUsuario )
				if !empty( lcExpresionValorSugeridoDefinidoPorElUsuario )
					try
						lvValorSugeridoDefinidoPorElUsuario = &lcExpresionValorSugeridoDefinidoPorElUsuario
	...
	*--------------------------------------------------------------------------------------------------------
	function Destroy()
		...
		this.lDestroy = .t.
		this.oCompFacturacionElectronicaExportacion = null
