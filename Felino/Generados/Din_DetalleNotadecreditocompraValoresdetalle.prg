...
define class Din_DetalleNOTADECREDITOCOMPRAValoresdetalle as DetalleValorescompra of DetalleValorescompra.Prg
	...
	*--------------------------------------------------------------------------------------------------------
	function oItem_Access() as variant
		if !this.ldestroy and ( !vartype( this.oItem ) = 'O' or isnull( this.oItem ) )
			this.oItem = _Screen.zoo.crearobjeto( 'Din_ItemNOTADECREDITOCOMPRAValoresdetalle' )
		...
		for lni = 1 to this.Count
			...
			if vartype( this.Item[lni].MONTO) = 'N'
				...
				if lnLargoEntero <= 7
					...
					llRetorno = .F.
					this.AgregarInformacion( 'La longitud entera del valor del atributo MONTO del item ' + transform( lni ) + ' es mayor a la permitida (Actual:' + transform(lnLargoEntero) + ' Máxima:7), en el detalle ' + this.cEtiqueta + '.' )
				Endif
			else
				llRetorno = .F.
