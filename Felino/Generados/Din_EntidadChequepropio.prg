...
define class Din_EntidadCHEQUEPROPIO as Ent_CarteraDeCheques of Ent_CarteraDeCheques.prg
	...
	*--------------------------------------------------------------------------------------------------------
	function ValidarFecha() as boolean
		...
		llRetorno = dodefault()
		if empty( this.Fecha )
			this.AgregarInformacion( 'Debe cargar el campo Fecha de pago', 9005, 'Fecha' )
			llRetorno = .F.
		endif
		return llRetorno
	endfunc
	...
	*--------------------------------------------------------------------------------------------------------
	function ValidarSerieOrigen() as boolean
		...
		if empty( this.SerieOrigen )
			this.AgregarInformacion( 'Debe cargar el campo Serie', 9005, 'SerieOrigen' )
			llRetorno = .F.
		endif
		return llRetorno
	endfunc
	...
	*--------------------------------------------------------------------------------------------------------
	Protected Function _Buscar() As void
		...
		if empty( This.&lcAtributo )
			...
			lcMensaje = 'El dato buscado PUNTO DE VENTA ' + transform( This.PuntoDeVenta ) + ', NÚMERO ' + transform( This.NumeroC ) + ' de la entidad ' + upper( this.cDescripcion ) + ' no existe.'
		else
