...
PropertyExclusionList = ;
 ...
 "width,height,top,left,tag,picture,onetomany,childalias,childorder,relationalexpr,timestamp_column,name" 
...
LOCAL lnX, lnProperties, lcFieldName, lcFieldType, lvValue, llExclusions, lcKey
...
FOR lnX = 1 TO lnProperties
      ...
      lcKey = iif( left(lcFieldName,1) = "x", substr(lcFieldName,2,len(lcFieldName) - 1), lcFieldName)

        do case
            case lcKey = "iva"
                lcKey = "IVA"
            case lcKey = "seversion"
                lcKey = "SEVersion"
            case lcKey = "sitefcode"
                lcKey = "SiTefCode"
            case lcKey = "productsku"
                lcKey = "ProductSKU"
            case lcKey = "paymentid"
                lcKey = "PaymentID" 
            case lcKey =  "nsusitef"
                lcKey = "NSUSitef"
            otherwise
                lcKey = proper(strtran(lcKey, "_", " "))
                lcKey = strtran(lcKey, " ", "")
        endcase
   
      this.cOutput = this.cOutput + ["] + lcKey + [":]
